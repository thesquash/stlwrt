			     ...
			     GtkTextIter *end)
{
   ...
   * start and end.
  */
  start_line = ___gtk_text_iter_get_text_line (start);
  start_line_prev = _gtk_text_line_previous (start_line);
  end_line = ___gtk_text_iter_get_text_line (end);
                        ...
                        GtkTextIter *end)
{
  ...
  start_byte_offset = __gtk_text_iter_get_line_index (start);
                        ...
                        gint         len)
{
  ...
  start_line = line;
  start_byte_index = __gtk_text_iter_get_line_index (iter);
     ...
     passed in to point to the end of the inserted text. */
  {
    ...
    /* We could almost certainly be more efficient here
       ...
       above. FIXME */
    __gtk_text_iter_forward_chars (&end, char_count_delta);
...

{
  ...
  line = ___gtk_text_iter_get_text_line (iter);
  tree = _gtk_text_iter_get_btree (iter);
  start_byte_offset = __gtk_text_iter_get_line_index (iter);
  ...
  *iter = start;
  __gtk_text_iter_forward_char (iter); /* skip forward past the segment */
                                     ...
                                     GtkTextChildAnchor *anchor)
{
  ...
  seg->body.child.line = ___gtk_text_iter_get_text_line (iter);
                     ...
                     gboolean           add)
{
  ...
  g_return_if_fail (gtk_text_tag_get_props (tag)->table == _gtk_text_iter_get_btree (start_orig)->table);
  ...
  if (__gtk_text_iter_equal (start_orig, end_orig))
    return;
   ...
   * start.
   */
  while (__gtk_text_iter_forward_to_tag_toggle (&iter, tag))
    {
  ...
  toggled_on = __gtk_text_iter_has_tag (&start, tag);
  if ( (add && !toggled_on) ||
       (!add && toggled_on) )
    {
...
_gtk_text_btree_char_is_invisible (const GtkTextIter *iter)
{
  ...
  numTags = __gtk_text_tag_table_get_size (tree->table);
      ...
      if ((seg->type == &gtk_text_toggle_on_type)
          || (seg->type == &gtk_text_toggle_off_type))
        {
          ...
          if (gtk_text_tag_get_props (tag)->invisible_set)
            {
              tags[gtk_text_tag_get_props (tag)->priority] = gtk_text_tag_get_props (tag);
              tagCnts[gtk_text_tag_get_props (tag)->priority]++;
            }
        }
    }
              ...
              if (summary->toggle_count & 1)
                {
                  ...
                  if (gtk_text_tag_get_props (tag)->invisible_set)
                    {
                      tags[gtk_text_tag_get_props (tag)->priority] = gtk_text_tag_get_props (tag);
                      tagCnts[gtk_text_tag_get_props (tag)->priority] += summary->toggle_count;
                    }
                }
            }
        }
...
ensure_not_off_end (GtkTextBTree *tree,
                    ...
                    GtkTextIter *iter)
{
  if (__gtk_text_iter_get_line (iter) == _gtk_text_btree_line_count (tree))
               ...
               gboolean           redraw_selections)
{
  ...
  if (existing_mark)
    {
      if (__gtk_text_mark_get_buffer (existing_mark) != NULL)
  ...
  if (mark != NULL)
    {
      ...
      gtk_text_btree_unlink_segment (tree, mark,
                                     mark->body.mark.line);
      mark->body.mark.line = ___gtk_text_iter_get_text_line (&iter);
      g_assert (mark->body.mark.line == ___gtk_text_iter_get_text_line (&iter));
	...
	existing_mark = __gtk_text_mark_new (name, left_gravity);
                                     ...
                                     GtkTextIter  *end)
{
  ...
  if (__gtk_text_iter_equal (&tmp_start, &tmp_end))
    {
      if (start)
        *start = tmp_start;
    ...
    {
      __gtk_text_iter_order (&tmp_start, &tmp_end);
                              ...
                              const GtkTextIter *bound)
{
  ...
  /* Check if it's no-op since __gtk_text_buffer_place_cursor()
   * also calls this, and this will redraw the cursor line. */
  if (!__gtk_text_iter_equal (&old_ins, ins) ||
      !__gtk_text_iter_equal (&old_bound, bound))
    {
 ...
 **/
void
__gtk_text_mark_set_visible (GtkTextMark       *mark,
                           gboolean           setting)
{
  GtkTextLineSegment *seg;
                  ...
                  if (summary2 == NULL)
                    {
                      ...
                      g_error ("gtk_text_btree_node_check_consistency: GtkTextBTreeNode tag \"%s\" not %s",
                               gtk_text_tag_get_props (summary->info->tag)->name,
...
list_of_tags (GtkTextTagTable *table)
{
  ...
  __gtk_text_tag_table_foreach (table, listify_foreach, &list);
              ...
              if (info->toggle_count != 0)
                {
                  g_error ("_gtk_text_btree_check found \"%s\" with toggles (%d) but no root",
                           gtk_text_tag_get_props (tag)->name, info->toggle_count);
                }
          ...
          if (count != info->toggle_count)
            {
              g_error ("_gtk_text_btree_check toggle_count (%d) wrong for \"%s\" should be (%d)",
                       info->toggle_count, gtk_text_tag_get_props (tag)->name, count);
            }
        }
    }
  ...
  while (seg != NULL)
    {
      if (seg->type == &gtk_text_char_type)
        ...
        {
          printf ("%s tag `%s' %s\n",
                  spaces, gtk_text_tag_get_props (seg->body.toggle.info->tag)->name,
  ...
  while (s)
    {
      printf ("%s %d toggles of `%s' below this node\n",
              spaces, s->toggle_count, gtk_text_tag_get_props (s->info->tag)->name);
      s = s->next;
    }
  ...
  if (seg->type == &gtk_text_char_type)
           ...
           seg->type == &gtk_text_toggle_off_type)
    {
      printf ("       tag `%s' priority %d\n",
              gtk_text_tag_get_props (seg->body.toggle.info->tag)->name,
              gtk_text_tag_get_props (seg->body.toggle.info->tag)->priority);
    }
}
