          ...
          gboolean            copy)
{
  ...
  while (i < n_items)
      ...
      if (item->modifier & NON_STATIC_MASK)
	{
      ...
      if (copy)
	{
	  item = __gtk_stock_item_copy (item);
	  ...
	  if (((GtkStockItem *)old_value)->modifier & NON_STATIC_MASK)
	    __gtk_stock_item_free (old_value);
        }
 ...
 * __gtk_stock_add_static() if @items is persistent and GTK+ need not
 * copy the array.
 * 
 **/
void
__gtk_stock_add (const GtkStockItem *items,
               guint               n_items)
{
...
/**
 * __gtk_stock_add_static:
 * @items: (array length=n_items): a #GtkStockItem or array of #GtkStockItem
 * @n_items: number of items
 *
 * Same as __gtk_stock_add(), but doesn't copy @items, so
 * @items must persist until application exit.
 * 
 **/
void
__gtk_stock_add_static (const GtkStockItem *items,
                      guint               n_items)
{
  g_return_if_fail (items != NULL);
 ...
 **/
gboolean
__gtk_stock_lookup (const gchar  *stock_id,
                  GtkStockItem *item)
{
...
/**
 * __gtk_stock_list_ids:
 * 
 * Retrieves a list of all known stock IDs added to a #GtkIconFactory
 * or registered with __gtk_stock_add(). The list must be freed with g_slist_free(),
 * and each string in the list must be freed with g_free().
 *
 * Return value: (element-type utf8) (transfer full): a list of known stock IDs
 **/
GSList*
__gtk_stock_list_ids (void)
{
  GList *ids;
  GList *icon_ids;
  GSList *retval;
  ...
  icon_ids = ______gtk_icon_factory_list_ids ();
 ...
 **/
GtkStockItem *
__gtk_stock_item_copy (const GtkStockItem *item)
{
  GtkStockItem *copy;
...
/**
 * __gtk_stock_item_free:
 * @item: a #GtkStockItem
 *
 * Frees a stock item allocated on the heap, such as one returned by
 * __gtk_stock_item_copy(). Also frees the fields inside the stock item,
 * if they are not %NULL.
 * 
 **/
void
__gtk_stock_item_free (GtkStockItem *item)
{
...
/**
 * __gtk_stock_set_translate_func: 
 ...
 * /&ast; ... &ast;/
 *
 * __gtk_stock_add (items, G_N_ELEMENTS (items));
 * __gtk_stock_set_translate_func ("odd-item-domain", my_translate_func, "odd items"); 
 * __gtk_stock_set_translate_func ("even-item-domain", my_translate_func, "even items"); 
 * ]|
 * 
 * Since: 2.8
 */
void
__gtk_stock_set_translate_func (const gchar      *domain,
			      GtkTranslateFunc  func,
			      gpointer          data,
			      GDestroyNotify    notify)
{
  ...
  if (stock_hash == NULL)
    {
      ...
      __gtk_stock_add_static (builtin_items, G_N_ELEMENTS (builtin_items));
    }
  ...
  if (translate_hash == NULL)
    {
      ...
      __gtk_stock_set_translate_func (GETTEXT_PACKAGE, 
				    sgettext_swapped,
				    "Stock label",
				    NULL);
      __gtk_stock_set_translate_func (GETTEXT_PACKAGE "-navigation", 
				    sgettext_swapped,
				    "Stock label, navigation",
				    NULL);
      __gtk_stock_set_translate_func (GETTEXT_PACKAGE "-media", 
				    sgettext_swapped,
				    "Stock label, media",
				    NULL);
    }
