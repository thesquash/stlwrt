...
void
______gtk_container_dequeue_resize_handler (GtkContainer *container)
{
  g_return_if_fail (GTK_IS_CONTAINER (container));
...
void
______gtk_container_queue_resize (GtkContainer *container)
{
...
gchar*
______gtk_container_child_composite_name (GtkContainer *container,
				    GtkWidget    *child)
{
				    ...
				    GtkWidget        *child)
{
  ...
  if (container->focus_child)
      ...
      if (hadj || vadj) 
	{
	  ...
	  while (GTK_IS_CONTAINER (focus_child) && 
		 GTK_CONTAINER (focus_child)->focus_child)
	    {
	      focus_child = GTK_CONTAINER (focus_child)->focus_child;
           ...
           if (!__gtk_widget_translate_coordinates (focus_child, container->focus_child,
                                                  0, 0, &x, &y))
             return;
	  ...
	  if (vadj)
	    __gtk_adjustment_clamp_page (vadj, y, y + focus_child->allocation.height);
	  
	  if (hadj)
	    __gtk_adjustment_clamp_page (hadj, x, x + focus_child->allocation.width);
	}
    }
}
                     ...
                     GtkDirectionType  direction)
{
  ...
  if (__gtk_widget_get_can_focus (widget))
	...
	sorted_children = ______gtk_container_focus_sort (container, children, direction, NULL);
 ...
 **/
GList *
______gtk_container_focus_sort (GtkContainer     *container,
			   GList            *children,
			   GtkDirectionType  direction,
			   GtkWidget        *old_focus)
{
