      ...
      if (type == CAIRO_SURFACE_TYPE_PS)
        {
          ...
          switch (__gtk_page_setup_get_orientation (page_setup))
            {
						      ...
						      const gchar       *filename)
{
  ...
  if (parent)
    screen = __gtk_window_get_screen (parent);
  else
  ...
  print_settings = __gtk_print_settings_copy (__gtk_print_operation_get_print_settings (op));

  if (print_settings != NULL)
    {
      __gtk_print_settings_set_reverse (print_settings, FALSE);
      __gtk_print_settings_set_page_set (print_settings, GTK_PAGE_SET_ALL);
      __gtk_print_settings_set_scale (print_settings, 1.0);
      __gtk_print_settings_set_number_up (print_settings, 1);
      __gtk_print_settings_set_number_up_layout (print_settings, GTK_NUMBER_UP_LAYOUT_LEFT_TO_RIGHT_TOP_TO_BOTTOM);

      /*  These removals are neccessary because cups-* settings have higher priority
       *  than normal settings.
       */
      __gtk_print_settings_unset (print_settings, "cups-reverse");
      __gtk_print_settings_unset (print_settings, "cups-page-set");
      __gtk_print_settings_unset (print_settings, "cups-scale");
      __gtk_print_settings_unset (print_settings, "cups-number-up");
      __gtk_print_settings_unset (print_settings, "cups-number-up-layout");

      __gtk_print_settings_to_key_file (print_settings, key_file, NULL);
      g_object_unref (print_settings);
    }

  page_setup = __gtk_print_context_get_page_setup (op->priv->print_context);
  __gtk_page_setup_to_key_file (page_setup, key_file, NULL);
  ...
  __gdk_spawn_on_screen (screen, NULL, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, &error);
  ...
  if (error != NULL)
    {
      ...
      __gtk_show_uri (screen, uri, GDK_CURRENT_TIME, &error);
      g_free (uri);
    }
 ...
 out:
  if (error != NULL)
    {
      GtkWidget *edialog;
      edialog = __gtk_message_dialog_new (parent, 
                                        GTK_DIALOG_DESTROY_WITH_PARENT,
                                        GTK_MESSAGE_ERROR,
                                        GTK_BUTTONS_CLOSE,
                                        _("Error launching preview") /* FIXME better text */);
      __gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (edialog),
                                                "%s", error->message);
      g_signal_connect (edialog, "response",
                        G_CALLBACK (__gtk_widget_destroy), NULL);

      __gtk_window_present (GTK_WINDOW (edialog));
                   ...
                   GError      *error)
{
  ...
  if (error != NULL)
    {
      GtkWidget *edialog;
      edialog = __gtk_message_dialog_new (op_unix->parent, 
                                        GTK_DIALOG_DESTROY_WITH_PARENT,
                                        GTK_MESSAGE_ERROR,
                                        GTK_BUTTONS_CLOSE,
      ...
      __gtk_message_dialog_format_secondary_text (GTK_MESSAGE_DIALOG (edialog),
                                                "%s", error->message);
      __gtk_window_set_modal (GTK_WINDOW (edialog), TRUE);
      g_signal_connect (edialog, "response",
                        G_CALLBACK (__gtk_widget_destroy), NULL);
                  ...
                  GtkWindow         *parent)
{
  ...
  if (priv->custom_widget) 
      ...
      if (custom_tab_label == NULL)
	{
      ...
      label = __gtk_label_new (custom_tab_label);
	      ...
	      gboolean           page_setup_set)
{
  ...
  if (rdata->do_print)
    {
      __gtk_print_operation_set_print_settings (op, settings);
      priv->print_context = _gtk_print_context_new (op);

      if (__gtk_print_settings_get_number_up (settings) < 2)
        {
      ...
      if (page_setup != NULL &&
          (__gtk_print_operation_get_default_page_setup (op) == NULL ||
           page_setup_set))
        __gtk_print_operation_set_default_page_setup (op, page_setup);
	  ...
	  __gtk_print_context_set_cairo_context (priv->print_context, cr, 72, 72);
	  cairo_destroy (cr);
		       ...
		       gpointer   data)
{
  ...
  if (rdata->do_print)
    {
       ...
       */
      __gtk_print_operation_set_print_settings (rdata->op, settings);
  ...
  __gtk_widget_destroy (GTK_WIDGET (pd));
  ...
  if (printer != NULL) 
    {
      ...
      if (priv->print_settings)
	settings = __gtk_print_settings_copy (priv->print_settings);
      else
  ...
  if (show_dialog)
    {
      ...
      __gtk_window_set_modal (GTK_WINDOW (pd), TRUE);

      g_signal_connect (pd, "response", 
			G_CALLBACK (handle_print_response), rdata);
      
      __gtk_window_present (GTK_WINDOW (pd));
    }
  else
    {
      printer_name = NULL;
      if (op->priv->print_settings)
	printer_name = __gtk_print_settings_get_printer (op->priv->print_settings);
  ...
  if (show_dialog)
    {
      ...
      response = __gtk_dialog_run (GTK_DIALOG (pd));
...
/**
 * __gtk_print_run_page_setup_dialog:
 ...
 * setup dialog. See __gtk_print_run_page_setup_dialog_async() if this is 
 * a problem.
 * 
 ...
 */
GtkPageSetup *
__gtk_print_run_page_setup_dialog (GtkWindow        *parent,
				 GtkPageSetup     *page_setup,
				 GtkPrintSettings *settings)
{
  GtkWidget *dialog;
  ...
  if (rdata.page_setup)
    ...
    return __gtk_page_setup_copy (page_setup);
  else
    return __gtk_page_setup_new ();
}

/**
 * __gtk_print_run_page_setup_dialog_async:
 ...
 *
 * In contrast to __gtk_print_run_page_setup_dialog(), this function  returns after 
 ...
 */
void
__gtk_print_run_page_setup_dialog_async (GtkWindow            *parent,
				       GtkPageSetup         *page_setup,
				       GtkPrintSettings     *settings,
				       GtkPageSetupDoneFunc  done_cb,
				       gpointer              data)
