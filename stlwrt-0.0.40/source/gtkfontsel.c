...
static GdkFont* __gtk_font_selection_get_font_internal (GtkFontSelection *fontsel);
static PangoFontDescription *__gtk_font_selection_get_font_description (GtkFontSelection *fontsel);
				 ...
				 GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FONT_NAME:
      __gtk_font_selection_set_font_name (fontsel, g_value_get_string (value));
      break;
    case PROP_PREVIEW_TEXT:
      __gtk_font_selection_set_preview_text (fontsel, g_value_get_string (value));
      break;
    default:
					     ...
					     GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_FONT_NAME:
      g_value_take_string (value, __gtk_font_selection_get_font_name (fontsel));
      break;
    case PROP_FONT:
      g_value_set_boxed (value, __gtk_font_selection_get_font_internal (fontsel));
      break;
    case PROP_PREVIEW_TEXT:
      g_value_set_string (value, __gtk_font_selection_get_preview_text (fontsel));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
list_row_activated (GtkWidget *widget)
{
  ...
  window = GTK_WINDOW (__gtk_widget_get_toplevel (GTK_WIDGET (widget)));
  if (!__gtk_widget_is_toplevel (GTK_WIDGET (window)))
    window = NULL;
...
gtk_font_selection_init (GtkFontSelection *fontsel)
{
  ...
  font_label = __gtk_label_new_with_mnemonic (_("_Family:"));
  __gtk_misc_set_alignment (GTK_MISC (font_label), 0.0, 0.5);
  __gtk_widget_show (font_label);
  __gtk_table_attach (GTK_TABLE (table), font_label, 0, 1, 0, 1,
		    GTK_FILL, 0, 0, 0);  

  style_label = __gtk_label_new_with_mnemonic (_("_Style:"));
  __gtk_misc_set_alignment (GTK_MISC (style_label), 0.0, 0.5);
  __gtk_widget_show (style_label);
  __gtk_table_attach (GTK_TABLE (table), style_label, 1, 2, 0, 1,
		    GTK_FILL, 0, 0, 0);
  ...
  if (GTK_IS_ACCESSIBLE (atk_obj))
    {
      ...
      atk_label = __gtk_widget_get_accessible (label);
  ...
  __gtk_box_pack_start (GTK_BOX (fontsel), vbox, FALSE, TRUE, 0);
  
  /* create the text entry widget */
  label = __gtk_label_new_with_mnemonic (_("_Preview:"));
  __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  __gtk_widget_show (label);
  __gtk_box_pack_start (GTK_BOX (vbox), label, FALSE, TRUE, 0);
  
  text_box = __gtk_hbox_new (FALSE, 0);
...
{
  if (__gtk_widget_has_screen (GTK_WIDGET (fontsel)))
    {
      PangoFontDescription *desc;
      desc = __gtk_font_selection_get_font_description (fontsel);
...
{
  GtkTreeSelection *selection = __gtk_tree_view_get_selection (tree_view);
  GtkTreeModel *model;
  GtkTreeIter iter;

  if (__gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      GtkTreePath *path = __gtk_tree_model_get_path (model, &iter);
      __gtk_tree_view_scroll_to_cell (tree_view, path, NULL, TRUE, 0.5, 0.5);
...
set_cursor_to_iter (GtkTreeView *view,
		    GtkTreeIter *iter)
{
  GtkTreeModel *model = __gtk_tree_view_get_model (view);
  GtkTreePath *path = __gtk_tree_model_get_path (model, iter);
  
  __gtk_tree_view_set_cursor (view, path, NULL, FALSE);
  ...
  for (i=0; i<n_families; i++)
    {
      ...
      __gtk_list_store_set (model, &iter,
  ...
  if (__gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      ...
      __gtk_tree_model_get (model, &iter, FACE_COLUMN, &face, -1);
					 ...
					 gboolean          first_time)
{
  ...
  if (first_time)
      ...
      for (i = 0; i < G_N_ELEMENTS (font_sizes) && !found; i++)
	{
	  ...
	  __gtk_tree_model_iter_next (GTK_TREE_MODEL (model), &iter);
	}
				...
				gpointer          data)
{
  ...
  if (__gtk_tree_selection_get_selected (selection, &model, &iter))
    {
      __gtk_tree_model_get (model, &iter, SIZE_COLUMN, &new_size, -1);
...
__gtk_font_selection_get_font_description (GtkFontSelection *fontsel)
{
...
gtk_font_selection_update_preview (GtkFontSelection *fontsel)
{
  ...
  rc_style->font_desc = __gtk_font_selection_get_font_description (fontsel);
  
  __gtk_widget_modify_style (preview_entry, rc_style);
  ...
  text = __gtk_entry_get_text (GTK_ENTRY (preview_entry));
  if (strlen (text) == 0)
    __gtk_entry_set_text (GTK_ENTRY (preview_entry), _(PREVIEW_TEXT));
  __gtk_editable_set_position (GTK_EDITABLE (preview_entry), 0);
}
...
/**
 * __gtk_font_selection_get_family_list:
 ...
 */
GtkWidget *
__gtk_font_selection_get_family_list (GtkFontSelection *fontsel)
{
...
/**
 * __gtk_font_selection_get_face_list:
 * @fontsel: a #GtkFontSelection
 *
 ...
 */
GtkWidget *
__gtk_font_selection_get_face_list (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);
...
/**
 * __gtk_font_selection_get_size_entry:
 * @fontsel: a #GtkFontSelection
 *
 ...
 */
GtkWidget *
__gtk_font_selection_get_size_entry (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);
...
/**
 * __gtk_font_selection_get_size_list:
 * @fontsel: a #GtkFontSelection
 *
 ...
 */
GtkWidget *
__gtk_font_selection_get_size_list (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);
...
/**
 * __gtk_font_selection_get_preview_entry:
 * @fontsel: a #GtkFontSelection
 *
 ...
 */
GtkWidget *
__gtk_font_selection_get_preview_entry (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);
...
/**
 * __gtk_font_selection_get_family:
 * @fontsel: a #GtkFontSelection
 *
 ...
 **/
gint
__gtk_font_selection_get_size (GtkFontSelection *fontsel)
{
 ...
 *
 * Deprecated: 2.0: Use __gtk_font_selection_get_font_name() instead.
 */
GdkFont *
__gtk_font_selection_get_font (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);

  return __gtk_font_selection_get_font_internal (fontsel);
}

/**
 * __gtk_font_selection_get_font_name:
 * @fontsel: a #GtkFontSelection
 * 
 * Gets the currently-selected font name. 
 *
 * Note that this can be a different string than what you set with 
 * __gtk_font_selection_set_font_name(), as the font selection widget may 
 ...
 */
gchar *
__gtk_font_selection_get_font_name (GtkFontSelection *fontsel)
{
  gchar *result;
...
/**
 * __gtk_font_selection_set_font_name:
 * @fontsel: a #GtkFontSelection
 ...
 */
gboolean
__gtk_font_selection_set_font_name (GtkFontSelection *fontsel,
...
/**
 * __gtk_font_selection_get_preview_text:
 * @fontsel: a #GtkFontSelection
 *
 ...
 */
const gchar*
__gtk_font_selection_get_preview_text (GtkFontSelection *fontsel)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION (fontsel), NULL);
...
/**
 * __gtk_font_selection_set_preview_text:
 * @fontsel: a #GtkFontSelection
 ...
 */
void
__gtk_font_selection_set_preview_text  (GtkFontSelection *fontsel,
				      const gchar      *text)
{
...
gtk_font_selection_dialog_init (GtkFontSelectionDialog *fontseldiag)
{
  ...
  __gtk_window_set_title (GTK_WINDOW (fontseldiag),
                        _("Font Selection"));

  __gtk_widget_pop_composite_child ();

  ______gtk_dialog_set_ignore_separator (dialog, TRUE);
}

/**
 * __gtk_font_selection_dialog_new:
 ...
 */
GtkWidget*
__gtk_font_selection_dialog_new (const gchar *title)
{
  ...
  if (title)
    __gtk_window_set_title (GTK_WINDOW (fontseldiag), title);
  
  return GTK_WIDGET (fontseldiag);
}

/**
 * __gtk_font_selection_dialog_get_font_selection:
 ...
 **/
GtkWidget*
__gtk_font_selection_dialog_get_font_selection (GtkFontSelectionDialog *fsd)
{
...
/**
 * __gtk_font_selection_dialog_get_ok_button:
 * @fsd: a #GtkFontSelectionDialog
 *
 * Gets the 'OK' button.
 *
 ...
 */
GtkWidget *
__gtk_font_selection_dialog_get_ok_button (GtkFontSelectionDialog *fsd)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION_DIALOG (fsd), NULL);
...
/**
 * __gtk_font_selection_dialog_get_apply_button:
 * @fsd: a #GtkFontSelectionDialog
 *
 ...
 */
GtkWidget *
__gtk_font_selection_dialog_get_apply_button (GtkFontSelectionDialog *fsd)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION_DIALOG (fsd), NULL);
...
/**
 * __gtk_font_selection_dialog_get_cancel_button:
 * @fsd: a #GtkFontSelectionDialog
 *
 ...
 */
GtkWidget *
__gtk_font_selection_dialog_get_cancel_button (GtkFontSelectionDialog *fsd)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION_DIALOG (fsd), NULL);
...
/**
 * __gtk_font_selection_dialog_get_font_name:
 * @fsd: a #GtkFontSelectionDialog
 * 
 * Gets the currently-selected font name.
 *
 * Note that this can be a different string than what you set with 
 * __gtk_font_selection_dialog_set_font_name(), as the font selection widget
 ...
 */
gchar*
__gtk_font_selection_dialog_get_font_name (GtkFontSelectionDialog *fsd)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION_DIALOG (fsd), NULL);
...
/**
 * __gtk_font_selection_dialog_get_font:
 * @fsd: a #GtkFontSelectionDialog
 *
 * Gets the currently-selected font.
 *
 ...
 *
 * Deprecated: 2.0: Use __gtk_font_selection_dialog_get_font_name() instead.
 */
GdkFont*
__gtk_font_selection_dialog_get_font (GtkFontSelectionDialog *fsd)
{
  g_return_val_if_fail (GTK_IS_FONT_SELECTION_DIALOG (fsd), NULL);
...
/**
 * __gtk_font_selection_dialog_set_font_name:
 * @fsd: a #GtkFontSelectionDialog
 * @fontname: a font name like "Helvetica 12" or "Times Bold 18"
 *
 * Sets the currently selected font. 
 ...
 */
gboolean
__gtk_font_selection_dialog_set_font_name (GtkFontSelectionDialog *fsd,
					 const gchar	        *fontname)
{
...
/**
 * __gtk_font_selection_dialog_get_preview_text:
 * @fsd: a #GtkFontSelectionDialog
 *
 * Gets the text displayed in the preview area.
 * 
...
/**
 * __gtk_font_selection_dialog_set_preview_text:
 * @fsd: a #GtkFontSelectionDialog
 * @text: the text to display in the preview area
 *
 * Sets the text displayed in the preview area. 
 */
void
__gtk_font_selection_dialog_set_preview_text (GtkFontSelectionDialog *fsd,
					    const gchar	           *text)
{
