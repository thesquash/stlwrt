...
{
  ______gtk_rc_free_widget_class_path (pspec->path);
  if (pspec->pspec)
...
_____gtk_binding_entry_add_signall (GtkBindingSet  *binding_set,
                               ...
                               GSList	      *binding_args)
{
  ______gtk_binding_entry_add_signall (binding_set,
                                  keyval, modifiers,
                                  signal_name, binding_args);
}

void
______gtk_binding_entry_add_signall (GtkBindingSet  *binding_set,
                                guint	       keyval,
                                GdkModifierType modifiers,
                                const gchar    *signal_name,
                                GSList	      *binding_args)
  ...
  if (i == n_args || i == 0)
    {
      ...
      ______gtk_binding_entry_add_signall (binding_set, keyval, modifiers, signal_name, slist);
    }
			  ...
			  GtkPathPriorityType priority)
{
  ...
  if (path_type == GTK_PATH_WIDGET_CLASS)
    {
      pspec->pspec = NULL;
      pspec->path = ______gtk_rc_parse_widget_class_path (path_pattern);
    }
  else
    {
  ...
  for (slist = pspec_list; slist; slist = slist->next)
    {
      ...
      if (pspec->type == GTK_PATH_WIDGET_CLASS)
        {
          if (______gtk_rc_match_widget_class (pspec->path, path_length, path, path_reversed))
...
guint
______gtk_binding_parse_binding (GScanner *scanner)
{
  gchar *name;
  GtkBindingSet *binding_set;
 ...
 */
void
______gtk_binding_reset_parsed (void)
{
  GSList *slist, *next;
