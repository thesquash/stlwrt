			     ...
			     GdkEventButton *event)
{
  ...
  if (menu_item && ______gtk_menu_item_is_selectable (menu_item) &&
  ...
  if (menu_item && ______gtk_menu_item_is_selectable (menu_item) &&
      ...
      !__gtk_widget_get_visible (GTK_MENU_ITEM (menu_item)->submenu))
    {
      ...
      ______gtk_menu_item_popup_submenu (menu_item, FALSE);
      ...
      if ((event->time - menu_shell->activate_time) > MENU_SHELL_TIMEOUT)
        {
          if (menu_item && (menu_shell->active_menu_item == menu_item) &&
              ______gtk_menu_item_is_selectable (menu_item))
            {
              ...
              if (submenu == NULL)
                       ...
                       priv->activated_submenu)
                {
                  ...
                  if (!priv->activated_submenu &&
                    ...
                    {
                      ______gtk_menu_item_popdown_submenu (menu_item);
                    }
                  else
                    {
...
void
______gtk_menu_shell_set_keyboard_mode (GtkMenuShell *menu_shell,
...
gboolean
______gtk_menu_shell_get_keyboard_mode (GtkMenuShell *menu_shell)
{
  return menu_shell->keyboard_mode;
}

void
______gtk_menu_shell_update_mnemonics (GtkMenuShell *menu_shell)
{
  GtkMenuShell *target;
      ...
      if (GTK_IS_MENU_BAR (target))
        {
          ...
          ______gtk_label_mnemonics_visible_apply_recursively (GTK_WIDGET (target),
                                                          mnemonics_visible);
        }
      else
                  ...
                  if (!__gtk_widget_get_visible (GTK_MENU_ITEM (menu_item)->submenu))
                    {
                      ...
                      if (touchscreen_mode)
                        ______gtk_menu_item_popup_submenu (menu_item, TRUE);
                    }
                }
	    }
	}
			     ...
			     GdkEventCrossing *event)
{
  ...
  if (__gtk_widget_get_visible (widget))
	...
	return TRUE;
      ...
      if (!______gtk_menu_item_is_selectable (event_widget))
        {
          GtkMenuShellPrivate *priv;
  ...
  if (menu_shell->active_menu_item)
      ...
      if (distance > 0)
	{
	  ...
	  while (node != start_node && 
		 (!node || !______gtk_menu_item_is_selectable (node->data)))
	    {
	      if (node)
		node = node->next;
              else if (wrap_around)
  ...
  while (tmp_list)
    {
	  ...
	  ______gtk_menu_item_is_selectable (child))
	{
	  to_select = child;
...
void
______gtk_menu_shell_select_last (GtkMenuShell *menu_shell,
			     gboolean      search_sensitive)
{
  GtkWidget *to_select = NULL;
  GList *tmp_list;
...
gtk_menu_shell_select_submenu_first (GtkMenuShell     *menu_shell)
{
  ...
  if (menu_item->submenu)
    {
      ______gtk_menu_item_popup_submenu (GTK_WIDGET (menu_item), FALSE);
  ...
  switch (direction)
    ...
    case GTK_MENU_DIR_PARENT:
      if (touchscreen_mode &&
          ...
          __gtk_widget_get_visible (GTK_MENU_ITEM (menu_shell->active_menu_item)->submenu))
        {
           ...
           * menu.
           */
          ______gtk_menu_item_popdown_submenu (menu_shell->active_menu_item);
          ______gtk_menu_shell_update_mnemonics (menu_shell);
        }
      else if (parent_menu_shell)
	{
          if (touchscreen_mode)
            {
              ...
              ______gtk_menu_item_popdown_submenu (GTK_MENU (menu_shell)->parent_menu_item);
              ______gtk_menu_shell_update_mnemonics (parent_menu_shell);
              break;
            }
       ...
       */
	       ...
	       ______gtk_menu_item_is_selectable (menu_shell->active_menu_item) &&
	       GTK_MENU_ITEM (menu_shell->active_menu_item)->submenu)
	{
	  ...
	  if (GTK_MENU_SHELL_GET_CLASS (menu_shell)->submenu_placement !=
	    ...
	    ______gtk_menu_shell_select_last (submenu, TRUE);
	}
      break;
				 ...
				 GtkDirectionType   dir)
{
  ...
  if (menu_shell)
    ______gtk_menu_bar_cycle_focus (GTK_MENU_BAR (menu_shell), dir);
}

gint
______gtk_menu_shell_get_popup_delay (GtkMenuShell *menu_shell)
{
...
void
______gtk_menu_shell_add_mnemonic (GtkMenuShell *menu_shell,
			      guint      keyval,
			      GtkWidget *target)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
...
void
______gtk_menu_shell_remove_mnemonic (GtkMenuShell *menu_shell,
				 guint      keyval,
				 GtkWidget *target)
{
  g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
