...
tree_path_new_from_node (GtkFileSystemModel *model, guint id)
{
  ...
  return __gtk_tree_path_new_from_indices (r, -1);
}
...
emit_row_inserted_for_node (GtkFileSystemModel *model, guint id)
{
  ...
  __gtk_tree_model_row_inserted (GTK_TREE_MODEL (model), path, &iter);
...
emit_row_changed_for_node (GtkFileSystemModel *model, guint id)
{
  ...
  path = tree_path_new_from_node (model, id);
  ITER_INIT_FROM_INDEX (model, &iter, id);
  __gtk_tree_model_row_changed (GTK_TREE_MODEL (model), path, &iter);
  __gtk_tree_path_free (path);
}
...
emit_row_deleted_for_row (GtkFileSystemModel *model, guint row)
{
  ...
  __gtk_tree_model_row_deleted (GTK_TREE_MODEL (model), path);
  __gtk_tree_path_free (path);
}
...
node_should_be_filtered_out (GtkFileSystemModel *model, guint id)
{
  ...
  /* fill info */
  required = __gtk_file_filter_get_needed (model->filter);
  ...
  result = !__gtk_file_filter_filter (model->filter, &filter_info);
...
gtk_file_system_model_get_iter (GtkTreeModel *tree_model,
				...
				GtkTreePath  *path)
{
  g_return_val_if_fail (__gtk_tree_path_get_depth (path) > 0, FALSE);
      ...
      if (n_visible_rows)
        {
          ...
          __gtk_tree_model_rows_reordered (GTK_TREE_MODEL (model),
                                         path,
                                         NULL,
                                         new_order);
          __gtk_tree_path_free (path);
      ...
      if (model->dir_thaw_source == 0)
        {
          ...
          model->dir_thaw_source = __gdk_threads_add_timeout_full (IO_PRIORITY + 1,
                                                                 50,
                                                                 thaw_func,
                                                                 model,
                                                                 NULL);
 ...
 * __gtk_tree_model_get() or __gtk_tree_model_get_value(), which copy 
