...
foreach_unref (GtkTextTag *tag, gpointer data)
{
  ...
  while (tmp != NULL)
    {
      ______gtk_text_buffer_notify_will_remove_tag (GTK_TEXT_BUFFER (tmp->data),
                                               tag);
...
void
______gtk_text_tag_table_add_buffer (GtkTextTagTable *table,
                                gpointer         buffer)
{
  g_return_if_fail (GTK_IS_TEXT_TAG_TABLE (table));
...
foreach_remove_tag (GtkTextTag *tag, gpointer data)
{
  ...
  ______gtk_text_buffer_notify_will_remove_tag (buffer, tag);
}

void
______gtk_text_tag_table_remove_buffer (GtkTextTagTable *table,
                                   gpointer         buffer)
{
  g_return_if_fail (GTK_IS_TEXT_TAG_TABLE (table));
