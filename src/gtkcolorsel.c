...
STLWRT_DEFINE_FTYPE_VPARENT (GtkColorSelection, gtk_color_selection, GTK_TYPE_VBOX,
                             G_TYPE_FLAG_NONE, G_ADD_PRIVATE (GtkColorSelection))
...
gtk_color_selection_init (GtkColorSelection *colorsel)
{
  ...
  top_hbox = __gtk_hbox_new (FALSE, 12);
  __gtk_box_pack_start (GTK_BOX (colorsel), top_hbox, FALSE, FALSE, 0);
  
  vbox = __gtk_vbox_new (FALSE, 6);
  priv->triangle_colorsel = __gtk_hsv_new ();
  g_signal_connect (priv->triangle_colorsel, "changed",
                    G_CALLBACK (hsv_changed), colorsel);
  __gtk_hsv_set_metrics (GTK_HSV (priv->triangle_colorsel), 174, 15);
  __gtk_box_pack_start (GTK_BOX (top_hbox), vbox, FALSE, FALSE, 0);
  __gtk_box_pack_start (GTK_BOX (vbox), priv->triangle_colorsel, FALSE, FALSE, 0);
  __gtk_widget_set_tooltip_text (priv->triangle_colorsel,
  ...
  __gtk_box_pack_end (GTK_BOX (vbox), hbox, FALSE, FALSE, 0);
  
  frame = __gtk_frame_new (NULL);
  __gtk_widget_set_size_request (frame, -1, 30);
  __gtk_frame_set_shadow_type (GTK_FRAME (frame), GTK_SHADOW_IN);
  color_sample_new (colorsel);
  __gtk_container_add (GTK_CONTAINER (frame), priv->sample_area);
  __gtk_box_pack_start (GTK_BOX (hbox), frame, TRUE, TRUE, 0);
  
  button = __gtk_button_new ();

  __gtk_widget_set_events (button, GDK_POINTER_MOTION_MASK | GDK_POINTER_MOTION_HINT_MASK);
  g_object_set_data (G_OBJECT (button), I_("COLORSEL"), colorsel); 
  g_signal_connect (button, "clicked",
                    G_CALLBACK (get_screen_color), NULL);
  picker_image = __gtk_image_new_from_stock (GTK_STOCK_COLOR_PICKER, GTK_ICON_SIZE_BUTTON);
  __gtk_container_add (GTK_CONTAINER (button), picker_image);
  __gtk_widget_show (GTK_WIDGET (picker_image));
  __gtk_box_pack_end (GTK_BOX (hbox), button, FALSE, FALSE, 0);

  __gtk_widget_set_tooltip_text (button,
                        _("Click the eyedropper, then click a color anywhere on your screen to select that color."));
  
  top_right_vbox = __gtk_vbox_new (FALSE, 6);
  __gtk_box_pack_start (GTK_BOX (top_hbox), top_right_vbox, FALSE, FALSE, 0);
  table = __gtk_table_new (8, 6, FALSE);
  __gtk_box_pack_start (GTK_BOX (top_right_vbox), table, FALSE, FALSE, 0);
  ...
  __gtk_spin_button_set_wrap (GTK_SPIN_BUTTON (priv->hue_spinbutton), TRUE);
  ...
  __gtk_table_attach_defaults (GTK_TABLE (table), __gtk_hseparator_new (), 0, 8, 3, 4); 

  priv->opacity_label = __gtk_label_new_with_mnemonic (_("Op_acity:")); 
  __gtk_misc_set_alignment (GTK_MISC (priv->opacity_label), 0.0, 0.5); 
  __gtk_table_attach_defaults (GTK_TABLE (table), priv->opacity_label, 0, 1, 4, 5); 
  adjust = GTK_ADJUSTMENT (__gtk_adjustment_new (0.0, 0.0, 255.0, 1.0, 1.0, 0.0)); 
  g_object_set_data (G_OBJECT (adjust), I_("COLORSEL"), colorsel); 
  priv->opacity_slider = __gtk_hscale_new (adjust);
  __gtk_widget_set_tooltip_text (priv->opacity_slider,
                        _("Transparency of the color."));
  __gtk_label_set_mnemonic_widget (GTK_LABEL (priv->opacity_label),
                                 priv->opacity_slider);
  __gtk_scale_set_draw_value (GTK_SCALE (priv->opacity_slider), FALSE);
  g_signal_connect (adjust, "value-changed",
                    G_CALLBACK (adjustment_changed),
                    GINT_TO_POINTER (COLORSEL_OPACITY));
  __gtk_table_attach_defaults (GTK_TABLE (table), priv->opacity_slider, 1, 7, 4, 5); 
  priv->opacity_entry = __gtk_entry_new (); 
  __gtk_widget_set_tooltip_text (priv->opacity_entry,
                        _("Transparency of the color."));
  __gtk_widget_set_size_request (priv->opacity_entry, 40, -1); 

  g_signal_connect (priv->opacity_entry, "activate",
                    G_CALLBACK (opacity_entry_changed), colorsel);
  __gtk_table_attach_defaults (GTK_TABLE (table), priv->opacity_entry, 7, 8, 4, 5);
  
  label = __gtk_label_new_with_mnemonic (_("Color _name:"));
  __gtk_table_attach_defaults (GTK_TABLE (table), label, 0, 1, 5, 6);
  __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  priv->hex_entry = __gtk_entry_new ();

  __gtk_label_set_mnemonic_widget (GTK_LABEL (label), priv->hex_entry);
  ...
  __gtk_widget_set_tooltip_text (priv->hex_entry,
                        _("You can enter an HTML-style hexadecimal color value, or simply a color name such as 'orange' in this entry."));
  
  __gtk_entry_set_width_chars (GTK_ENTRY (priv->hex_entry), 7);
  __gtk_table_attach_defaults (GTK_TABLE (table), priv->hex_entry, 1, 5, 5, 6);
  ...
  __gtk_container_set_focus_chain (GTK_CONTAINER (table), focus_chain);
  g_list_free (focus_chain);

  /* Set up the palette */
  table = __gtk_table_new (GTK_CUSTOM_PALETTE_HEIGHT, GTK_CUSTOM_PALETTE_WIDTH, TRUE);
  __gtk_table_set_row_spacings (GTK_TABLE (table), 1);
  __gtk_table_set_col_spacings (GTK_TABLE (table), 1);
  ...
  priv->palette_frame = __gtk_vbox_new (FALSE, 6);
  label = __gtk_label_new_with_mnemonic (_("_Palette:"));
  __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  __gtk_box_pack_start (GTK_BOX (priv->palette_frame), label, FALSE, FALSE, 0);

  __gtk_label_set_mnemonic_widget (GTK_LABEL (label),
                                 priv->custom_palette[0][0]);
  
  __gtk_box_pack_end (GTK_BOX (top_right_vbox), priv->palette_frame, FALSE, FALSE, 0);
  __gtk_box_pack_start (GTK_BOX (priv->palette_frame), table, FALSE, FALSE, 0);
  
  __gtk_widget_show_all (top_hbox);

  /* hide unused stuff */
  
  if (priv->has_opacity == FALSE)
    {
      __gtk_widget_hide (priv->opacity_label);
      __gtk_widget_hide (priv->opacity_slider);
      __gtk_widget_hide (priv->opacity_entry);
    }
  
  if (priv->has_palette == FALSE)
    {
      __gtk_widget_hide (priv->palette_frame);
    }

  atk_obj = __gtk_widget_get_accessible (priv->triangle_colorsel);
  if (GTK_IS_ACCESSIBLE (atk_obj))
    {
      atk_object_set_name (atk_obj, _("Color Wheel"));
      atk_object_set_role (__gtk_widget_get_accessible (GTK_WIDGET (colorsel)), ATK_ROLE_COLOR_CHOOSER);
				  ...
				  GParamSpec      *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_HAS_OPACITY_CONTROL:
      __gtk_color_selection_set_has_opacity_control (colorsel, 
						   g_value_get_boolean (value));
      break;
    case PROP_HAS_PALETTE:
      __gtk_color_selection_set_has_palette (colorsel, 
					   g_value_get_boolean (value));
      break;
    case PROP_CURRENT_COLOR:
      __gtk_color_selection_set_current_color (colorsel, g_value_get_boxed (value));
      break;
    case PROP_CURRENT_ALPHA:
      __gtk_color_selection_set_current_alpha (colorsel, g_value_get_uint (value));
      break;
    default:
				  ...
				  GParamSpec  *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_HAS_OPACITY_CONTROL:
      g_value_set_boolean (value, __gtk_color_selection_get_has_opacity_control (colorsel));
      break;
    case PROP_HAS_PALETTE:
      g_value_set_boolean (value, __gtk_color_selection_get_has_palette (colorsel));
      break;
    case PROP_CURRENT_COLOR:
      ...
      break;
    case PROP_CURRENT_ALPHA:
      g_value_set_uint (value, __gtk_color_selection_get_current_alpha (colorsel));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gtk_color_selection_realize (GtkWidget *widget)
{
  ...
  GtkColorSelectionPrivate *priv = gtk_color_selection_get_instance_private (colorsel);
		...
		gdouble        *colors)
{
  ...
  __gtk_drag_set_icon_pixbuf (context, pixbuf, -2, -2);
			  ...
			  gpointer          data)
{
  ...
  __gtk_selection_data_set (selection_data,
			  __gdk_atom_intern_static_string ("application/x-color"),
			  16, (guchar *)vals, 8);
}
...
color_sample_draw_sample (GtkColorSelection *colorsel, int which)
{
  ...
  if (!__gtk_widget_is_drawable (priv->sample_area))
    return;
...
{
  GtkColorSelectionPrivate *priv = gtk_color_selection_get_instance_private (colorsel);
  __gtk_widget_queue_draw (priv->old_sample);
  __gtk_widget_queue_draw (priv->cur_sample);
}
...
update_tooltips (GtkColorSelection *colorsel)
{
  ...
  if (priv->has_palette == TRUE)
    {
      __gtk_widget_set_tooltip_text (priv->old_sample,
      ...
      __gtk_widget_set_tooltip_text (priv->cur_sample,
...
color_sample_new (GtkColorSelection *colorsel)
{
  ...
  priv->sample_area = __gtk_hbox_new (FALSE, 0);
  priv->old_sample = __gtk_drawing_area_new ();
  priv->cur_sample = __gtk_drawing_area_new ();

  __gtk_box_pack_start (GTK_BOX (priv->sample_area), priv->old_sample,
		      TRUE, TRUE, 0);
  __gtk_box_pack_start (GTK_BOX (priv->sample_area), priv->cur_sample,
		      TRUE, TRUE, 0);
  ...
  __gtk_widget_show_all (priv->sample_area);
}
	       ...
	       gpointer      data)
{
  ...
  if (__gtk_widget_has_focus (drawing_area))
    {
			   ...
			   gint      *focus_width)
{
  ...
  __gtk_widget_style_get (drawing_area,
...
get_current_colors (GtkColorSelection *colorsel)
{
  ...
  settings = __gtk_widget_get_settings (GTK_WIDGET (colorsel));
  g_object_get (settings, "gtk-color-palette", &palette, NULL);
  
  if (!__gtk_color_selection_palette_from_string (palette, &colors, &n_colors))
    {
      __gtk_color_selection_palette_from_string (DEFAULT_COLOR_PALETTE,
                                               &colors,
                                               &n_colors);
    }
  else
                      ...
                      gdouble           *color)
{
 ...
 out:
  ...
  screen = __gtk_widget_get_screen (GTK_WIDGET (colorsel));
  if (change_palette_hook != default_change_palette_func)
    ...
    {
      if (screen != __gdk_screen_get_default ())
	g_warning ("__gtk_color_selection_set_change_palette_hook used by widget is not on the default screen.");
		   ...
		   gdouble           *color)
{
  ...
  __gtk_widget_modify_bg (drawing_area, GTK_STATE_NORMAL, &gdk_color);
  ...
  if (GPOINTER_TO_INT (g_object_get_data (G_OBJECT (drawing_area), "color_set")) == 0)
	...
	{ "application/x-color", 0 }
      ...
      __gtk_drag_source_set (drawing_area,
			   GDK_BUTTON1_MASK | GDK_BUTTON3_MASK,
			   targets, 1,
			   GDK_ACTION_COPY | GDK_ACTION_MOVE);
          ...
          guint32            timestamp)
{
  ...
  mi = __gtk_menu_item_new_with_mnemonic (_("_Save color here"));

  g_signal_connect (mi, "activate",
                    G_CALLBACK (save_color_selected),
                    drawing_area);
  
  __gtk_menu_shell_append (GTK_MENU_SHELL (menu), mi);

  __gtk_widget_show_all (mi);
	       ...
	       gpointer        data)
{
  ...
  __gtk_widget_grab_focus (drawing_area);
...
palette_new (GtkColorSelection *colorsel)
{
  ...
  __gtk_widget_set_events (retval, GDK_BUTTON_PRESS_MASK
  ...
  __gtk_widget_set_tooltip_text (retval,
...
make_picker_cursor (GdkScreen *screen)
{
  ...
  cursor = __gdk_cursor_new_from_name (__gdk_screen_get_display (screen),
				     "color-picker");
  ...
  if (!cursor)
    {
      ...
      cursor = __gdk_cursor_new_from_pixmap (pixmap, mask, &fg, &bg,
		     ...
		     gpointer   data)
{
  ...
  pixbuf = __gdk_pixbuf_get_from_drawable (NULL, root_window, NULL,
                                         x_root, y_root,
                                         0, 0,
                                         1, 1);
  if (!pixbuf)
    {
      ...
      GdkWindow *window = __gdk_display_get_window_at_pointer (display, &x, &y);
      if (!window)
	return;
      pixbuf = __gdk_pixbuf_get_from_drawable (NULL, window, NULL,
                                             x, y,
                                             0, 0,
                                             1, 1);
      if (!pixbuf)
...
shutdown_eyedropper (GtkWidget *widget)
{
  ...
  if (priv->has_grab)
    {
      __gdk_display_keyboard_ungrab (display, priv->grab_time);
      __gdk_display_pointer_ungrab (display, priv->grab_time);
      __gtk_grab_remove (priv->dropper_grab_widget);
...
mouse_motion (GtkWidget      *invisible,
	      ...
	      gpointer        data)
{
  grab_color_at_mouse (__gdk_event_get_screen ((GdkEvent *)event),
...
key_press (GtkWidget   *invisible,
           ...
           gpointer     data)
{  
  GdkDisplay *display = __gtk_widget_get_display (invisible);
  GdkScreen *screen = __gdk_event_get_screen ((GdkEvent *)event);
  guint state = event->state & __gtk_accelerator_get_default_mod_mask ();
  gint x, y;
  gint dx, dy;
  ...
  __gdk_display_warp_pointer (display, screen, x + dx, y + dy);
...
get_screen_color (GtkWidget *button)
{
  ...
  GtkColorSelectionPrivate *priv = gtk_color_selection_get_instance_private (colorsel);
  GdkScreen *screen = __gtk_widget_get_screen (GTK_WIDGET (button));
  GdkCursor *picker_cursor;
  GdkGrabStatus grab_status;
  GtkWidget *grab_widget, *toplevel;

  guint32 time = __gtk_get_current_event_time ();
  
  if (priv->dropper_grab_widget == NULL)
    {
      grab_widget = __gtk_window_new (GTK_WINDOW_POPUP);
      __gtk_window_set_screen (GTK_WINDOW (grab_widget), screen);
      __gtk_window_resize (GTK_WINDOW (grab_widget), 1, 1);
      __gtk_window_move (GTK_WINDOW (grab_widget), -100, -100);
      __gtk_widget_show (grab_widget);

      __gtk_widget_add_events (grab_widget,
                             GDK_BUTTON_RELEASE_MASK | GDK_BUTTON_PRESS_MASK | GDK_POINTER_MOTION_MASK);
      
      toplevel = __gtk_widget_get_toplevel (GTK_WIDGET (colorsel));
  ...
  grab_status = __gdk_pointer_grab (gtk_widget_get_props (priv->dropper_grab_widget)->window,
				  ...
				  picker_cursor,
				  time);
  __gdk_cursor_unref (picker_cursor);
  
  if (grab_status != GDK_GRAB_SUCCESS)
    {
      __gdk_display_keyboard_ungrab (__gtk_widget_get_display (button), time);
      return;
    }

  __gtk_grab_add (priv->dropper_grab_widget);
  priv->grab_time = time;
  priv->has_grab = TRUE;
	     ...
	     gpointer   data)
{
  ...
  text = __gtk_editable_get_chars (GTK_EDITABLE (priv->hex_entry), 0, -1);
  if (__gdk_color_parse (text, &color))
    {
	     ...
	     gpointer   data)
{
  ...
  __gtk_hsv_get_color (GTK_HSV (hsv),
		     &priv->color[COLORSEL_HUE],
		     &priv->color[COLORSEL_SATURATION],
		     &priv->color[COLORSEL_VALUE]);
		       ...
		       gpointer   data)
{
  ...
  text = __gtk_editable_get_chars (GTK_EDITABLE (priv->opacity_entry), 0, -1);
  adj = __gtk_range_get_adjustment (GTK_RANGE (priv->opacity_slider));
  __gtk_adjustment_set_value (adj, g_strtod (text, NULL)); 
                       ...
                       const gchar       *tooltip)
{
  ...
  if (channel_type == COLORSEL_HUE)
    {
      adjust = GTK_ADJUSTMENT (__gtk_adjustment_new (0.0, 0.0, 360.0, 1.0, 1.0, 0.0));
    }
  else if (channel_type == COLORSEL_SATURATION ||
	   channel_type == COLORSEL_VALUE)
    {
      adjust = GTK_ADJUSTMENT (__gtk_adjustment_new (0.0, 0.0, 100.0, 1.0, 1.0, 0.0));
    }
  else
    {
      adjust = GTK_ADJUSTMENT (__gtk_adjustment_new (0.0, 0.0, 255.0, 1.0, 1.0, 0.0));
  ...
  g_object_set_data (G_OBJECT (adjust), I_("COLORSEL"), colorsel);
  *spinbutton = __gtk_spin_button_new (adjust, 10.0, 0);

  __gtk_widget_set_tooltip_text (*spinbutton, tooltip);  

  g_signal_connect (adjust, "value-changed",
                    G_CALLBACK (adjustment_changed),
                    GINT_TO_POINTER (channel_type));
  label = __gtk_label_new_with_mnemonic (text);
  __gtk_label_set_mnemonic_widget (GTK_LABEL (label), *spinbutton);

  __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  __gtk_table_attach_defaults (GTK_TABLE (table), label, i, i+1, j, j+1);
  __gtk_table_attach_defaults (GTK_TABLE (table), *spinbutton, i+1, i+2, j, j+1);
}
		    ...
		    gint               j)
{
  ...
  __gtk_widget_set_size_request (priv->custom_palette[i][j], CUSTOM_PALETTE_ENTRY_WIDTH, CUSTOM_PALETTE_ENTRY_HEIGHT);
  __gtk_container_add (GTK_CONTAINER (frame), priv->custom_palette[i][j]);
  __gtk_table_attach_defaults (GTK_TABLE (table), frame, i, i+1, j, j+1);
}
...
set_selected_palette (GtkColorSelection *colorsel, int x, int y)
{
  ...
  __gtk_widget_grab_focus (priv->custom_palette[x][y]);
}
...
update_color (GtkColorSelection *colorsel)
{
  ...
  __gtk_hsv_set_color (GTK_HSV (priv->triangle_colorsel),
		     priv->color[COLORSEL_HUE],
		     priv->color[COLORSEL_SATURATION],
		     priv->color[COLORSEL_VALUE]);
  __gtk_adjustment_set_value (__gtk_spin_button_get_adjustment
  ...
  __gtk_adjustment_set_value (__gtk_range_get_adjustment
  ...
  __gtk_entry_set_text (GTK_ENTRY (priv->opacity_entry), opacity_text);
  ...
  for (ptr = entryval; *ptr; ptr++)
    if (*ptr == ' ')
      *ptr = '0';
  __gtk_entry_set_text (GTK_ENTRY (priv->hex_entry), entryval);
  priv->changing = FALSE;
...
default_noscreen_change_palette_func (const GdkColor *colors,
				      gint            n_colors)
{
  default_change_palette_func (__gdk_screen_get_default (), colors, n_colors);
}
                             ...
                             gint            n_colors)
{
  ...
  str = __gtk_color_selection_palette_to_string (colors, n_colors);

  __gtk_settings_set_string_property (__gtk_settings_get_for_screen (screen),
                                    "gtk-color-palette",
                                    str,
                                    "__gtk_color_selection_palette_to_string");

  g_free (str);
}

/**
 * __gtk_color_selection_new:
 * 
 ...
 **/
GtkWidget *
__gtk_color_selection_new (void)
{
  GtkColorSelection *colorsel;
  GtkColorSelectionPrivate *priv;
  gdouble color[4];
  ...
  priv = gtk_color_selection_get_instance_private (colorsel);
  set_color_internal (colorsel, color);
  __gtk_color_selection_set_has_opacity_control (colorsel, TRUE);
...
void
__gtk_color_selection_set_update_policy (GtkColorSelection *colorsel,
...
/**
 * __gtk_color_selection_get_has_opacity_control:
 ...
 **/
gboolean
__gtk_color_selection_get_has_opacity_control (GtkColorSelection *colorsel)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_set_has_opacity_control:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_set_has_opacity_control (GtkColorSelection *colorsel,
  ...
  if (priv->has_opacity != has_opacity)
    {
      ...
      if (has_opacity)
	{
	  __gtk_widget_show (priv->opacity_slider);
	  __gtk_widget_show (priv->opacity_label);
	  __gtk_widget_show (priv->opacity_entry);
	}
      else
	{
	  __gtk_widget_hide (priv->opacity_slider);
...
/**
 * __gtk_color_selection_get_has_palette:
 * @colorsel: a #GtkColorSelection.
 * 
 ...
 **/
gboolean
__gtk_color_selection_get_has_palette (GtkColorSelection *colorsel)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_set_has_palette:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_set_has_palette (GtkColorSelection *colorsel,
  ...
  if (priv->has_palette != has_palette)
    {
      ...
      if (has_palette)
	__gtk_widget_show (priv->palette_frame);
      else
	__gtk_widget_hide (priv->palette_frame);
...
/**
 * __gtk_color_selection_set_current_color:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_set_current_color (GtkColorSelection *colorsel,
...
/**
 * __gtk_color_selection_set_current_alpha:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_set_current_alpha (GtkColorSelection *colorsel,
				       guint16            alpha)
{
  GtkColorSelectionPrivate *priv;
  gint i;
...
/**
 * __gtk_color_selection_set_color:
 * @colorsel: a #GtkColorSelection.
 ...
 * also set the original color to be @color too.
 *
 * Deprecated: 2.0: Use __gtk_color_selection_set_current_color() instead.
 **/
void
__gtk_color_selection_set_color (GtkColorSelection    *colorsel,
			       gdouble              *color)
{
  g_return_if_fail (GTK_IS_COLOR_SELECTION (colorsel));
...
/**
 * __gtk_color_selection_get_current_color:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_get_current_color (GtkColorSelection *colorsel,
				       GdkColor          *color)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_get_current_alpha:
 * @colorsel: a #GtkColorSelection.
 *
 ...
 **/
guint16
__gtk_color_selection_get_current_alpha (GtkColorSelection *colorsel)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_get_color:
 * @colorsel: a #GtkColorSelection.
 * @color: an array of 4 #gdouble to fill in with the current color.
 *
 * Sets @color to be the current color in the GtkColorSelection widget.
 *
 * Deprecated: 2.0: Use __gtk_color_selection_get_current_color() instead.
 **/
void
__gtk_color_selection_get_color (GtkColorSelection *colorsel,
			       gdouble           *color)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_set_previous_color:
 * @colorsel: a #GtkColorSelection.
 ...
 * Calling __gtk_color_selection_set_current_color() will also set this color the first
 * time it is called.
 **/
void
__gtk_color_selection_set_previous_color (GtkColorSelection *colorsel,
					const GdkColor    *color)
{
  GtkColorSelectionPrivate *priv;
  ...
  __gtk_rgb_to_hsv (priv->old_color[COLORSEL_RED],
		  priv->old_color[COLORSEL_GREEN],
		  priv->old_color[COLORSEL_BLUE],
...
/**
 * __gtk_color_selection_set_previous_alpha:
 * @colorsel: a #GtkColorSelection.
 * @alpha: an integer between 0 and 65535.
 *
 ...
 **/
void
__gtk_color_selection_set_previous_alpha (GtkColorSelection *colorsel,
					guint16            alpha)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_get_previous_color:
 * @colorsel: a #GtkColorSelection.
 ...
 **/
void
__gtk_color_selection_get_previous_color (GtkColorSelection *colorsel,
					GdkColor           *color)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_get_previous_alpha:
 * @colorsel: a #GtkColorSelection.
 *
 ...
 **/
guint16
__gtk_color_selection_get_previous_alpha (GtkColorSelection *colorsel)
{
  GtkColorSelectionPrivate *priv;
...
/**
 * __gtk_color_selection_is_adjusting:
 * @colorsel: a #GtkColorSelection.
 *
 ...
 **/
gboolean
__gtk_color_selection_is_adjusting (GtkColorSelection *colorsel)
{
  GtkColorSelectionPrivate *priv;
  ...
  return (__gtk_hsv_is_adjusting (GTK_HSV (priv->triangle_colorsel)));
}


/**
 * __gtk_color_selection_palette_from_string:
 ...
 * list of color names readable by __gdk_color_parse().
 * 
 * Return value: %TRUE if a palette was successfully parsed.
 **/
gboolean
__gtk_color_selection_palette_from_string (const gchar *str,
                                         GdkColor   **colors,
                                         gint        *n_colors)
{
  GdkColor *retval;
      ...
      if (*p == ':' || *p == '\0')
        {
          ...
          if (!__gdk_color_parse (start, retval + count))
            {
              goto failed;
            }
 ...
 **/
gchar*
__gtk_color_selection_palette_to_string (const GdkColor *colors,
                                       gint            n_colors)
{
  gint i;
  gchar **strs = NULL;
...
/**
 * __gtk_color_selection_set_change_palette_hook:
 ...
 *     Use __gtk_color_selection_set_change_palette_with_screen_hook() instead. 
 * 
 **/
GtkColorSelectionChangePaletteFunc
__gtk_color_selection_set_change_palette_hook (GtkColorSelectionChangePaletteFunc func)
{
...
/**
 * __gtk_color_selection_set_change_palette_with_screen_hook:
 * @func: a function to call when the custom palette needs saving.
 * 
 * Installs a global function to be called whenever the user tries to
 * modify the palette in a color selection. This function should save
...
__gtk_color_selection_set_change_palette_with_screen_hook (GtkColorSelectionChangePaletteWithScreenFunc func)
{
                        ...
                        GtkWidget *widget)
{
  ...
  obj = __gtk_widget_get_accessible (widget);
