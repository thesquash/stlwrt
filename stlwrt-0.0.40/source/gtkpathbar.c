			   ...
			   GtkRequisition *requisition)
{
  ...
  __gtk_widget_size_request (path_bar->up_slider_button, &child_requisition);
  __gtk_widget_size_request (path_bar->down_slider_button, &child_requisition);
...
gtk_path_bar_update_slider_buttons (GtkPathBar *path_bar)
{
  if (path_bar->button_list)
	...
	{
	  gtk_path_bar_stop_scrolling (path_bar);
	  __gtk_widget_set_sensitive (path_bar->down_slider_button, FALSE);
	}
      else
	__gtk_widget_set_sensitive (path_bar->down_slider_button, TRUE);

      button = BUTTON_DATA (g_list_last (path_bar->button_list)->data)->button;
      if (__gtk_widget_get_child_visible (button))
	{
	  gtk_path_bar_stop_scrolling (path_bar);
	  __gtk_widget_set_sensitive (path_bar->up_slider_button, FALSE);
	}
      else
	__gtk_widget_set_sensitive (path_bar->up_slider_button, TRUE);
    }
}
...
gtk_path_bar_realize (GtkWidget *widget)
{
  ...
  path_bar->event_window = __gdk_window_new (__gtk_widget_get_parent_window (widget),
							&attributes, attributes_mask);
  __gdk_window_set_user_data (path_bar->event_window, widget);
...
gtk_path_bar_unrealize (GtkWidget *widget)
{
  ...
  __gdk_window_set_user_data (path_bar->event_window, NULL);
  __gdk_window_destroy (path_bar->event_window);
			    ...
			    GtkAllocation *allocation)
{
  ...
  if (__gtk_widget_get_realized (widget))
    __gdk_window_move_resize (path_bar->event_window,
  ...
  if (need_sliders || path_bar->fake_root)
    {
      ...
      __gtk_widget_size_allocate (path_bar->up_slider_button, &child_allocation);

      __gtk_widget_set_child_visible (path_bar->up_slider_button, TRUE);
      __gtk_widget_show_all (path_bar->up_slider_button);
    }
  else
    __gtk_widget_set_child_visible (path_bar->up_slider_button, FALSE);
  ...
  if (need_sliders)
	...
	child_allocation.x = allocation->width - border_width - path_bar->slider_width;
      ...
      __gtk_widget_size_allocate (path_bar->down_slider_button, &child_allocation);

      __gtk_widget_set_child_visible (path_bar->down_slider_button, TRUE);
      __gtk_widget_show_all (path_bar->down_slider_button);
      gtk_path_bar_update_slider_buttons (path_bar);
    }
  else
    __gtk_widget_set_child_visible (path_bar->down_slider_button, FALSE);
}
...
gtk_path_bar_scroll_down (GtkPathBar *path_bar)
{
  ...
  if (__gtk_widget_get_child_visible (BUTTON_DATA (path_bar->button_list->data)->button))
    {
...
gtk_path_bar_scroll_timeout (GtkPathBar *path_bar)
{
  ...
  if (path_bar->timer)
      ...
      if (path_bar->need_timer) 
	{
	  ...
	  path_bar->timer = __gdk_threads_add_timeout (timeout * SCROLL_DELAY_FACTOR,
                                    ...
                                    GtkPathBar     *path_bar)
{
  ...
  if ((!__gtk_widget_is_sensitive (path_bar->up_slider_button)) ||
      (!__gtk_widget_get_child_visible (path_bar->up_slider_button)))
                                    ...
                                    GtkPathBar     *path_bar)
{
  ...
  /* don't let the focus vanish */
  if ((!__gtk_widget_is_sensitive (path_bar->down_slider_button)) ||
      (!__gtk_widget_get_child_visible (path_bar->down_slider_button)))
  ...
  if (!path_bar->timer)
    {
      ...
      path_bar->timer = __gdk_threads_add_timeout (timeout,
				       (GSourceFunc)gtk_path_bar_scroll_timeout,
				       path_bar);
    }
...
gtk_path_bar_clear_buttons (GtkPathBar *path_bar)
{
  while (path_bar->button_list != NULL)
    {
      __gtk_container_remove (GTK_CONTAINER (path_bar), BUTTON_DATA (path_bar->button_list->data)->button);
    }
  ...
  switch (button_data->type)
    ...
    case ROOT_BUTTON:
      ...
      if (path_bar->root_icon != NULL)
        {
          __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), path_bar->root_icon);
	  break;
	}
    ...
    case HOME_BUTTON:
      if (path_bar->home_icon != NULL)
        {
	  __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), path_bar->home_icon);
	  break;
	}
    ...
    case DESKTOP_BUTTON:
      if (path_bar->desktop_icon != NULL)
        {
	  __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), path_bar->desktop_icon);
	  break;
	}
