...
gboolean
______gtk_accel_path_is_valid (const gchar *accel_path)
{
  gchar *p;
 ...
 */
void
__gtk_accel_map_add_entry (const gchar    *accel_path,
			 guint           accel_key,
			 GdkModifierType accel_mods)
{
  ...
  if (entry)
	  ...
	  (accel_key || accel_mods))
	{
	  ...
	  if (!entry->changed)
	    __gtk_accel_map_change_entry (entry->accel_path, accel_key, accel_mods, TRUE);
	}
    }
  else
    {
 ...
 */
gboolean
__gtk_accel_map_lookup_entry (const gchar *accel_path,
			    GtkAccelKey *key)
{
  AccelEntry *entry;

  g_return_val_if_fail (______gtk_accel_path_is_valid (accel_path), FALSE);
		       ...
		       gboolean	       simulate)
{
  ...
  for (slist = win_list; slist; slist = slist->next)
    for (node = __gtk_accel_groups_from_object (slist->data); node; node = node->next)
  ...
  if (accel_key)
	...
	if (______gtk_window_query_nonaccels (slist->data, accel_key, accel_mods))
	  {
	    seen_accel = TRUE;
	    break;
	  }
  ...
  if (removable)
	...
	n = 0;
	ag_entry = entry->accel_key ? __gtk_accel_group_query (group, entry->accel_key, entry->accel_mods, &n) : NULL;
	for (i = 0; i < n; i++)
	...
	n = 0;
	ag_entry = accel_key ? __gtk_accel_group_query (group, accel_key, accel_mods, &n) : NULL;
	for (i = 0; i < n; i++)
	  {
	    seen_accel = TRUE;
 ...
 break_loop_step5:
  ...
  if (change_accel && !simulate)
	...
	internal_change_entry (g_quark_to_string (GPOINTER_TO_UINT (slist->data)), 0, 0, FALSE, FALSE);
      ...
      for (slist = group_list; slist; slist = slist->next)
	______gtk_accel_group_reconnect (slist->data, g_quark_from_string (entry->accel_path));
 ...
 */
gboolean
__gtk_accel_map_change_entry (const gchar    *accel_path,
			    guint           accel_key,
			    GdkModifierType accel_mods,
			    gboolean        replace)
{
...
accel_map_parse_accel_path (GScanner *scanner)
{
  ...
  __gtk_accel_map_add_entry (path, 0, 0);

  /* and propagate it */
  __gtk_accelerator_parse (accel, &accel_key, &accel_mods);
  __gtk_accel_map_change_entry (path, accel_key, accel_mods, TRUE);
...
/**
 * __gtk_accel_map_load_scanner:
 * @scanner: a #GScanner which has already been provided with an input file
 *
 * #GScanner variant of __gtk_accel_map_load().
 */
void
__gtk_accel_map_load_scanner (GScanner *scanner)
{
...
/**
 * __gtk_accel_map_load_fd:
 * @fd: a valid readable file descriptor
 *
 * Filedescriptor variant of __gtk_accel_map_load().
 *
 * Note that the file descriptor will not be closed by this function.
 */
void
__gtk_accel_map_load_fd (gint fd)
{
  GScanner *scanner;
  ...
  __gtk_accel_map_load_scanner (scanner);

  g_scanner_destroy (scanner);
}

/**
 * __gtk_accel_map_load:
 * @file_name: a file containing accelerator specifications,
 *   in the GLib file name encoding
 *
 * Parses a file previously saved with __gtk_accel_map_save() for
 * accelerator specifications, and propagates them accordingly.
 */
void
__gtk_accel_map_load (const gchar *file_name)
{
  gint fd;
		 ...
		 gboolean        changed)
{
  ...
  name = __gtk_accelerator_name (accel_key, accel_mods);
...
/**
 * __gtk_accel_map_save_fd:
 * @fd: a valid writable file descriptor
 *
 * Filedescriptor variant of __gtk_accel_map_save().
 *
 * Note that the file descriptor will not be closed by this function.
 */
void
__gtk_accel_map_save_fd (gint fd)
{
  GString *gstring;
  ...
  __gtk_accel_map_foreach (GINT_TO_POINTER (fd), accel_map_print);
}

/**
 * __gtk_accel_map_save:
 ...
 */
void
__gtk_accel_map_save (const gchar *file_name)
{
  gint fd;
  ...
  __gtk_accel_map_save_fd (fd);

  close (fd);
}

/**
 * __gtk_accel_map_foreach:
 * @data:         data to be passed into @foreach_func
 * @foreach_func: function to be executed for each accel map entry which
 *                is not filtered out
 *
 * Loops over the entries in the accelerator map whose accel path 
 * doesn't match any of the filters added with __gtk_accel_map_add_filter(), 
 ...
 */
void
__gtk_accel_map_foreach (gpointer           data,
...
/**
 * __gtk_accel_map_foreach_unfiltered:
 * @data:         data to be passed into @foreach_func
 ...
 */
void
__gtk_accel_map_foreach_unfiltered (gpointer           data,
				  GtkAccelMapForeach foreach_func)
{
...
/**
 * __gtk_accel_map_add_filter:
 * @filter_pattern: a pattern (see #GPatternSpec)
 *
 * Adds a filter to the global list of accel path filters.
 *
 * Accel map entries whose accel path matches one of the filters
 * are skipped by __gtk_accel_map_foreach().
 *
 ...
 */
void
__gtk_accel_map_add_filter (const gchar *filter_pattern)
{
  GPatternSpec *pspec;
  GSList *slist;
...
void
______gtk_accel_map_add_group (const gchar   *accel_path,
...
void
______gtk_accel_map_remove_group (const gchar   *accel_path,
			     GtkAccelGroup *accel_group)
{
  AccelEntry *entry;
 ...
 * __gtk_accel_map_unlock_path(). Refer to __gtk_accel_map_change_entry() 
 * for information about runtime accelerator changes.
 *
 * If called more than once, @accel_path remains locked until
 * __gtk_accel_map_unlock_path() has been called an equivalent number
 * of times.
 *
 ...
 **/
void 
__gtk_accel_map_lock_path (const gchar *accel_path)
{
  AccelEntry *entry;
...
/**
 * __gtk_accel_map_unlock_path:
 * @accel_path: a valid accelerator path
 * 
 * Undoes the last call to __gtk_accel_map_lock_path() on this @accel_path.
 * Refer to __gtk_accel_map_lock_path() for information about accelerator path locking.
 *
 * Since: 2.4
 **/
void 
__gtk_accel_map_unlock_path (const gchar *accel_path)
{
  AccelEntry *entry;
...
__gtk_accel_map_load (const gchar *file_name)
{
  ...
  __gtk_accel_map_load_utf8 (utf8_file_name);

  g_free (utf8_file_name);
}

#undef __gtk_accel_map_save
...
__gtk_accel_map_save (const gchar *file_name)
{
  ...
  __gtk_accel_map_save_utf8 (utf8_file_name);
