...
#include <gtkaccellabel.h> /* For ______gtk_accel_label_class_get_accelerator_label */
#include <gtkaccelmap.h>
#include <gtkintl.h>
#include <gtkmain.h>		/* For ______gtk_boolean_handled_accumulator */
 ...
 * @See_also:__gtk_window_add_accel_group(), __gtk_accel_map_change_entry(),
 * gtk_item_factory_new(), __gtk_label_new_with_mnemonic()
 * 
 * A #GtkAccelGroup represents a group of keyboard accelerators,
 * typically attached to a toplevel #GtkWindow (with
 * __gtk_window_add_accel_group()). Usually you won't need to create a
 ...
 * __gtk_label_new_with_mnemonic(). Menu items can have both accelerators
...
gtk_accel_group_finalize (GObject *object)
{
  ...
  for (i = 0; i < accel_group->n_accels; i++)
      ...
      if (entry->accel_path_quark)
	{
	  ...
	  ______gtk_accel_map_remove_group (accel_path, accel_group);
	}
...
gtk_accel_group_init (GtkAccelGroup *accel_group)
{
  ...
  accel_group->modifier_mask = __gtk_accelerator_get_default_mod_mask ();
...
/**
 * __gtk_accel_group_new:
 * @returns: a new #GtkAccelGroup object
 * 
 * Creates a new #GtkAccelGroup. 
 */
GtkAccelGroup*
__gtk_accel_group_new (void)
{
  return g_object_new (GTK_TYPE_ACCEL_GROUP, NULL);
}

/**
 * __gtk_accel_group_get_is_locked:
 * @accel_group: a #GtkAccelGroup
 *
 * Locks are added and removed using __gtk_accel_group_lock() and
 * __gtk_accel_group_unlock().
 *
 ...
 */
gboolean
__gtk_accel_group_get_is_locked (GtkAccelGroup *accel_group)
{
...
/**
 * __gtk_accel_group_get_modifier_mask:
 * @accel_group: a #GtkAccelGroup
 *
...
void
______gtk_accel_group_attach (GtkAccelGroup *accel_group,
			 GObject       *object)
{
  GSList *slist;
...
void
______gtk_accel_group_detach (GtkAccelGroup *accel_group,
			 GObject       *object)
{
  GSList *slist;
...
/**
 * __gtk_accel_groups_from_object:
 ...
 */
GSList*
__gtk_accel_groups_from_object (GObject *object)
{
 ...
 */
GtkAccelKey*
__gtk_accel_group_find (GtkAccelGroup        *accel_group,
 ...
 * __gtk_accel_map_change_entry() about runtime accelerator changes.
 *
 * If called more than once, @accel_group remains locked until
 * __gtk_accel_group_unlock() has been called an equivalent number
 * of times.
 */
void
__gtk_accel_group_lock (GtkAccelGroup *accel_group)
{
  g_return_if_fail (GTK_IS_ACCEL_GROUP (accel_group));
 ...
 * @accel_group: a #GtkAccelGroup
 * 
 * Undoes the last call to __gtk_accel_group_lock() on this @accel_group.
 */
void
__gtk_accel_group_unlock (GtkAccelGroup *accel_group)
{
  g_return_if_fail (GTK_IS_ACCEL_GROUP (accel_group));
			  ...
			  GClosure *closure)
{
  ...
  __gtk_accel_group_disconnect (accel_group, closure);
}
		 ...
		 GQuark          path_quark)
{
  ...
  if (path_quark)
    ______gtk_accel_map_add_group (g_quark_to_string (path_quark), accel_group);

  /* connect and notify changed */
  if (accel_key)
    {
      gchar *accel_name = __gtk_accelerator_name (accel_key, accel_mods);
                    ...
                    guint               pos)
{
  ...
  if (entry->accel_path_quark)
    ______gtk_accel_map_remove_group (g_quark_to_string (entry->accel_path_quark), accel_group);
 ...
 * in response to a call to __gtk_accel_groups_activate(), @closure will be
 * invoked if the @accel_key and @accel_mods from __gtk_accel_groups_activate()
 ...
 */
void
__gtk_accel_group_connect (GtkAccelGroup	*accel_group,
			 guint		 accel_key,
			 GdkModifierType accel_mods,
			 GtkAccelFlags	 accel_flags,
			 GClosure	*closure)
{
  ...
  g_return_if_fail (__gtk_accel_group_from_accel_closure (closure) == NULL);

  g_object_ref (accel_group);
  if (!closure->is_invalid)
    quick_accel_add (accel_group,
		     __gdk_keyval_to_lower (accel_key),
		     accel_mods, accel_flags, closure, 0);
  g_object_unref (accel_group);
}

/**
 * __gtk_accel_group_connect_by_path:
 * @accel_group:      the accelerator group to install an accelerator in
 ...
 * up the appropriate key and modifiers (see __gtk_accel_map_add_entry()).
 * When @accel_group is being activated in response to a call to
 * __gtk_accel_groups_activate(), @closure will be invoked if the @accel_key and
 * @accel_mods from __gtk_accel_groups_activate() match the key and modifiers
 * for the path.
 *
 * The signature used for the @closure is that of #GtkAccelGroupActivate.
 * 
 ...
 */
void
__gtk_accel_group_connect_by_path (GtkAccelGroup	*accel_group,
				 ...
				 GClosure	*closure)
{
  ...
  if (__gtk_accel_map_lookup_entry (accel_path, &key))
    {
      accel_key = __gdk_keyval_to_lower (key.accel_key);
 ...
 */
gboolean
__gtk_accel_group_disconnect (GtkAccelGroup *accel_group,
			    GClosure      *closure)
{
  guint i;
...
/**
 * __gtk_accel_group_disconnect_key:
 * @accel_group:      the accelerator group to install an accelerator in
 * @accel_key:        key value of the accelerator
 * @accel_mods:       modifier combination of the accelerator
 ...
 */
gboolean
__gtk_accel_group_disconnect_key (GtkAccelGroup  *accel_group,
				guint	        accel_key,
				GdkModifierType accel_mods)
{
  ...
  for (slist = clist; slist; slist = slist->next)
    {
      ...
      removed_one |= __gtk_accel_group_disconnect (accel_group, closure);
      g_closure_unref (closure);
    }
  g_slist_free (clist);
...
void
______gtk_accel_group_reconnect (GtkAccelGroup *accel_group,
  ...
  for (slist = clist; slist; slist = slist->next)
    {
      ...
      __gtk_accel_group_disconnect (accel_group, closure);
      __gtk_accel_group_connect_by_path (accel_group, g_quark_to_string (accel_path_quark), closure);
      g_closure_unref (closure);
    }
  g_slist_free (clist);
 ...
 */
GtkAccelGroupEntry*
__gtk_accel_group_query (GtkAccelGroup  *accel_group,
		       guint           accel_key,
		       GdkModifierType accel_mods,
		       guint          *n_entries)
{
  ...
  entries = quick_accel_find (accel_group, __gdk_keyval_to_lower (accel_key), accel_mods, &n);
 ...
 * see __gtk_accel_group_connect().
 */
GtkAccelGroup*
__gtk_accel_group_from_accel_closure (GClosure *closure)
{
  guint i;
 ...
 */
gboolean
__gtk_accel_group_activate (GtkAccelGroup   *accel_group,
                          GQuark	   accel_quark,
 ...
 */
gboolean
__gtk_accel_groups_activate (GObject	  *object,
			   guint	   accel_key,
			   GdkModifierType accel_mods)
{
  g_return_val_if_fail (G_IS_OBJECT (object), FALSE);
  
  if (__gtk_accelerator_valid (accel_key, accel_mods))
    {
      gchar *accel_name;
      GQuark accel_quark;
      GSList *slist;

      accel_name = __gtk_accelerator_name (accel_key, (accel_mods & __gtk_accelerator_get_default_mod_mask ()));
      accel_quark = g_quark_from_string (accel_name);
      g_free (accel_name);
      
      for (slist = __gtk_accel_groups_from_object (object); slist; slist = slist->next)
	if (__gtk_accel_group_activate (slist->data, accel_quark, object, accel_key, accel_mods))
	  return TRUE;
    }
 ...
 */
gboolean
__gtk_accelerator_valid (guint		  keyval,
 ...
 * Key names are parsed using __gdk_keyval_from_name(). For character keys the
 ...
 */
void
__gtk_accelerator_parse (const gchar     *accelerator,
		       ...
		       GdkModifierType *accelerator_mods)
{
  ...
  while (len)
	...
	{
	  keyval = __gdk_keyval_from_name (accelerator);
	  accelerator += len;
	  len -= len;
	}
    }
  ...
  if (accelerator_key)
    *accelerator_key = __gdk_keyval_to_lower (keyval);
  if (accelerator_mods)
    *accelerator_mods = mods;
}

/**
 * __gtk_accelerator_name:
 * @accelerator_key:  accelerator keyval
 * @accelerator_mods: accelerator modifier mask
 * 
 * Converts an accelerator keyval and modifier mask
 * into a string parseable by __gtk_accelerator_parse().
 * For example, if you pass in #GDK_q and #GDK_CONTROL_MASK,
 * this function returns "&lt;Control&gt;q". 
 *
 * If you need to display accelerators in the user interface,
 * see __gtk_accelerator_get_label().
 *
 * Returns: a newly-allocated accelerator name
 */
gchar*
__gtk_accelerator_name (guint           accelerator_key,
		      GdkModifierType accelerator_mods)
{
  ...
  keyval_name = __gdk_keyval_name (__gdk_keyval_to_lower (accelerator_key));
  if (!keyval_name)
    keyval_name = "";
 ...
 */
gchar*
__gtk_accelerator_get_label (guint           accelerator_key,
			   GdkModifierType accelerator_mods)
{
  ...
  label = ______gtk_accel_label_class_get_accelerator_label (klass, 
							accelerator_key, 
							accelerator_mods);
...
/**
 * __gtk_accelerator_set_default_mod_mask:
 ...
 */
void
__gtk_accelerator_set_default_mod_mask (GdkModifierType default_mod_mask)
{
 ...
 *
 * Gets the value set by __gtk_accelerator_set_default_mod_mask().
 */
guint
__gtk_accelerator_get_default_mod_mask (void)
{
