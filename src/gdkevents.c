...
/**
 * __gdk_event_handler_set:
 * @func: the function to call to handle events from GDK.
 * @data: user data to pass to the function. 
 * @notify: the function to call when the handler function is removed, i.e. when
 *          __gdk_event_handler_set() is called with another event handler.
 * 
 * Sets the function to call to handle all events from GDK.
 *
 * Note that GTK+ uses this to install its own event handler, so it is
 * usually not useful for GTK+ applications. (Although an application
 * can call this function then call __gtk_main_do_event() to pass
 * events to GTK+.)
 **/
void 
__gdk_event_handler_set (GdkEventFunc   func,
		       gpointer       data,
		       GDestroyNotify notify)
{
 ...
 **/
GdkEvent*
__gdk_event_get (void)
{
  GSList *tmp_list;

  for (tmp_list = _gdk_displays; tmp_list; tmp_list = tmp_list->next)
    {
      GdkEvent *event = __gdk_display_get_event (tmp_list->data);
      if (event)
	return event;
    }
 ...
 * display, returns a copy of it. See __gdk_display_peek_event().
 * 
 ...
 **/
GdkEvent*
__gdk_event_peek (void)
{
  GSList *tmp_list;

  for (tmp_list = _gdk_displays; tmp_list; tmp_list = tmp_list->next)
    {
      GdkEvent *event = __gdk_display_peek_event (tmp_list->data);
      if (event)
	return event;
    }
 ...
 * queue if event->any.window is %NULL. See __gdk_display_put_event().
 **/
void
__gdk_event_put (const GdkEvent *event)
{
  GdkDisplay *display;
  ...
  if (event->any.window)
    ...
    {
      GDK_NOTE (MULTIHEAD,
		g_message ("Falling back to default display for __gdk_event_put()"));
      display = __gdk_display_get_default ();
    }

  __gdk_display_put_event (display, event);
}
 ...
 * should be freed with __gdk_event_free().
 *
 * Since: 2.2
 **/
GdkEvent*
__gdk_event_new (GdkEventType type)
{
 ...
 **/
GdkEvent*
__gdk_event_copy (const GdkEvent *event)
{
  GdkEventPrivate *new_private;
  GdkEvent *new_event;
  ...
  new_event = __gdk_event_new (GDK_NOTHING);
  ...
  *new_event = *event;
  ...
  switch (event->any.type)
	...
	new_event->expose.region = __gdk_region_copy (event->expose.region);
      break;
 ...
 * functions such as __gdk_event_peek(), __gdk_event_get(),
 * __gdk_event_get_graphics_expose() and __gdk_event_copy() and __gdk_event_new().
 **/
void
__gdk_event_free (GdkEvent *event)
{
  g_return_if_fail (event != NULL);
  ...
  switch (event->any.type)
    ...
    case GDK_DAMAGE:
      if (event->expose.region)
	__gdk_region_destroy (event->expose.region);
      break;
 ...
 **/
guint32
__gdk_event_get_time (const GdkEvent *event)
{
  if (event)
    switch (event->type)
      {
 ...
 **/
gboolean
__gdk_event_get_state (const GdkEvent        *event,
 ...
 **/
gboolean
__gdk_event_get_coords (const GdkEvent *event,
		      gdouble        *x_win,
		      gdouble        *y_win)
{
  gdouble x = 0, y = 0;
 ...
 **/
gboolean
__gdk_event_get_root_coords (const GdkEvent *event,
			   gdouble        *x_root,
			   gdouble        *y_root)
{
  gdouble x = 0, y = 0;
 ...
 **/
gboolean
__gdk_event_get_axis (const GdkEvent *event,
		    GdkAxisUse      axis_use,
		    gdouble        *value)
{
  gdouble *axes;
  ...
  return __gdk_device_get_axis (device, axes, axis_use, value);
}

/**
 * __gdk_event_request_motions:
 * @event: a valid #GdkEvent
 *
 * Request more motion notifies if @event is a motion notify hint event.
 * This function should be used instead of __gdk_window_get_pointer() to
 ...
 *   __gdk_event_request_motions (motion_event); /&ast; handles is_hint events &ast;/
 * }
 * ]|
 *
 * Since: 2.12
 **/
void
__gdk_event_request_motions (const GdkEventMotion *event)
{
  GdkDisplay *display;
 ...
 **/
void
__gdk_event_set_screen (GdkEvent  *event,
		      GdkScreen *screen)
{
 ...
 **/
GdkScreen *
__gdk_event_get_screen (const GdkEvent *event)
{
  if (gdk_event_is_allocated (event))
    {
      GdkEventPrivate *private = (GdkEventPrivate *)event;
 ...
 **/
void
__gdk_set_show_events (gboolean show_events)
{
  if (show_events)
 ...
 */
gint
__gdk_input_add_full (gint	      source,
		    GdkInputCondition condition,
		    GdkInputFunction  function,
		    gpointer	      data,
 ...
 */
gint
__gdk_input_add (gint		 source,
	       GdkInputCondition condition,
	       GdkInputFunction	 function,
	       gpointer		 data)
{
  return __gdk_input_add_full (source, condition, function, data, NULL);
}

void
__gdk_input_remove (gint tag)
{
  g_source_remove (tag);
}
...
/**
 * __gdk_display_set_double_click_time:
 * @display: a #GdkDisplay
 ...
 **/
void
__gdk_display_set_double_click_time (GdkDisplay *display,
				   guint       msec)
{
...
/**
 * __gdk_set_double_click_time:
 * @msec: double click time in milliseconds (thousandths of a second)
 *
 * Set the double click time for the default display. See
 * __gdk_display_set_double_click_time(). 
 * See also __gdk_display_set_double_click_distance().
 * Applications should <emphasis>not</emphasis> set this, it is a 
 * global user-configured setting.
 **/
void
__gdk_set_double_click_time (guint msec)
{
  __gdk_display_set_double_click_time (__gdk_display_get_default (), msec);
}

/**
 * __gdk_display_set_double_click_distance:
 * @display: a #GdkDisplay
 * @distance: distance in pixels
 * 
 * Sets the double click distance (two clicks within this distance
 * count as a double click and result in a #GDK_2BUTTON_PRESS event).
 * See also __gdk_display_set_double_click_time().
 * Applications should <emphasis>not</emphasis> set this, it is a global 
 * user-configured setting.
 *
 * Since: 2.4
 **/
void
__gdk_display_set_double_click_distance (GdkDisplay *display,
				       guint       distance)
{
 ...
 * for the default screen. See __gdk_screen_get_setting().
 *
 * Returns: %TRUE if the setting existed and a value was stored
 *   in @value, %FALSE otherwise.
 **/
gboolean
__gdk_setting_get (const gchar *name,
		 GValue      *value)
{
