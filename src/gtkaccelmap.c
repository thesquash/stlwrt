...
gboolean
___gtk_accel_path_is_valid (const gchar *accel_path)
{
  gchar *p;
			    ...
			    GtkAccelKey *key)
{
  ...
  g_return_val_if_fail (___gtk_accel_path_is_valid (accel_path), FALSE);
		       ...
		       gboolean	       simulate)
{
  ...
  if (accel_key)
	...
	if (___gtk_window_query_nonaccels (slist->data, accel_key, accel_mods))
	  {
	    seen_accel = TRUE;
	    break;
	  }
 ...
 break_loop_step5:
  ...
  if (change_accel && !simulate)
	...
	internal_change_entry (g_quark_to_string (GPOINTER_TO_UINT (slist->data)), 0, 0, FALSE, FALSE);
      ...
      for (slist = group_list; slist; slist = slist->next)
	___gtk_accel_group_reconnect (slist->data, g_quark_from_string (entry->accel_path));
...
void
___gtk_accel_map_add_group (const gchar   *accel_path,
...
void
___gtk_accel_map_remove_group (const gchar   *accel_path,
			     GtkAccelGroup *accel_group)
{
  AccelEntry *entry;
