...
static GHashTable*
__gtk_gc_get_drawable_ht (GdkScreen *screen)
{
...
gtk_gc_new (gpointer key)
{
  ...
  keyval = key;
  screen = __gdk_colormap_get_screen (keyval->colormap);
  
  ht = __gtk_gc_get_drawable_ht (screen);
...
gtk_gc_key_hash (gpointer key)
{
  ...
  if (keyval->mask & GDK_GC_FONT)
    {
      hash_val += __gdk_font_id (keyval->values.font);
    }
		  ...
		  gpointer b)
{
  ...
  if (akey->mask & GDK_GC_FONT)
    {
      if (!__gdk_font_equal (avalues->font, bvalues->font))
	return FALSE;
    }
