                          ...
                          GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_TRANSLATION_DOMAIN:
      __gtk_builder_set_translation_domain (builder, g_value_get_string (value));
      break;
    default:
  ...
  for (l = properties; l; l = l->next)
    {
      ...
      if (G_IS_PARAM_SPEC_OBJECT (pspec) &&
        ...
        {
          GObject *object = __gtk_builder_get_object (builder, prop->data);
      ...
      else if (!__gtk_builder_value_from_string (builder, pspec,
			...
			GError **error)
{
  ...
  if (info->constructor)
    {
      ...
      constructor = __gtk_builder_get_object (builder, info->constructor);
      if (constructor == NULL)
	{
	  g_set_error (error,
		       GTK_BUILDER_ERROR,
      ...
      obj = __gtk_buildable_construct_child (GTK_BUILDABLE (constructor),
                                           builder,
                                           info->id);
      g_assert (obj != NULL);
  ...
  if (GTK_IS_BUILDABLE (obj))
    __gtk_buildable_set_name (buildable, info->id);
  else
 ...
 **/
GtkBuilder *
__gtk_builder_new (void)
{
  return g_object_new (GTK_TYPE_BUILDER, NULL);
}

/**
 * __gtk_builder_add_from_file:
 ...
 **/
guint
__gtk_builder_add_from_file (GtkBuilder   *builder,
...
/**
 * __gtk_builder_add_objects_from_file:
 * @builder: a #GtkBuilder
 * @filename: the name of the file to parse
 ...
 **/
guint
__gtk_builder_add_objects_from_file (GtkBuilder   *builder,
                                   const gchar  *filename,
                                   gchar       **object_ids,
                                   GError      **error)
{
 ...
 **/
guint
__gtk_builder_add_from_string (GtkBuilder   *builder,
                             const gchar  *buffer,
                             gsize         length,
                             GError      **error)
{
...
/**
 * __gtk_builder_add_objects_from_string:
 * @builder: a #GtkBuilder
 * @buffer: the string to parse
 * @length: the length of @buffer (may be -1 if @buffer is nul-terminated)
 * @object_ids: nul-terminated array of objects to build
 ...
 **/
guint
__gtk_builder_add_objects_from_string (GtkBuilder   *builder,
                                     const gchar  *buffer,
                                     gsize         length,
                                     gchar       **object_ids,
                                     GError      **error)
 ...
 **/
GObject *
__gtk_builder_get_object (GtkBuilder  *builder,
                        const gchar *name)
{
 ...
 **/
GSList *
__gtk_builder_get_objects (GtkBuilder *builder)
{
  GSList *objects = NULL;
...
/**
 * __gtk_builder_set_translation_domain:
 * @builder: a #GtkBuilder
 ...
 **/
void
__gtk_builder_set_translation_domain (GtkBuilder  *builder,
                                    const gchar *domain)
{
  gchar *new_domain;
...
/**
 * __gtk_builder_get_translation_domain:
 * @builder: a #GtkBuilder
 *
 ...
 **/
const gchar *
__gtk_builder_get_translation_domain (GtkBuilder *builder)
{
  g_return_val_if_fail (GTK_IS_BUILDER (builder), NULL);
...
static void
__gtk_builder_connect_signals_default (GtkBuilder    *builder,
				     GObject       *object,
...
/**
 * __gtk_builder_connect_signals:
 * @builder: a #GtkBuilder
 * @user_data: a pointer to a structure sent in as user data to all signals
 *
 * This method is a simpler variation of __gtk_builder_connect_signals_full().
 ...
 **/
void
__gtk_builder_connect_signals (GtkBuilder *builder,
			     gpointer    user_data)
{
  connect_args *args;
  ...
  if (!g_module_supported ())
    g_error ("__gtk_builder_connect_signals() requires working GModule");

  args = g_slice_new0 (connect_args);
  args->module = g_module_open (NULL, G_MODULE_BIND_LAZY);
  args->data = user_data;
  
  __gtk_builder_connect_signals_full (builder,
                                    __gtk_builder_connect_signals_default,
                                    args);
 ...
 * by the __gtk_builder_connect_signals() and __gtk_builder_connect_signals_full()
...
/**
 * __gtk_builder_connect_signals_full:
 * @builder: a #GtkBuilder
 ...
 * version of __gtk_builder_connect_signals(), except that it does not
 * require GModule to function correctly.
 *
 * Since: 2.12
 */
void
__gtk_builder_connect_signals_full (GtkBuilder            *builder,
...
/**
 * __gtk_builder_value_from_string:
 * @builder: a #GtkBuilder
 ...
 */
gboolean
__gtk_builder_value_from_string (GtkBuilder   *builder,
			       GParamSpec   *pspec,
                               const gchar  *string,
                               GValue       *value,
			       GError      **error)
{
  ...
  return __gtk_builder_value_from_string_type (builder,
					     G_PARAM_SPEC_VALUE_TYPE (pspec),
                                             string, value, error);
}

/**
 * __gtk_builder_value_from_string_type:
 * @builder: a #GtkBuilder
 ...
 * @error: (allow-none): return location for an error, or %NULL
 *
 * Like __gtk_builder_value_from_string(), this function demarshals 
 ...
 */
gboolean
__gtk_builder_value_from_string_type (GtkBuilder   *builder,
				    GType         type,
                                    const gchar  *string,
                                    GValue       *value,
				    GError      **error)
{
      ...
      if (G_VALUE_HOLDS (value, GDK_TYPE_COLOR))
        {
          ...
          if (__gdk_color_parse (string, &colour) &&
              __gdk_colormap_alloc_color (__gtk_widget_get_default_colormap (),
                                        &colour, FALSE, TRUE))
      ...
      if (G_VALUE_HOLDS (value, GDK_TYPE_PIXBUF))
        {
          ...
          if (__gtk_builder_get_object (builder, string))
            {
              g_set_error (error,
                           GTK_BUILDER_ERROR,
                           GTK_BUILDER_ERROR_INVALID_VALUE,
   ...
   * without calling __gtk_builder_value_from_string_type().
   */
...
GQuark
__gtk_builder_error_quark (void)
{
