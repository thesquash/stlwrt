...
STLWRT_DEFINE_FTYPE (GdkKeymap, gdk_keymap, G_TYPE_OBJECT, 0, ;)
...
gdk_keymap_class_init (GdkKeymapClass *klass)
{
   ...
   * See __gdk_keymap_get_caps_lock_state().
   *
   * Since: 2.16
   */
			 ...
			 guint *upper)
{
  ...
  if ((symbol & 0xff000000) == 0x01000000)
    {
      if (lower)
	*lower = __gdk_unicode_to_keyval (g_unichar_tolower (symbol & 0x00ffffff));
      if (upper)
	*upper = __gdk_unicode_to_keyval (g_unichar_toupper (symbol & 0x00ffffff));
      return;
    }
...
guint
__gdk_keyval_to_upper (guint keyval)
{
  guint result;
  
  __gdk_keyval_convert_case (keyval, NULL, &result);

  return result;
}

guint
__gdk_keyval_to_lower (guint keyval)
{
  guint result;
  
  __gdk_keyval_convert_case (keyval, &result, NULL);

  return result;
}

gboolean
__gdk_keyval_is_upper (guint keyval)
{
  if (keyval)
    {
      guint upper_val = 0;
      
      __gdk_keyval_convert_case (keyval, NULL, &upper_val);
...
gboolean
__gdk_keyval_is_lower (guint keyval)
{
  if (keyval)
    {
      guint lower_val = 0;
      
      __gdk_keyval_convert_case (keyval, &lower_val, NULL);
...
/** 
 * __gdk_keymap_get_default:
 * @returns: the #GdkKeymap attached to the default display.
 *
 * Returns the #GdkKeymap attached to the default display.
 **/
GdkKeymap*
__gdk_keymap_get_default (void)
{
