	       ...
	       GtkWidget *proxy)
{
  ...
  if (action->private_data->action_group)
    ___gtk_action_group_emit_connect_proxy (action->private_data->action_group, action, proxy);
		  ...
		  GtkWidget *proxy)
{
  ...
  if (action->private_data->action_group)
    ___gtk_action_group_emit_disconnect_proxy (action->private_data->action_group, action, proxy);
}

/**
 * ___gtk_action_sync_menu_visible:
 ...
 **/
void
___gtk_action_sync_menu_visible (GtkAction *action,
			       GtkWidget *proxy,
			       gboolean   empty)
{
...
void
___gtk_action_emit_activate (GtkAction *action)
{
  ...
  if (group != NULL)
    {
      g_object_ref (action);
      g_object_ref (group);
      ___gtk_action_group_emit_pre_activate (group, action);
    }
  ...
  if (group != NULL)
    {
      ___gtk_action_group_emit_post_activate (group, action);
...
__gtk_action_activate (GtkAction *action)
{
  ...
  if (__gtk_action_is_sensitive (action))
    ___gtk_action_emit_activate (action);
}
...
void
___gtk_action_add_to_proxy_list (GtkAction     *action,
			       GtkWidget     *proxy)
{
  g_return_if_fail (GTK_IS_ACTION (action));
...
void
___gtk_action_remove_from_proxy_list (GtkAction     *action,
				    GtkWidget     *proxy)
{
  g_return_if_fail (GTK_IS_ACTION (action));
  g_return_if_fail (GTK_IS_WIDGET (proxy));
                        ...
                        gpointer      marshal_data)
{
  if (__gtk_action_is_sensitive (GTK_ACTION (closure->data)))
    {
      ___gtk_action_emit_activate (GTK_ACTION (closure->data));
