...
/* Color usage */
#define HEADER_FG_COLOR(widget)		 (& (gtk_widget_get_props (widget))->style->fg[__gtk_widget_get_state (widget)])
#define HEADER_BG_COLOR(widget)		 (& (gtk_widget_get_props (widget))->style->bg[__gtk_widget_get_state (widget)])
#define SELECTED_BG_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->base[__gtk_widget_has_focus (widget) ? GTK_STATE_SELECTED : GTK_STATE_ACTIVE])
#define SELECTED_FG_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->text[__gtk_widget_has_focus (widget) ? GTK_STATE_SELECTED : GTK_STATE_ACTIVE])
#define NORMAL_DAY_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->text[__gtk_widget_get_state (widget)])
#define PREV_MONTH_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->mid[__gtk_widget_get_state (widget)])
#define NEXT_MONTH_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->mid[__gtk_widget_get_state (widget)])
#define MARKED_COLOR(widget)		 (& (gtk_widget_get_props (widget))->style->text[__gtk_widget_get_state (widget)])
#define BACKGROUND_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->base[__gtk_widget_get_state (widget)])
#define HIGHLIGHT_BACK_COLOR(widget)	 (& (gtk_widget_get_props (widget))->style->mid[__gtk_widget_get_state (widget)])
...
static void calendar_invalidate_day     (GtkCalendar *calendar,
					 gint       row,
					 gint       col);
static void calendar_invalidate_day_num (GtkCalendar *calendar,
					 gint       day);
static void calendar_invalidate_arrow   (GtkCalendar *calendar,
					 guint      arrow);
 ...
 * A value of 0 allows any width. See __gtk_calendar_set_detail_func().
 *
 * Since: 2.14
 */
  g_object_class_install_property (gobject_class,
...
gtk_calendar_init (GtkCalendar *calendar)
{
  ...
  priv = gtk_calendar_get_instance_private (calendar);
  ...
  __gtk_drag_dest_set (widget, 0, NULL, 0, GDK_ACTION_COPY);
  __gtk_drag_dest_add_text_targets (widget);
...
calendar_set_month_next (GtkCalendar *calendar)
{
  ...
  if (month_len < gtk_calendar_get_props (calendar)->selected_day)
    ...
    __gtk_calendar_select_day (calendar, gtk_calendar_get_props (calendar)->selected_day);
...
{
  GtkCalendarPrivate *priv = gtk_calendar_get_instance_private (GTK_WIDGET (calendar));
  gint month;
  gint year;
  gint ndays_in_month;
...
{
  return (gtk_calendar_get_instance_private (calendar)->main_h - CALENDAR_MARGIN
			    ...
			    gint	 column)
{
  ...
  if (__gtk_widget_get_direction (GTK_WIDGET (calendar)) == GTK_TEXT_DIR_RTL)
    column = 6 - column;

  width = gtk_calendar_get_instance_private (calendar)->day_width;
  ...
  for (c = 0; c < 7; c++)
    {
      ...
      x_right = x_left + gtk_calendar_get_instance_private (calendar)->day_width;
			...
			gint	     row)
{
  ...
  return (gtk_calendar_get_instance_private (calendar)->main_h 
			   ...
			   GParamSpec   *pspec)
{
  ...
  switch (prop_id) 
    ...
    case PROP_DETAIL_WIDTH_CHARS:
      __gtk_calendar_set_detail_width_chars (calendar,
                                           g_value_get_int (value));
      break;
    case PROP_DETAIL_HEIGHT_ROWS:
      __gtk_calendar_set_detail_height_rows (calendar,
                                           g_value_get_int (value));
      break;
    default:
...
gtk_calendar_get_property (GObject      *object,
			   ...
			   GParamSpec   *pspec)
{
  GtkCalendarPrivate *priv = gtk_calendar_get_instance_private (object);
...
calendar_realize_arrows (GtkCalendar *calendar)
{
  ...
  if (! (gtk_calendar_get_props (calendar)->display_flags & GTK_CALENDAR_NO_MONTH_CHANGE)
      ...
      for (i = 0; i < 4; i++)
	{
	  ...
	  priv->arrow_win[i] = __gdk_window_new (priv->header_win,
					       &attributes, 
					       attributes_mask);
	  ...
	  __gdk_window_set_background (priv->arrow_win[i],
				     HEADER_BG_COLOR (GTK_WIDGET (calendar)));
	  __gdk_window_show (priv->arrow_win[i]);
	  __gdk_window_set_user_data (priv->arrow_win[i], widget);
	}
    }
  else
    {
  ...
  if (gtk_calendar_get_props (calendar)->display_flags & GTK_CALENDAR_SHOW_HEADING)
    {
      ...
      __gdk_window_set_background (priv->header_win,
				 HEADER_BG_COLOR (GTK_WIDGET (calendar)));
      __gdk_window_show (priv->header_win);
      __gdk_window_set_user_data (priv->header_win, widget);
...
calendar_get_inner_border (GtkCalendar *calendar)
{
  ...
  __gtk_widget_style_get (GTK_WIDGET (calendar),
  ...
  if ( gtk_calendar_get_props (calendar)->display_flags & GTK_CALENDAR_SHOW_DAY_NAMES)
    {
      ...
      __gdk_window_show (priv->day_name_win);
      __gdk_window_set_user_data (priv->day_name_win, widget);
    }
  else
    {
  ...
  if (gtk_calendar_get_props (calendar)->display_flags & GTK_CALENDAR_SHOW_WEEK_NUMBERS)
	...
	attributes.x = gtk_widget_get_props (widget)->allocation.width - priv->week_width - (gtk_widget_get_props (widget)->style->xthickness + inner_border);
      ...
      __gdk_window_set_background (priv->week_win,  
				 BACKGROUND_COLOR (GTK_WIDGET (calendar)));
      __gdk_window_show (priv->week_win);
      __gdk_window_set_user_data (priv->week_win, widget);
    } 
  else
    {
      priv->week_win = NULL;
...
gtk_calendar_realize (GtkWidget *widget)
{
  ...
  GtkCalendarPrivate *priv = gtk_calendar_get_instance_private (widget);
  GdkWindowAttr attributes;
  gint attributes_mask;
  gint inner_border = calendar_get_inner_border (calendar);
...
gtk_calendar_unrealize (GtkWidget *widget)
{
  ...
  if (priv->header_win)
      ...
      for (i = 0; i < 4; i++)
	{
	  if (priv->arrow_win[i])
	    {
	      __gdk_window_set_user_data (priv->arrow_win[i], NULL);
	      __gdk_window_destroy (priv->arrow_win[i]);
	      priv->arrow_win[i] = NULL;
	    }
	}
      __gdk_window_set_user_data (priv->header_win, NULL);
      __gdk_window_destroy (priv->header_win);
      priv->header_win = NULL;
    }
  
  if (priv->week_win)
    {
      __gdk_window_set_user_data (priv->week_win, NULL);
      __gdk_window_destroy (priv->week_win);
      priv->week_win = NULL;      
    }
  
  if (priv->main_win)
    {
      __gdk_window_set_user_data (priv->main_win, NULL);
      __gdk_window_destroy (priv->main_win);
      priv->main_win = NULL;      
    }
  if (priv->day_name_win)
    {
      __gdk_window_set_user_data (priv->day_name_win, NULL);
      __gdk_window_destroy (priv->day_name_win);
      priv->day_name_win = NULL;      
    }
                            ...
                            GtkTooltip *tooltip)
{
  ...
  if (priv->main_win)
    {
      ...
      __gdk_window_get_position (priv->main_win, &x0, &y0);
  ...
  if (detail)
    {
      __gtk_tooltip_set_tip_area (tooltip, &day_rect);
      __gtk_tooltip_set_markup (tooltip, detail);
  ...
  if (__gtk_widget_get_realized (widget))
    {
      ...
      if (priv->header_win)
	__gdk_window_move_resize (priv->header_win,
	  ...
	  if (priv->arrow_win[i])
	    {
	      ...
	      __gdk_window_move_resize (priv->arrow_win[i],
...
calendar_paint_header (GtkCalendar *calendar)
{
  ...
  cr = __gdk_cairo_create ((GdkDrawable *) (priv->header_win));
  ...
  layout = __gtk_widget_create_pango_layout (widget, str);
  g_free (str);
  ...
  __gdk_cairo_set_source_color (cr, HEADER_FG_COLOR (GTK_WIDGET (calendar)));
...
calendar_paint_day_names (GtkCalendar *calendar)
{
  ...
  cr = __gdk_cairo_create ((GdkDrawable *) (priv->day_name_win));
  ...
  __gdk_cairo_set_source_color (cr, SELECTED_FG_COLOR (widget));
  for (i = 0; i < 7; i++)
    {
      if (__gtk_widget_get_direction (GTK_WIDGET (calendar)) == GTK_TEXT_DIR_RTL)
	day = 6 - i;
...
calendar_paint_week_numbers (GtkCalendar *calendar)
{
  ...
  cr = __gdk_cairo_create ((GdkDrawable *) (priv->week_win));
  ...
  if (priv->main_win)
    {
      ...
      calendar_day_rectangle (calendar, row, col, &day_rect);
      __gdk_window_invalidate_rect (priv->main_win, &day_rect, FALSE);
    }
}
		    ...
		    gint	     col)
{
  ...
  cr = __gdk_cairo_create ((GdkDrawable *) (priv->main_win));
  ...
  layout = __gtk_widget_create_pango_layout (widget, buffer);
			   ...
			   guint        arrow)
{
  ...
  if (window)
    __gdk_window_invalidate_rect (window, NULL, FALSE);
}
...
calendar_timer (gpointer data)
{
  ...
  if (priv->timer)
      ...
      if (priv->need_timer)
	{
          ...
          settings = __gtk_widget_get_settings (GTK_WIDGET (calendar));
          g_object_get (settings, "gtk-timeout-repeat", &timeout, NULL);

	  priv->need_timer = FALSE;
	  priv->timer = __gdk_threads_add_timeout_full (G_PRIORITY_DEFAULT_IDLE,
      ...
      if (priv->in_drag) 
	{
	  if (__gtk_drag_check_threshold (widget,
					...
					event->x, event->y))
	    {
	      ...
	      context = __gtk_drag_begin (widget, target_list, GDK_ACTION_COPY,
					1, (GdkEvent *)event);
...
move_focus (GtkCalendar *calendar, 
	    gint         direction)
{
  GtkTextDirection text_dir = __gtk_widget_get_direction (GTK_WIDGET (calendar));
			...
			GdkEventKey *event)
{
  ...
  switch (event->keyval)
      ...
      else
	{
	  ...
	  calendar_invalidate_day (calendar, old_focus_row, old_focus_col);
	  calendar_invalidate_day (calendar, gtk_calendar_get_props (calendar)->focus_row,
				   gtk_calendar_get_props (calendar)->focus_col);
	}
      break;
    case GDK_KP_Right:
			  ...
			  guint           time)
{
  ...
  if (target == GDK_NONE || __gdk_drag_context_get_suggested_action (context) == 0)
    __gdk_drag_status (context, 0, time);
  else
    {
      set_status_pending (context, __gdk_drag_context_get_suggested_action (context));
			...
			guint           time)
{
  ...
  if (target != GDK_NONE)
    {
      __gtk_drag_get_data (widget, context, 
			 target, 
			 time);
      return TRUE;
    }
 ...
 **/
GtkWidget*
__gtk_calendar_new (void)
{
  return g_object_new (GTK_TYPE_CALENDAR, NULL);
}

/**
 * __gtk_calendar_display_options:
 * @calendar: a #GtkCalendar.
 * @flags: the display options to set.
 *
 * Sets display options (whether to display the heading and the month headings).
 * 
 * Deprecated: 2.4: Use __gtk_calendar_set_display_options() instead
 **/
void
__gtk_calendar_display_options (GtkCalendar	       *calendar,
			      GtkCalendarDisplayOptions flags)
{
  __gtk_calendar_set_display_options (calendar, flags);
}
...
/**
 * __gtk_calendar_get_display_options:
 * @calendar: a #GtkCalendar
 * 
...
__gtk_calendar_get_display_options (GtkCalendar         *calendar)
{
...
/**
 * __gtk_calendar_set_display_options:
 * @calendar: a #GtkCalendar
 * @flags: the display options to set
 * 
 ...
 **/
void
__gtk_calendar_set_display_options (GtkCalendar	       *calendar,
				  GtkCalendarDisplayOptions flags)
{
  GtkWidget *widget = GTK_WIDGET (calendar);
  GtkCalendarPrivate *priv = gtk_calendar_get_instance_private (calendar);
      ...
      if ((flags ^ gtk_calendar_get_props (calendar)->display_flags) & GTK_CALENDAR_NO_MONTH_CHANGE)
	{
	  ...
	  if (! (flags & GTK_CALENDAR_NO_MONTH_CHANGE)
	      ...
	      for (i = 0; i < 4; i++)
		{
		  if (priv->arrow_win[i])
		    {
		      __gdk_window_set_user_data (priv->arrow_win[i], 
						NULL);
		      __gdk_window_destroy (priv->arrow_win[i]);
		      priv->arrow_win[i] = NULL;
		    }
 ...
 **/
gboolean
__gtk_calendar_select_month (GtkCalendar *calendar,
			   guint	month,
			   guint	year)
{
 ...
 **/
void
__gtk_calendar_select_day (GtkCalendar *calendar,
			 guint	      day)
{
  g_return_if_fail (GTK_IS_CALENDAR (calendar));
...
/**
 * __gtk_calendar_clear_marks:
 * @calendar: a #GtkCalendar
 * 
 * Remove all visual markers.
 **/
void
__gtk_calendar_clear_marks (GtkCalendar *calendar)
{
  guint day;
 ...
 */
gboolean
__gtk_calendar_mark_day (GtkCalendar *calendar,
		       guint	    day)
{
  g_return_val_if_fail (GTK_IS_CALENDAR (calendar), FALSE);
 ...
 */
gboolean
__gtk_calendar_unmark_day (GtkCalendar *calendar,
			 guint	      day)
{
  g_return_val_if_fail (GTK_IS_CALENDAR (calendar), FALSE);
 ...
 **/
void
__gtk_calendar_get_date (GtkCalendar *calendar,
		       guint	   *year,
		       guint	   *month,
		       guint	   *day)
{
...
/**
 * __gtk_calendar_set_detail_func:
 * @calendar: a #GtkCalendar.
 ...
 */
void
__gtk_calendar_set_detail_func (GtkCalendar           *calendar,
                              ...
                              GDestroyNotify         destroy)
{
  ...
  __gtk_widget_set_has_tooltip (GTK_WIDGET (calendar),
                              NULL != priv->detail_func);
  __gtk_widget_queue_resize (GTK_WIDGET (calendar));
}

/**
 * __gtk_calendar_set_detail_width_chars:
 * @calendar: a #GtkCalendar.
 ...
 */
void
__gtk_calendar_set_detail_width_chars (GtkCalendar *calendar,
                                     gint         chars)
{
  GtkCalendarPrivate *priv;
  ...
  if (chars != priv->detail_width_chars)
    {
      ...
      __gtk_widget_queue_resize_no_redraw (GTK_WIDGET (calendar));
    }
}

/**
 * __gtk_calendar_set_detail_height_rows:
 * @calendar: a #GtkCalendar.
 ...
 */
void
__gtk_calendar_set_detail_height_rows (GtkCalendar *calendar,
                                     gint         rows)
{
  GtkCalendarPrivate *priv;
...
/**
 * __gtk_calendar_get_detail_width_chars:
 * @calendar: a #GtkCalendar.
 *
 ...
 */
gint
__gtk_calendar_get_detail_width_chars (GtkCalendar *calendar)
{
  g_return_val_if_fail (GTK_IS_CALENDAR (calendar), 0);
  return gtk_calendar_get_instance_private (calendar)->detail_width_chars;
}

/**
 * __gtk_calendar_get_detail_height_rows:
 * @calendar: a #GtkCalendar.
 *
 ...
 */
gint
__gtk_calendar_get_detail_height_rows (GtkCalendar *calendar)
{
  g_return_val_if_fail (GTK_IS_CALENDAR (calendar), 0);
  return gtk_calendar_get_instance_private (calendar)->detail_height_rows;
}

/**
 * __gtk_calendar_freeze:
 * @calendar: a #GtkCalendar
 * 
 * Does nothing. Previously locked the display of the calendar until
 * it was thawed with __gtk_calendar_thaw().
 *
 * Deprecated: 2.8: 
 **/
void
__gtk_calendar_freeze (GtkCalendar *calendar)
{
  g_return_if_fail (GTK_IS_CALENDAR (calendar));
}
 ...
 * since the last __gtk_calendar_freeze() were displayed.
 *
 * Deprecated: 2.8: 
 **/
void
__gtk_calendar_thaw (GtkCalendar *calendar)
{
  g_return_if_fail (GTK_IS_CALENDAR (calendar));
}
