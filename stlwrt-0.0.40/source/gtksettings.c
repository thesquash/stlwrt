...
gtk_settings_finalize (GObject *object)
{
  ...
  ______gtk_rc_context_destroy (settings);
  ...
  else
    {
	  ...
	  !__gdk_screen_get_setting (settings->screen, pspec->name, &val))
      ...
      else
        {
          ...
          if (parser && ______gtk_settings_parse_convert (parser, &gstring_value,
                                                     pspec, &tmp_value))
            {
...
gboolean
______gtk_settings_parse_convert (GtkRcPropertyParser parser,
		      ...
		      GtkSettingsValuePrivate *qvalue)
{
  ...
  g_value_init (&tmp_value, G_PARAM_SPEC_VALUE_TYPE (pspec));
  if (______gtk_settings_parse_convert (parser, &qvalue->public.value,
				   pspec, &tmp_value))
    {
      if (pspec->param_id == PROP_COLOR_SCHEME) 
...
GtkRcPropertyParser
______gtk_rc_property_parser_from_type (GType type)
{
...
void
______gtk_settings_set_property_value_from_rc (GtkSettings            *settings,
					  const gchar            *prop_name,
					  const GtkSettingsValue *new_value)
{
  g_return_if_fail (GTK_SETTINGS (settings));
...
void
______gtk_settings_handle_event (GdkEventSetting *event)
{
  GtkSettings *settings;
  GParamSpec *pspec;
  guint property_id;
...
void
______gtk_settings_reset_rc_values (GtkSettings *settings)
{
...
settings_update_modules (GtkSettings *settings)
{
  ...
  ______gtk_modules_settings_changed (settings, modules);
