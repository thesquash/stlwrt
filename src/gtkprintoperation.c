...
STLWRT_DEFINE_TYPE_WITH_CODE (GtkPrintOperation, gtk_print_operation, G_TYPE_OBJECT,
		  ...
		  GtkPrintContext   *print_context)
{
  ...
  cr = __gtk_print_context_get_cairo_context (print_context);
				  ...
				  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_DEFAULT_PAGE_SETUP:
      __gtk_print_operation_set_default_page_setup (op, g_value_get_object (value));
      break;
    case PROP_PRINT_SETTINGS:
      __gtk_print_operation_set_print_settings (op, g_value_get_object (value));
      break;
    case PROP_JOB_NAME:
      __gtk_print_operation_set_job_name (op, g_value_get_string (value));
      break;
    case PROP_N_PAGES:
      __gtk_print_operation_set_n_pages (op, g_value_get_int (value));
      break;
    case PROP_CURRENT_PAGE:
      __gtk_print_operation_set_current_page (op, g_value_get_int (value));
      break;
    case PROP_USE_FULL_PAGE:
      __gtk_print_operation_set_use_full_page (op, g_value_get_boolean (value));
      break;
    case PROP_TRACK_PRINT_STATUS:
      __gtk_print_operation_set_track_print_status (op, g_value_get_boolean (value));
      break;
    case PROP_UNIT:
      __gtk_print_operation_set_unit (op, g_value_get_enum (value));
      break;
    case PROP_ALLOW_ASYNC:
      __gtk_print_operation_set_allow_async (op, g_value_get_boolean (value));
      break;
    case PROP_SHOW_PROGRESS:
      __gtk_print_operation_set_show_progress (op, g_value_get_boolean (value));
      break;
    case PROP_EXPORT_FILENAME:
      __gtk_print_operation_set_export_filename (op, g_value_get_string (value));
      break;
    case PROP_CUSTOM_TAB_LABEL:
      __gtk_print_operation_set_custom_tab_label (op, g_value_get_string (value));
      break;
    case PROP_EMBED_PAGE_SETUP:
      __gtk_print_operation_set_embed_page_setup (op, g_value_get_boolean (value));
      break;
    case PROP_HAS_SELECTION:
      __gtk_print_operation_set_has_selection (op, g_value_get_boolean (value));
      break;
    case PROP_SUPPORT_SELECTION:
      __gtk_print_operation_set_support_selection (op, g_value_get_boolean (value));
      break;
    default:
...
preview_print_idle_done (gpointer data)
{
  ...
  __gtk_print_operation_preview_end_preview (pop->preview);
      ...
      if (!pop->pages_data->initialized)
      ...
      else
        {
          ...
          if (!pop->pages_data->done)
            __gtk_print_operation_preview_render_page (pop->preview, pop->pages_data->page);
          else
		       ...
		       PreviewOp                *pop)
{
  ...
  cr = __gtk_print_context_get_cairo_context (pop->print_context);
	       ...
	       PreviewOp                *pop)
{
  ...
  __gdk_threads_add_idle_full (G_PRIORITY_DEFAULT_IDLE + 10,
	                     preview_print_idle,
		             pop,
				     ...
				     GtkWindow                *parent)
{
  ...
  page_setup = __gtk_print_context_get_page_setup (context);
  ...
  __gtk_print_context_set_cairo_context (op->priv->print_context, cr,
				       dpi_x, dpi_y);
  cairo_destroy (cr);
...
gtk_print_operation_class_init (GtkPrintOperationClass *class)
{
   ...
   * __gtk_print_operation_get_error() for more information.
   *
   * If you enabled print status tracking then 
   * __gtk_print_operation_is_finished() may still return %FALSE 
   ...
   * set the number of pages with __gtk_print_operation_set_n_pages().
   *
   * Since: 2.10
   */
  signals[BEGIN_PRINT] =
   ...
   * __gtk_print_operation_set_n_pages(), and return %TRUE if the document
   ...
   * from @context using __gtk_print_context_get_cairo_context().
   * |[
   * static void
   ...
   *   
   *   cr = __gtk_print_context_get_cairo_context (context);
   *   width = __gtk_print_context_get_width (context);
   *   
   ...
   *   cairo_fill (cr);
   *   
   *   layout = __gtk_print_context_create_pango_layout (context);
   *   
   ...
   * ]|
   *
   * Use __gtk_print_operation_set_use_full_page() and 
   * __gtk_print_operation_set_unit() before starting the print operation
   ...
   * Use __gtk_print_operation_get_status() to find out the current
   * status.
   *
   * Since: 2.10
   */
   ...
   * given a suitable cairo context with __gtk_print_context_set_cairo_context().
   * 
   * The custom preview implementation can use 
   * __gtk_print_operation_preview_is_selected() and 
   * __gtk_print_operation_preview_render_page() to find pages which
   * are selected for print and render them. The preview must be
   * finished by calling __gtk_print_operation_preview_end_preview()
   ...
   * 
   * This page setup will be used by __gtk_print_operation_run(),
   ...
   *
   * If this is set before __gtk_print_operation_run(), 
   ...
   * and call __gtk_print_operation_get_n_pages_to_print() when
 ...
 */
GtkPrintOperation *
__gtk_print_operation_new (void)
{
...
/**
 * __gtk_print_operation_set_default_page_setup:
 ...
 **/
void
__gtk_print_operation_set_default_page_setup (GtkPrintOperation *op,
...
/**
 * __gtk_print_operation_get_default_page_setup:
 * @op: a #GtkPrintOperation
 *
 ...
 */
GtkPageSetup *
__gtk_print_operation_get_default_page_setup (GtkPrintOperation *op)
{
...
/**
 * __gtk_print_operation_set_print_settings:
 * @op: a #GtkPrintOperation
 * @print_settings: (allow-none): #GtkPrintSettings
 *
 * Sets the print settings for @op. This is typically used to
 * re-establish print settings from a previous print operation,
 * see __gtk_print_operation_run().
 *
 * Since: 2.10
 **/
void
__gtk_print_operation_set_print_settings (GtkPrintOperation *op,
...
/**
 * __gtk_print_operation_get_print_settings:
 * @op: a #GtkPrintOperation
 *
 * Returns the current print settings.
 *
 * Note that the return value is %NULL until either
 * __gtk_print_operation_set_print_settings() or
 * __gtk_print_operation_run() have been called.
 *
 ...
 **/
GtkPrintSettings *
__gtk_print_operation_get_print_settings (GtkPrintOperation *op)
{
  g_return_val_if_fail (GTK_IS_PRINT_OPERATION (op), NULL);
...
/**
 * __gtk_print_operation_set_job_name:
 * @op: a #GtkPrintOperation
 ...
 **/
void
__gtk_print_operation_set_job_name (GtkPrintOperation *op,
...
/**
 * __gtk_print_operation_set_n_pages:
 * @op: a #GtkPrintOperation
 ...
 **/
void
__gtk_print_operation_set_n_pages (GtkPrintOperation *op,
				 gint               n_pages)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_current_page:
 * @op: a #GtkPrintOperation
 * @current_page: the current page, 0-based
 *
 * Sets the current page.
 *
 * If this is called before __gtk_print_operation_run(), 
 * the user will be able to select to print only the current page.
 *
 * Note that this only makes sense for pre-paginated documents.
 * 
 ...
 **/
void
__gtk_print_operation_set_current_page (GtkPrintOperation *op,
				      gint               current_page)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_use_full_page:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_use_full_page (GtkPrintOperation *op,
				       gboolean           full_page)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_unit:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_unit (GtkPrintOperation *op,
			      GtkUnit            unit)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_track_print_status:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_track_print_status (GtkPrintOperation  *op,
...
/**
 * __gtk_print_operation_get_status:
 * @op: a #GtkPrintOperation
 * 
 * Returns the status of the print operation. 
 * Also see __gtk_print_operation_get_status_string().
 * 
 ...
 **/
GtkPrintStatus
__gtk_print_operation_get_status (GtkPrintOperation *op)
{
...
/**
 * __gtk_print_operation_get_status_string:
 * @op: a #GtkPrintOperation
 * 
 ...
 *
 * Use __gtk_print_operation_get_status() to obtain a status
 ...
 **/
const gchar *
__gtk_print_operation_get_status_string (GtkPrintOperation *op)
{
  g_return_val_if_fail (GTK_IS_PRINT_OPERATION (op), "");
...
/**
 * __gtk_print_operation_is_finished:
 * @op: a #GtkPrintOperation
 * 
 ...
 **/
gboolean
__gtk_print_operation_is_finished (GtkPrintOperation *op)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_show_progress:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_show_progress (GtkPrintOperation  *op,
...
/**
 * __gtk_print_operation_set_allow_async:
 * @op: a #GtkPrintOperation
 * @allow_async: %TRUE to allow asynchronous operation
 *
 * Sets whether the __gtk_print_operation_run() may return
 ...
 */
void
__gtk_print_operation_set_allow_async (GtkPrintOperation  *op,
...
/**
 * __gtk_print_operation_set_custom_tab_label:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_custom_tab_label (GtkPrintOperation  *op,
					  const gchar        *label)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_set_export_filename:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_export_filename (GtkPrintOperation *op,
...
create_page_setup (GtkPrintOperation *op)
{
  ...
  if (priv->default_page_setup)
    page_setup = __gtk_page_setup_copy (priv->default_page_setup);
  else
  ...
  if (settings)
    {
      ...
      if (__gtk_print_settings_has_key (settings, GTK_PRINT_SETTINGS_ORIENTATION))
	__gtk_page_setup_set_orientation (page_setup,
					__gtk_print_settings_get_orientation (settings));


      paper_size = __gtk_print_settings_get_paper_size (settings);
      if (paper_size)
	{
		...
		GtkPageSetup      *page_setup)
{
  ...
  w = __gtk_paper_size_get_width (paper_size, GTK_UNIT_POINTS);
  h = __gtk_paper_size_get_height (paper_size, GTK_UNIT_POINTS);
	 ...
	 gboolean           *do_print)
{
  ...
  width = __gtk_page_setup_get_paper_width (page_setup, GTK_UNIT_POINTS);
  height = __gtk_page_setup_get_paper_height (page_setup, GTK_UNIT_POINTS);
  ...
  __gtk_print_context_set_cairo_context (op->priv->print_context,
				       cr, 72, 72);
  cairo_destroy (cr);
...
print_pages_idle_done (gpointer user_data)
{
  ...
  if (data->progress)
    __gtk_widget_destroy (data->progress);
...
/**
 * __gtk_print_operation_set_defer_drawing:
 * @op: a #GtkPrintOperation
 * 
 * Sets up the #GtkPrintOperation to wait for calling of
 * __gtk_print_operation_draw_page_finish() from application. It can
 ...
 **/
void
__gtk_print_operation_set_defer_drawing (GtkPrintOperation *op)
{
  GtkPrintOperationPrivate *priv = op->priv;
...
/**
 * __gtk_print_operation_set_embed_page_setup:
 * @op: a #GtkPrintOperation
 ...
 **/
void
__gtk_print_operation_set_embed_page_setup (GtkPrintOperation  *op,
                                          gboolean            embed)
{
  GtkPrintOperationPrivate *priv;
...
/**
 * __gtk_print_operation_get_embed_page_setup:
 * @op: a #GtkPrintOperation
 *
 ...
 */
gboolean
__gtk_print_operation_get_embed_page_setup (GtkPrintOperation *op)
{
...
/**
 * __gtk_print_operation_draw_page_finish:
 * @op: a #GtkPrintOperation
 * 
 ...
 * thread).
 * If __gtk_print_operation_set_defer_drawing() was called before, then this function
 ...
 **/
void
__gtk_print_operation_draw_page_finish (GtkPrintOperation *op)
{
  GtkPrintOperationPrivate *priv = op->priv;
  GtkPageSetup *page_setup;
  GtkPrintContext *print_context;
  ...
  page_setup = __gtk_print_context_get_page_setup (print_context);
		    ...
		    gint               page_nr)
{
  ...
  if (priv->manual_number_up > 1)
    {
      ...
      page_setup = __gtk_print_context_get_page_setup (print_context);
      orientation = __gtk_page_setup_get_orientation (page_setup);

      top_margin = __gtk_page_setup_get_top_margin (page_setup, GTK_UNIT_POINTS);
      bottom_margin = __gtk_page_setup_get_bottom_margin (page_setup, GTK_UNIT_POINTS);
      left_margin = __gtk_page_setup_get_left_margin (page_setup, GTK_UNIT_POINTS);
      right_margin = __gtk_page_setup_get_right_margin (page_setup, GTK_UNIT_POINTS);

      paper_width = __gtk_page_setup_get_paper_width (page_setup, GTK_UNIT_POINTS);
      paper_height = __gtk_page_setup_get_paper_height (page_setup, GTK_UNIT_POINTS);

      context_width = __gtk_print_context_get_width (print_context);
      context_height = __gtk_print_context_get_height (print_context);
  ...
  if (priv->page_drawing_state == GTK_PAGE_DRAWING_STATE_DRAWING)
    __gtk_print_operation_draw_page_finish (op);
}
			  ...
			  gpointer   data)
{
  ...
  __gtk_widget_hide (dialog);
  __gtk_print_operation_cancel (op);
}

static gboolean
show_progress_timeout (PrintPagesData *data)
{
  __gtk_window_present (GTK_WINDOW (data->progress));
  ...
  if (priv->show_progress)
    {
      ...
      progress = __gtk_message_dialog_new (parent, 0, 
					 GTK_MESSAGE_OTHER,
					 GTK_BUTTONS_CANCEL,
					 _("Preparing"));
	...
	__gdk_threads_add_timeout (SHOW_PROGRESS_TIME, 
  ...
  if (data->is_preview)
    {
      ...
      if (!handled)
        {
          ...
          error_dialog = __gtk_message_dialog_new (parent,
      ...
      if (__gtk_print_context_get_cairo_context (priv->print_context) == NULL)
        {
          /* Programmer error */
      ...
      priv->print_pages = __gtk_print_settings_get_print_pages (priv->print_settings);
      priv->page_ranges = __gtk_print_settings_get_page_ranges (priv->print_settings,
							      &priv->num_page_ranges);
      priv->manual_num_copies = 1;
      priv->manual_collation = FALSE;
      priv->manual_reverse = __gtk_print_settings_get_reverse (priv->print_settings);
      priv->manual_page_set = __gtk_print_settings_get_page_set (priv->print_settings);
      priv->manual_scale = __gtk_print_settings_get_scale (priv->print_settings) / 100.0;
      priv->manual_orientation = TRUE;
      priv->manual_number_up = __gtk_print_settings_get_number_up (priv->print_settings);
      priv->manual_number_up_layout = __gtk_print_settings_get_number_up_layout (priv->print_settings);
    }
  
  priv->print_pages_idle_id = __gdk_threads_add_idle_full (G_PRIORITY_DEFAULT_IDLE + 10,
					                 print_pages_idle, 
					                 data, 
...
/**
 * __gtk_print_operation_get_error:
 * @op: a #GtkPrintOperation
 * @error: return location for the error
 * 
 * Call this when the result of a print operation is
 * %GTK_PRINT_OPERATION_RESULT_ERROR, either as returned by 
 * __gtk_print_operation_run(), or in the #GtkPrintOperation::done signal 
 * handler. The returned #GError will contain more details on what went wrong.
 *
 * Since: 2.10
 **/
void
__gtk_print_operation_get_error (GtkPrintOperation  *op,
			       GError            **error)
{
  g_return_if_fail (GTK_IS_PRINT_OPERATION (op));
 ...
 *
 * If you call __gtk_print_operation_set_allow_async() or set the 
 ...
 * |[
 * if (settings != NULL)
 *   __gtk_print_operation_set_print_settings (print, settings);
 *   
 * if (page_setup != NULL)
 *   __gtk_print_operation_set_default_page_setup (print, page_setup);
 *   
 ...
 *  
 * res = __gtk_print_operation_run (print, 
 ...
 *  {
 *    error_dialog = __gtk_message_dialog_new (GTK_WINDOW (parent),
 ...
 *    settings = g_object_ref (__gtk_print_operation_get_print_settings (print));
 *  }
 * ]|
 *
 * Note that __gtk_print_operation_run() can only be called once on a
 * given #GtkPrintOperation.
 *
 * Return value: the result of the print operation. A return value of 
 *   %GTK_PRINT_OPERATION_RESULT_APPLY indicates that the printing was
 *   completed successfully. In this case, it is a good idea to obtain 
 *   the used print settings with __gtk_print_operation_get_print_settings() 
...
__gtk_print_operation_run (GtkPrintOperation        *op,
			 ...
			 GError                  **error)
{
  ...
  if (priv->print_settings == NULL)
    priv->print_settings = __gtk_print_settings_new ();
 ...
 */
void
__gtk_print_operation_cancel (GtkPrintOperation *op)
{
  g_return_if_fail (GTK_IS_PRINT_OPERATION (op));
...
/**
 * __gtk_print_operation_set_support_selection:
 * @op: a #GtkPrintOperation
 ...
 */
void
__gtk_print_operation_set_support_selection (GtkPrintOperation  *op,
...
/**
 * __gtk_print_operation_get_support_selection:
 * @op: a #GtkPrintOperation
 *
 ...
 */
gboolean
__gtk_print_operation_get_support_selection (GtkPrintOperation *op)
{
  g_return_val_if_fail (GTK_IS_PRINT_OPERATION (op), FALSE);
...
/**
 * __gtk_print_operation_set_has_selection:
 * @op: a #GtkPrintOperation
 ...
 * will draw by __gtk_print_operation_set_n_pages() in a callback of
 * #GtkPrintOperation::begin-print.
 *
 * Since: 2.18
 */
void
__gtk_print_operation_set_has_selection (GtkPrintOperation  *op,
...
/**
 * __gtk_print_operation_get_has_selection:
 * @op: a #GtkPrintOperation
 *
 ...
 */
gboolean
__gtk_print_operation_get_has_selection (GtkPrintOperation *op)
{
  g_return_val_if_fail (GTK_IS_PRINT_OPERATION (op), FALSE);
...
/**
 * __gtk_print_operation_get_n_pages_to_print:
 * @op: a #GtkPrintOperation
 *
 ...
 **/
gint
__gtk_print_operation_get_n_pages_to_print (GtkPrintOperation *op)
{
