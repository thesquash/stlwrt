...
gdk_rgb_cmap_fail (const char *msg, GdkColormap *cmap, gulong *pixels)
{
  ...
  if (n_free)
    __gdk_colormap_free_colors (cmap, free_colors, n_free);
  return 0;
}
...
static gboolean
__gdk_rgb_try_colormap (GdkRgbInfo *image_info, gboolean force,
		      gint nr, gint ng, gint nb)
{
  gint r, g, b;
  gint ri, gi, bi;
  ...
  screen = __gdk_visual_get_screen (image_info->visual);

  if (image_info->cmap)
    cmap = image_info->cmap;
  else
    cmap = __gdk_screen_get_system_colormap (screen);
...
#ifndef GAMMA
  if (cmap == __gdk_screen_get_system_colormap (screen))
	...
	if (d2 < best[idx])
	  {
		...
		__gdk_colormap_free_colors (cmap, &color, 1);
	      }
	    else
	      colors_needed--;
	    color = cmap->colors[i];
	    if (!__gdk_colormap_alloc_color (cmap, &color, FALSE, FALSE))
	  ...
	  if (pixels[i] == 256)
	    {
	      ...
	      if (!__gdk_colormap_alloc_color (cmap, &color, FALSE, force))
		{
		  char tmp_str[80];
...
gdk_rgb_do_colormaps (GdkRgbInfo *image_info, gboolean force)
{
   ...
   */
  for (i = 0; i < n_sizes; i++)
    if (__gdk_rgb_try_colormap (image_info,
			      (i == n_sizes - 1 ) && force,
  ...
  for (i = 0; i < 8; i++)
    {
      ...
      __gdk_colormap_alloc_color (image_info->cmap, &color, FALSE, TRUE);
      image_info->colorcube_d[((i & 4) << 4) | ((i & 2) << 2) | (i & 1)] = color.pixel;
    }
}

void
__gdk_rgb_set_verbose (gboolean verbose)
{
  gdk_rgb_verbose = verbose;
}

void
__gdk_rgb_set_install (gboolean install)
{
  gdk_rgb_install_cmap = install;
}

void
__gdk_rgb_set_min_colors (gint min_colors)
{
...
gdk_rgb_score_visual (GdkVisual *visual)
{
  ...
  sys = (visual == __gdk_screen_get_system_visual (__gdk_visual_get_screen (visual)));
...
gdk_rgb_choose_visual (GdkScreen *screen)
{
  ...
  visuals = __gdk_screen_list_visuals (screen);
  tmp_list = visuals;
...
gdk_rgb_free_info (GdkRgbInfo *image_info)
{
  ...
  if (image_info->gray_cmap)
    __gdk_rgb_cmap_free (image_info->gray_cmap);
...
{
  GdkRgbInfo *image_info;
  GdkScreen *screen = __gdk_visual_get_screen (visual);
  ...
  if ((image_info->visual->type == GDK_VISUAL_PSEUDO_COLOR &&
    ...
    {
      if (!image_info->cmap)
	image_info->cmap = g_object_ref (__gdk_screen_get_system_colormap (screen));
    ...
    {
      if (!image_info->cmap &&
	  (gdk_rgb_install_cmap || image_info->visual != __gdk_screen_get_system_visual (screen)))
	{
	  image_info->cmap = __gdk_colormap_new (image_info->visual, FALSE);
      ...
      if (!image_info->cmap)
	{
	  ...
	  if (image_info->visual->type != GDK_VISUAL_DIRECT_COLOR &&
	      image_info->visual == __gdk_screen_get_system_visual (screen))
	    image_info->cmap = g_object_ref (__gdk_screen_get_system_colormap (screen));
	  else
	    {
	      image_info->cmap = __gdk_colormap_new (image_info->visual, FALSE);
  ...
  image_info->bpp = (_gdk_windowing_get_bits_for_depth (__gdk_screen_get_display (screen), image_info->visual->depth) + 7) / 8;
...
__gdk_rgb_init (void)
{
  ...
  if (((char *)byte_order)[0] == 1)
    g_error ("__gdk_rgb_init: compiled for big endian, but this is a little endian machine.\n\n");
#else
  if (((char *)byte_order)[0] != 1)
    g_error ("__gdk_rgb_init: compiled for little endian, but this is a big endian machine.\n\n");
#endif
}
...
gdk_rgb_get_info_from_colormap (GdkColormap *cmap)
{
  ...
  if (!image_info)
    image_info = gdk_rgb_create_info (__gdk_colormap_get_visual (cmap), cmap);
...
static gulong
__gdk_rgb_xpixel_from_rgb_internal (GdkColormap *colormap,
...
gulong
__gdk_rgb_xpixel_from_rgb (guint32 rgb)
{
  guint32 r = rgb & 0xff0000;
  guint32 g = rgb & 0xff00;
  guint32 b = rgb & 0xff;

  return __gdk_rgb_xpixel_from_rgb_internal (__gdk_screen_get_rgb_colormap (__gdk_screen_get_default ()),
					   (r >> 8) + (r >> 16), g + (g >> 8), b + (b << 8));
}

void
__gdk_rgb_gc_set_foreground (GdkGC *gc, guint32 rgb)
{
  GdkColor color;

  color.pixel = __gdk_rgb_xpixel_from_rgb (rgb);
  __gdk_gc_set_foreground (gc, &color);
}

void
__gdk_rgb_gc_set_background (GdkGC *gc, guint32 rgb)
{
  GdkColor color;
 ...
 * call __gtk_widget_get_colormap().
 *
 * @color should have its %red, %green, and %blue fields initialized;
 * __gdk_rgb_find_color() will fill in the %pixel field with the best
 * matching pixel from a color cube. The color is then ready to be
 * used for drawing, e.g. you can call __gdk_gc_set_foreground() which
 * expects %pixel to be initialized.
 *
 * In many cases, you can avoid this whole issue by calling
 * __gdk_gc_set_rgb_fg_color() or __gdk_gc_set_rgb_bg_color(), which
 * do not expect %pixel to be initialized in advance. If you use those
 * functions, there's no need for __gdk_rgb_find_color().
 * 
 * Deprecated: 2.22: Cairo handles colors automatically.
 **/
void
__gdk_rgb_find_color (GdkColormap *colormap, GdkColor *color)
{
  color->pixel = __gdk_rgb_xpixel_from_rgb_internal (colormap,
...
gdk_rgb_select_conv (GdkRgbInfo *image_info)
{
  ...
  GdkScreen *screen = __gdk_visual_get_screen (image_info->visual);

  depth = image_info->visual->depth;

  bpp = _gdk_windowing_get_bits_for_depth (__gdk_screen_get_display (screen),
...
static void
__gdk_draw_rgb_image_core (GdkRgbInfo     *image_info,
			 ...
			 gint            ydith)
{
  ...
  if (image_info->bitmap)
	...
	image_info->own_gc = __gdk_gc_new (drawable);
  ...
  for (y0 = 0; y0 < height; y0 += GDK_SCRATCH_IMAGE_HEIGHT)
      ...
      for (x0 = 0; x0 < width; x0 += GDK_SCRATCH_IMAGE_WIDTH)
	{
	  ...
	  image = _gdk_image_get_scratch (__gdk_drawable_get_screen (drawable), 
					  width1, height1,
...
{
  GdkColormap *cmap = __gdk_drawable_get_colormap (drawable);
  GdkScreen *screen = __gdk_drawable_get_screen (drawable);
  ...
  if (!cmap)
    {
      ...
      gint depth = __gdk_drawable_get_depth (drawable);
      GdkColormap *rgb_cmap = __gdk_screen_get_rgb_colormap (screen);
      if (depth == __gdk_colormap_get_visual (rgb_cmap)->depth)
	cmap = rgb_cmap;
      else
	{
...
void
__gdk_draw_rgb_image (GdkDrawable  *drawable,
		    GdkGC        *gc,
		    gint          x,
		    gint          y,
		    gint          width,
		    ...
		    gint          rowstride)
{
  ...
  if (dith == GDK_RGB_DITHER_NONE || (dith == GDK_RGB_DITHER_NORMAL &&
    ...
    __gdk_draw_rgb_image_core (image_info, drawable, gc, x, y, width, height,
...
void
__gdk_draw_rgb_image_dithalign (GdkDrawable  *drawable,
			      GdkGC        *gc,
			      gint          x,
			      gint          y,
			      gint          width,
...
void
__gdk_draw_rgb_32_image (GdkDrawable  *drawable,
		       GdkGC        *gc,
		       gint          x,
		       gint          y,
		       gint          width,
...
/**
 * __gdk_draw_rgb_32_image_dithalign:
 ...
 *
 * Like __gdk_draw_rgb_32_image(), but allows you to specify the dither
 * offsets. See __gdk_draw_rgb_image_dithalign() for more details.
 * 
 * Deprecated: 2.22: Cairo handles colors automatically.
 **/
void
__gdk_draw_rgb_32_image_dithalign (GdkDrawable  *drawable,
				 GdkGC        *gc,
				 gint          x,
				 gint          y,
				 gint          width,
...
gdk_rgb_make_gray_cmap (GdkRgbInfo *info)
{
  ...
  for (i = 0; i < 256; i++)
    rgb[i] = (i << 16)  | (i << 8) | i;
  info->gray_cmap = __gdk_rgb_cmap_new (rgb, 256);
}

void
__gdk_draw_gray_image (GdkDrawable  *drawable,
		     GdkGC        *gc,
		     gint          x,
		     gint          y,
		     gint          width,
...
GdkRgbCmap *
__gdk_rgb_cmap_new (guint32 *colors, gint n_colors)
{
  GdkRgbCmap *cmap;
...
void
__gdk_rgb_cmap_free (GdkRgbCmap *cmap)
{
  GSList *tmp_list;
...
void
__gdk_draw_indexed_image (GdkDrawable  *drawable,
			GdkGC        *gc,
			gint          x,
			gint          y,
			gint          width,
...
gboolean
__gdk_rgb_colormap_ditherable (GdkColormap *cmap)
{
...
gboolean
__gdk_rgb_ditherable (void)
{
  return __gdk_rgb_colormap_ditherable (__gdk_rgb_get_colormap ());
}

/**
 * __gdk_rgb_get_colormap:
 * 
 ...
 *
 * Deprecated: 2.22: Use __gdk_screen_get_system_colormap (__gdk_screen_get_default ()) instead.
 **/
GdkColormap *
__gdk_rgb_get_colormap (void)
{
  static GdkColormap *cmap = NULL;
  if (!cmap)
    {
      GdkRgbInfo *image_info = gdk_rgb_create_info (gdk_rgb_choose_visual (__gdk_screen_get_default ()), NULL);
      cmap = image_info->cmap;
    }
 ...
 * Since: 2.2
 *
 * Deprecated: 2.22: Use __gdk_screen_get_system_colormap()
 **/
GdkColormap *
__gdk_screen_get_rgb_colormap (GdkScreen *screen)
{
  GdkColormap *cmap;
 ...
 * Since: 2.2
 *
 * Deprecated: 2.22: Use __gdk_screen_get_system_visual()
 **/
GdkVisual *
__gdk_screen_get_rgb_visual (GdkScreen *screen)
{
  g_return_val_if_fail (GDK_IS_SCREEN (screen), NULL);
  return __gdk_colormap_get_visual (__gdk_screen_get_rgb_colormap (screen));
}

/**
 * __gdk_rgb_get_visual:
 * 
 * Gets a "preferred visual" chosen by GdkRGB for rendering image data
 ...
 * Return value: (transfer none): The #GdkVisual chosen by GdkRGB.
 *
 * Deprecated: 2.22: Use __gdk_visual_get_system_visual (__gdk_screen_get_default ()) instead.
 **/
GdkVisual *
__gdk_rgb_get_visual (void)
{
