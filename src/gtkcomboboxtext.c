 ...
 *
 * To create a GtkComboBoxText, use __gtk_combo_box_text_new() or
 * __gtk_combo_box_text_new_with_entry().
 *
 * You can add items to a GtkComboBoxText with
 * __gtk_combo_box_text_append_text(), __gtk_combo_box_text_insert_text()
 * or __gtk_combo_box_text_prepend_text() and remove options with
 * __gtk_combo_box_text_remove().
 *
 * If the GtkComboBoxText contains an entry (via the 'has-entry' property),
 * its contents can be retrieved using __gtk_combo_box_text_get_active_text().
 * The entry itself can be accessed by calling __gtk_bin_get_child() on the
 * combo box.
 *
                                ...
                                GObjectConstructParam *construct_properties)
{
  ...
  if (!__gtk_combo_box_get_has_entry (GTK_COMBO_BOX (object)))
    {
      GtkCellRenderer *cell;

      cell = __gtk_cell_renderer_text_new ();
      __gtk_cell_layout_pack_start (GTK_CELL_LAYOUT (object), cell, TRUE);
      __gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (object), cell,
                                      "text", 0,
                                      NULL);
    }
		  ...
		  GError             **error)
{
  ...
  if (data->string)
    __gtk_combo_box_text_append_text (GTK_COMBO_BOX_TEXT (data->object), data->string);
...
/**
 * __gtk_combo_box_text_new:
 *
 * Creates a new #GtkComboBoxText, which is a #GtkComboBox just displaying
 * strings. See __gtk_combo_box_entry_new_with_text().
 *
 ...
 */
GtkWidget *
__gtk_combo_box_text_new_with_entry (void)
{
  return g_object_new (GTK_TYPE_COMBO_BOX_TEXT,
                       "has-entry", TRUE,
                       "entry-text-column", 0,
...
/**
 * __gtk_combo_box_text_append_text:
 ...
 */
void
__gtk_combo_box_text_append_text (GtkComboBoxText *combo_box,
                                const gchar     *text)
{
  GtkListStore *store;
  GtkTreeIter iter;
  ...
  store = GTK_LIST_STORE (__gtk_combo_box_get_model (GTK_COMBO_BOX (combo_box)));
  g_return_if_fail (GTK_IS_LIST_STORE (store));

  text_column = __gtk_combo_box_get_entry_text_column (GTK_COMBO_BOX (combo_box));
  if (__gtk_combo_box_get_has_entry (GTK_COMBO_BOX (combo_box)))
    g_return_if_fail (text_column >= 0);
  else if (text_column < 0)
    text_column = 0;

  column_type = __gtk_tree_model_get_column_type (GTK_TREE_MODEL (store), text_column);
  g_return_if_fail (column_type == G_TYPE_STRING);

  __gtk_list_store_append (store, &iter);
  __gtk_list_store_set (store, &iter, text_column, text, -1);
}

/**
 * __gtk_combo_box_text_insert_text:
 * @combo_box: A #GtkComboBoxText
 ...
 */
void
__gtk_combo_box_text_insert_text (GtkComboBoxText *combo_box,
                                gint             position,
                                const gchar     *text)
{
  GtkListStore *store;
...
/**
 * __gtk_combo_box_text_prepend_text:
 ...
 */
void
__gtk_combo_box_text_prepend_text (GtkComboBoxText *combo_box,
                                 const gchar     *text)
{
  GtkListStore *store;
  GtkTreeIter iter;
 ...
 */
void
__gtk_combo_box_text_remove (GtkComboBoxText *combo_box,
                           gint             position)
{
  GtkTreeModel *model;
  GtkListStore *store;
  ...
  model = __gtk_combo_box_get_model (GTK_COMBO_BOX (combo_box));
  store = GTK_LIST_STORE (model);
  g_return_if_fail (GTK_IS_LIST_STORE (store));

  if (__gtk_tree_model_iter_nth_child (model, &iter, NULL, position))
    __gtk_list_store_remove (store, &iter);
}

/**
 * __gtk_combo_box_text_get_active_text:
 * @combo_box: A #GtkComboBoxText
 *
 ...
 */
gchar *
__gtk_combo_box_text_get_active_text (GtkComboBoxText *combo_box)
{
  GtkTreeIter iter;
  gchar *text = NULL;
 ...
 if (__gtk_combo_box_get_has_entry (GTK_COMBO_BOX (combo_box)))
   {
     ...
     entry = __gtk_bin_get_child (GTK_BIN (combo_box));
     text = g_strdup (__gtk_entry_get_text (GTK_ENTRY (entry)));
   }
  else if (__gtk_combo_box_get_active_iter (GTK_COMBO_BOX (combo_box), &iter))
    {
      GtkTreeModel *model;
      gint text_column;
      gint column_type;
      ...
      text_column = __gtk_combo_box_get_entry_text_column (GTK_COMBO_BOX (combo_box));
      column_type = __gtk_tree_model_get_column_type (model, text_column);
      g_return_val_if_fail (column_type == G_TYPE_STRING, NULL);
      __gtk_tree_model_get (model, &iter, text_column, &text, -1);
    }
