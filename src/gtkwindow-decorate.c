				    ...
				    GdkEventMotion  *event)
{
  ...
  if (deco->moving)
    {
      ...
      ___gtk_window_reposition (window, win_x + dx, win_y + dy);
    }
  ...
  if (deco->resize != RESIZE_NONE)
	...
	break;
      ...
      if ((w > 0) && (h > 0))
	{
	  ___gtk_window_constrain_size (window, w,h, &w, &h);
  ...
  if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED)
      ...
      if (event->new_window_state & GDK_WINDOW_STATE_MAXIMIZED)
	{
	  ...
	  if (w != deco->last_w || h != deco->last_h)
	    {
	      ___gtk_window_reposition (window, DECORATION_BORDER_LEFT, DECORATION_BORDER_TOP);
	      __gdk_window_resize (widget->window, w, h);
	      deco->maximized = TRUE;
	    }
	}
	...
	{
	  ___gtk_window_reposition (window, deco->last_x, deco->last_y);
	  ___gtk_window_constrain_size (window, deco->last_w, deco->last_h,
