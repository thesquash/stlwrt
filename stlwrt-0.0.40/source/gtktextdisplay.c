...
text_renderer_get_error_color (GtkTextRenderer *text_renderer)
{
  ...
  if (!text_renderer->error_color)
    __gtk_widget_style_get (text_renderer->widget,
			  "error-underline-color", &text_renderer->error_color,
			  NULL);
  
  if (!text_renderer->error_color)
    text_renderer->error_color = __gdk_color_copy (&red);
			       ...
			       PangoLayoutRun *run)
{
  ...
  if (text_renderer->state == SELECTED)
    {
      if (__gtk_widget_has_focus (text_renderer->widget))
	fg_color = &text_renderer->widget->style->text[GTK_STATE_SELECTED];
      else
	fg_color = &text_renderer->widget->style->text[GTK_STATE_ACTIVE];
    }
  else if (text_renderer->state == CURSOR && __gtk_widget_has_focus (text_renderer->widget))
  ...
  if (fg_stipple && text_renderer->screen != __gdk_drawable_get_screen (fg_stipple))
    {
  ...
  __gdk_pango_renderer_set_stipple (gdk_renderer, PANGO_RENDER_PART_FOREGROUND, fg_stipple);
  __gdk_pango_renderer_set_stipple (gdk_renderer, PANGO_RENDER_PART_STRIKETHROUGH, fg_stipple);
  __gdk_pango_renderer_set_stipple (gdk_renderer, PANGO_RENDER_PART_UNDERLINE, fg_stipple);

  bg_stipple = appearance->draw_bg ? appearance->bg_stipple : NULL;
  
  if (bg_stipple && text_renderer->screen != __gdk_drawable_get_screen (bg_stipple))
    {
      g_warning ("gtk_text_renderer_prepare_run:\n"
  ...
  __gdk_pango_renderer_set_stipple (gdk_renderer, PANGO_RENDER_PART_BACKGROUND, bg_stipple);
}
  ...
  if (attr->data == NULL)
    {
      ...
      if (__gdk_rectangle_intersect (&shape_rect, &text_renderer->clip_rect,
				   &draw_rect))
	{
	  __gdk_draw_rectangle (text_renderer->drawable, fg_gc,
			      FALSE, shape_rect.x, shape_rect.y,
			      shape_rect.width, shape_rect.height);
	  
	  __gdk_draw_line (text_renderer->drawable, fg_gc,
  ...
  else if (GDK_IS_PIXBUF (attr->data))
    {
      ...
      if (__gdk_rectangle_intersect (&pixbuf_rect, &text_renderer->clip_rect,
				   &draw_rect))
	{
	  __gdk_draw_pixbuf (text_renderer->drawable,
			   fg_gc,
			   pixbuf,
		     ...
		     GdkRectangle    *clip_rect)
{
  ...
  __gdk_pango_renderer_set_drawable (GDK_PANGO_RENDERER (text_renderer), drawable);
  __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer),
...
text_renderer_end (GtkTextRenderer *text_renderer)
{
  ...
  if (text_renderer->error_color)
    {
      __gdk_color_free (text_renderer->error_color);
      text_renderer->error_color = NULL;
    }

  __gdk_pango_renderer_set_drawable (GDK_PANGO_RENDERER (text_renderer), NULL);
  __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), NULL);
                   ...
                   int                 end_index)
{
  gint *ranges;
  gint n_ranges, i;
  GdkRegion *clip_region = __gdk_region_new ();
  ...
  tmp_region = __gdk_region_rectangle (&text_renderer->clip_rect);
  __gdk_region_intersect (clip_region, tmp_region);
  ...
  do
    {
      ...
      if (selection_start_index < byte_offset &&
        ...
        {
          __gdk_draw_rectangle (text_renderer->drawable,
                              selection_gc,
                              TRUE,
          ...
          if (line_display->pg_bg_color)
            {
              ...
              bg_gc = __gdk_gc_new (text_renderer->drawable);
              __gdk_gc_set_fill (bg_gc, GDK_SOLID);
              __gdk_gc_set_rgb_fg_color (bg_gc, line_display->pg_bg_color);
	   ...
	   */
	       ...
	       */
	      __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), NULL);
              __gdk_gc_set_clip_region (selection_gc, clip_region);
              __gdk_gc_set_clip_region (fg_gc, clip_region);
	      __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), fg_gc);
	      ...
	      __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), NULL);
              __gdk_gc_set_clip_region (selection_gc, NULL);
              __gdk_gc_set_clip_region (fg_gc, NULL);
	      __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), fg_gc);
		   ...
		   __gtk_widget_has_focus (text_renderer->widget) &&
		    ...
		    (at_last_line && line_display->insert_index == byte_offset + line->length)))
	    {
	      ...
	      cursor_gc = ______gtk_widget_get_cursor_gc (text_renderer->widget);
	      ...
	      __gdk_gc_set_clip_rectangle (cursor_gc, &cursor_rect);
              ...
              __gdk_gc_set_clip_region (cursor_gc, NULL);
	      ...
	      if (!line_display->cursor_at_line_end)
		{
		  ...
		  __gdk_gc_set_clip_rectangle (cursor_text_gc, &cursor_rect);

		  __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), cursor_text_gc);
		  ...
		  __gdk_pango_renderer_set_gc (GDK_PANGO_RENDERER (text_renderer), fg_gc);
		  __gdk_gc_set_clip_region (cursor_text_gc, NULL);
		}
	    }
        }
  ...
  if (!text_renderer)
    {
      ...
      g_signal_connect_object (__gdk_screen_get_display (screen), "closed",
                      ...
                      GList **widgets)
{
  ...
  text_renderer = get_text_renderer (__gdk_drawable_get_screen (drawable));

  text_renderer_begin (text_renderer, widget, drawable, &clip);

  gtk_text_layout_wrap_loop_start (layout);

  if (__gtk_text_buffer_get_selection_bounds (layout->buffer,
                                            &selection_start,
                                            &selection_end))
    have_selection = TRUE;
  ...
  while (tmp_list != NULL)
    {
      ...
      if (line_display->height > 0)
		...
		__gtk_text_iter_forward_to_line_end (&line_end);
              byte_count = __gtk_text_iter_get_visible_line_index (&line_end);     

              if (__gtk_text_iter_compare (&selection_start, &line_end) <= 0 &&
                  __gtk_text_iter_compare (&selection_end, &line_start) >= 0)
                {
                  if (__gtk_text_iter_compare (&selection_start, &line_start) >= 0)
                    selection_start_index = __gtk_text_iter_get_visible_line_index (&selection_start);
                  else
                    selection_start_index = -1;

                  if (__gtk_text_iter_compare (&selection_end, &line_end) <= 0)
                    selection_end_index = __gtk_text_iter_get_visible_line_index (&selection_end);
                  else
	  ...
	  while (cursor_list)
	    {
 	      ...
 	      if (have_strong && have_weak)
 		{
	      ...
	      __gtk_draw_insertion_cursor (widget, drawable, &clip, &cursor_location,
					 cursor->is_strong,
