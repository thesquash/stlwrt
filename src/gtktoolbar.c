...
gtk_toolbar_init (GtkToolbar *toolbar)
{
  ...
  __gtk_widget_set_can_focus (GTK_WIDGET (toolbar), FALSE);
  __gtk_widget_set_has_window (GTK_WIDGET (toolbar), FALSE);
  
  priv = gtk_toolbar_get_instance_private (toolbar);
  
  gtk_toolbar_get_props (toolbar)->orientation = GTK_ORIENTATION_HORIZONTAL;
  ...
  __gtk_button_set_focus_on_click (GTK_BUTTON (priv->arrow_button), FALSE);
  
  priv->arrow = __gtk_arrow_new (GTK_ARROW_DOWN, GTK_SHADOW_NONE);
  __gtk_widget_set_name (priv->arrow, "gtk-toolbar-arrow");
  __gtk_widget_show (priv->arrow);
  __gtk_container_add (GTK_CONTAINER (priv->arrow_button), priv->arrow);
  
  __gtk_widget_set_parent (priv->arrow_button, GTK_WIDGET (toolbar));
			  ...
			  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_TOOLBAR_STYLE:
      __gtk_toolbar_set_style (toolbar, g_value_get_enum (value));
      break;
    case PROP_SHOW_ARROW:
      __gtk_toolbar_set_show_arrow (toolbar, g_value_get_boolean (value));
      break;
    case PROP_TOOLTIPS:
      __gtk_toolbar_set_tooltips (toolbar, g_value_get_boolean (value));
      break;
    case PROP_ICON_SIZE:
      __gtk_toolbar_set_icon_size (toolbar, g_value_get_int (value));
      break;
    case PROP_ICON_SIZE_SET:
      if (g_value_get_boolean (value))
	gtk_toolbar_get_props (toolbar)->icon_size_set = TRUE;
      else
	__gtk_toolbar_unset_icon_size (toolbar);
      break;
    default:
			  ...
			  GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_ORIENTATION:
      g_value_set_enum (value, gtk_toolbar_get_props (toolbar)->orientation);
      break;
    case PROP_TOOLBAR_STYLE:
      g_value_set_enum (value, gtk_toolbar_get_props (toolbar)->style);
      break;
    case PROP_SHOW_ARROW:
      g_value_set_boolean (value, priv->show_arrow);
      break;
    case PROP_TOOLTIPS:
      g_value_set_boolean (value, __gtk_toolbar_get_tooltips (toolbar));
      break;
    case PROP_ICON_SIZE:
      g_value_set_int (value, __gtk_toolbar_get_icon_size (toolbar));
      break;
    case PROP_ICON_SIZE_SET:
      g_value_set_boolean (value, gtk_toolbar_get_props (toolbar)->icon_size_set);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
{
  GtkToolbarPrivate *priv = gtk_toolbar_get_instance_private (widget);
  
  GTK_WIDGET_CLASS (gtk_toolbar_parent_class)->map (widget);
  
  if (priv->event_window)
    __gdk_window_show_unraised (priv->event_window);
}
...
gtk_toolbar_realize (GtkWidget *widget)
{
  ...
  __gdk_window_set_user_data (priv->event_window, toolbar);
}
			  ...
			  GtkRequisition *requisition)
{
  ...
  if (priv->show_arrow && priv->api_mode == NEW_API)
    {
      __gtk_widget_size_request (priv->arrow_button, &arrow_requisition);
  ...
  gtk_toolbar_get_props (toolbar)->button_maxw = max_homogeneous_child_width;
  gtk_toolbar_get_props (toolbar)->button_maxh = max_homogeneous_child_height;
}
          ...
          gdouble     elapsed)
{
  ...
  if (! gtk_toolbar_get_instance_private (toolbar)->animation)
    return to;
	       ...
	       ToolbarContent *content)
{
  ...
  if (gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      if (toolbar_content_is_homogeneous (content, toolbar))
	return gtk_toolbar_get_props (toolbar)->button_maxw;
      else
    ...
    {
      if (toolbar_content_is_homogeneous (content, toolbar))
	return gtk_toolbar_get_props (toolbar)->button_maxh;
      else
...
slide_idle_handler (gpointer data)
{
  ...
  for (list = priv->content; list != NULL; list = list->next)
	  ...
	  toolbar_content_child_visible (content))
	{
      ...
      if (cont)
	{
	  __gtk_widget_queue_resize_no_redraw (GTK_WIDGET (toolbar));
...
gtk_toolbar_begin_sliding (GtkToolbar *toolbar)
{
   ...
   * the equivalent in __gtk_widget_size_allocate().
   */
  priv->is_sliding = TRUE;
  
  if (!priv->idle_id)
    priv->idle_id = __gdk_threads_add_idle (slide_idle_handler, toolbar);
  ...
  if (rtl)
    {
      cur_x = gtk_widget_get_props (widget)->allocation.width - border_width - gtk_widget_get_props (widget)->style->xthickness;
      cur_y = gtk_widget_get_props (widget)->allocation.height - border_width - gtk_widget_get_props (widget)->style->ythickness;
    }
  else
    {
      cur_x = border_width + gtk_widget_get_props (widget)->style->xthickness;
      cur_y = border_width + gtk_widget_get_props (widget)->style->ythickness;
    }
  
  cur_x += gtk_widget_get_props (widget)->allocation.x;
  cur_y += gtk_widget_get_props (widget)->allocation.y;
  ...
  for (list = priv->content; list != NULL; list = list->next)
    {
      ...
      if ((state == NORMAL &&
	   rect_within (&item_allocation, &(gtk_widget_get_props (widget)->allocation))) ||
	  state == OVERFLOWN)
	{
      ...
      else
	{
	  ...
	  if (vertical)
	    {
	      new_start_allocation.width = gtk_widget_get_props (widget)->allocation.width -
		2 * border_width - 2 * gtk_widget_get_props (widget)->style->xthickness;
	      new_start_allocation.height = 0;
	    }
	  else
	    {
	      new_start_allocation.width = 0;
	      new_start_allocation.height = gtk_widget_get_props (widget)->allocation.height -
		2 * border_width - 2 * gtk_widget_get_props (widget)->style->ythickness;
	    }
	}
...
remove_item (GtkWidget *menu_item,
	     gpointer   data)
{
  __gtk_container_remove (GTK_CONTAINER (gtk_widget_get_props (menu_item)->parent), gtk_widget_get_props (menu_item));
}
...
rebuild_menu (GtkToolbar *toolbar)
{
  ...
  if (!priv->menu)
    {
      priv->menu = GTK_MENU (__gtk_menu_new());
      __gtk_menu_attach_to_widget (priv->menu,
				 GTK_WIDGET (toolbar),
				 menu_detached);
  ...
  __gtk_container_foreach (GTK_CONTAINER (priv->menu), remove_item, NULL);
	  ...
	  if (menu_item)
	    {
	      ...
	      __gtk_menu_shell_append (GTK_MENU_SHELL (priv->menu), menu_item);
	    }
	}
    }

  /* Remove leading and trailing separator items */
  children = __gtk_container_get_children (GTK_CONTAINER (priv->menu));
  ...
  while (list && GTK_IS_SEPARATOR_MENU_ITEM (list->data))
    {
      ...
      __gtk_container_remove (GTK_CONTAINER (priv->menu), child);
      list = list->next;
    }
			   ...
			   GtkAllocation *allocation)
{
  ...
  size_changed = FALSE;
  if (gtk_widget_get_props (widget)->allocation.x != allocation->x		||
      gtk_widget_get_props (widget)->allocation.y != allocation->y		||
  ...
  __gtk_widget_get_child_requisition (GTK_WIDGET (priv->arrow_button),
				    &arrow_requisition);
  ...
  if (gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_HORIZONTAL)
	...
	{
	  available_size -= 2 * gtk_widget_get_props (widget)->style->xthickness;
	  short_size -= 2 * gtk_widget_get_props (widget)->style->ythickness;
	}
    }
  else
    {
      ...
      if (shadow_type != GTK_SHADOW_NONE)
	{
	  available_size -= 2 * gtk_widget_get_props (widget)->style->ythickness;
	  short_size -= 2 * gtk_widget_get_props (widget)->style->xthickness;
	}
    }
  ...
  if (gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_VERTICAL)
    {
      for (i = 0; i < n_items; ++i)
  ...
  else if (__gtk_widget_get_direction (GTK_WIDGET (toolbar)) == GTK_TEXT_DIR_RTL)
    {
      for (i = 0; i < n_items; ++i)
  ...
  for (i = 0; i < n_items; ++i)
    {
      ...
      if (shadow_type != GTK_SHADOW_NONE)
	{
	  allocations[i].x += gtk_widget_get_props (widget)->style->xthickness;
	  allocations[i].y += gtk_widget_get_props (widget)->style->ythickness;
	}
    }
  ...
  if (need_arrow)
    {
      ...
      if (shadow_type != GTK_SHADOW_NONE)
	{
	  arrow_allocation.x += gtk_widget_get_props (widget)->style->xthickness;
	  arrow_allocation.y += gtk_widget_get_props (widget)->style->ythickness;
	}
    }
  ...
  if (shadow_type != GTK_SHADOW_NONE)
    {
      item_area.x += gtk_widget_get_props (widget)->style->xthickness;
      item_area.y += gtk_widget_get_props (widget)->style->ythickness;
    }
  ...
  if (need_arrow)
    {
      __gtk_widget_size_allocate (GTK_WIDGET (priv->arrow_button),
				&arrow_allocation);
      __gtk_widget_show (GTK_WIDGET (priv->arrow_button));
    }
  else
    {
      __gtk_widget_hide (GTK_WIDGET (priv->arrow_button));
...
gtk_toolbar_update_button_relief (GtkToolbar *toolbar)
{
  ...
  if (relief != __gtk_button_get_relief (GTK_BUTTON (priv->arrow_button)))
    {
      gtk_toolbar_reconfigured (toolbar);
  
      __gtk_button_set_relief (GTK_BUTTON (priv->arrow_button), relief);
    }
}
					  ...
					  GtkDirectionType  dir)
{
  ...
  rtl = (__gtk_widget_get_direction (GTK_WIDGET (toolbar)) == GTK_TEXT_DIR_RTL);
			       ...
			       gboolean    focus_home)
{
  ...
  if (__gtk_widget_get_direction (GTK_WIDGET (toolbar)) == GTK_TEXT_DIR_RTL)
    {
			...
			GtkDirectionType  dir)
{
  ...
  if (gtk_container_get_props (container)->focus_child &&
      __gtk_widget_child_focus (gtk_container_get_props (container)->focus_child, dir))
    {
      return;
    }
...
{
  if (!gtk_toolbar_get_props (toolbar)->style_set)
    {
      /* pretend it was set, then unset, thus reverting to new default */
      gtk_toolbar_get_props (toolbar)->style_set = TRUE;
      __gtk_toolbar_unset_style (toolbar);
    }
}
...
{
  if (!gtk_toolbar_get_props (toolbar)->icon_size_set)
    {
      /* pretend it was set, then unset, thus reverting to new default */
      gtk_toolbar_get_props (toolbar)->icon_size_set = TRUE;
      __gtk_toolbar_unset_icon_size (toolbar);
			    ...
			    GdkScreen *previous_screen)
{
  ...
  if (__gtk_widget_has_screen (GTK_WIDGET (toolbar)))
    settings = __gtk_widget_get_settings (GTK_WIDGET (toolbar));
  else
    settings = NULL;
		 ...
		 gint        y)
{
  ...
  orientation = gtk_toolbar_get_props (toolbar)->orientation;
  direction = __gtk_widget_get_direction (GTK_WIDGET (toolbar));
...
/**
 * __gtk_toolbar_set_drop_highlight_item:
 ...
 **/
void
__gtk_toolbar_set_drop_highlight_item (GtkToolbar  *toolbar,
				     ...
				     gint         index_)
{
  ...
  if (!tool_item)
	...
	{
	  __gtk_widget_unparent (GTK_WIDGET (priv->highlight_tool_item));
  ...
  n_items = __gtk_toolbar_get_n_items (toolbar);
  ...
  if (tool_item != priv->highlight_tool_item)
	...
	g_object_unref (priv->highlight_tool_item);
      ...
      __gtk_widget_set_parent (GTK_WIDGET (priv->highlight_tool_item),
			     GTK_WIDGET (toolbar));
    }
  ...
  if (!content || !toolbar_content_is_placeholder (content))
    {
      ...
      placeholder = GTK_WIDGET (__gtk_separator_tool_item_new ());

      content = toolbar_content_new_tool_item (toolbar,
					       GTK_TOOL_ITEM (placeholder),
					       TRUE, index_);
      __gtk_widget_show (placeholder);
    }
  
  g_assert (content);
  g_assert (toolbar_content_is_placeholder (content));
  
  __gtk_widget_size_request (GTK_WIDGET (priv->highlight_tool_item),
			   &requisition);

  toolbar_content_set_expand (content, __gtk_tool_item_get_expand (tool_item));
				...
				GParamSpec   *pspec)
{
  ...
  switch (property_id)
    ...
    case CHILD_PROP_HOMOGENEOUS:
      g_value_set_boolean (value, __gtk_tool_item_get_homogeneous (item));
      break;
      
    case CHILD_PROP_EXPAND:
      g_value_set_boolean (value, __gtk_tool_item_get_expand (item));
      break;
				...
				GParamSpec   *pspec)
{
  switch (property_id)
    {
    case CHILD_PROP_HOMOGENEOUS:
      __gtk_tool_item_set_homogeneous (GTK_TOOL_ITEM (child), g_value_get_boolean (value));
      break;
      
    case CHILD_PROP_EXPAND:
      __gtk_tool_item_set_expand (GTK_TOOL_ITEM (child), g_value_get_boolean (value));
      break;
		 ...
		 GtkWidget    *widget)
{
  ...
  if (GTK_IS_TOOL_ITEM (widget))
    __gtk_toolbar_insert (toolbar, GTK_TOOL_ITEM (widget), -1);
  else
    __gtk_toolbar_append_widget (toolbar, widget, NULL, NULL);
}
...
{
  GtkToolbarPrivate *priv = gtk_toolbar_get_instance_private (toolbar);
  if (gtk_toolbar_get_props (toolbar)->orientation != orientation)
    {
      gtk_toolbar_get_props (toolbar)->orientation = orientation;
	...
	__gtk_arrow_set (GTK_ARROW (priv->arrow), GTK_ARROW_RIGHT, GTK_SHADOW_NONE);
      ...
      __gtk_widget_queue_resize (GTK_WIDGET (toolbar));
...
gtk_toolbar_real_style_changed (GtkToolbar     *toolbar,
				GtkToolbarStyle style)
{
  if (gtk_toolbar_get_props (toolbar)->style != style)
    {
      gtk_toolbar_get_props (toolbar)->style = style;
	   ...
	   GdkEventButton *event)
{
  ...
  __gtk_widget_show_all (GTK_WIDGET (priv->menu));
				  ...
				  GtkToolbar *toolbar)
{
  ...
  if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->arrow_button)) &&
      (!priv->menu || !__gtk_widget_get_visible (GTK_WIDGET (priv->menu))))
    {
				...
				GtkToolbar     *toolbar)
{
  ...
  __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button), TRUE);
 ...
 **/
void
__gtk_toolbar_insert (GtkToolbar  *toolbar,
		    GtkToolItem *item,
		    gint         pos)
{
  g_return_if_fail (GTK_IS_TOOLBAR (toolbar));
 ...
 **/
gint
__gtk_toolbar_get_item_index (GtkToolbar  *toolbar,
			    GtkToolItem *item)
{
  GtkToolbarPrivate *priv;
  GList *list;
 ...
 **/
void
__gtk_toolbar_set_orientation (GtkToolbar     *toolbar,
			     GtkOrientation  orientation)
{
  g_return_if_fail (GTK_IS_TOOLBAR (toolbar));
 ...
 **/
GtkOrientation
__gtk_toolbar_get_orientation (GtkToolbar *toolbar)
{
  g_return_val_if_fail (GTK_IS_TOOLBAR (toolbar), GTK_ORIENTATION_HORIZONTAL);
  
  return gtk_toolbar_get_props (toolbar)->orientation;
}

/**
 * __gtk_toolbar_set_style:
 * @toolbar: a #GtkToolbar.
 ...
 **/
void
__gtk_toolbar_set_style (GtkToolbar      *toolbar,
		       GtkToolbarStyle  style)
{
  g_return_if_fail (GTK_IS_TOOLBAR (toolbar));
 ...
 **/
GtkToolbarStyle
__gtk_toolbar_get_style (GtkToolbar *toolbar)
{
  g_return_val_if_fail (GTK_IS_TOOLBAR (toolbar), DEFAULT_TOOLBAR_STYLE);
  
  return gtk_toolbar_get_props (toolbar)->style;
}

/**
 * __gtk_toolbar_unset_style:
 * @toolbar: a #GtkToolbar
 * 
 * Unsets a toolbar style set with __gtk_toolbar_set_style(), so that
 * user preferences will be used to determine the toolbar style.
 **/
void
__gtk_toolbar_unset_style (GtkToolbar *toolbar)
{
  GtkToolbarStyle style;
  ...
  if (gtk_toolbar_get_props (toolbar)->style_set)
	...
	style = DEFAULT_TOOLBAR_STYLE;
      ...
      if (style != gtk_toolbar_get_props (toolbar)->style)
	g_signal_emit (toolbar, toolbar_signals[STYLE_CHANGED], 0, style);
      
      gtk_toolbar_get_props (toolbar)->style_set = FALSE;
    }
}

/**
 * __gtk_toolbar_set_tooltips:
 * @toolbar: a #GtkToolbar.
 ...
 **/
gint
__gtk_toolbar_get_n_items (GtkToolbar *toolbar)
{
  GtkToolbarPrivate *priv;
 ...
 **/
GtkToolItem *
__gtk_toolbar_get_nth_item (GtkToolbar *toolbar,
			  gint        n)
{
  GtkToolbarPrivate *priv;
  ToolbarContent *content;
...
/**
 * __gtk_toolbar_get_icon_size:
 * @toolbar: a #GtkToolbar
 *
 * Retrieves the icon size for the toolbar. See __gtk_toolbar_set_icon_size().
 *
 * Return value: (type int): the current icon size for the icons on
 * the toolbar.
 **/
GtkIconSize
__gtk_toolbar_get_icon_size (GtkToolbar *toolbar)
{
  g_return_val_if_fail (GTK_IS_TOOLBAR (toolbar), DEFAULT_ICON_SIZE);
  
  return gtk_toolbar_get_props (toolbar)->icon_size;
}

/**
 * __gtk_toolbar_get_relief_style:
 * @toolbar: a #GtkToolbar
 * 
 * Returns the relief style of buttons on @toolbar. See
 * __gtk_button_set_relief().
 * 
 ...
 **/
GtkReliefStyle
__gtk_toolbar_get_relief_style (GtkToolbar *toolbar)
{
 ...
 * and which have a proxy menu item set by __gtk_tool_item_set_proxy_menu_item()
 ...
 **/
void
__gtk_toolbar_set_show_arrow (GtkToolbar *toolbar,
			    gboolean    show_arrow)
{
  GtkToolbarPrivate *priv;
  ...
  if (priv->show_arrow != show_arrow)
    {
      ...
      if (!priv->show_arrow)
	__gtk_widget_hide (priv->arrow_button);
...
/**
 * __gtk_toolbar_get_show_arrow:
 * @toolbar: a #GtkToolbar
 * 
 * Returns whether the toolbar has an overflow menu.
 * See __gtk_toolbar_set_show_arrow().
 * 
 ...
 **/
gboolean
__gtk_toolbar_get_show_arrow (GtkToolbar *toolbar)
{
  GtkToolbarPrivate *priv;
 ...
 **/
gint
__gtk_toolbar_get_drop_index (GtkToolbar *toolbar,
			    gint        x,
			    gint        y)
{
  g_return_val_if_fail (GTK_IS_TOOLBAR (toolbar), -1);
...
gtk_toolbar_dispose (GObject *object)
{
  ...
  if (priv->arrow_button)
    {
      __gtk_widget_unparent (priv->arrow_button);
      priv->arrow_button = NULL;
    }

  if (priv->menu)
    __gtk_widget_destroy (GTK_WIDGET (priv->menu));
...
gtk_toolbar_finalize (GObject *object)
{
  ...
  g_list_free (gtk_toolbar_get_props (toolbar)->children);
 ...
 **/
void
__gtk_toolbar_set_icon_size (GtkToolbar  *toolbar,
			   GtkIconSize  icon_size)
{
  ...
  if (gtk_toolbar_get_props (toolbar)->icon_size == icon_size)
    return;
  
  gtk_toolbar_get_props (toolbar)->icon_size = icon_size;
...
/**
 * __gtk_toolbar_unset_icon_size:
 * @toolbar: a #GtkToolbar
 * 
 * Unsets toolbar icon size set with __gtk_toolbar_set_icon_size(), so that
 * user preferences will be used to determine the icon size.
 **/
void
__gtk_toolbar_unset_icon_size (GtkToolbar *toolbar)
{
  GtkIconSize size;
  ...
  if (gtk_toolbar_get_props (toolbar)->icon_size_set)
	...
	size = DEFAULT_ICON_SIZE;
      ...
      if (size != gtk_toolbar_get_props (toolbar)->icon_size)
	{
	  __gtk_toolbar_set_icon_size (toolbar, size);
	  g_object_notify (G_OBJECT (toolbar), "icon-size");	  
	}
      
      gtk_toolbar_get_props (toolbar)->icon_size_set = FALSE;
      g_object_notify (G_OBJECT (toolbar), "icon-size-set");      
    }
}
 ...
 *
 * Deprecated: 2.4: Use __gtk_toolbar_insert() instead.
 **/
GtkWidget *
__gtk_toolbar_append_item (GtkToolbar    *toolbar,
			 const char    *text,
			 const char    *tooltip_text,
			 const char    *tooltip_private_text,
			 GtkWidget     *icon,
			 ...
			 gpointer       user_data)
{
  return __gtk_toolbar_insert_element (toolbar, GTK_TOOLBAR_CHILD_BUTTON,
				     NULL, text,
				     tooltip_text, tooltip_private_text,
				     icon, callback, user_data,
				     gtk_toolbar_get_props (toolbar)->num_children);
}

/**
 * __gtk_toolbar_prepend_item:
 * @toolbar: a #GtkToolbar.
 * @text: give your toolbar button a label.
 * @tooltip_text: a string that appears when the user holds the mouse over this item.
 * @tooltip_private_text: use with #GtkTipsQuery.
 ...
 **/
GtkWidget *
__gtk_toolbar_prepend_item (GtkToolbar    *toolbar,
			  const char    *text,
			  const char    *tooltip_text,
			  const char    *tooltip_private_text,
			  GtkWidget     *icon,
 ...
 **/
GtkWidget *
__gtk_toolbar_insert_item (GtkToolbar    *toolbar,
			 const char    *text,
			 const char    *tooltip_text,
			 const char    *tooltip_private_text,
			 GtkWidget     *icon,
 ...
 */
GtkWidget*
__gtk_toolbar_insert_stock (GtkToolbar      *toolbar,
 ...
 **/
void
__gtk_toolbar_append_space (GtkToolbar *toolbar)
{
  __gtk_toolbar_insert_element (toolbar, GTK_TOOLBAR_CHILD_SPACE,
			      NULL, NULL,
			      NULL, NULL,
			      NULL, NULL, NULL,
			      gtk_toolbar_get_props (toolbar)->num_children);
 ...
 **/
void
__gtk_toolbar_prepend_space (GtkToolbar *toolbar)
{
  __gtk_toolbar_insert_element (toolbar, GTK_TOOLBAR_CHILD_SPACE,
			      NULL, NULL,
			      NULL, NULL,
 ...
 **/
void
__gtk_toolbar_insert_space (GtkToolbar *toolbar,
			  gint        position)
{
  __gtk_toolbar_insert_element (toolbar, GTK_TOOLBAR_CHILD_SPACE,
			      NULL, NULL,
 ...
 **/
void
__gtk_toolbar_remove_space (GtkToolbar *toolbar,
			  gint        position)
{
  GtkToolbarPrivate *priv;
  ToolbarContent *content;
 ...
 **/
void
__gtk_toolbar_append_widget (GtkToolbar  *toolbar,
			   GtkWidget   *widget,
			   const gchar *tooltip_text,
			   const gchar *tooltip_private_text)
{
  __gtk_toolbar_insert_element (toolbar, GTK_TOOLBAR_CHILD_WIDGET,
			      widget, NULL,
			      tooltip_text, tooltip_private_text,
			      NULL, NULL, NULL,
			      gtk_toolbar_get_props (toolbar)->num_children);
 ...
 **/
void
__gtk_toolbar_prepend_widget (GtkToolbar  *toolbar,
			    GtkWidget   *widget,
			    const gchar *tooltip_text,
			    const gchar *tooltip_private_text)
{
 ...
 **/ 
void
__gtk_toolbar_insert_widget (GtkToolbar *toolbar,
			   GtkWidget  *widget,
			   const char *tooltip_text,
			   const char *tooltip_private_text,
			   gint        position)
 ...
 **/
GtkWidget*
__gtk_toolbar_append_element (GtkToolbar          *toolbar,
			    GtkToolbarChildType  type,
			    GtkWidget           *widget,
			    const char          *text,
			    const char          *tooltip_text,
			    ...
			    gpointer             user_data)
{
  return __gtk_toolbar_insert_element (toolbar, type, widget, text,
				     tooltip_text, tooltip_private_text,
				     icon, callback, user_data,
				     gtk_toolbar_get_props (toolbar)->num_children);
}
 ...
 **/
GtkWidget *
__gtk_toolbar_prepend_element (GtkToolbar          *toolbar,
			     GtkToolbarChildType  type,
			     GtkWidget           *widget,
			     const char          *text,
			     const char          *tooltip_text,
 ...
 **/
GtkWidget *
__gtk_toolbar_insert_element (GtkToolbar          *toolbar,
			    GtkToolbarChildType  type,
			    GtkWidget           *widget,
			    const char          *text,
			    const char          *tooltip_text,
                                 ...
                                 GtkToolbarChild *child)
{
  ...
  box = __gtk_bin_get_child (GTK_BIN (child->widget));
  
  g_return_if_fail (GTK_IS_BOX (box));
  
  if (child->label)
    {
      expand = (gtk_toolbar_get_props (toolbar)->style != GTK_TOOLBAR_BOTH);
      
      __gtk_box_set_child_packing (GTK_BOX (box), child->label,
                                 expand, expand, 0, GTK_PACK_END);
      
      if (gtk_toolbar_get_props (toolbar)->style != GTK_TOOLBAR_ICONS)
        __gtk_widget_show (child->label);
      else
        __gtk_widget_hide (child->label);
    }
  
  if (child->icon)
    {
      expand = (gtk_toolbar_get_props (toolbar)->style != GTK_TOOLBAR_BOTH_HORIZ);
      
      __gtk_box_set_child_packing (GTK_BOX (box), child->icon,
                                 expand, expand, 0, GTK_PACK_END);
      
      if (gtk_toolbar_get_props (toolbar)->style != GTK_TOOLBAR_TEXT)
        __gtk_widget_show (child->icon);
      else
        __gtk_widget_hide (child->icon);
    }
}
			 ...
			 gboolean             use_stock)
{
  ...
  switch (type)
	...
	{
	  child_widget = __gtk_button_new ();
	}
      else if (type == GTK_TOOLBAR_CHILD_TOGGLEBUTTON)
	{
	  child_widget = __gtk_toggle_button_new ();
	  __gtk_toggle_button_set_mode (GTK_TOGGLE_BUTTON (child_widget), FALSE);
	}
      else /* type == GTK_TOOLBAR_CHILD_RADIOBUTTON */
	{
	  ...
	  if (widget)
	    group = __gtk_radio_button_get_group (GTK_RADIO_BUTTON (widget));
	  
	  child_widget = __gtk_radio_button_new (group);
	  __gtk_toggle_button_set_mode (GTK_TOGGLE_BUTTON (child_widget), FALSE);
	}

      __gtk_button_set_relief (GTK_BUTTON (child_widget), get_button_relief (toolbar));
      __gtk_button_set_focus_on_click (GTK_BUTTON (child_widget), FALSE);
      ...
      if (callback)
	{
      ...
      if (gtk_toolbar_get_props (toolbar)->style == GTK_TOOLBAR_BOTH_HORIZ)
	box = __gtk_hbox_new (FALSE, 0);
      else
	box = __gtk_vbox_new (FALSE, 0);

      __gtk_container_add (GTK_CONTAINER (child_widget), box);
      __gtk_widget_show (box);
      
      if (text && use_stock)
	{
	  GtkStockItem stock_item;
	  if (__gtk_stock_lookup (text, &stock_item))
	    {
	      if (!icon)
		icon = __gtk_image_new_from_stock (text, gtk_toolbar_get_props (toolbar)->icon_size);
	  
	      text = free_me = ___gtk_toolbar_elide_underscores (stock_item.label);
	    }
	}
      
      if (text)
	{
	  child_label = __gtk_label_new (text);
	  
	  __gtk_container_add (GTK_CONTAINER (box), child_label);
	}
      
      if (icon)
	{
	  child_icon = GTK_WIDGET (icon);
          __gtk_container_add (GTK_CONTAINER (box), child_icon);
	}
      
      __gtk_widget_show (child_widget);
      break;
  ...
  if ((type != GTK_TOOLBAR_CHILD_SPACE) && tooltip_text)
    {
      gtk_tooltips_set_tip (gtk_toolbar_get_props (toolbar)->tooltips, child_widget,
			    tooltip_text, tooltip_private_text);
    }
			       ...
			       gint	    pos)
{
  ...
  __gtk_widget_set_parent (GTK_WIDGET (item), GTK_WIDGET (toolbar));
				   ...
				   gint			 pos)
{
  ...
  if (type != GTK_TOOLBAR_CHILD_SPACE)
    {
      __gtk_widget_set_parent (child->widget, GTK_WIDGET (toolbar));
    }
  else
    {
  ...
  priv->content = g_list_insert (priv->content, content, pos);
  gtk_toolbar_get_props (toolbar)->children = g_list_insert (gtk_toolbar_get_props (toolbar)->children, child, pos);
  priv->need_rebuild = TRUE;
			...
			GtkToolbar     *toolbar)
{
  ...
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_widget_unparent (GTK_WIDGET (content->u.tool_item.item));
      break;
      ...
      if (child->type != GTK_TOOLBAR_CHILD_SPACE)
	{
	  ...
	  __gtk_widget_unparent (child->widget);
	  __gtk_widget_destroy (child->widget);
	  g_object_unref (child->widget);
	}
      
      gtk_toolbar_get_props (toolbar)->children = g_list_remove (gtk_toolbar_get_props (toolbar)->children, child);
      break;
    }
			...
			GdkEventExpose *expose)
{
  ...
  switch (content->type)
      ...
      if (child->type == GTK_TOOLBAR_CHILD_SPACE)
	{
	  if (content->u.compatibility.space_visible &&
	     ...
	     ___gtk_toolbar_paint_space_line (GTK_WIDGET (toolbar), toolbar,
					    &expose->area,
  ...
  if (widget)
    __gtk_container_propagate_expose (container, widget, expose);
}
			 ...
			 GtkToolbar     *toolbar)
{
  ...
  switch (content->type)
    ...
    case TOOL_ITEM:
      ...
      if (!__gtk_widget_get_visible (GTK_WIDGET (item)))
	return FALSE;
      
      if (gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_HORIZONTAL &&
	  __gtk_tool_item_get_visible_horizontal (item))
	return TRUE;
      
      if ((gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_VERTICAL &&
	   __gtk_tool_item_get_visible_vertical (item)))
	return TRUE;
	...
	return __gtk_widget_get_visible (content->u.compatibility.child.widget);
      else
	return TRUE;
      break;
    }
			      ...
			      GtkRequisition *requisition)
{
  ...
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_widget_size_request (GTK_WIDGET (content->u.tool_item.item),
			       requisition);
      if (content->u.tool_item.is_placeholder &&
    ...
    case COMPATIBILITY:
      ...
      if (content->u.compatibility.child.type != GTK_TOOLBAR_CHILD_SPACE)
	{
	  __gtk_widget_size_request (content->u.compatibility.child.widget,
				   requisition);
	}
      else
	{
				...
				GtkToolbar     *toolbar)
{
  ...
  switch (content->type)
    {
    case TOOL_ITEM:
      result = __gtk_tool_item_get_homogeneous (content->u.tool_item.item) &&
	!GTK_IS_SEPARATOR_TOOL_ITEM (content->u.tool_item.item);
      
      if (__gtk_tool_item_get_is_important (content->u.tool_item.item) &&
	  gtk_toolbar_get_props (toolbar)->style == GTK_TOOLBAR_BOTH_HORIZ &&
	  gtk_toolbar_get_props (toolbar)->orientation == GTK_ORIENTATION_HORIZONTAL)
	{
	  result = FALSE;
	}
				...
				GtkAllocation  *allocation)
{
  ...
  switch (content->type)
    {
    case TOOL_ITEM:
      *allocation = gtk_widget_get_props (GTK_WIDGET (content->u.tool_item.item))->allocation;
      break;
...
toolbar_content_get_expand (ToolbarContent *content)
{
  if (content->type == TOOL_ITEM &&
      __gtk_tool_item_get_expand (content->u.tool_item.item) &&
      !content->u.tool_item.disappearing)
    {
      return TRUE;
    }
				   ...
				   gboolean        visible)
{
  ...
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_widget_set_child_visible (GTK_WIDGET (content->u.tool_item.item),
				    visible);
      break;
    ...
    case COMPATIBILITY:
      ...
      if (child->type != GTK_TOOLBAR_CHILD_SPACE)
	{
	  __gtk_widget_set_child_visible (child->widget, visible);
	}
      else
	{
	  if (content->u.compatibility.space_visible != visible)
	    {
	      ...
	      __gtk_widget_queue_draw (GTK_WIDGET (toolbar));
	    }
	}
      break;
    }
			       ...
			       GtkAllocation  *allocation)
{
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_widget_size_allocate (GTK_WIDGET (content->u.tool_item.item),
				allocation);
      break;
    ...
    case COMPATIBILITY:
      if (content->u.compatibility.child.type != GTK_TOOLBAR_CHILD_SPACE)
	{
	  __gtk_widget_size_allocate (content->u.compatibility.child.widget,
				    allocation);
	}
      else
	{
				  ...
				  gint            height)
{
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_widget_set_size_request (GTK_WIDGET (content->u.tool_item.item),
				   width, height);
      break;
			   ...
			   GtkToolbarChild *child)
{
  ...
  style = __gtk_toolbar_get_style (toolbar);
  icon_size = __gtk_toolbar_get_icon_size (toolbar);
  relief = __gtk_toolbar_get_relief_style (toolbar);
  ...
  if (child->type == GTK_TOOLBAR_CHILD_BUTTON ||
      ...
      if (style == GTK_TOOLBAR_BOTH && GTK_IS_HBOX (box))
	{
	  ...
	  if (child->label)
	    __gtk_widget_reparent (child->label, vbox);
	  if (child->icon)
	    __gtk_widget_reparent (child->icon, vbox);
	  
	  __gtk_widget_destroy (box);
	  __gtk_container_add (GTK_CONTAINER (child->widget), vbox);
      ...
      else if (style == GTK_TOOLBAR_BOTH_HORIZ && GTK_IS_VBOX (box))
	{
	  ...
	  if (child->label)
	    __gtk_widget_reparent (child->label, hbox);
	  if (child->icon)
	    __gtk_widget_reparent (child->icon, hbox);
	  
	  __gtk_widget_destroy (box);
	  __gtk_container_add (GTK_CONTAINER (child->widget), hbox);
  ...
  if ((child->type == GTK_TOOLBAR_CHILD_BUTTON ||
      ...
      GTK_IS_IMAGE (child->icon))
    {
      ...
      if (__gtk_image_get_storage_type (image) == GTK_IMAGE_STOCK)
	{
	  __gtk_image_get_stock (image, &stock_id, NULL);
	  stock_id = g_strdup (stock_id);
	  __gtk_image_set_from_stock (image,
				    stock_id,
				    icon_size);
	  g_free (stock_id);
	}
  ...
  if (child->type == GTK_TOOLBAR_CHILD_BUTTON ||
      ...
      child->type == GTK_TOOLBAR_CHILD_TOGGLEBUTTON)
    {
      __gtk_button_set_relief (GTK_BUTTON (child->widget), relief);
    }
}
				      ...
				      GtkToolbar     *toolbar)
{
  switch (content->type)
    {
    case TOOL_ITEM:
      __gtk_tool_item_toolbar_reconfigured (content->u.tool_item.item);
      break;
...
{
  if (content->type == TOOL_ITEM)
    return __gtk_tool_item_retrieve_proxy_menu_item (content->u.tool_item.item);
...
{
  if (content->type == TOOL_ITEM)
    __gtk_tool_item_set_expand (content->u.tool_item.item, expand);
}
...
get_space_size (GtkToolbar *toolbar)
{
  ...
  if (toolbar)
    {
      __gtk_widget_style_get (GTK_WIDGET (toolbar),
...
get_button_relief (GtkToolbar *toolbar)
{
  ...
  __gtk_widget_ensure_style (GTK_WIDGET (toolbar));
...
gint
___gtk_toolbar_get_default_space_size (void)
{
  return DEFAULT_SPACE_SIZE;
}

void
___gtk_toolbar_paint_space_line (GtkWidget           *widget,
			       GtkToolbar          *toolbar,
			       ...
			       const GtkAllocation *allocation)
{
  ...
  orientation = gtk_toolbar_get_props (toolbar)? gtk_toolbar_get_props (toolbar)->orientation : GTK_ORIENTATION_HORIZONTAL;
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      ...
      if (wide_separators)
        __gtk_paint_box (gtk_widget_get_props (widget)->style, gtk_widget_get_props (widget)->window,
                       __gtk_widget_get_state (widget), GTK_SHADOW_ETCHED_OUT,
        ...
        __gtk_paint_vline (gtk_widget_get_props (widget)->style, gtk_widget_get_props (widget)->window,
                         __gtk_widget_get_state (widget), area, widget,
                         "toolbar",
                         allocation->y + allocation->height * start_fraction,
                         allocation->y + allocation->height * end_fraction,
                         allocation->x + (allocation->width - gtk_widget_get_props (widget)->style->xthickness) / 2);
    }
  else
    {
      gboolean wide_separators;
      ...
      if (wide_separators)
        ...
        __gtk_paint_hline (gtk_widget_get_props (widget)->style, gtk_widget_get_props (widget)->window,
                         ...
                         allocation->y + (allocation->height - gtk_widget_get_props (widget)->style->ythickness) / 2);
    }
}

gchar *
___gtk_toolbar_elide_underscores (const gchar *original)
{
  gchar *q, *result;
  const gchar *p, *end;
  gsize len;
...
{
  GtkToolbarPrivate *priv = gtk_toolbar_get_instance_private (shell);
  GList *list;
