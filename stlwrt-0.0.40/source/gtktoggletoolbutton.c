...
gtk_toggle_tool_button_init (GtkToggleToolButton *button)
{
  ...
  GtkToggleButton *toggle_button = GTK_TOGGLE_BUTTON (______gtk_tool_button_get_button (tool_button));
				     ...
				     GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
      case PROP_ACTIVE:
	__gtk_toggle_tool_button_set_active (button, 
				     ...
				     GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
      case PROP_ACTIVE:
        g_value_set_boolean (value, __gtk_toggle_tool_button_get_active (button));
        break;
...
gtk_toggle_tool_button_create_menu_proxy (GtkToolItem *item)
{
  ...
  if (______gtk_tool_item_create_menu_proxy (item))
    return TRUE;

  label_widget = __gtk_tool_button_get_label_widget (tool_button);
  label_text = __gtk_tool_button_get_label (tool_button);
  stock_id = __gtk_tool_button_get_stock_id (tool_button);

  if (GTK_IS_LABEL (label_widget))
    {
      label = __gtk_label_get_label (GTK_LABEL (label_widget));
      use_mnemonic = __gtk_label_get_use_underline (GTK_LABEL (label_widget));
    }
  else if (label_text)
    {
      label = label_text;
      use_mnemonic = __gtk_tool_button_get_use_underline (tool_button);
    }
  else if (stock_id && __gtk_stock_lookup (stock_id, &stock_item))
    {
  ...
  if (use_mnemonic)
    menu_item = __gtk_check_menu_item_new_with_mnemonic (label);
  else
  ...
  if (GTK_IS_RADIO_TOOL_BUTTON (toggle_tool_button))
    {
      __gtk_check_menu_item_set_draw_as_radio (GTK_CHECK_MENU_ITEM (menu_item),
					     TRUE);
    }
  ...
  __gtk_tool_item_set_proxy_menu_item (item, MENU_ID, menu_item);
  ...
  if (toggle_tool_button->priv->active != menu_active)
    {
      ...
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (______gtk_tool_button_get_button (tool_button)),
				    toggle_tool_button->priv->active);
  ...
  if (strcmp (property_name, "active") == 0)
    {
      ...
      __gtk_toggle_tool_button_set_active (button, __gtk_toggle_action_get_active (GTK_TOGGLE_ACTION (action)));
 ...
 **/
GtkToolItem *
__gtk_toggle_tool_button_new (void)
{
...
/**
 * __gtk_toggle_tool_button_new_from_stock:
 ...
 **/
GtkToolItem *
__gtk_toggle_tool_button_new_from_stock (const gchar *stock_id)
{
  GtkToolButton *button;
...
/**
 * __gtk_toggle_tool_button_set_active:
 ...
 **/
void
__gtk_toggle_tool_button_set_active (GtkToggleToolButton *button,
				   gboolean is_active)
{
  ...
  if (button->priv->active != is_active)
    __gtk_button_clicked (GTK_BUTTON (______gtk_tool_button_get_button (GTK_TOOL_BUTTON (button))));
}

/**
 * __gtk_toggle_tool_button_get_active:
 * @button: a #GtkToggleToolButton
 * 
 ...
 **/
gboolean
__gtk_toggle_tool_button_get_active (GtkToggleToolButton *button)
{
