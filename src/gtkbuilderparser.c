		 ...
		 GError      **error)
{
  ...
  for (i = 0; names[i] != NULL; i++)
      ...
      if (strcmp (names[i], "domain") == 0)
	{
	  ...
	  __gtk_builder_set_translation_domain (data->builder, data->domain);
	}
      else
	       ...
	       GError             **error)
{
  ...
  __gtk_buildable_custom_tag_end (GTK_BUILDABLE (data->subparser->object),
				data->builder,
	      ...
	      GError             **error)
{
  ...
  if (!__gtk_buildable_custom_tag_start (GTK_BUILDABLE (object),
				       data->builder,
				       child,
				       element_name,
				       &parser,
                                  ...
                                  GError      **error)
{
   ...
   * __gtk_builder_get_translation_domain() without breaking the ABI or API
   */
  domain = __gtk_builder_get_translation_domain (builder);
  ...
  for (l = data->custom_finalizers; l; l = l->next)
    {
      ...
      __gtk_buildable_custom_finished (GTK_BUILDABLE (sub->object),
                                     builder,
                                     sub->child,
                                     sub->tagname,
                                     sub->data);
  ...
  for (l = data->finalizers; l; l = l->next)
    {
      ...
      __gtk_buildable_parser_finished (GTK_BUILDABLE (buildable), builder);
    }
 ...
 out:
  ...
  __gtk_builder_set_translation_domain (builder, domain);
}
