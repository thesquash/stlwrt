		       ...
		       gboolean  is_child)
{
  ...
  if (is_child)
	...
	__gtk_widget_unmap (GTK_WIDGET (plug));
      ...
      ______gtk_window_set_is_toplevel (GTK_WINDOW (plug), FALSE);
      __gtk_container_set_resize_mode (GTK_CONTAINER (plug), GTK_RESIZE_PARENT);

      ______gtk_widget_propagate_hierarchy_changed (GTK_WIDGET (plug), GTK_WIDGET (plug));
    }
  else
    {
	...
	__gtk_window_set_default (GTK_WINDOW (plug), NULL);
      ...
      ______gtk_window_set_is_toplevel (GTK_WINDOW (plug), TRUE);
      __gtk_container_set_resize_mode (GTK_CONTAINER (plug), GTK_RESIZE_QUEUE);

      ______gtk_widget_propagate_hierarchy_changed (GTK_WIDGET (plug), NULL);
    }
}
 ...
 **/
void
______gtk_plug_add_to_socket (GtkPlug   *plug,
			 GtkSocket *socket_)
{
  GtkWidget *widget;
  gint w, h;
...
/**
 * ______gtk_plug_remove_from_socket:
 * @plug: a #GtkPlug
 * @socket_: a #GtkSocket
 * 
 ...
 **/
void
______gtk_plug_remove_from_socket (GtkPlug   *plug,
			      GtkSocket *socket_)
{
  GtkWidget *widget;
  gboolean result;
  ...
  if (socket_id)
      ...
      if (plug->socket_window)
	{
	  ...
	  if (user_data)
		...
		______gtk_plug_add_to_socket (plug, user_data);
	      else
		{
...
gtk_plug_keys_changed (GtkWindow *window)
{
  ...
  ______gtk_window_keys_foreach (window, keys_foreach, new_grabbed_keys);
