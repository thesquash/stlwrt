		       ...
		       GtkKeyHashEntry *entry)
{
  ...
  g_free (entry->keys);
  __gdk_keymap_get_entries_for_keyval (key_hash->keymap,
				     entry->keyval,
...
{                 
  ...
  __gdk_keymap_get_entries_for_keyval (keymap,
				     keyval,
				     &entry.keys, &entry.n_keys);
  ...
  if (keys)
      ...
      while (tmp_list)
	{
	   ...
	   */
          ...
          if (__gdk_keymap_map_virtual_modifiers (key_hash->keymap, &modifiers) &&
			     ...
			     GdkModifierType modifiers)
{
   ...
   */
  __gdk_keymap_get_entries_for_keyval (key_hash->keymap, keyval,
				     &keys, &n_keys);
