 ...
 **/
gchar **
______gtk_get_module_path (const gchar *type)
{
 ...
 **/
gchar *
______gtk_find_module (const gchar *name,
		  const gchar *type)
{
  gchar **paths;
  gchar **path;
  ...
  paths = ______gtk_get_module_path (type);
  for (path = paths; *path; path++)
    {
      gchar *tmp_name;
...
find_module (const gchar *name)
{
  ...
  module_name = ______gtk_find_module (name, "modules");
  if (!module_name)
    {
		   ...
		   GdkDisplay        *display)
{
  ...
  for (i = 0; i < __gdk_display_get_n_screens (display); i++)
	...
	{
	  settings = __gtk_settings_get_for_screen (screen);
	  ______gtk_modules_settings_changed (settings, g_value_get_string (&value));
	  g_value_unset (&value);
	}
    }
...
void
______gtk_modules_init (gint        *argc, 
		   gchar     ***argv, 
...
void
______gtk_modules_settings_changed (GtkSettings *settings, 
			       const gchar *modules)
{
