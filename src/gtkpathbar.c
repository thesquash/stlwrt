...
on_slider_unmap (GtkWidget  *widget,
		 GtkPathBar *path_bar)
{
  if (gtk_path_bar_get_props (path_bar)->timer &&
      ((widget == gtk_path_bar_get_props (path_bar)->up_slider_button && gtk_path_bar_get_props (path_bar)->scrolling_up) ||
       (widget == gtk_path_bar_get_props (path_bar)->down_slider_button && gtk_path_bar_get_props (path_bar)->scrolling_down)))
		   ...
		   GtkArrowType arrow_type)
{
  ...
  atk_obj = __gtk_widget_get_accessible (button);
  ...
  __gtk_container_add (GTK_CONTAINER (button),
                     __gtk_arrow_new (arrow_type, GTK_SHADOW_OUT));
  __gtk_container_add (GTK_CONTAINER (path_bar), button);
  __gtk_widget_show_all (button);
...
{
  __gtk_widget_set_has_window (GTK_WIDGET (path_bar), FALSE);
  __gtk_widget_set_redraw_on_allocate (GTK_WIDGET (path_bar), FALSE);
  ...
  g_signal_connect_swapped (gtk_path_bar_get_props (path_bar)->down_slider_button, "clicked",
                            G_CALLBACK (gtk_path_bar_scroll_down), path_bar);

  g_signal_connect (gtk_path_bar_get_props (path_bar)->up_slider_button, "focus-out-event",
                    G_CALLBACK (gtk_path_bar_slider_up_defocus), path_bar);
  g_signal_connect (gtk_path_bar_get_props (path_bar)->down_slider_button, "focus-out-event",
                    G_CALLBACK (gtk_path_bar_slider_down_defocus), path_bar);

  g_signal_connect (gtk_path_bar_get_props (path_bar)->up_slider_button, "button-press-event",
                    G_CALLBACK (gtk_path_bar_slider_button_press), path_bar);
  g_signal_connect (gtk_path_bar_get_props (path_bar)->up_slider_button, "button-release-event",
                    G_CALLBACK (gtk_path_bar_slider_button_release), path_bar);
  g_signal_connect (gtk_path_bar_get_props (path_bar)->down_slider_button, "button-press-event",
                    G_CALLBACK (gtk_path_bar_slider_button_press), path_bar);
  g_signal_connect (gtk_path_bar_get_props (path_bar)->down_slider_button, "button-release-event",
                    G_CALLBACK (gtk_path_bar_slider_button_release), path_bar);
}
...
gtk_path_bar_finalize (GObject *object)
{
  ...
  g_list_free (gtk_path_bar_get_props (path_bar)->button_list);
  if (gtk_path_bar_get_props (path_bar)->root_file)
    g_object_unref (gtk_path_bar_get_props (path_bar)->root_file);
  if (gtk_path_bar_get_props (path_bar)->home_file)
    g_object_unref (gtk_path_bar_get_props (path_bar)->home_file);
  if (gtk_path_bar_get_props (path_bar)->desktop_file)
    g_object_unref (gtk_path_bar_get_props (path_bar)->desktop_file);

  if (gtk_path_bar_get_props (path_bar)->root_icon)
    g_object_unref (gtk_path_bar_get_props (path_bar)->root_icon);
  if (gtk_path_bar_get_props (path_bar)->home_icon)
    g_object_unref (gtk_path_bar_get_props (path_bar)->home_icon);
  if (gtk_path_bar_get_props (path_bar)->desktop_icon)
    g_object_unref (gtk_path_bar_get_props (path_bar)->desktop_icon);

  if (gtk_path_bar_get_props (path_bar)->file_system)
    g_object_unref (gtk_path_bar_get_props (path_bar)->file_system);
...
remove_settings_signal (GtkPathBar *path_bar,
			GdkScreen  *screen)
{
  if (gtk_path_bar_get_props (path_bar)->settings_signal_id)
    {
      GtkSettings *settings;

      settings = __gtk_settings_get_for_screen (screen);
      g_signal_handler_disconnect (settings,
				   gtk_path_bar_get_props (path_bar)->settings_signal_id);
      gtk_path_bar_get_props (path_bar)->settings_signal_id = 0;
    }
}
...
gtk_path_bar_dispose (GObject *object)
{
  ...
  remove_settings_signal (path_bar, __gtk_widget_get_screen (GTK_WIDGET (object)));

  if (gtk_path_bar_get_props (path_bar)->get_info_cancellable)
    g_cancellable_cancel (gtk_path_bar_get_props (path_bar)->get_info_cancellable);
  gtk_path_bar_get_props (path_bar)->get_info_cancellable = NULL;
			   ...
			   GtkRequisition *requisition)
{
  ...
  for (list = gtk_path_bar_get_props (path_bar)->button_list; list; list = list->next)
    {
      button_data = BUTTON_DATA (list->data);
      __gtk_widget_size_request (button_data->button, &child_requisition);
   ...
   */
  gtk_path_bar_get_props (path_bar)->slider_width = MIN(requisition->height * 2 / 3 + 5, requisition->height);
  if (gtk_path_bar_get_props (path_bar)->button_list && gtk_path_bar_get_props (path_bar)->button_list->next != NULL)
    requisition->width += (gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width) * 2;
...
{
  if (gtk_path_bar_get_props (path_bar)->button_list)
    {
      GtkWidget *button;

      button = BUTTON_DATA (gtk_path_bar_get_props (path_bar)->button_list->data)->button;
      if (__gtk_widget_get_child_visible (button))
	{
	  gtk_path_bar_stop_scrolling (path_bar);
	  __gtk_widget_set_sensitive (gtk_path_bar_get_props (path_bar)->down_slider_button, FALSE);
	}
      else
	__gtk_widget_set_sensitive (gtk_path_bar_get_props (path_bar)->down_slider_button, TRUE);

      button = BUTTON_DATA (g_list_last (gtk_path_bar_get_props (path_bar)->button_list)->data)->button;
      if (__gtk_widget_get_child_visible (button))
	{
	  gtk_path_bar_stop_scrolling (path_bar);
	  __gtk_widget_set_sensitive (gtk_path_bar_get_props (path_bar)->up_slider_button, FALSE);
	}
      else
	__gtk_widget_set_sensitive (gtk_path_bar_get_props (path_bar)->up_slider_button, TRUE);
    }
}
...
gtk_path_bar_realize (GtkWidget *widget)
{
  ...
  gtk_path_bar_get_props (path_bar)->event_window = __gdk_window_new (__gtk_widget_get_parent_window (widget),
							&attributes, attributes_mask);
  __gdk_window_set_user_data (gtk_path_bar_get_props (path_bar)->event_window, widget);
...
gtk_path_bar_unrealize (GtkWidget *widget)
{
  ...
  __gdk_window_set_user_data (gtk_path_bar_get_props (path_bar)->event_window, NULL);
  __gdk_window_destroy (gtk_path_bar_get_props (path_bar)->event_window);
  gtk_path_bar_get_props (path_bar)->event_window = NULL;
			    ...
			    GtkAllocation *allocation)
{
  ...
  if (__gtk_widget_get_realized (widget))
    __gdk_window_move_resize (gtk_path_bar_get_props (path_bar)->event_window,
			    allocation->x, allocation->y,
			    allocation->width, allocation->height);

  /* No path is set; we don't have to allocate anything. */
  if (gtk_path_bar_get_props (path_bar)->button_list == NULL)
    return;
  ...
  if (gtk_path_bar_get_props (path_bar)->fake_root)
    width = gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width;
  else
      width = 0;
  ...
  for (list = gtk_path_bar_get_props (path_bar)->button_list; list; list = list->next)
    {
      ...
      width += gtk_widget_get_props (child)->requisition.width + gtk_path_bar_get_props (path_bar)->spacing;
      if (list == gtk_path_bar_get_props (path_bar)->fake_root)
	break;
    }
  ...
  if (width <= allocation_width)
    {
      if (gtk_path_bar_get_props (path_bar)->fake_root)
	first_button = gtk_path_bar_get_props (path_bar)->fake_root;
      else
	first_button = g_list_last (gtk_path_bar_get_props (path_bar)->button_list);
    }
  else
    {
      gboolean reached_end = FALSE;
      gint slider_space = 2 * (gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width);

      if (gtk_path_bar_get_props (path_bar)->first_scrolled_button)
	first_button = gtk_path_bar_get_props (path_bar)->first_scrolled_button;
      else
	first_button = gtk_path_bar_get_props (path_bar)->button_list;
      need_sliders = TRUE;
      ...
      while (list && !reached_end)
	{
	  ...
	  if (width + gtk_widget_get_props (child)->requisition.width +
	      gtk_path_bar_get_props (path_bar)->spacing + slider_space > allocation_width)
	    reached_end = TRUE;
	  else if (list == gtk_path_bar_get_props (path_bar)->fake_root)
	    break;
	  else
	    width += gtk_widget_get_props (child)->requisition.width + gtk_path_bar_get_props (path_bar)->spacing;
      ...
      while (first_button->next && !reached_end)
	{
	  ...
	  if (width + gtk_widget_get_props (child)->requisition.width + gtk_path_bar_get_props (path_bar)->spacing + slider_space > allocation_width)
	    {
	      reached_end = TRUE;
	    }
	  else
	    {
	      width += gtk_widget_get_props (child)->requisition.width + gtk_path_bar_get_props (path_bar)->spacing;
	      if (first_button == gtk_path_bar_get_props (path_bar)->fake_root)
		break;
  ...
  if (direction == GTK_TEXT_DIR_RTL)
    {
      ...
      if (need_sliders || gtk_path_bar_get_props (path_bar)->fake_root)
	{
	  child_allocation.x -= (gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width);
	  up_slider_offset = allocation->width - border_width - gtk_path_bar_get_props (path_bar)->slider_width;
	}
    }
  else
    {
  ...
  for (list = first_button; list; list = list->prev)
    {
      ...
      child_allocation.width = MIN (gtk_widget_get_props (child)->requisition.width,
				    allocation_width - (gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width) * 2);
	...
	{
	  if (child_allocation.x - gtk_path_bar_get_props (path_bar)->spacing - gtk_path_bar_get_props (path_bar)->slider_width < gtk_widget_get_props (widget)->allocation.x + border_width)
	    break;
	}
      else if (need_sliders && direction == GTK_TEXT_DIR_LTR)
	{
	  if (child_allocation.x + child_allocation.width + gtk_path_bar_get_props (path_bar)->spacing + gtk_path_bar_get_props (path_bar)->slider_width >
	      gtk_widget_get_props (widget)->allocation.x + border_width + allocation_width)
	    break;
	}

      if (child_allocation.width < gtk_widget_get_props (child)->requisition.width)
	{
	  if (!__gtk_widget_get_has_tooltip (child))
	    __gtk_widget_set_tooltip_text (child, button_data->dir_name);
	}
      else if (__gtk_widget_get_has_tooltip (child))
	__gtk_widget_set_tooltip_text (child, NULL);
      
      __gtk_widget_set_child_visible (child, TRUE);
      __gtk_widget_size_allocate (child, &child_allocation);

      if (direction == GTK_TEXT_DIR_RTL)
	child_allocation.x -= gtk_path_bar_get_props (path_bar)->spacing;
      else
	child_allocation.x += child_allocation.width + gtk_path_bar_get_props (path_bar)->spacing;
    }
  /* Now we go hide all the widgets that don't fit */
  while (list)
    {
      __gtk_widget_set_child_visible (BUTTON_DATA (list->data)->button, FALSE);
      list = list->prev;
    }
  ...
  if (need_sliders || gtk_path_bar_get_props (path_bar)->fake_root)
    {
      child_allocation.width = gtk_path_bar_get_props (path_bar)->slider_width;
      child_allocation.x = up_slider_offset + allocation->x;
      __gtk_widget_size_allocate (gtk_path_bar_get_props (path_bar)->up_slider_button, &child_allocation);

      __gtk_widget_set_child_visible (gtk_path_bar_get_props (path_bar)->up_slider_button, TRUE);
      __gtk_widget_show_all (gtk_path_bar_get_props (path_bar)->up_slider_button);
    }
  else
    __gtk_widget_set_child_visible (gtk_path_bar_get_props (path_bar)->up_slider_button, FALSE);
  ...
  if (need_sliders)
	...
	child_allocation.x = allocation->width - border_width - gtk_path_bar_get_props (path_bar)->slider_width;

      child_allocation.x += allocation->x;
      
      __gtk_widget_size_allocate (gtk_path_bar_get_props (path_bar)->down_slider_button, &child_allocation);

      __gtk_widget_set_child_visible (gtk_path_bar_get_props (path_bar)->down_slider_button, TRUE);
      __gtk_widget_show_all (gtk_path_bar_get_props (path_bar)->down_slider_button);
      gtk_path_bar_update_slider_buttons (path_bar);
    }
  else
    __gtk_widget_set_child_visible (gtk_path_bar_get_props (path_bar)->down_slider_button, FALSE);
}
		     ...
		     GtkWidget    *widget)
{
  ...
  if (widget == gtk_path_bar_get_props (path_bar)->up_slider_button)
    {
      gtk_path_bar_remove_1 (container, widget);
      gtk_path_bar_get_props (path_bar)->up_slider_button = NULL;
      return;
    }

  if (widget == gtk_path_bar_get_props (path_bar)->down_slider_button)
    {
      gtk_path_bar_remove_1 (container, widget);
      gtk_path_bar_get_props (path_bar)->down_slider_button = NULL;
      return;
    }

  children = gtk_path_bar_get_props (path_bar)->button_list;
  while (children)
    {
      if (widget == BUTTON_DATA (children->data)->button)
	{
	  gtk_path_bar_remove_1 (container, widget);
	  gtk_path_bar_get_props (path_bar)->button_list = g_list_remove_link (gtk_path_bar_get_props (path_bar)->button_list, children);
		     ...
		     gpointer      callback_data)
{
  ...
  if (gtk_path_bar_get_props (path_bar)->up_slider_button)
    (* callback) (gtk_path_bar_get_props (path_bar)->up_slider_button, callback_data);

  if (gtk_path_bar_get_props (path_bar)->down_slider_button)
    (* callback) (gtk_path_bar_get_props (path_bar)->down_slider_button, callback_data);
}
...
gtk_path_bar_scroll_down (GtkPathBar *path_bar)
{
  ...
  if (gtk_path_bar_get_props (path_bar)->ignore_click)
    {
      gtk_path_bar_get_props (path_bar)->ignore_click = FALSE;
      return;   
    }

  if (__gtk_widget_get_child_visible (BUTTON_DATA (gtk_path_bar_get_props (path_bar)->button_list->data)->button))
    {
      /* Return if the last button is already visible */
      return;
    }

  __gtk_widget_queue_resize (GTK_WIDGET (path_bar));

  /* We find the button at the 'down' end that we have to make
   * visible */
  for (list = gtk_path_bar_get_props (path_bar)->button_list; list; list = list->next)
    {
      if (list->next && __gtk_widget_get_child_visible (BUTTON_DATA (list->next->data)->button))
	{
	  down_button = list;
	  break;
	}
...
gtk_path_bar_scroll_up (GtkPathBar *path_bar)
{
  ...
  list = g_list_last (gtk_path_bar_get_props (path_bar)->button_list);

  if (__gtk_widget_get_child_visible (BUTTON_DATA (list->data)->button))
    {
  ...
  for ( ; list; list = list->prev)
    {
      if (list->prev && __gtk_widget_get_child_visible (BUTTON_DATA (list->prev->data)->button))
	{
	  if (list->prev == gtk_path_bar_get_props (path_bar)->fake_root)
	    gtk_path_bar_get_props (path_bar)->fake_root = NULL;
	  gtk_path_bar_get_props (path_bar)->first_scrolled_button = list;
	  return;
	}
    }
}
...
gtk_path_bar_scroll_timeout (GtkPathBar *path_bar)
{
  ...
  if (gtk_path_bar_get_props (path_bar)->timer)
    {
      if (gtk_path_bar_get_props (path_bar)->scrolling_up)
	gtk_path_bar_scroll_up (path_bar);
      else if (gtk_path_bar_get_props (path_bar)->scrolling_down)
	gtk_path_bar_scroll_down (path_bar);

      if (gtk_path_bar_get_props (path_bar)->need_timer) 
	{
          GtkSettings *settings = __gtk_widget_get_settings (GTK_WIDGET (path_bar));
          guint        timeout;

          g_object_get (settings, "gtk-timeout-repeat", &timeout, NULL);

	  gtk_path_bar_get_props (path_bar)->need_timer = FALSE;

	  gtk_path_bar_get_props (path_bar)->timer = __gdk_threads_add_timeout (timeout * SCROLL_DELAY_FACTOR,
...
gtk_path_bar_stop_scrolling (GtkPathBar *path_bar)
{
  if (gtk_path_bar_get_props (path_bar)->timer)
    {
      g_source_remove (gtk_path_bar_get_props (path_bar)->timer);
      gtk_path_bar_get_props (path_bar)->timer = 0;
      gtk_path_bar_get_props (path_bar)->need_timer = FALSE;
    }
}
                                    ...
                                    GtkPathBar     *path_bar)
{
  ...
  for (list = g_list_last (gtk_path_bar_get_props (path_bar)->button_list); list; list = list->prev)
    {
      if (__gtk_widget_get_child_visible (BUTTON_DATA (list->data)->button))
        {
          up_button = list;
  ...
  if ((!__gtk_widget_is_sensitive (gtk_path_bar_get_props (path_bar)->up_slider_button)) ||
      (!__gtk_widget_get_child_visible (gtk_path_bar_get_props (path_bar)->up_slider_button)))
    __gtk_widget_grab_focus (BUTTON_DATA (up_button->data)->button);
                                    ...
                                    GtkPathBar     *path_bar)
{
  ...
  /* don't let the focus vanish */
  if ((!__gtk_widget_is_sensitive (gtk_path_bar_get_props (path_bar)->down_slider_button)) ||
      (!__gtk_widget_get_child_visible (gtk_path_bar_get_props (path_bar)->down_slider_button)))
    __gtk_widget_grab_focus (BUTTON_DATA (down_button->data)->button);
				  ...
				  GtkPathBar     *path_bar)
{
  ...
  if (widget == gtk_path_bar_get_props (path_bar)->up_slider_button)
    {
      gtk_path_bar_get_props (path_bar)->scrolling_down = FALSE;
      gtk_path_bar_get_props (path_bar)->scrolling_up = TRUE;
      gtk_path_bar_scroll_up (path_bar);
    }
  else if (widget == gtk_path_bar_get_props (path_bar)->down_slider_button)
    {
      gtk_path_bar_get_props (path_bar)->scrolling_up = FALSE;
      gtk_path_bar_get_props (path_bar)->scrolling_down = TRUE;
      gtk_path_bar_scroll_down (path_bar);
    }
  ...
  if (!gtk_path_bar_get_props (path_bar)->timer)
    {
      ...
      gtk_path_bar_get_props (path_bar)->need_timer = TRUE;
      gtk_path_bar_get_props (path_bar)->timer = __gdk_threads_add_timeout (timeout,
				       (GSourceFunc)gtk_path_bar_scroll_timeout,
				       path_bar);
    }
				    ...
				    GtkPathBar     *path_bar)
{
  ...
  gtk_path_bar_get_props (path_bar)->ignore_click = TRUE;
  gtk_path_bar_stop_scrolling (path_bar);
...
reload_icons (GtkPathBar *path_bar)
{
  ...
  if (gtk_path_bar_get_props (path_bar)->root_icon)
    {
      g_object_unref (gtk_path_bar_get_props (path_bar)->root_icon);
      gtk_path_bar_get_props (path_bar)->root_icon = NULL;
    }
  if (gtk_path_bar_get_props (path_bar)->home_icon)
    {
      g_object_unref (gtk_path_bar_get_props (path_bar)->home_icon);
      gtk_path_bar_get_props (path_bar)->home_icon = NULL;
    }
  if (gtk_path_bar_get_props (path_bar)->desktop_icon)
    {
      g_object_unref (gtk_path_bar_get_props (path_bar)->desktop_icon);
      gtk_path_bar_get_props (path_bar)->desktop_icon = NULL;
    }
  ...
  for (list = gtk_path_bar_get_props (path_bar)->button_list; list; list = list->next)
    {
      ...
      if (button_data->type != NORMAL_BUTTON)
	{
	  current_dir = __gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button_data->button));
...
change_icon_theme (GtkPathBar *path_bar)
{
  ...
  settings = __gtk_settings_get_for_screen (__gtk_widget_get_screen (GTK_WIDGET (path_bar)));

  if (__gtk_icon_size_lookup_for_settings (settings, GTK_ICON_SIZE_MENU, &width, &height))
    gtk_path_bar_get_props (path_bar)->icon_size = MAX (width, height);
  else
    gtk_path_bar_get_props (path_bar)->icon_size = FALLBACK_ICON_SIZE;
...
gtk_path_bar_check_icon_theme (GtkPathBar *path_bar)
{
  ...
  settings = __gtk_settings_get_for_screen (__gtk_widget_get_screen (GTK_WIDGET (path_bar)));
  gtk_path_bar_get_props (path_bar)->settings_signal_id = g_signal_connect (settings, "notify", G_CALLBACK (settings_notify_cb), gtk_path_bar_get_props (path_bar));
		  ...
		  ButtonData *button_data)
{
  ...
  switch (button_data->type)
    ...
    case ROOT_BUTTON:
      ...
      if (gtk_path_bar_get_props (path_bar)->root_icon != NULL)
        {
          __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), gtk_path_bar_get_props (path_bar)->root_icon);
	  break;
	}

      volume = _gtk_file_system_get_volume_for_file (gtk_path_bar_get_props (path_bar)->file_system, gtk_path_bar_get_props (path_bar)->root_file);
      if (volume == NULL)
	return;

      gtk_path_bar_get_props (path_bar)->root_icon = _gtk_file_system_volume_render_icon (volume,
								 GTK_WIDGET (path_bar),
								 gtk_path_bar_get_props (path_bar)->icon_size,
								 NULL);
    ...
    case HOME_BUTTON:
      if (gtk_path_bar_get_props (path_bar)->home_icon != NULL)
        {
	  __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), gtk_path_bar_get_props (path_bar)->home_icon);
	  break;
	}
	...
	g_cancellable_cancel (button_data->cancellable);
      ...
      button_data->cancellable =
        _gtk_file_system_get_info (gtk_path_bar_get_props (path_bar)->file_system,
				   gtk_path_bar_get_props (path_bar)->home_file,
				   "standard::icon",
    ...
    case DESKTOP_BUTTON:
      if (gtk_path_bar_get_props (path_bar)->desktop_icon != NULL)
        {
	  __gtk_image_set_from_pixbuf (GTK_IMAGE (button_data->image), gtk_path_bar_get_props (path_bar)->desktop_icon);
	  break;
	}
	...
	g_cancellable_cancel (button_data->cancellable);
      ...
      button_data->cancellable =
        _gtk_file_system_get_info (gtk_path_bar_get_props (path_bar)->file_system,
				   gtk_path_bar_get_props (path_bar)->desktop_file,
				   "standard::icon",
				   set_button_image_get_info_cb,
				   data);
      break;
		       ...
		       ButtonData     *button_data)
{
  ...
  PangoLayout *layout = __gtk_widget_create_pango_layout (button_data->label, dir_name);
				       ...
				       gboolean    current_dir)
{
  ...
  if (button_data->label != NULL)
      ...
      if (current_dir)
	{
	  ...
	  markup = g_markup_printf_escaped ("<b>%s</b>", dir_name);
	  __gtk_label_set_markup (GTK_LABEL (button_data->label), markup);
	  g_free (markup);
	}
      else
	{
	  __gtk_label_set_text (GTK_LABEL (button_data->label), dir_name);
	}
    }
  ...
  if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button_data->button)) != current_dir)
    {
      button_data->ignore_changes = TRUE;
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button_data->button), current_dir);
...
find_button_type (GtkPathBar  *path_bar,
		  GFile       *file)
{
  if (gtk_path_bar_get_props (path_bar)->root_file != NULL &&
      g_file_equal (file, gtk_path_bar_get_props (path_bar)->root_file))
    return ROOT_BUTTON;
  if (gtk_path_bar_get_props (path_bar)->home_file != NULL &&
      g_file_equal (file, gtk_path_bar_get_props (path_bar)->home_file))
    return HOME_BUTTON;
  if (gtk_path_bar_get_props (path_bar)->desktop_file != NULL &&
      g_file_equal (file, gtk_path_bar_get_props (path_bar)->desktop_file))
			 ...
			 gpointer            data)
{
  ...
  __gtk_selection_data_set_uris (selection_data, uris);
  g_free (uris[0]);
}
		       ...
		       gboolean     file_is_hidden)
{
  ...
  button_data->button = __gtk_toggle_button_new ();
  atk_obj = __gtk_widget_get_accessible (button_data->button);
  __gtk_button_set_focus_on_click (GTK_BUTTON (button_data->button), FALSE);

  switch (button_data->type)
    {
    case ROOT_BUTTON:
      button_data->image = __gtk_image_new ();
    ...
    case DESKTOP_BUTTON:
      button_data->image = __gtk_image_new ();
      button_data->label = __gtk_label_new (NULL);
      label_alignment = __gtk_alignment_new (0.5, 0.5, 1.0, 1.0);
      __gtk_container_add (GTK_CONTAINER (label_alignment), button_data->label);
      child = __gtk_hbox_new (FALSE, 2);
      __gtk_box_pack_start (GTK_BOX (child), button_data->image, FALSE, FALSE, 0);
      __gtk_box_pack_start (GTK_BOX (child), label_alignment, FALSE, FALSE, 0);
      break;
    case NORMAL_BUTTON:
    default:
      button_data->label = __gtk_label_new (NULL);
      __gtk_label_set_ellipsize (GTK_LABEL (button_data->label), PANGO_ELLIPSIZE_END);
      label_alignment = __gtk_alignment_new (0.5, 0.5, 1.0, 1.0);
      __gtk_container_add (GTK_CONTAINER (label_alignment), button_data->label);
  ...
  __gtk_container_add (GTK_CONTAINER (button_data->button), child);
  __gtk_widget_show_all (button_data->button);
  ...
  __gtk_drag_source_set (button_data->button,
		       GDK_BUTTON1_MASK,
		       NULL, 0,
		       GDK_ACTION_COPY);
  __gtk_drag_source_add_uri_targets (button_data->button);
  ...
  if (current_path)
      ...
      if (need_new_fake_root)
	{
	  ...
	  for (list = current_path; list; list = list->next)
		...
		{
		  gtk_path_bar_get_props (path_bar)->fake_root = list;
		  break;
		}
	    }
	}
      ...
      for (list = gtk_path_bar_get_props (path_bar)->button_list; list; list = list->next)
	{
      ...
      if (!__gtk_widget_get_child_visible (BUTTON_DATA (current_path->data)->button))
	{
	  gtk_path_bar_get_props (path_bar)->first_scrolled_button = current_path;
	  __gtk_widget_queue_resize (GTK_WIDGET (path_bar));
	}
                              ...
                              gboolean            result)
{
  if (result)
      ...
      for (l = info->new_buttons; l; l = l->next)
	{
	  ...
	  __gtk_widget_destroy (button_data->button);
	}
			...
			GError            **error)
{
  ...
  gtk_path_bar_get_props (path_bar)->get_info_cancellable =
    _gtk_file_system_get_info (gtk_path_bar_get_props (path_bar)->file_system,
			       info->file,
			       "standard::display-name,standard::is-hidden,standard::is-backup",
			       gtk_path_bar_get_info_callback,
			       ...
			       GtkFileSystem *file_system)
{
  ...
  g_assert (gtk_path_bar_get_props (path_bar)->file_system == NULL);

  gtk_path_bar_get_props (path_bar)->file_system = g_object_ref (file_system);
  ...
  if (home != NULL)
    {
      ...
      gtk_path_bar_get_props (path_bar)->home_file = g_file_new_for_path (home);
      /* FIXME: Need file system backend specific way of getting the
       * Desktop path.
       */
      desktop = g_get_user_special_dir (G_USER_DIRECTORY_DESKTOP);
      if (desktop != NULL)
        gtk_path_bar_get_props (path_bar)->desktop_file = g_file_new_for_path (desktop);
      else 
        gtk_path_bar_get_props (path_bar)->desktop_file = NULL;
    }
  else
    {
      gtk_path_bar_get_props (path_bar)->home_file = NULL;
      gtk_path_bar_get_props (path_bar)->desktop_file = NULL;
    }
  gtk_path_bar_get_props (path_bar)->root_file = g_file_new_for_path ("/");
}
...
_gtk_path_bar_up (GtkPathBar *path_bar)
{
  ...
  for (l = gtk_path_bar_get_props (path_bar)->button_list; l; l = l->next)
    {
      GtkWidget *button = BUTTON_DATA (l->data)->button;
      if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (button)))
	{
	  if (l->next)
	    {
