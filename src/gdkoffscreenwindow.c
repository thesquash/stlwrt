...
STLWRT_DEFINE_FTYPE (GdkOffscreenWindow, gdk_offscreen_window, GDK_TYPE_DRAWABLE,
                     G_TYPE_FLAG_NONE,
...
gdk_offscreen_window_finalize (GObject *object)
{
  ...
  if (offscreen->cursor)
    __gdk_cursor_unref (offscreen->cursor);
			      ...
			      gboolean   foreign_destroy)
{
  ...
  __gdk_offscreen_window_set_embedder (window, NULL);
	    ...
	    gboolean is_line)
{
  ...
  damage = __gdk_region_rectangle (&rect);
  _gdk_window_add_damage (offscreen->wrapper, damage);
  __gdk_region_destroy (damage);
}
				    ...
				    GdkDrawable *original_src)
{
  ...
  __gdk_draw_drawable (real_drawable, gc,
		     src, xsrc, ysrc,
		     xdest, ydest,
		     width, height);
				     ...
				     gint	   height)
{
  ...
  __gdk_draw_rectangle (real_drawable,
				   ...
				   gint		npoints)
{
  ...
  __gdk_draw_polygon (real_drawable,
		    gc,
		    filled,
		    points,
		    npoints);
				...
				gint		text_length)
{
  GdkOffscreenWindow *offscreen = GDK_OFFSCREEN_WINDOW (drawable);
  GdkDrawable *real_drawable = get_real_drawable (offscreen);
  GdkWindow *private = GDK_WINDOW (offscreen->wrapper);

  __gdk_draw_text (real_drawable,
		 font,
		 gc,
		 x,
		 y,
				   ...
				   gint		  text_length)
{
  ...
  __gdk_draw_text_wc (real_drawable,
		    font,
		    gc,
		    x,
		    y,
				  ...
				  gint		npoints)
{
  ...
  __gdk_draw_points (real_drawable,
		   gc,
		   points,
		   npoints);
				    ...
				    gint	  nsegs)
{
  ...
  __gdk_draw_segments (real_drawable,
		     gc,
		     segs,
		     nsegs);
				 ...
				 gint          npoints)
{
  ...
  __gdk_draw_lines (real_drawable,
		  gc,
		  points,
		  npoints);
				 ...
				 gint	       height)
{
  ...
  __gdk_draw_image (real_drawable,
		  gc,
		  image,
		  xsrc,
		  ysrc,
				  ...
				  gint         y_dither)
{
  ...
  __gdk_draw_pixbuf (real_drawable,
		   gc,
		   pixbuf,
		   src_x,
		   src_y,
			   ...
			   gint           attributes_mask)
{
  ...
  if (attributes_mask & GDK_WA_COLORMAP)
    ...
    {
      if (__gdk_screen_get_system_visual (screen) == visual)
	{
	  offscreen->colormap = __gdk_screen_get_system_colormap (screen);
	  g_object_ref (offscreen->colormap);
	}
      else
	offscreen->colormap = __gdk_colormap_new (visual, FALSE);
    }

  offscreen->pixmap = __gdk_pixmap_new ((GdkDrawable *)private->parent,
				      private->width,
				      private->height,
				      private->depth);
			       ...
			       gint       y)
{
  ...
  GdkWindow *new_parent_private = (GdkWindow *)new_parent;
  ...
  if (offscreen->embedder)
    {
      ...
      tmpx = floor (dx + 0.5);
      tmpy = floor (dy + 0.5);
      __gdk_window_get_root_coords (offscreen->embedder,
				  tmpx, tmpy,
				  &tmpx, &tmpy);
					      ...
					      gint      *y)
{
  ...
  if (offscreen->embedder)
    {
      double dx, dy;
      __gdk_window_get_deskrelative_origin (offscreen->embedder,
					  &tmpx, &tmpy);
				  ...
				  GdkModifierType *mask)
{
  ...
  if (offscreen->embedder != NULL)
    {
      __gdk_window_get_pointer (offscreen->embedder, &tmpx, &tmpy, &tmpmask);
...
/**
 * __gdk_offscreen_window_get_pixmap:
 * @window: a #GdkWindow
 *
 ...
 */
GdkPixmap *
__gdk_offscreen_window_get_pixmap (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkOffscreenWindow *offscreen;
...
{
  /* __gdk_window_raise already changed the stacking order */
  _gdk_synthesize_crossing_events_for_geometry_change (window);
}
...
{
  /* __gdk_window_lower already changed the stacking order */
  _gdk_synthesize_crossing_events_for_geometry_change (window);
}
					   ...
					   gboolean   send_expose_events)
{
  ...
  if (private->width != width ||
      private->height != height)
    {
      ...
      offscreen->pixmap = __gdk_pixmap_new (GDK_DRAWABLE (old_pixmap),
					  width,
					  height,
					  private->depth);
  ...
  if (GDK_WINDOW_IS_MAPPED (private))
    {
      ...
      __gdk_window_invalidate_rect (window, NULL, TRUE);
      _gdk_synthesize_crossing_events_for_geometry_change (window);
    }
}
			   ...
			   gboolean already_mapped)
{
  ...
  __gdk_window_clear_area_e (window, 0, 0,
				 ...
				 GdkCursor *cursor)
{
  ...
  if (cursor)
    offscreen->cursor = __gdk_cursor_ref (cursor);
...
/**
 * __gdk_offscreen_window_set_embedder:
 * @window: a #GdkWindow
 ...
 */
void
__gdk_offscreen_window_set_embedder (GdkWindow     *window,
				   GdkWindow     *embedder)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkOffscreenWindow *offscreen;
  ...
  if (embedder)
    {
      ...
      GDK_WINDOW (embedder)->num_offscreen_children++;
    }

  if (offscreen->embedder)
    {
      g_object_unref (offscreen->embedder);
      GDK_WINDOW (offscreen->embedder)->num_offscreen_children--;
    }
...
/**
 * __gdk_offscreen_window_get_embedder:
 * @window: a #GdkWindow
 *
 ...
 */
GdkWindow *
__gdk_offscreen_window_get_embedder (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkOffscreenWindow *offscreen;
