...
static void      __gtk_dialog_add_buttons_valist   (GtkDialog    *dialog,
...
gtk_dialog_class_init (GtkDialogClass *class)
{
   ...
   * delete event, or the application programmer calls __gtk_dialog_response(). 
   ...
   * __gtk_dialog_get_content_area(), unless __gtk_box_set_spacing()
...
update_spacings (GtkDialog *dialog)
{
  ...
  __gtk_widget_style_get (GTK_WIDGET (dialog),
  ...
  __gtk_container_set_border_width (GTK_CONTAINER (dialog->vbox),
                                  content_area_border);
  if (!______gtk_box_get_spacing_set (GTK_BOX (dialog->vbox)))
    {
      __gtk_box_set_spacing (GTK_BOX (dialog->vbox), content_area_spacing);
      ______gtk_box_set_spacing_set (GTK_BOX (dialog->vbox), FALSE);
    }
...
gtk_dialog_init (GtkDialog *dialog)
{
  ...
  __gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_CENTER_ON_PARENT);
}
                         ...
                         GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    ...
    case PROP_HAS_SEPARATOR:
      __gtk_dialog_set_has_separator (dialog, g_value_get_boolean (value));
      break;
                                 ...
                                 gpointer     user_data)
{
  ...
  __gtk_dialog_response (GTK_DIALOG (widget), GTK_RESPONSE_DELETE_EVENT);
...
GtkWidget*
__gtk_dialog_new (void)
{
  return g_object_new (GTK_TYPE_DIALOG, NULL);
}

static GtkWidget*
__gtk_dialog_new_empty (const gchar     *title,
                      GtkWindow       *parent,
                      GtkDialogFlags   flags)
{
  ...
  if (title)
    __gtk_window_set_title (GTK_WINDOW (dialog), title);
 ...
 * title; see __gtk_window_set_title()) and transient parent @parent (or
 * %NULL for none; see __gtk_window_set_transient_for()). The @flags
 ...
 * |[
 *  GtkWidget *dialog = __gtk_dialog_new_with_buttons ("My dialog",
 *                                                   main_app_window,
 ...
 **/
GtkWidget*
__gtk_dialog_new_with_buttons (const gchar    *title,
                             GtkWindow      *parent,
                             GtkDialogFlags  flags,
                             const gchar    *first_button_text,
                             ...)
{
  ...
  dialog = GTK_DIALOG (__gtk_dialog_new_empty (title, parent, flags));

  va_start (args, first_button_text);

  __gtk_dialog_add_buttons_valist (dialog,
                                 first_button_text,
                                 args);
...
action_widget_activated (GtkWidget *widget, GtkDialog *dialog)
{
  ...
  response_id = __gtk_dialog_get_response_for_widget (dialog, widget);

  __gtk_dialog_response (dialog, response_id);
}

/**
 * __gtk_dialog_add_action_widget:
 * @dialog: a #GtkDialog
 ...
 **/
void
__gtk_dialog_add_action_widget (GtkDialog *dialog,
                              GtkWidget *child,
                              gint       response_id)
{
  ResponseData *ad;
 ...
 **/
GtkWidget*
__gtk_dialog_add_button (GtkDialog   *dialog,
                       ...
                       gint         response_id)
{
  ...
  button = __gtk_button_new_from_stock (button_text);

  __gtk_widget_set_can_default (button, TRUE);
  
  __gtk_widget_show (button);
  
  __gtk_dialog_add_action_widget (dialog,
                                button,
                                response_id);
...
static void
__gtk_dialog_add_buttons_valist (GtkDialog      *dialog,
                               const gchar    *first_button_text,
                               va_list         args)
{
  const gchar* text;
  ...
  while (text != NULL)
    {
      __gtk_dialog_add_button (dialog, text, response_id);
...
/**
 * __gtk_dialog_add_buttons:
 * @dialog: a #GtkDialog
 * @first_button_text: button text or stock ID
 * @Varargs: response ID for first button, then more text-response_id pairs
 * 
 * Adds more buttons, same as calling __gtk_dialog_add_button()
 * repeatedly.  The variable argument list should be %NULL-terminated
 * as with __gtk_dialog_new_with_buttons(). Each button must have both
 * text and response ID.
 **/
void
__gtk_dialog_add_buttons (GtkDialog   *dialog,
                        const gchar *first_button_text,
                        ...)
{  
  va_list args;
...
/**
 * __gtk_dialog_set_response_sensitive:
 * @dialog: a #GtkDialog
 * @response_id: a response ID
 * @setting: %TRUE for sensitive
 *
 * Calls <literal>__gtk_widget_set_sensitive (widget, @setting)</literal> 
 * for each widget in the dialog's action area with the given @response_id.
 * A convenient way to sensitize/desensitize dialog buttons.
 **/
void
__gtk_dialog_set_response_sensitive (GtkDialog *dialog,
                                   gint       response_id,
                                   gboolean   setting)
{
  GList *children;
  ...
  while (tmp_list != NULL)
    {
      ...
      if (rd && rd->response_id == response_id)
        __gtk_widget_set_sensitive (widget, setting);
...
/**
 * __gtk_dialog_set_default_response:
 * @dialog: a #GtkDialog
 * @response_id: a response ID
 * 
 ...
 **/
void
__gtk_dialog_set_default_response (GtkDialog *dialog,
                                 gint       response_id)
{
  GList *children;
  GList *tmp_list;
...
/**
 * __gtk_dialog_set_has_separator:
 * @dialog: a #GtkDialog
 ...
 **/
void
__gtk_dialog_set_has_separator (GtkDialog *dialog,
                              gboolean   setting)
{
  GtkDialogPrivate *priv;
...
/**
 * __gtk_dialog_get_has_separator:
 * @dialog: a #GtkDialog
 * 
 ...
 **/
gboolean
__gtk_dialog_get_has_separator (GtkDialog *dialog)
{
...
/**
 * __gtk_dialog_response:
 * @dialog: a #GtkDialog
 * @response_id: response ID 
 * 
 * Emits the #GtkDialog::response signal with the given response ID. 
 * Used to indicate that the user has responded to the dialog in some way;
 * typically either you or __gtk_dialog_run() will be monitoring the
 * ::response signal and take appropriate action.
 **/
void
__gtk_dialog_response (GtkDialog *dialog,
                     gint       response_id)
{
  g_return_if_fail (GTK_IS_DIALOG (dialog));
 ...
 * destroyed during the call to __gtk_dialog_run(), __gtk_dialog_run() returns 
 * #GTK_RESPONSE_NONE. Otherwise, it returns the response ID from the 
 * ::response signal emission.
 *
 * Before entering the recursive main loop, __gtk_dialog_run() calls
 * __gtk_widget_show() on the dialog for you. Note that you still
 * need to show any children of the dialog yourself.
 *
 * During __gtk_dialog_run(), the default behavior of #GtkWidget::delete-event 
 * is disabled; if the dialog receives ::delete_event, it will not be
 * destroyed as windows usually are, and __gtk_dialog_run() will return
 * #GTK_RESPONSE_DELETE_EVENT. Also, during __gtk_dialog_run() the dialog 
 * will be modal. You can force __gtk_dialog_run() to return at any time by
 * calling __gtk_dialog_response() to emit the ::response signal. Destroying 
 * the dialog during __gtk_dialog_run() is a very bad idea, because your 
 * post-run code won't know whether the dialog was destroyed or not.
 *
 * After __gtk_dialog_run() returns, you are responsible for hiding or
 * destroying the dialog if you wish to do so.
 *
 * Typical usage of this function might be:
 * |[
 *   gint result = __gtk_dialog_run (GTK_DIALOG (dialog));
 *   switch (result)
 *     {
 ...
 * be triggered during a __gtk_dialog_run() call.
 * 
 * Return value: response ID
 **/
gint
__gtk_dialog_run (GtkDialog *dialog)
{
  ...
  if (!__gtk_widget_get_visible (GTK_WIDGET (dialog)))
    __gtk_widget_show (GTK_WIDGET (dialog));
  ...
  if (!ri.destroyed)
    {
      if (!was_modal)
        __gtk_window_set_modal (GTK_WINDOW(dialog), FALSE);
...
void
______gtk_dialog_set_ignore_separator (GtkDialog *dialog,
...
/**
 * __gtk_dialog_get_widget_for_response:
 * @dialog: a #GtkDialog
 ...
 */
GtkWidget*
__gtk_dialog_get_widget_for_response (GtkDialog *dialog,
				    gint       response_id)
{
  GList *children;
  GList *tmp_list;
...
/**
 * __gtk_dialog_get_response_for_widget:
 * @dialog: a #GtkDialog
 ...
 */
gint
__gtk_dialog_get_response_for_widget (GtkDialog *dialog,
				    GtkWidget *widget)
{
  ResponseData *rd;
...
/**
 * __gtk_alternative_dialog_button_order:
 * @screen: (allow-none): a #GdkScreen, or %NULL to use the default screen
 *
 * Returns %TRUE if dialogs are expected to use an alternative
 * button order on the screen @screen. See
 * __gtk_dialog_set_alternative_button_order() for more details
 ...
 */
gboolean 
__gtk_alternative_dialog_button_order (GdkScreen *screen)
{
...
static void
__gtk_dialog_set_alternative_button_order_valist (GtkDialog *dialog,
...
/**
 * __gtk_dialog_set_alternative_button_order:
 * @dialog: a #GtkDialog
 ...
 * |[
 * cancel_button = __gtk_dialog_add_button (GTK_DIALOG (dialog),
 *                                        GTK_STOCK_CANCEL,
 *                                        GTK_RESPONSE_CANCEL);
 *  
 * ok_button = __gtk_dialog_add_button (GTK_DIALOG (dialog),
 *                                    GTK_STOCK_OK,
 *                                    GTK_RESPONSE_OK);
 *   
 * __gtk_widget_grab_default (ok_button);
 *   
 * help_button = __gtk_dialog_add_button (GTK_DIALOG (dialog),
 *                                      GTK_STOCK_HELP,
 *                                      GTK_RESPONSE_HELP);
 *  
 ...
 */
void 
__gtk_dialog_set_alternative_button_order (GtkDialog *dialog,
					 gint       first_response_id,
					 ...)
{
  GdkScreen *screen;
  ...
  screen = __gtk_widget_get_screen (GTK_WIDGET (dialog));
  if (!__gtk_alternative_dialog_button_order (screen))
      return;

  va_start (args, first_response_id);

  __gtk_dialog_set_alternative_button_order_valist (dialog,
						  first_response_id,
						  args);
  va_end (args);
}
/**
 * __gtk_dialog_set_alternative_button_order_from_array:
 * @dialog: a #GtkDialog
 ...
 *
 * See __gtk_dialog_set_alternative_button_order() for more information.
 *
 ...
 */
void 
__gtk_dialog_set_alternative_button_order_from_array (GtkDialog *dialog,
                                                    gint       n_params,
                                                    gint      *new_order)
{
  GdkScreen *screen;
  ...
  for (l = parser_data->items; l; l = l->next)
    {
      ...
      object = __gtk_builder_get_object (builder, item->widget_name);
      if (!object)
	{
	  g_warning ("Unknown object %s specified in action-widgets of %s",
		     item->widget_name,
		     __gtk_buildable_get_name (GTK_BUILDABLE (buildable)));
	  continue;
	}
 ...
 **/
GtkWidget *
__gtk_dialog_get_action_area (GtkDialog *dialog)
{
  g_return_val_if_fail (GTK_IS_DIALOG (dialog), NULL);
 ...
 **/
GtkWidget *
__gtk_dialog_get_content_area (GtkDialog *dialog)
{
  g_return_val_if_fail (GTK_IS_DIALOG (dialog), NULL);
