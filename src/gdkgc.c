...
STLWRT_DEFINE_FTYPE (GdkGC, gdk_gc, G_TYPE_OBJECT, 0, G_ADD_PRIVATE (GdkGC))
 ...
 **/
GdkGC*
__gdk_gc_new (GdkDrawable *drawable)
{
  g_return_val_if_fail (drawable != NULL, NULL);

  return __gdk_gc_new_with_values (drawable, NULL, 0);
}

/**
 * __gdk_gc_new_with_values:
 * @drawable: a #GdkDrawable. The created GC must always be used
 *   with drawables of the same depth as this one.
 ...
 **/
GdkGC*
__gdk_gc_new_with_values (GdkDrawable	*drawable,
			GdkGCValues	*values,
	      ...
	      GdkGCValuesMask  values_mask)
{
  ...
  gc->colormap = __gdk_drawable_get_colormap (drawable);
  if (gc->colormap)
...
gdk_gc_finalize (GObject *object)
{
  ...
  if (priv->old_clip_region)
    __gdk_region_destroy (priv->old_clip_region);
  if (priv->clip_mask)
 ...
 * are filled, use __gdk_colormap_query_color() to obtain the rgb values
 * if you need them.
 *
 * Deprecated: 2.22: Use Cairo for rendering.
 **/
void
__gdk_gc_get_values (GdkGC       *gc,
		   GdkGCValues *values)
{
  g_return_if_fail (GDK_IS_GC (gc));
...
/**
 * __gdk_gc_set_foreground:
 * @gc: a #GdkGC.
 * @color: the new foreground color.
 * 
 * Sets the foreground color for a graphics context.
 * Note that this function uses @color->pixel, use 
 * __gdk_gc_set_rgb_fg_color() to specify the foreground 
 * color as red, green, blue components.
 *
 * Deprecated: 2.22: Use __gdk_cairo_set_source_color() to use a #GdkColor
 * as the source in Cairo.
 **/
void
__gdk_gc_set_foreground (GdkGC	      *gc,
		       const GdkColor *color)
{
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_FOREGROUND);
}

/**
 * __gdk_gc_set_background:
 * @gc: a #GdkGC.
 * @color: the new background color.
 * 
 * Sets the background color for a graphics context.
 * Note that this function uses @color->pixel, use 
 * __gdk_gc_set_rgb_bg_color() to specify the background 
 * color as red, green, blue components.
 *
 * Deprecated: 2.22: Use __gdk_cairo_set_source_color() to use a #GdkColor
 ...
 **/
void
__gdk_gc_set_background (GdkGC	      *gc,
		       const GdkColor *color)
{
  GdkGCValues values;
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_BACKGROUND);
}
		 ...
		 GdkFont *font)
{
  ...
  values.font = font;
  __gdk_gc_set_values (gc, &values, GDK_GC_FONT);
}

/**
 * __gdk_gc_set_function:
 * @gc: a #GdkGC.
 ...
 **/
void
__gdk_gc_set_function (GdkGC	 *gc,
		     GdkFunction  function)
{
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_FUNCTION);
}
 ...
 * deprecation comments on __gdk_gc_set_stipple().
 **/
void
__gdk_gc_set_fill (GdkGC	 *gc,
		 GdkFill  fill)
{
  GdkGCValues values;
  ...
  values.fill = fill;
  __gdk_gc_set_values (gc, &values, GDK_GC_FILL);
}
 ...
 * as the source in Cairo:
 * |[__gdk_cairo_set_source_pixmap (cr, tile, ts_origin_x, ts_origin_y);
 * cairo_pattern_set_extend (cairo_get_source (cr), CAIRO_EXTEND_REPEAT);]|
 **/
void
__gdk_gc_set_tile (GdkGC	   *gc,
		 GdkPixmap *tile)
{
  GdkGCValues values;
  ...
  values.tile = tile;
  __gdk_gc_set_values (gc, &values, GDK_GC_TILE);
}

/**
 * __gdk_gc_set_stipple:
 * @gc: a #GdkGC.
		    ...
		    GdkPixmap *stipple)
{
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_STIPPLE);
}

/**
 * __gdk_gc_set_ts_origin:
 * @gc:  a #GdkGC.
 ...
 * can specify it with __gdk_cairo_set_source_pixmap() as shown in the example
 * for __gdk_gc_set_tile().
 **/
void
__gdk_gc_set_ts_origin (GdkGC *gc,
		      gint   x,
		      gint   y)
{
  GdkGCValues values;
  ...
  __gdk_gc_set_values (gc, &values,
		     GDK_GC_TS_X_ORIGIN | GDK_GC_TS_Y_ORIGIN);
}

/**
 * __gdk_gc_set_clip_origin:
 * @gc: a #GdkGC.
 * @x: the x-coordinate of the origin.
 * @y: the y-coordinate of the origin.
 * 
 ...
 **/
void
__gdk_gc_set_clip_origin (GdkGC *gc,
			gint   x,
			gint   y)
{
  GdkGCValues values;
...
/**
 * __gdk_gc_set_clip_mask:
 * @gc: the #GdkGC.
 * @mask: a bitmap.
 * 
 * Sets the clip mask for a graphics context from a bitmap.
 * The clip mask is interpreted relative to the clip
 * origin. (See __gdk_gc_set_clip_origin()).
 *
 * Deprecated: 2.22: Use cairo_mask() instead.
 **/
void
__gdk_gc_set_clip_mask (GdkGC	*gc,
		      GdkBitmap *mask)
{
  GdkGCValues values;
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_CLIP_MASK);
}

/* Takes ownership of passed in region */
static void
___gdk_gc_set_clip_region_real (GdkGC     *gc,
			      GdkRegion *region,
...
{
  _gdk_gc_remove_drawable_clip (gc);
  ___gdk_gc_set_clip_region_real (gc, region, reset_origin);
}
			   ...
			   int        offset_y)
{
  ...
  region = __gdk_region_copy (region);
  if (offset_x != 0 || offset_y != 0)
    __gdk_region_offset (region, offset_x, offset_y);
  ...
  if (priv->clip_mask)
    {
       ...
       */
      overlap = __gdk_region_rect_in (region, &r);
  ...
  else
    {
      ...
      priv->clip_region = region;
      if (priv->old_clip_region)
	__gdk_region_intersect (region, priv->old_clip_region);
  ...
  if (priv->region_tag_applied)
    {
      ...
      if (priv->old_clip_mask)
	{
	  __gdk_gc_set_clip_mask (gc, priv->old_clip_mask);
	  g_object_unref (priv->old_clip_mask);
	...
	{
	  ___gdk_gc_set_clip_region_real (gc, priv->old_clip_region, FALSE);
 ...
 **/
void
__gdk_gc_set_clip_rectangle (GdkGC              *gc,
			   const GdkRectangle *rectangle)
{
  ...
  ___gdk_gc_set_clip_region_real (gc, region, TRUE);
}

/**
 * __gdk_gc_set_clip_region:
 * @gc: a #GdkGC.
 ...
 * __gdk_gc_set_clip_origin()).
 *
 * Deprecated: 2.22: Use __gdk_cairo_region() and cairo_clip() in Cairo.
 **/
void
__gdk_gc_set_clip_region (GdkGC           *gc,
			const GdkRegion *region)
{
  ...
  ___gdk_gc_set_clip_region_real (gc, copy, TRUE);
}
 ...
 **/
void
__gdk_gc_set_subwindow (GdkGC	       *gc,
		      GdkSubwindowMode	mode)
{
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_SUBWINDOW);
}
 ...
 * drawable. (See __gdk_draw_drawable()).
 *
 * Deprecated: 2.22: There is no replacement. If you need to control
 * exposures, you must use drawing operations of the underlying window
 * system or use __gdk_window_invalidate_rect(). Cairo will never
 * generate exposures.
 **/
void
__gdk_gc_set_exposures (GdkGC     *gc,
		      gboolean   exposures)
{
  GdkGCValues values;
  ...
  __gdk_gc_set_values (gc, &values, GDK_GC_EXPOSURES);
}
 ...
 **/
void
__gdk_gc_set_line_attributes (GdkGC	*gc,
			    gint	 line_width,
 ...
 * be changed with __gdk_gc_set_line_attributes().)
 *
 ...
 * convenience function for __gdk_gc_set_ts_origin and __gdk_gc_set_clip_origin().
 **/
void
__gdk_gc_offset (GdkGC *gc,
	       gint   x_offset,
	       gint   y_offset)
{
	     ...
	     GdkGC *src_gc)
{
  ...
  if (dst_priv->clip_region)
    __gdk_region_destroy (dst_priv->clip_region);

  if (src_priv->clip_region)
    dst_priv->clip_region = __gdk_region_copy (src_priv->clip_region);
  else
  ...
  if (dst_priv->old_clip_region)
    __gdk_region_destroy (dst_priv->old_clip_region);

  if (src_priv->old_clip_region)
    dst_priv->old_clip_region = __gdk_region_copy (src_priv->old_clip_region);
  else
 ...
 **/
void
__gdk_gc_set_colormap (GdkGC       *gc,
 ...
 **/
GdkColormap *
__gdk_gc_get_colormap (GdkGC *gc)
{
  g_return_val_if_fail (GDK_IS_GC (gc), NULL);
...
static GdkColormap *
__gdk_gc_get_colormap_warn (GdkGC *gc)
{
  GdkColormap *colormap = __gdk_gc_get_colormap (gc);
  if (!colormap)
    {
      g_warning ("__gdk_gc_set_rgb_fg_color() and __gdk_gc_set_rgb_bg_color() can\n"
		 "only be used on GC's with a colormap. A GC will have a colormap\n"
		 "if it is created for a drawable with a colormap, or if a\n"
		 "colormap has been set explicitly with __gdk_gc_set_colormap.\n");
      return NULL;
    }
 ...
 *
 * Deprecated: 2.22: Use __gdk_cairo_set_source_color() instead.
 **/
void
__gdk_gc_set_rgb_fg_color (GdkGC          *gc,
			 const GdkColor *color)
{
  GdkColormap *cmap;
  GdkColor tmp_color;
  ...
  cmap = __gdk_gc_get_colormap_warn (gc);
  if (!cmap)
    return;

  tmp_color = *color;
  __gdk_rgb_find_color (cmap, &tmp_color);
  __gdk_gc_set_foreground (gc, &tmp_color);
}

/**
 * __gdk_gc_set_rgb_bg_color:
 * @gc: a #GdkGC
 * @color: an unallocated #GdkColor.
 * 
 ...
 **/
void
__gdk_gc_set_rgb_bg_color (GdkGC          *gc,
			 const GdkColor *color)
{
  GdkColormap *cmap;
  GdkColor tmp_color;
  ...
  tmp_color = *color;
  __gdk_rgb_find_color (cmap, &tmp_color);
  __gdk_gc_set_background (gc, &tmp_color);
}
			   ...
			   GdkColor  *background)
{
  ...
  if (background)
      __gdk_cairo_set_source_color (tmp_cr, background);
  else
  ...
  __gdk_cairo_set_source_color (tmp_cr, foreground);
		   ...
		   GdkColor *color)
{
  ...
  if (gc->colormap)
    __gdk_colormap_query_color (gc->colormap, priv->fg_pixel, color);
  else
		   ...
		   GdkColor *color)
{
  ...
  if (gc->colormap)
    __gdk_colormap_query_color (gc->colormap, priv->bg_pixel, color);
  else
  ...
  if (priv->clip_region)
    {
      ...
      cairo_new_path (cr);
      __gdk_cairo_region (cr, priv->clip_region);
