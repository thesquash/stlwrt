...
#include <gtksettings.h>	/* ______gtk_settings_parse_convert() */
#include <gtkintl.h>
...
/**
 * ______gtk_style_init_for_settings:
 * @style: a #GtkStyle
 ...
 **/
void
______gtk_style_init_for_settings (GtkStyle    *style,
			      GtkSettings *settings)
{
  const gchar *font_name = ______gtk_rc_context_get_default_font_name (settings);
...
gtk_style_init (GtkStyle *style)
{
  ...
  if (settings)
    ______gtk_style_init_for_settings (style, settings);
  else
                  ...
                  GdkWindow *window)
{
  ...
  colormap = __gdk_drawable_get_colormap (window);
			     ...
			     GtkRcStyle *rc_style)
{
  ...
  priv->color_hashes = g_slist_copy (______gtk_rc_style_get_color_hashes (rc_style));
  g_slist_foreach (priv->color_hashes, (GFunc) g_hash_table_ref, NULL);
}
                              ...
                              GValue      *value)
{
  ...
  peek_value = ______gtk_style_peek_property_value (style, widget_type, pspec, parser);
...
const GValue*
______gtk_style_peek_property_value (GtkStyle           *style,
				GType               widget_type,
				GParamSpec         *pspec,
				GtkRcPropertyParser parser)
{
  ...
  if (style->rc_style)
    {
      ...
      do
	{
	  rcprop = ______gtk_rc_style_lookup_rc_property (style->rc_style,
  ...
  if (rcprop && !______gtk_settings_parse_convert (parser, &rcprop->value,
					      pspec, &pcache->value))
    {
...
gtk_style_real_realize (GtkStyle *style)
{
  ...
  for (i = 0; i < 5; i++)
    {
      ______gtk_style_shade (&style->bg[i], &style->light[i], LIGHTNESS_MULT);
      ______gtk_style_shade (&style->bg[i], &style->dark[i], DARKNESS_MULT);
                                    ...
                                    gint                height)
{
  ...
  if (!style->bg_pixmap[state_type] ||
      ...
      (!set_bg && style->bg_pixmap[state_type] != (GdkPixmap*) GDK_PARENT_RELATIVE))
    {
      ...
      __gdk_draw_rectangle (window, gc, TRUE, 
...
{
  if ((*width == -1) && (*height == -1))
    __gdk_drawable_get_size (window, width, height);
  else if (*width == -1)
    __gdk_drawable_get_size (window, width, NULL);
  else if (*height == -1)
    __gdk_drawable_get_size (window, NULL, height);
}
                         ...
                         gint           height)
{
  ...
  if (shadow_type == GTK_SHADOW_IN)
      ...
      if (detail && strcmp (detail, "buttondefault") == 0)
	{
	  ...
	  __gdk_draw_rectangle (window, style->black_gc, FALSE,
                          ...
                          gboolean       fill)
{
  ...
  if (fill)
    __gdk_draw_polygon (window, style->bg_gc[state_type], TRUE, points, npoints);
                 ...
                 gint            darken_count)
{
  ...
  while (darken_count)
    {
      ______gtk_style_shade (&src, &shaded, 0.93);
      src = shaded;
      --darken_count;
    }
                           ...
                           gint           height)
{
  ...
  if (!style->bg_pixmap[state_type] || gc1 != style->bg_gc[state_type] ||
	...
	__gdk_gc_set_clip_rectangle (gc1, area);
      ...
      __gdk_draw_rectangle (window, gc1, TRUE,
                          x, y, width, height);
	  ...
	  gushort       size)
{
  ...
  if (size == 2)
    {
      __gdk_draw_point (window, light_gc, x, y);
      __gdk_draw_point (window, light_gc, x+1, y+1);
    }
  else if (size == 3)
    {
      __gdk_draw_point (window, light_gc, x, y);
      __gdk_draw_point (window, light_gc, x+1, y);
      __gdk_draw_point (window, light_gc, x, y+1);
      __gdk_draw_point (window, dark_gc, x+1, y+2);
      __gdk_draw_point (window, dark_gc, x+2, y+1);
      __gdk_draw_point (window, dark_gc, x+2, y+2);
    }
}
			 ...
			 GtkOrientation orientation)
{
  ...
  if (detail && !strcmp (detail, "paned"))
      ...
      if (state_type == GTK_STATE_SELECTED && widget && !__gtk_widget_has_focus (widget))
	{
	  ...
	  ______gtk_style_shade (&style->base[GTK_STATE_ACTIVE], &unfocused_light,
                            LIGHTNESS_MULT);
  ...
  if (state_type == GTK_STATE_INSENSITIVE)
    {
      ...
      __gdk_draw_layout (window, gc, x, y, ins);

      g_object_unref (ins);
    }
  else
    {
      __gdk_draw_layout (window, gc, x, y, layout);
    }
                              ...
                              gint            height)
{
  ...
  __gdk_draw_polygon (window, style->bg_gc[state_type], TRUE, 
		    points, skip < 0 ? 4 : 3);
...
void
______gtk_style_shade (const GdkColor *a,
                  GdkColor       *b,
                  gdouble         k)
{
  gdouble red;
                 ...
                 gint                y)
{
  g_return_if_fail (GTK_IS_STYLE (style));
  g_return_if_fail (GTK_STYLE_GET_CLASS (style)->draw_hline != NULL);
  g_return_if_fail (style->depth == __gdk_drawable_get_depth (window));
...
GdkGC *
______gtk_widget_get_cursor_gc (GtkWidget *widget)
{
...
void
______gtk_widget_get_cursor_color (GtkWidget *widget,
			      GdkColor  *color)
{
