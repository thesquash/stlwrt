...
STLWRT_DEFINE_FTYPE_VPARENT (GtkVolumeButton, gtk_volume_button, GTK_TYPE_SCALE_BUTTON,
                             G_TYPE_FLAG_NONE, ;)
...
gtk_volume_button_init (GtkVolumeButton *button)
{
  ...
  atk_object_set_name (__gtk_widget_get_accessible (GTK_WIDGET (button)),
		       _("Volume"));
  atk_object_set_description (__gtk_widget_get_accessible (GTK_WIDGET (button)),
		       _("Turns volume down or up"));
  atk_action_set_description (ATK_ACTION (__gtk_widget_get_accessible (GTK_WIDGET (button))),
			      1,
  ...
  adj = __gtk_adjustment_new (0., 0., 1., 0.02, 0.2, 0.);
		  ...
		  gpointer    user_data)
{
  ...
  image = ATK_IMAGE (__gtk_widget_get_accessible (button));
  ...
  __gtk_tooltip_set_text (tooltip, str);
