...
{
  return __gtk_bin_get_child (GTK_BIN (gtk_tool_item_group_get_props (group)->priv->header));
}

static GtkOrientation
gtk_tool_item_group_get_orientation (GtkToolShell *shell)
{
  GtkWidget *parent = __gtk_widget_get_parent (GTK_WIDGET (shell));

  if (GTK_IS_TOOL_PALETTE (parent))
    return __gtk_orientable_get_orientation (GTK_ORIENTABLE (parent));
...
gtk_tool_item_group_get_style (GtkToolShell *shell)
{
  ...
  if (GTK_IS_TOOL_PALETTE (parent))
    return __gtk_tool_palette_get_style (GTK_TOOL_PALETTE (parent));
...
gtk_tool_item_group_get_icon_size (GtkToolShell *shell)
{
  ...
  if (GTK_IS_TOOL_PALETTE (parent))
    return __gtk_tool_palette_get_icon_size (GTK_TOOL_PALETTE (parent));
...
{
  GtkSettings *settings = gtk_tool_item_group_get_props (group)->priv->settings;
  gboolean animation;
  ...
  gtk_tool_item_group_get_props (group)->priv->animation = animation;
}
                                    ...
                                    GdkScreen *previous_screen)
{
  ...
  if (__gtk_widget_has_screen (GTK_WIDGET (group)))
    settings = __gtk_widget_get_settings (GTK_WIDGET (group));
  else
    settings = NULL;
...
gtk_tool_item_group_tool_shell_init (GtkToolShellIface *iface)
{
  ...
  iface->get_ellipsize_mode = __gtk_tool_item_group_get_ellipsize_mode;
}
  ...
  if (GTK_ORIENTATION_VERTICAL == orientation)
    {
      if (GTK_TEXT_DIR_RTL == direction)
        x = gtk_widget_get_props (widget)->allocation.x + gtk_widget_get_props (widget)->allocation.width - priv->expander_size / 2;
      else
        x = gtk_widget_get_props (widget)->allocation.x + priv->expander_size / 2;
  ...
  else
    {
      ...
      y = gtk_widget_get_props (widget)->allocation.y + priv->expander_size / 2;

      /* Unfortunatly __gtk_paint_expander() doesn't support rotated drawing
...
{
  GtkToolItemGroup *group = GTK_TOOL_ITEM_GROUP (data);
  requisition->height = MAX (requisition->height, gtk_tool_item_group_get_props (group)->priv->expander_size);
}
                                       ...
                                       gpointer   data)
{
  GtkToolItemGroup *group = GTK_TOOL_ITEM_GROUP (data);
  GtkToolItemGroupPrivate* priv = gtk_tool_item_group_get_props (group)->priv;
  GtkWidget *parent = __gtk_widget_get_parent (data);

  if (priv->collapsed ||
      !GTK_IS_TOOL_PALETTE (parent) ||
      !__gtk_tool_palette_get_exclusive (GTK_TOOL_PALETTE (parent), data))
    __gtk_tool_item_group_set_collapsed (group, !priv->collapsed);
}
...
gtk_tool_item_group_header_adjust_style (GtkToolItemGroup *group)
{
  ...
  GtkWidget *label_widget = __gtk_bin_get_child (GTK_BIN (alignment));
  ...
  switch (__gtk_tool_shell_get_orientation (GTK_TOOL_SHELL (group)))
    {
      case GTK_ORIENTATION_HORIZONTAL:
        ...
        if (GTK_IS_LABEL (label_widget))
          {
            __gtk_label_set_ellipsize (GTK_LABEL (label_widget), PANGO_ELLIPSIZE_NONE);
            if (GTK_TEXT_DIR_RTL == direction)
              __gtk_label_set_angle (GTK_LABEL (label_widget), -90);
            else
              __gtk_label_set_angle (GTK_LABEL (label_widget), 90);
          }
       break;
      ...
      case GTK_ORIENTATION_VERTICAL:
        ...
        if (GTK_IS_LABEL (label_widget))
          {
            __gtk_label_set_ellipsize (GTK_LABEL (label_widget), priv->ellipsize);
            __gtk_label_set_angle (GTK_LABEL (label_widget), 0);
          }
        break;
    }

  __gtk_alignment_set_padding (GTK_ALIGNMENT (alignment), dy, 0, dx, 0);
}
...
gtk_tool_item_group_init (GtkToolItemGroup *group)
{
  ...
  priv->label_widget = __gtk_label_new (NULL);
  __gtk_misc_set_alignment (GTK_MISC (priv->label_widget), 0.0, 0.5);
  alignment = __gtk_alignment_new (0.5, 0.5, 1.0, 1.0);
  __gtk_container_add (GTK_CONTAINER (alignment), priv->label_widget);
  __gtk_widget_show_all (alignment);

  __gtk_widget_push_composite_child ();
  priv->header = __gtk_button_new ();
  __gtk_widget_set_composite_name (priv->header, "header");
  __gtk_widget_pop_composite_child ();

  g_object_ref_sink (priv->header);
  __gtk_button_set_focus_on_click (GTK_BUTTON (priv->header), FALSE);
  __gtk_container_add (GTK_CONTAINER (priv->header), alignment);
  __gtk_widget_set_parent (priv->header, GTK_WIDGET (group));
                                  ...
                                  GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
      case PROP_LABEL:
        __gtk_tool_item_group_set_label (group, g_value_get_string (value));
        break;

      case PROP_LABEL_WIDGET:
        __gtk_tool_item_group_set_label_widget (group, g_value_get_object (value));
	break;

      case PROP_COLLAPSED:
        __gtk_tool_item_group_set_collapsed (group, g_value_get_boolean (value));
        break;

      case PROP_ELLIPSIZE:
        __gtk_tool_item_group_set_ellipsize (group, g_value_get_enum (value));
        break;

      case PROP_RELIEF:
        __gtk_tool_item_group_set_header_relief (group, g_value_get_enum(value));
        break;
                                  ...
                                  GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
      case PROP_LABEL:
        g_value_set_string (value, __gtk_tool_item_group_get_label (group));
        break;

      case PROP_LABEL_WIDGET:
        g_value_set_object (value,
			    __gtk_tool_item_group_get_label_widget (group));
        break;

      case PROP_COLLAPSED:
        g_value_set_boolean (value, __gtk_tool_item_group_get_collapsed (group));
        break;

      case PROP_ELLIPSIZE:
        g_value_set_enum (value, __gtk_tool_item_group_get_ellipsize (group));
        break;

      case PROP_RELIEF:
        g_value_set_enum (value, __gtk_tool_item_group_get_header_relief (group));
        break;
...
gtk_tool_item_group_finalize (GObject *object)
{
  ...
  if (gtk_tool_item_group_get_props (group)->priv->children)
    {
      g_list_free (gtk_tool_item_group_get_props (group)->priv->children);
      gtk_tool_item_group_get_props (group)->priv->children = NULL;
    }
...
gtk_tool_item_group_get_item_size (GtkToolItemGroup *group,
                                   ...
                                   gint             *requested_rows)
{
  GtkWidget *parent = __gtk_widget_get_parent (GTK_WIDGET (group));
                                  ...
                                  GtkRequisition *requisition)
{
  ...
  if (priv->children && __gtk_tool_item_group_get_label_widget (group))
    {
      __gtk_widget_size_request (priv->header, requisition);
      __gtk_widget_show (priv->header);
    }
  else
    {
      requisition->width = requisition->height = 0;
      __gtk_widget_hide (priv->header);
    }
                                     ...
                                     GtkToolItemGroupChild *child)
{
  ...
  orientation = __gtk_tool_shell_get_orientation (GTK_TOOL_SHELL (group));
  style = __gtk_tool_shell_get_style (GTK_TOOL_SHELL (group));
  ...
  return
    (__gtk_widget_get_visible (GTK_WIDGET (child->item))) &&
    (GTK_ORIENTATION_VERTICAL == orientation ?
     __gtk_tool_item_get_visible_vertical (child->item) :
     __gtk_tool_item_get_visible_horizontal (child->item));
}
          ...
          for (it = priv->children; it != NULL; it = it->next)
            {
              ...
              if (child->homogeneous)
              ...
              else
                {
                  ...
                  __gtk_widget_size_request (GTK_WIDGET (child->item), &req);
  ...
  if (__gtk_widget_get_visible (priv->header))
    {
      ...
      __gtk_widget_size_request (priv->header, &child_requisition);
  ...
  if (__gtk_widget_get_visible (priv->header))
    {
      ...
      __gtk_widget_size_allocate (priv->header, &child_allocation);
      ...
      for (it = priv->children; it != NULL; it = it->next)
        {
          ...
          if (!gtk_tool_item_group_is_item_visible (group, child))
            {
              __gtk_widget_set_child_visible (GTK_WIDGET (child->item), FALSE);
          ...
          if (!child->homogeneous)
            {
              __gtk_widget_size_request (GTK_WIDGET (child->item), &child_requisition);
          ...
          __gtk_widget_size_allocate (GTK_WIDGET (child->item), &child_allocation);
          __gtk_widget_set_child_visible (GTK_WIDGET (child->item), TRUE);
        }
                                  ...
                                  gpointer   user_data)
{
  ...
  for (p = widget; p; p = __gtk_widget_get_parent (p))
    if (p == user_data)
      {
        p = __gtk_widget_get_parent (p);
        break;
      }
  ...
  if (GTK_IS_TOOL_PALETTE (p))
    {
      ...
      adjustment = __gtk_tool_palette_get_hadjustment (GTK_TOOL_PALETTE (p));
      adjustment = __gtk_tool_palette_get_vadjustment (GTK_TOOL_PALETTE (p));
      ...
      if (adjustment)
        {
          ...
          if (__gtk_widget_translate_coordinates
            ...
            {
              y += gtk_adjustment_get_props (adjustment)->value;
              __gtk_adjustment_clamp_page (adjustment, y - gtk_widget_get_props (widget)->allocation.height, y);
            }
        }
      ...
      if (adjustment)
        {
          ...
          if (__gtk_widget_translate_coordinates
            ...
            {
              x += gtk_adjustment_get_props (adjustment)->value;
              __gtk_adjustment_clamp_page (adjustment, x - gtk_widget_get_props (widget)->allocation.width, x);
            }
...
gtk_tool_item_group_realize (GtkWidget *widget)
{
  ...
  toplevel_window = __gtk_widget_get_ancestor (widget, GTK_TYPE_WINDOW);
                         ...
                         GtkWidget    *widget)
{
  ...
  __gtk_tool_item_group_insert (GTK_TOOL_ITEM_GROUP (container),
                            ...
                            GtkWidget    *child)
{
  ...
  priv = gtk_tool_item_group_get_props (group)->priv;
                               ...
                               GList            **link)
{
  ...
  for (it = gtk_tool_item_group_get_props (group)->priv->children, i = 0; it != NULL; it = it->next, ++i)
    {
      GtkToolItemGroupChild *child = it->data;
                                      ...
                                      gboolean          new_row)
{
  ...
  __gtk_widget_freeze_child_notify (GTK_WIDGET (item));

  if (child->homogeneous != homogeneous)
    {
      child->homogeneous = homogeneous;
      changed = TRUE;
      __gtk_widget_child_notify (GTK_WIDGET (item), "homogeneous");
    }
  if (child->expand != expand)
    {
      child->expand = expand;
      changed = TRUE;
      __gtk_widget_child_notify (GTK_WIDGET (item), "expand");
    }
  if (child->fill != fill)
    {
      child->fill = fill;
      changed = TRUE;
      __gtk_widget_child_notify (GTK_WIDGET (item), "fill");
    }
  if (child->new_row != new_row)
    {
      child->new_row = new_row;
      changed = TRUE;
      __gtk_widget_child_notify (GTK_WIDGET (item), "new-row");
    }

  __gtk_widget_thaw_child_notify (GTK_WIDGET (item));

  if (changed
      && __gtk_widget_get_visible (GTK_WIDGET (group))
      && __gtk_widget_get_visible (GTK_WIDGET (item)))
    __gtk_widget_queue_resize (GTK_WIDGET (group));
}
  ...
  switch (prop_id)
    {
      ...
      case CHILD_PROP_POSITION:
        __gtk_tool_item_group_set_item_position (group, item, g_value_get_int (value));
        break;
  ...
  switch (prop_id)
    {
     ...
     case CHILD_PROP_POSITION:
        g_value_set_int (value, __gtk_tool_item_group_get_item_position (group, item));
        break;
...
gtk_tool_item_group_class_init (GtkToolItemGroupClass *cls)
{
  ...
  __gtk_widget_class_install_style_property (wclass,
  ...
  __gtk_container_class_install_child_property (cclass, CHILD_PROP_HOMOGENEOUS,
  ...
  __gtk_container_class_install_child_property (cclass, CHILD_PROP_EXPAND,
  ...
  __gtk_container_class_install_child_property (cclass, CHILD_PROP_FILL,
  ...
  __gtk_container_class_install_child_property (cclass, CHILD_PROP_NEW_ROW,
  ...
  __gtk_container_class_install_child_property (cclass, CHILD_PROP_POSITION,
 ...
 */
GtkWidget*
__gtk_tool_item_group_new (const gchar *label)
{
  return g_object_new (GTK_TYPE_TOOL_ITEM_GROUP, "label", label, NULL);
}

/**
 * __gtk_tool_item_group_set_label:
 ...
 */
void
__gtk_tool_item_group_set_label (GtkToolItemGroup *group,
                               const gchar      *label)
{
  g_return_if_fail (GTK_IS_TOOL_ITEM_GROUP (group));

  if (!label)
    __gtk_tool_item_group_set_label_widget (group, NULL);
  else
    {
      GtkWidget *child = __gtk_label_new (label);
      __gtk_widget_show (child);

      __gtk_tool_item_group_set_label_widget (group, child);
    }
...
/**
 * __gtk_tool_item_group_set_label_widget:
 * @group: a #GtkToolItemGroup
 ...
 */
void
__gtk_tool_item_group_set_label_widget (GtkToolItemGroup *group,
                                      GtkWidget        *label_widget)
{
  GtkToolItemGroupPrivate* priv;
  GtkWidget *alignment;
  ...
  if (priv->label_widget)
    {
      ...
      __gtk_container_remove (GTK_CONTAINER (alignment), priv->label_widget);
    }


  if (label_widget)
      __gtk_container_add (GTK_CONTAINER (alignment), label_widget);

  priv->label_widget = label_widget;

  if (__gtk_widget_get_visible (GTK_WIDGET (group)))
    __gtk_widget_queue_resize (GTK_WIDGET (group));
...
/**
 * __gtk_tool_item_group_set_header_relief:
 * @group: a #GtkToolItemGroup
 * @style: the #GtkReliefStyle
 *
 * Set the button relief of the group header.
 * See __gtk_button_set_relief() for details.
 *
 * Since: 2.20
 */
void
__gtk_tool_item_group_set_header_relief (GtkToolItemGroup *group,
                                       GtkReliefStyle    style)
{
  g_return_if_fail (GTK_IS_TOOL_ITEM_GROUP (group));

  __gtk_button_set_relief (GTK_BUTTON (gtk_tool_item_group_get_props (group)->priv->header), style);
}

static gint64
gtk_tool_item_group_get_animation_timestamp (GtkToolItemGroup *group)
{
  return (g_source_get_time (gtk_tool_item_group_get_props (group)->priv->animation_timeout) -
          gtk_tool_item_group_get_props (group)->priv->animation_start) / 1000;
}
...
gtk_tool_item_group_force_expose (GtkToolItemGroup *group)
{
  ...
  if (__gtk_widget_get_realized (priv->header))
    {
      GtkWidget *alignment = gtk_tool_item_group_get_alignment (group);
      GdkRectangle area;

      /* Find the header button's arrow area... */
      area.x = gtk_widget_get_props (alignment)->allocation.x;
      area.y = gtk_widget_get_props (alignment)->allocation.y + (gtk_widget_get_props (alignment)->allocation.height - priv->expander_size) / 2;
  ...
  if (__gtk_widget_get_realized (widget))
    {
      GtkWidget *parent = __gtk_widget_get_parent (widget);
      ...
      __gtk_widget_translate_coordinates (widget, parent, 0, 0, &x, &y);
      ...
      /* ... and invalidated it to get it animated. */
      __gtk_widget_queue_draw_area (parent, x, y, width, height);
    }
}
...
gtk_tool_item_group_animation_cb (gpointer data)
{
  ...
  __gtk_widget_queue_resize_no_redraw (GTK_WIDGET (group));
...
/**
 * __gtk_tool_item_group_set_collapsed:
 * @group: a #GtkToolItemGroup
 ...
 */
void
__gtk_tool_item_group_set_collapsed (GtkToolItemGroup *group,
                                   gboolean          collapsed)
{
  GtkWidget *parent;
  GtkToolItemGroupPrivate* priv;
  ...
  parent = __gtk_widget_get_parent (GTK_WIDGET (group));
  if (GTK_IS_TOOL_PALETTE (parent) && !collapsed)
    ___gtk_tool_palette_set_expanding_child (GTK_TOOL_PALETTE (parent),
                                           GTK_WIDGET (group));
...
/**
 * __gtk_tool_item_group_set_ellipsize:
 * @group: a #GtkToolItemGroup
 ...
 */
void
__gtk_tool_item_group_set_ellipsize (GtkToolItemGroup   *group,
                                   PangoEllipsizeMode  ellipsize)
{
  g_return_if_fail (GTK_IS_TOOL_ITEM_GROUP (group));

  if (ellipsize != gtk_tool_item_group_get_props (group)->priv->ellipsize)
    {
      gtk_tool_item_group_get_props (group)->priv->ellipsize = ellipsize;
      gtk_tool_item_group_header_adjust_style (group);
...
/**
 * __gtk_tool_item_group_get_label:
 * @group: a #GtkToolItemGroup
 *
 ...
 *     label has been set with __gtk_tool_item_group_set_label_widget()
 *
 * Since: 2.20
 */
const gchar*
__gtk_tool_item_group_get_label (GtkToolItemGroup *group)
{
  GtkToolItemGroupPrivate *priv;
...
/**
 * __gtk_tool_item_group_get_label_widget:
 * @group: a #GtkToolItemGroup
 *
 * Gets the label widget of @group.
 * See __gtk_tool_item_group_set_label_widget().
 *
 ...
 */
GtkWidget*
__gtk_tool_item_group_get_label_widget (GtkToolItemGroup *group)
{
  GtkWidget *alignment = gtk_tool_item_group_get_alignment (group);

  return __gtk_bin_get_child (GTK_BIN (alignment));
}

/**
 * __gtk_tool_item_group_get_collapsed:
 * @group: a GtkToolItemGroup
 *
 ...
 */
gboolean
__gtk_tool_item_group_get_collapsed (GtkToolItemGroup *group)
{
  g_return_val_if_fail (GTK_IS_TOOL_ITEM_GROUP (group), DEFAULT_COLLAPSED);

  return gtk_tool_item_group_get_props (group)->priv->collapsed;
}

/**
 * __gtk_tool_item_group_get_ellipsize:
 * @group: a #GtkToolItemGroup
 *
 ...
 */
PangoEllipsizeMode
__gtk_tool_item_group_get_ellipsize (GtkToolItemGroup *group)
{
  g_return_val_if_fail (GTK_IS_TOOL_ITEM_GROUP (group), DEFAULT_ELLIPSIZE);

  return gtk_tool_item_group_get_props (group)->priv->ellipsize;
}

/**
 * __gtk_tool_item_group_get_header_relief:
 * @group: a #GtkToolItemGroup
 *
 ...
 */
GtkReliefStyle
__gtk_tool_item_group_get_header_relief (GtkToolItemGroup   *group)
{
  g_return_val_if_fail (GTK_IS_TOOL_ITEM_GROUP (group), GTK_RELIEF_NORMAL);

  return __gtk_button_get_relief (GTK_BUTTON (gtk_tool_item_group_get_props (group)->priv->header));
}

/**
 * __gtk_tool_item_group_insert:
 * @group: a #GtkToolItemGroup
 ...
 */
void
__gtk_tool_item_group_insert (GtkToolItemGroup *group,
                            GtkToolItem      *item,
                            gint              position)
{
  ...
  gtk_tool_item_group_get_props (group)->priv->children = g_list_insert (gtk_tool_item_group_get_props (group)->priv->children, child, position);

  if (GTK_IS_TOOL_PALETTE (parent))
    _gtk_tool_palette_child_set_drag_source (GTK_WIDGET (item), parent);

  child_widget = __gtk_bin_get_child (GTK_BIN (item));

  if (GTK_IS_BUTTON (child_widget))
    __gtk_button_set_focus_on_click (GTK_BUTTON (child_widget), TRUE);

  __gtk_widget_set_parent (GTK_WIDGET (item), GTK_WIDGET (group));
}

/**
 * __gtk_tool_item_group_set_item_position:
 * @group: a #GtkToolItemGroup
 ...
 */
void
__gtk_tool_item_group_set_item_position (GtkToolItemGroup *group,
                                       GtkToolItem      *item,
                                       gint              position)
{
  gint old_position;
  ...
  __gtk_widget_child_notify (GTK_WIDGET (item), "position");
  if (__gtk_widget_get_visible (GTK_WIDGET (group)) &&
      __gtk_widget_get_visible (GTK_WIDGET (item)))
    __gtk_widget_queue_resize (GTK_WIDGET (group));
}
...
/**
 * __gtk_tool_item_group_get_item_position:
 * @group: a #GtkToolItemGroup
 * @item: a #GtkToolItem
 *
 ...
 */
gint
__gtk_tool_item_group_get_item_position (GtkToolItemGroup *group,
                                       GtkToolItem      *item)
{
  gint position;
...
/**
 * __gtk_tool_item_group_get_n_items:
 * @group: a #GtkToolItemGroup
 *
 ...
 */
guint
__gtk_tool_item_group_get_n_items (GtkToolItemGroup *group)
{
  g_return_val_if_fail (GTK_IS_TOOL_ITEM_GROUP (group), 0);

  return g_list_length (gtk_tool_item_group_get_props (group)->priv->children);
}

/**
 * __gtk_tool_item_group_get_nth_item:
 * @group: a #GtkToolItemGroup
 * @index: the index
 *
 ...
 */
GtkToolItem*
__gtk_tool_item_group_get_nth_item (GtkToolItemGroup *group,
                                  guint             index)
{
  GtkToolItemGroupChild *child;
  ...
  child = g_list_nth_data (gtk_tool_item_group_get_props (group)->priv->children, index);

  return child != NULL ? child->item : NULL;
}

/**
 * __gtk_tool_item_group_get_drop_item:
 * @group: a #GtkToolItemGroup
 * @x: the x position
 * @y: the y position
 *
 ...
 */
GtkToolItem*
__gtk_tool_item_group_get_drop_item (GtkToolItemGroup *group,
                                   gint              x,
                                   gint              y)
{
  GtkAllocation *allocation;
  ...
  for (it = gtk_tool_item_group_get_props (group)->priv->children; it != NULL; it = it->next)
    {
      GtkToolItemGroupChild *child = it->data;
                            ...
                            cairo_t          *cr)
{
  ...
  __gdk_cairo_set_source_pixmap (cr, gtk_widget_get_props (widget)->window,
  ...
  if (priv->animation_timeout)
    {
      ...
      if (GTK_ORIENTATION_VERTICAL == orientation)
        v1 = gtk_widget_get_props (widget)->allocation.height;
      else
        v1 = gtk_widget_get_props (widget)->allocation.width;
      ...
      if (GTK_ORIENTATION_VERTICAL == orientation)
        {
          v0 += gtk_widget_get_props (widget)->allocation.y;
          v1 += gtk_widget_get_props (widget)->allocation.y;

          mask = cairo_pattern_create_linear (0.0, v0, 0.0, v1);
        }
      else
        {
          v0 += gtk_widget_get_props (widget)->allocation.x;
          v1 += gtk_widget_get_props (widget)->allocation.x;
                                         ...
                                         gboolean          animation)
{
  ...
  __gtk_widget_size_request (GTK_WIDGET (group), &requisition);
                                            ...
                                            gpointer   data)
{
  if (GTK_IS_TOOL_ITEM (child))
    __gtk_tool_item_toolbar_reconfigured (GTK_TOOL_ITEM (child));
}
...
{
  __gtk_container_foreach (GTK_CONTAINER (group),
