...
delegate_set_sort_func (GtkRecentChooser  *chooser,
			...
			GDestroyNotify     data_destroy)
{
  __gtk_recent_chooser_set_sort_func (get_delegate (chooser),
		  		    sort_func,
				    sort_data,
				    data_destroy);
}
...
delegate_add_filter (GtkRecentChooser *chooser,
		     GtkRecentFilter  *filter)
{
  __gtk_recent_chooser_add_filter (get_delegate (chooser), filter);
}
...
delegate_remove_filter (GtkRecentChooser *chooser,
			GtkRecentFilter  *filter)
{
  __gtk_recent_chooser_remove_filter (get_delegate (chooser), filter);
}

static GSList *
delegate_list_filters (GtkRecentChooser *chooser)
{
  return __gtk_recent_chooser_list_filters (get_delegate (chooser));
}
...
delegate_select_uri (GtkRecentChooser  *chooser,
		     ...
		     GError           **error)
{
  return __gtk_recent_chooser_select_uri (get_delegate (chooser), uri, error);
}
...
delegate_unselect_uri (GtkRecentChooser *chooser,
		       const gchar      *uri)
{
 __gtk_recent_chooser_unselect_uri (get_delegate (chooser), uri);
}

static GList *
delegate_get_items (GtkRecentChooser *chooser)
{
  return __gtk_recent_chooser_get_items (get_delegate (chooser));
}
...
{
  __gtk_recent_chooser_select_all (get_delegate (chooser));
}

static void
delegate_unselect_all (GtkRecentChooser *chooser)
{
  __gtk_recent_chooser_unselect_all (get_delegate (chooser));
}
...
delegate_set_current_uri (GtkRecentChooser  *chooser,
			  ...
			  GError           **error)
{
  return __gtk_recent_chooser_set_current_uri (get_delegate (chooser), uri, error);
}

static gchar *
delegate_get_current_uri (GtkRecentChooser *chooser)
{
  return __gtk_recent_chooser_get_current_uri (get_delegate (chooser));
}
		       ...
		       gpointer       unused)
{
  ...
  return __gtk_recent_info_get_modified (b) - __gtk_recent_info_get_modified (a);
}
		       ...
		       gpointer       unused)
{
  ...
  return -1 * (__gtk_recent_info_get_modified (b) - __gtk_recent_info_get_modified (a));
}
			...
			GtkRecentInfo   *info)
{
  ...
  needed = __gtk_recent_filter_get_needed (filter);
  
  filter_info.contains = GTK_RECENT_FILTER_URI | GTK_RECENT_FILTER_MIME_TYPE;
  
  filter_info.uri = __gtk_recent_info_get_uri (info);
  filter_info.mime_type = __gtk_recent_info_get_mime_type (info);
  
  if (needed & GTK_RECENT_FILTER_DISPLAY_NAME)
    {
      filter_info.display_name = __gtk_recent_info_get_display_name (info);
  ...
  if (needed & GTK_RECENT_FILTER_APPLICATION)
    {
      filter_info.applications = (const gchar **) __gtk_recent_info_get_applications (info, NULL);
  ...
  if (needed & GTK_RECENT_FILTER_GROUP)
    {
      filter_info.groups = (const gchar **) __gtk_recent_info_get_groups (info, NULL);
  ...
  if (needed & GTK_RECENT_FILTER_AGE)
    {
      filter_info.age = __gtk_recent_info_get_age (info);
      filter_info.contains |= GTK_RECENT_FILTER_AGE;
    }
  else
    filter_info.age = -1;
  
  retval = __gtk_recent_filter_filter (filter, &filter_info);
...
/*
 * ___gtk_recent_chooser_get_items:
 ...
 */
GList *
___gtk_recent_chooser_get_items (GtkRecentChooser  *chooser,
                               GtkRecentFilter   *filter,
                               GtkRecentSortFunc  sort_func,
                               gpointer           sort_data)
{
  ...
  items = __gtk_recent_manager_get_items (manager);
  if (!items)
    return NULL;

  limit = __gtk_recent_chooser_get_limit (chooser);
  if (limit == 0)
    return NULL;
  ...
  sort_type = __gtk_recent_chooser_get_sort_type (chooser);
  switch (sort_type)
    {
  ...
  if ((limit != -1) && (length > limit))
    {
      ...
      g_list_foreach (l, (GFunc) __gtk_recent_info_unref, NULL);
      g_list_free (l);
    }
