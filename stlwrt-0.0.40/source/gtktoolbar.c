...
remove_item (GtkWidget *menu_item,
	     gpointer   data)
{
  __gtk_container_remove (GTK_CONTAINER (menu_item->parent), menu_item);
}
			...
			GtkDirectionType  dir)
{
  ...
  if (container->focus_child &&
      __gtk_widget_child_focus (container->focus_child, dir))
    {
      return;
    }
  ...
  switch (type)
      ...
      if (text && use_stock)
	{
	  ...
	  if (__gtk_stock_lookup (text, &stock_item))
	    {
	      if (!icon)
		icon = __gtk_image_new_from_stock (text, toolbar->icon_size);
	  
	      text = free_me = ______gtk_toolbar_elide_underscores (stock_item.label);
	    }
	}
  ...
  switch (content->type)
      ...
      if (child->type == GTK_TOOLBAR_CHILD_SPACE)
	{
	  if (content->u.compatibility.space_visible &&
	     ...
	     ______gtk_toolbar_paint_space_line (GTK_WIDGET (toolbar), toolbar,
					    &expose->area,
...
gint
______gtk_toolbar_get_default_space_size (void)
{
  return DEFAULT_SPACE_SIZE;
}

void
______gtk_toolbar_paint_space_line (GtkWidget           *widget,
			       GtkToolbar          *toolbar,
  ...
  if (orientation == GTK_ORIENTATION_HORIZONTAL)
    {
      ...
      if (wide_separators)
        ...
        __gtk_paint_vline (widget->style, widget->window,
  ...
  else
    {
      ...
      if (wide_separators)
        ...
        __gtk_paint_hline (widget->style, widget->window,
                         __gtk_widget_get_state (widget), area, widget,
                         "toolbar",
                         allocation->x + allocation->width * start_fraction,
...
gchar *
______gtk_toolbar_elide_underscores (const gchar *original)
{
  gchar *q, *result;
  const gchar *p, *end;
  gsize len;
