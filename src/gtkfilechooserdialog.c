...
STLWRT_DEFINE_TYPE_WITH_CODE (GtkFileChooserDialog, gtk_file_chooser_dialog, GTK_TYPE_DIALOG,
  ...
  for (l = children; l; l = l->next)
    {
      ...
      response_id = __gtk_dialog_get_response_for_widget (GTK_DIALOG (dialog), widget);
      if (is_stock_accept_response_id (response_id))
	{
	  __gtk_widget_activate (widget); /* Should we __gtk_dialog_response (dialog, response_id) instead? */
	  break;
	}
    }
					  ...
					  GtkFileChooserDialog *dialog)
{
  ...
  __gtk_window_resize (GTK_WINDOW (dialog), default_width, default_height);

  if (!__gtk_widget_get_mapped (GTK_WIDGET (dialog)))
    {
#if 0
      ...
      if (xpos >= 0 && ypos >= 0)
	{
	  __gtk_window_set_position (GTK_WINDOW (dialog), GTK_WIN_POS_NONE);
	  __gtk_window_move (GTK_WINDOW (dialog), xpos, ypos);
	}
#endif
    }
}
				    ...
				    gpointer   data)
{
  ...
  response_id = __gtk_dialog_get_response_for_widget (GTK_DIALOG (dialog), widget);
  if (is_stock_accept_response_id (response_id))
    __gtk_dialog_set_default_response (GTK_DIALOG (dialog), response_id);
}
...
static GtkWidget *
__gtk_file_chooser_dialog_new_valist (const gchar          *title,
				    GtkWindow            *parent,
  ...
  while (button_text)
    {
      ...
      __gtk_dialog_add_button (GTK_DIALOG (result), button_text, response_id);
 ...
 **/
GtkWidget *
__gtk_file_chooser_dialog_new (const gchar         *title,
			     GtkWindow           *parent,
			     GtkFileChooserAction action,
			     const gchar         *first_button_text,
			     ...)
{
  ...
  result = __gtk_file_chooser_dialog_new_valist (title, parent, action,
...
/**
 * __gtk_file_chooser_dialog_new_with_backend:
 * @title: (allow-none): Title of the dialog, or %NULL
 * @parent: (allow-none): Transient parent of the dialog, or %NULL
 * @action: Open or save mode for the dialog
 ...
 * especially useful if you use __gtk_file_chooser_set_local_only() to allow
 ...
 *
 * Since: 2.4
 * Deprecated: 2.14: Use __gtk_file_chooser_dialog_new() instead.
 **/
GtkWidget *
__gtk_file_chooser_dialog_new_with_backend (const gchar          *title,
					  GtkWindow            *parent,
					  GtkFileChooserAction  action,
					  const gchar          *backend,
					  const gchar          *first_button_text,
