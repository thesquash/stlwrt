                ...
                SerializationContext *context)
{
  ...
  do
    {
      ...
      new_tag_list = __gtk_text_iter_get_tags (&iter);
      ...
      for (tmp = added; tmp; tmp = tmp->next)
	{
      ...
      while (TRUE)
	{
	  gunichar ch = __gtk_text_iter_get_char (&iter);

	  if (ch == 0xFFFC)
	    {
	      GdkPixbuf *pixbuf = __gtk_text_iter_get_pixbuf (&iter);

	      if (pixbuf)
		{
		  /* Append the text before the pixbuf */
		  tmp_text = __gtk_text_iter_get_slice (&old_iter, &iter);
	  ...
	  if (__gtk_text_iter_toggles_tag (&iter, NULL))
	    break;
	}

      /* We might have moved too far */
      if (__gtk_text_iter_compare (&iter, &context->end) > 0)
	iter = context->end;
  ...
  while (!__gtk_text_iter_equal (&iter, &context->end));
...
guint8 *
___gtk_text_buffer_serialize_rich_text (GtkTextBuffer     *register_buffer,
	    ...
	    GError             **error)
{
  ...
  if (info->create_tags)
	...
	return g_hash_table_lookup (info->anonymous_tags,
      ...
      if (real_name)
	return __gtk_text_tag_table_lookup (gtk_text_buffer_get_props (info->buffer)->tag_table, real_name);

      /* Next, try the list of defined tags */
      if (g_hash_table_lookup (info->defined_tags, name) != NULL)
	return __gtk_text_tag_table_lookup (gtk_text_buffer_get_props (info->buffer)->tag_table, name);
      ...
      if (!name)
	{
      ...
      tag = __gtk_text_tag_table_lookup (gtk_text_buffer_get_props (info->buffer)->tag_table, name);
	      ...
	      const gchar *tag_name)
{
  ...
  while (__gtk_text_tag_table_lookup (gtk_text_buffer_get_props (info->buffer)->tag_table, name) != NULL)
    {
      g_free (name);
		   ...
		   GError              **error)
{
  ...
  if (ELEMENT_IS ("tag"))
      ...
      if (name)
	{
	  ...
	  info->current_tag = __gtk_text_tag_new (tag_name);
	  g_free (tag_name);
	}
      else
	{
	  info->current_tag = __gtk_text_tag_new (NULL);
		     ...
		     GError              **error)
{
  ...
  switch (peek_state (info))
      ...
      while (list)
	{
	  ...
	  if (info->create_tags)
	    __gtk_text_tag_table_add (gtk_text_buffer_get_props (info->buffer)->tag_table, prio->tag);
    ...
    case STATE_TAG:
      ...
      if (gtk_text_tag_get_props (info->current_tag)->name)
	{
	  /* Add tag to defined tags hash */
	  tmp = g_strdup (gtk_text_tag_get_props (info->current_tag)->name);
	     ...
	     GtkTextIter *iter)
{
  ...
  mark = __gtk_text_buffer_create_mark (info->buffer, "deserialize_insert_point",
  				      &start_iter, TRUE);
  ...
  while (tmp)
    {
      ...
      if (span->text)
	__gtk_text_buffer_insert (info->buffer, iter, span->text, -1);
      else
	{
	  __gtk_text_buffer_insert_pixbuf (info->buffer, iter, span->pixbuf);
	  g_object_unref (span->pixbuf);
	}
      __gtk_text_buffer_get_iter_at_mark (info->buffer, &start_iter, mark);
      ...
      while (tags)
	{
	  ...
	  __gtk_text_buffer_apply_tag (info->buffer, tag,
				     &start_iter, iter);

	  tags = tags->next;
	}

      __gtk_text_buffer_move_mark (info->buffer, mark, iter);

      tmp = tmp->next;
    }

  __gtk_text_buffer_delete_mark (info->buffer, mark);
}
...
gboolean
___gtk_text_buffer_deserialize_rich_text (GtkTextBuffer *register_buffer,
                                        GtkTextBuffer *content_buffer,
                                        GtkTextIter   *iter,
                                        const guint8  *text,
                                        gsize          length,
