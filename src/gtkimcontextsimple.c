 ...
 **/
GtkIMContext *
__gtk_im_context_simple_new (void)
{
...
beep_window (GdkWindow *window)
{
  ...
  __gdk_window_get_user_data (window, (gpointer) &widget);
  ...
  if (GTK_IS_WIDGET (widget))
  ...
  else
    {
      ...
      g_object_get (__gtk_settings_get_for_screen (screen),
                    "gtk-error-bell", &beep,
                    NULL);

      if (beep)
        __gdk_window_beep (window);
    }
}
...
{
  gunichar ch = __gdk_keyval_to_unicode (keyval);
...
{
  GdkKeymap *keymap = __gdk_keymap_get_for_display (__gdk_window_get_display (event->window));
  guint keyval;
  guint *keyvals = NULL;
  gint n_vals = 0;
  gint i;
   ...
   */
  __gdk_keymap_get_entries_for_keycode (keymap,
      ...
      switch (gdk_win32_keymap_check_compose (GDK_WIN32_KEYMAP (__gdk_keymap_get_default ()),
        ...
        case GDK_WIN32_KEYMAP_MATCH_PARTIAL:
          for (i = 0; i < output_size; i++)
            {
              guint32 output_char = __gdk_keyval_to_unicode (output[i]);
              gtk_im_context_simple_commit_char (GTK_IM_CONTEXT (context_simple),
                                                 output_char);
            }
          context_simple->compose_buffer[0] = 0;
...
/**
 * __gtk_im_context_simple_add_table:
 ...
 **/
void
__gtk_im_context_simple_add_table (GtkIMContextSimple *context_simple,
				 guint16            *data,
				 gint                max_seq_len,
				 gint                n_seqs)
{
