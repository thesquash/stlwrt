					     ...
					     gboolean   group_cycling)
{
  ...
  GtkPrinterOptionWidgetPrivate *priv = gtk_printer_option_widget_get_props (powidget)->priv;

  if (priv->check)
    return __gtk_widget_mnemonic_activate (priv->check, group_cycling);
  if (priv->combo)
    return __gtk_widget_mnemonic_activate (priv->combo, group_cycling);
  if (priv->entry)
    return __gtk_widget_mnemonic_activate (priv->entry, group_cycling);
...
combo_box_set_model (GtkWidget *combo_box)
{
  ...
  store = __gtk_list_store_new (N_COLUMNS, G_TYPE_STRING, G_TYPE_STRING);
  __gtk_combo_box_set_model (GTK_COMBO_BOX (combo_box), GTK_TREE_MODEL (store));
...
combo_box_entry_new (void)
{
  ...
  __gtk_combo_box_set_entry_text_column (GTK_COMBO_BOX (combo_box), NAME_COLUMN);
		  ...
		  const gchar *value)
{
  ...
  model = __gtk_combo_box_get_model (GTK_COMBO_BOX (combo));
	...
	gpointer      data)
{
  ...
  __gtk_tree_model_get (model, iter, VALUE_COLUMN, &value, -1);
  found = (strcmp (value, set_data->value) == 0);
  g_free (value);
  
  if (found)
    __gtk_combo_box_set_active_iter (set_data->combo, iter);
	       ...
	       const gchar *value)
{
  ...
  __gtk_tree_model_foreach (model, set_cb, &set_data);
}
...
combo_box_get (GtkWidget *combo, gboolean *custom)
{
  ...
  value = NULL;
  if (__gtk_combo_box_get_active_iter (GTK_COMBO_BOX (combo), &iter))
    {
      __gtk_tree_model_get (model, &iter, VALUE_COLUMN, &value, -1);
      *custom = FALSE;
    }
    ...
    {
      if (__gtk_combo_box_get_has_entry (GTK_COMBO_BOX (combo)))
        {
          value = g_strdup (__gtk_entry_get_text (GTK_ENTRY (__gtk_bin_get_child (GTK_BIN (combo)))));
          *custom = TRUE;
        }

      if (!value || !__gtk_tree_model_get_iter_first (model, &iter))
        return value;
      ...
      do
        {
          gchar *val, *name;
          __gtk_tree_model_get (model, &iter, VALUE_COLUMN, &val,
                                            NAME_COLUMN, &name, -1);
...
deconstruct_widgets (GtkPrinterOptionWidget *widget)
{
  ...
  if (priv->check)
    {
      __gtk_widget_destroy (priv->check);
      priv->check = NULL;
    }
  
  if (priv->combo)
    {
      __gtk_widget_destroy (priv->combo);
      priv->combo = NULL;
    }
  
  if (priv->entry)
    {
      __gtk_widget_destroy (priv->entry);
      priv->entry = NULL;
    }
  ...
  if (priv->filechooser)
    {
      __gtk_widget_destroy (priv->filechooser);
  ...
  if (priv->label)
    {
      __gtk_widget_destroy (priv->label);
      priv->label = NULL;
    }
}
		  ...
		  GtkPrinterOptionWidget *widget)
{
  ...
  gtk_printer_option_set_boolean (priv->source,
				  __gtk_toggle_button_get_active (toggle_button));
                     ...
                     GtkPrinterOptionWidget *widget)
{
  ...
  file = g_filename_from_utf8 (__gtk_entry_get_text (GTK_ENTRY (priv->entry)),
			       -1, NULL, NULL, NULL);
  if (file == NULL)
    return;
  ...
  directory = __gtk_file_chooser_get_filename (GTK_FILE_CHOOSER (priv->combo));
  ...
  if ((g_uri_parse_scheme (file) == NULL) && (directory != NULL))
  ...
  else
    {
      if (g_uri_parse_scheme (file) != NULL)
        ...
        {
          gchar *chooser_uri = __gtk_file_chooser_get_uri (GTK_FILE_CHOOSER (priv->combo));
          if (chooser_uri)
            {
  ...
  if (filtered_val)
    {
	  ...
	  entry = GTK_ENTRY (__gtk_bin_get_child (GTK_BIN (combo)));

          __gtk_entry_set_text (entry, filtered_val);
	}
      value = filtered_val;
    }
		  ...
		  GtkPrinterOptionWidget *widget)
{
  ...
  g_signal_handler_block (priv->source, priv->source_changed_handler);
  value = __gtk_entry_get_text (GTK_ENTRY (entry));
  if (value)
    gtk_printer_option_set (priv->source, value);
  g_signal_handler_unblock (priv->source, priv->source_changed_handler);
  emit_changed (widget);
...
alternative_set (GtkWidget   *box,
		 const gchar *value)
{
  __gtk_container_foreach (GTK_CONTAINER (box), 
		    ...
		    GSList                 *group)
{
  ...
  button = __gtk_radio_button_new_with_label (group, label);
  __gtk_widget_show (button);
  __gtk_box_pack_start (GTK_BOX (box), button, FALSE, FALSE, 0);

  g_object_set_data (G_OBJECT (button), "value", (gpointer)value);
  g_signal_connect (button, "toggled", 
		    G_CALLBACK (radio_changed_cb), widget);

  return __gtk_radio_button_get_group (GTK_RADIO_BUTTON (button));
}
...
construct_widgets (GtkPrinterOptionWidget *widget)
{
  ...
  __gtk_widget_set_sensitive (GTK_WIDGET (widget), TRUE);

  if (source == NULL)
    {
      priv->combo = combo_box_new ();
      combo_box_append (priv->combo,_("Not available"), "None");
      __gtk_combo_box_set_active (GTK_COMBO_BOX (priv->combo), 0);
      __gtk_widget_set_sensitive (GTK_WIDGET (widget), FALSE);
      __gtk_widget_show (priv->combo);
      __gtk_box_pack_start (GTK_BOX (widget), priv->combo, TRUE, TRUE, 0);
    }
  else switch (gtk_printer_option_get_props (source)->type)
    {
    case GTK_PRINTER_OPTION_TYPE_BOOLEAN:
      priv->check = __gtk_check_button_new_with_mnemonic (gtk_printer_option_get_props (source)->display_text);
      g_signal_connect (priv->check, "toggled", G_CALLBACK (check_toggled_cb), widget);
      __gtk_widget_show (priv->check);
      __gtk_box_pack_start (GTK_BOX (widget), priv->check, TRUE, TRUE, 0);
      break;
    ...
    case GTK_PRINTER_OPTION_TYPE_PICKONE_STRING:
      if (gtk_printer_option_get_props (source)->type == GTK_PRINTER_OPTION_TYPE_PICKONE)
        {
          priv->combo = combo_box_new ();
	}
      else
        {
          ...
          if (gtk_printer_option_get_props (source)->type == GTK_PRINTER_OPTION_TYPE_PICKONE_PASSWORD ||
	      gtk_printer_option_get_props (source)->type == GTK_PRINTER_OPTION_TYPE_PICKONE_PASSCODE)
	    {
              GtkEntry *entry;

	      entry = GTK_ENTRY (__gtk_bin_get_child (GTK_BIN (priv->combo)));

              __gtk_entry_set_visibility (entry, FALSE); 
	    }
        }
       

      for (i = 0; i < gtk_printer_option_get_props (source)->num_choices; i++)
	  combo_box_append (priv->combo,
			    gtk_printer_option_get_props (source)->choices_display[i],
			    gtk_printer_option_get_props (source)->choices[i]);
      __gtk_widget_show (priv->combo);
      __gtk_box_pack_start (GTK_BOX (widget), priv->combo, TRUE, TRUE, 0);
      g_signal_connect (priv->combo, "changed", G_CALLBACK (combo_changed_cb), widget);

      text = g_strdup_printf ("%s:", gtk_printer_option_get_props (source)->display_text);
      priv->label = __gtk_label_new_with_mnemonic (text);
      g_free (text);
    ...
    case GTK_PRINTER_OPTION_TYPE_ALTERNATIVE:
      group = NULL;
      priv->box = __gtk_hbox_new (FALSE, 12);
      __gtk_widget_show (priv->box);
      __gtk_box_pack_start (GTK_BOX (widget), priv->box, TRUE, TRUE, 0);
      for (i = 0; i < gtk_printer_option_get_props (source)->num_choices; i++)
	group = alternative_append (priv->box,
      ...
      if (gtk_printer_option_get_props (source)->display_text)
	{
	  text = g_strdup_printf ("%s:", gtk_printer_option_get_props (source)->display_text);
	  priv->label = __gtk_label_new_with_mnemonic (text);
	  g_free (text);
    ...
    case GTK_PRINTER_OPTION_TYPE_STRING:
      priv->entry = __gtk_entry_new ();
      __gtk_entry_set_activates_default (GTK_ENTRY (priv->entry),
                                       gtk_printer_option_get_activates_default (source));
      __gtk_widget_show (priv->entry);
      __gtk_box_pack_start (GTK_BOX (widget), priv->entry, TRUE, TRUE, 0);
    ...
    case GTK_PRINTER_OPTION_TYPE_FILESAVE:
      {
        ...
        priv->filechooser = __gtk_table_new (2, 2, FALSE);
        __gtk_table_set_row_spacings (GTK_TABLE (priv->filechooser), 6);
        __gtk_table_set_col_spacings (GTK_TABLE (priv->filechooser), 12);

        /* TODO: make this a gtkfilechooserentry once we move to GTK */
        priv->entry = __gtk_entry_new ();
        priv->combo = __gtk_file_chooser_button_new (gtk_printer_option_get_props (source)->display_text,
        ...
        label = __gtk_label_new_with_mnemonic (_("_Name:"));
        __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
        __gtk_label_set_mnemonic_widget (GTK_LABEL (label), priv->entry);

        __gtk_table_attach (GTK_TABLE (priv->filechooser), label,
                          0, 1, 0, 1, GTK_FILL, 0,
                          0, 0);

        __gtk_table_attach (GTK_TABLE (priv->filechooser), priv->entry,
                          1, 2, 0, 1, GTK_FILL, 0,
                          0, 0);

        label = __gtk_label_new_with_mnemonic (_("_Save in folder:"));
        __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
        __gtk_label_set_mnemonic_widget (GTK_LABEL (label), priv->combo);

        __gtk_table_attach (GTK_TABLE (priv->filechooser), label,
                          0, 1, 1, 2, GTK_FILL, 0,
                          0, 0);

        __gtk_table_attach (GTK_TABLE (priv->filechooser), priv->combo,
                          1, 2, 1, 2, GTK_FILL, 0,
                          0, 0);

        __gtk_widget_show_all (priv->filechooser);
        __gtk_box_pack_start (GTK_BOX (widget), priv->filechooser, TRUE, TRUE, 0);
  ...
  priv->image = __gtk_image_new_from_stock (GTK_STOCK_DIALOG_WARNING, GTK_ICON_SIZE_MENU);
  __gtk_box_pack_start (GTK_BOX (widget), priv->image, FALSE, FALSE, 0);
}
...
update_widgets (GtkPrinterOptionWidget *widget)
{
  ...
  switch (gtk_printer_option_get_props (source)->type)
    {
    case GTK_PRINTER_OPTION_TYPE_BOOLEAN:
      if (g_ascii_strcasecmp (gtk_printer_option_get_props (source)->value, "True") == 0)
	__gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->check), TRUE);
      else
	__gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->check), FALSE);
      break;
    case GTK_PRINTER_OPTION_TYPE_PICKONE:
      combo_box_set (priv->combo, gtk_printer_option_get_props (source)->value);
      break;
    case GTK_PRINTER_OPTION_TYPE_ALTERNATIVE:
      alternative_set (priv->box, gtk_printer_option_get_props (source)->value);
      break;
    case GTK_PRINTER_OPTION_TYPE_STRING:
      __gtk_entry_set_text (GTK_ENTRY (priv->entry), gtk_printer_option_get_props (source)->value);
      break;
    case GTK_PRINTER_OPTION_TYPE_PICKONE_PASSWORD:
    case GTK_PRINTER_OPTION_TYPE_PICKONE_PASSCODE:
    case GTK_PRINTER_OPTION_TYPE_PICKONE_REAL:
    ...
    case GTK_PRINTER_OPTION_TYPE_FILESAVE:
      {
        gchar *filename = g_filename_from_uri (gtk_printer_option_get_props (source)->value, NULL, NULL);
        if (filename != NULL)
          {
            ...
            if (g_path_is_absolute (dirname))
              __gtk_file_chooser_set_current_folder (GTK_FILE_CHOOSER (priv->combo),
                                                   dirname);
            g_free (text);
            g_free (basename);
            g_free (dirname);
  ...
  if (gtk_printer_option_get_props (source)->has_conflict)
...
{
  return gtk_printer_option_widget_get_props (widget)->priv->label != NULL;
}

GtkWidget *
gtk_printer_option_widget_get_external_label (GtkPrinterOptionWidget  *widget)
{
  return gtk_printer_option_widget_get_props (widget)->priv->label;
}
