 ...
 *        Any parameters understood by g_test_init() or __gtk_init() are
 *        stripped before return.
 * @Varargs: currently unused
 *
 * This function is used to initialize a GTK+ test program.
 *
 * It will in turn call g_test_init() and __gtk_init() to properly
 * initialize the testing framework and graphical toolkit. It'll 
 * also set the program's locale to "C" and prevent loading of rc 
 * files and Gtk+ modules. This is done to make tets program
 * environments as deterministic as possible.
 *
 * Like __gtk_init() and g_test_init(), any known arguments will be
                                ...
                                gboolean   input_only)
{
  ...
  GSList *matches = NULL;
  gpointer udata;
  __gdk_window_get_user_data (widget->window, &udata);
  ...
  children = __gdk_window_get_children (__gtk_widget_get_parent_window (widget));
  for (node = children; node; node = node->next)
    {
      __gdk_window_get_user_data (node->data, &udata);
 ...
 * location, see __gdk_test_simulate_key() for details.
 *
                          ...
                          GdkModifierType modifiers)
{
  ...
  if (!iwindows)
    return FALSE;
  k1res = __gdk_test_simulate_key (iwindows->data, -1, -1, keyval, modifiers, GDK_KEY_PRESS);
  k2res = __gdk_test_simulate_key (iwindows->data, -1, -1, keyval, modifiers, GDK_KEY_RELEASE);
 ...
 * location, see __gdk_test_simulate_button() for details.
 *
                       ...
                       GdkModifierType modifiers)
{
  ...
  if (!iwindows)
    return FALSE;
  b1res = __gdk_test_simulate_button (iwindows->data, -1, -1, button, modifiers, GDK_BUTTON_PRESS);
  b2res = __gdk_test_simulate_button (iwindows->data, -1, -1, button, modifiers, GDK_BUTTON_RELEASE);
  g_slist_free (iwindows);
  return b1res && b2res;
}
  ...
  if (spinner->panel)
    {
      ...
      __gdk_drawable_get_size (spinner->panel, &width, &height);
      pos = upwards ? 0 : height - 1;
      b1res = __gdk_test_simulate_button (spinner->panel, width - 1, pos, button, 0, GDK_BUTTON_PRESS);
      b2res = __gdk_test_simulate_button (spinner->panel, width - 1, pos, button, 0, GDK_BUTTON_RELEASE);
    }
  return b1res && b2res;
}
                     ...
                     const gchar  *label_pattern)
{
  if (GTK_IS_LABEL (widget))
    {
      const gchar *text = __gtk_label_get_text (GTK_LABEL (widget));
      if (g_pattern_match_simple (label_pattern, text))
        return widget;
    }
  if (GTK_IS_CONTAINER (widget))
    {
      GList *node, *list = __gtk_container_get_children (GTK_CONTAINER (widget));
                 ...
                 GtkWidget *base)
{
  ...
  if (!__gtk_widget_translate_coordinates (a, base, 0, 0, &ax0, &ay0) ||
      !__gtk_widget_translate_coordinates (a, base, a->allocation.width, a->allocation.height, &ax1, &ay1))
    return -G_MAXINT;
  if (!__gtk_widget_translate_coordinates (b, base, 0, 0, &bx0, &by0) ||
      !__gtk_widget_translate_coordinates (b, base, b->allocation.width, b->allocation.height, &bx1, &by1))
    return +G_MAXINT;
  if (bx0 >= ax1)
    xdist = bx0 - ax1;
  else if (ax0 >= bx1)
                       ...
                       GType      widget_type)
{
  ...
  data[0] = __gtk_widget_get_toplevel (base_widget);
  data[1] = base_widget;
                      ...
                      GType         widget_type)
{
  ...
  if (!label)
    label = gtk_test_find_label (__gtk_widget_get_toplevel (widget), label_pattern);
  if (label)
                          ...
                          double          percentage)
{
  ...
  if (GTK_IS_RANGE (widget))
    adjustment = __gtk_range_get_adjustment (GTK_RANGE (widget));
  else if (GTK_IS_SPIN_BUTTON (widget))
    adjustment = __gtk_spin_button_get_adjustment (GTK_SPIN_BUTTON (widget));
  if (adjustment)
    __gtk_adjustment_set_value (adjustment, adjustment->lower + (adjustment->upper - adjustment->lower - adjustment->page_size) * percentage * 0.01);
}
...
{
  if (GTK_IS_LABEL (widget))
    __gtk_label_set_text (GTK_LABEL (widget), string);
  else if (GTK_IS_EDITABLE (widget))
    {
      int pos = 0;
      __gtk_editable_delete_text (GTK_EDITABLE (widget), 0, -1);
      __gtk_editable_insert_text (GTK_EDITABLE (widget), string, -1, &pos);
    }
  else if (GTK_IS_TEXT_VIEW (widget))
    {
      GtkTextBuffer *tbuffer = __gtk_text_view_get_buffer (GTK_TEXT_VIEW (widget));
      __gtk_text_buffer_set_text (tbuffer, string, -1);
    }
}
...
{
  if (GTK_IS_LABEL (widget))
    return g_strdup (__gtk_label_get_text (GTK_LABEL (widget)));
  else if (GTK_IS_EDITABLE (widget))
    {
      return g_strdup (__gtk_editable_get_chars (GTK_EDITABLE (widget), 0, -1));
    }
  else if (GTK_IS_TEXT_VIEW (widget))
    {
      GtkTextBuffer *tbuffer = __gtk_text_view_get_buffer (GTK_TEXT_VIEW (widget));
      GtkTextIter start, end;
      __gtk_text_buffer_get_start_iter (tbuffer, &start);
      __gtk_text_buffer_get_end_iter (tbuffer, &end);
      return __gtk_text_buffer_get_text (tbuffer, &start, &end, FALSE);
    }
  return NULL;
}
  ...
  if (widget)
    {
      ...
      g_test_queue_destroy ((GDestroyNotify) __gtk_widget_destroy, widget);
    }
  return widget;
}
 ...
 * The window will be automatically shown with __gtk_widget_show_now() after
 * creation, so when this function returns it has already been mapped,
 * resized and positioned on screen.
 * The window will quit any running __gtk_main()-loop when destroyed, and it
