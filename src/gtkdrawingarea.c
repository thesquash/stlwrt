...
STLWRT_DEFINE_FTYPE_VPARENT (GtkDrawingArea, gtk_drawing_area, GTK_TYPE_WIDGET,
                             G_TYPE_FLAG_NONE, ;)
...
{
  gtk_drawing_area_get_props (darea)->draw_data = NULL;
}
		       ...
		       gint            height)
{
  ...
  gtk_widget_get_props (GTK_WIDGET (darea))->requisition.width = width;
  gtk_widget_get_props (GTK_WIDGET (darea))->requisition.height = height;
...
gtk_drawing_area_realize (GtkWidget *widget)
{
  ...
  if (!__gtk_widget_get_has_window (widget))
  ...
  else
    {
      ...
      __gdk_window_set_user_data (gtk_widget_get_props (widget)->window, darea);
...
gtk_drawing_area_send_configure (GtkDrawingArea *darea)
{
  ...
  event->configure.window = g_object_ref (gtk_widget_get_props (widget)->window);
  event->configure.send_event = TRUE;
  event->configure.x = gtk_widget_get_props (widget)->allocation.x;
  event->configure.y = gtk_widget_get_props (widget)->allocation.y;
  event->configure.width = gtk_widget_get_props (widget)->allocation.width;
  event->configure.height = gtk_widget_get_props (widget)->allocation.height;
