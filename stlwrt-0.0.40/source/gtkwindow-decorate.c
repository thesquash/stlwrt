...
gtk_decorated_window_init (GtkWindow   *window)
{
  ...
  __gtk_window_set_has_frame (window, TRUE);
...
gtk_decorated_window_calculate_frame_size (GtkWindow *window)
{
  ...
  if (__gdk_window_get_decorations (GTK_WIDGET (window)->window,
				  &decorations))
    {
				    ...
				    gpointer user_data)
{
  ...
  __gdk_window_get_position (window->frame, x, y);
...
gtk_decorated_window_realize (GtkWindow   *window)
{
  ...
  deco->title_layout = __gtk_widget_create_pango_layout (widget,
  ...
  __gdk_window_shape_combine_mask (window->frame, GDK_FB_USE_CHILD_SHAPE, 0, 0);
#endif
}
				   ...
				   GdkEventFocus     *event)
{
  ...
  __gdk_window_invalidate_rect (window->frame, NULL, FALSE);
  return FALSE;
}
				    ...
				    GdkEventMotion  *event)
{
  ...
  win = widget->window;
  __gdk_window_get_pointer (window->frame, &x, &y, &mask);
  
  __gdk_window_get_position (window->frame, &win_x, &win_y);
  win_x += DECORATION_BORDER_LEFT;
  win_y += DECORATION_BORDER_TOP;
  
  __gdk_window_get_geometry (win, NULL, NULL, &win_w, &win_h, NULL);
  ...
  if (deco->moving)
    {
      ...
      ______gtk_window_reposition (window, win_x + dx, win_y + dy);
    }
  ...
  if (deco->resize != RESIZE_NONE)
	...
	break;
      ...
      if ((w > 0) && (h > 0))
	{
	  ______gtk_window_constrain_size (window, w,h, &w, &h);
	  
	  if ((w != win_w) || (h != win_h))
	    __gdk_window_resize (widget->window, w, h);
	}
    }
  ...
  if (deco->closing)
  ...
  else if (deco->maximizing)
    {
      ...
      if (type == GTK_WINDOW_REGION_MAXIMIZE)
        {
	  if (deco->maximized)
	    __gtk_window_unmaximize (window);
	  else
	    __gtk_window_maximize (window);
	}
    }
				   ...
				   GdkEventWindowState *event)
{
  ...
  if (event->changed_mask & GDK_WINDOW_STATE_MAXIMIZED)
	...
	{
	  int w, h;
	  __gdk_window_get_geometry (widget->window, NULL, NULL,
				   &deco->last_w, &deco->last_h, NULL);
	  __gdk_window_get_origin (widget->window, &deco->last_x, &deco->last_y);
	  w = __gdk_screen_get_width(__gdk_screen_get_default()) - DECORATION_BORDER_TOT_X;
	  h = __gdk_screen_get_height(__gdk_screen_get_default()) - DECORATION_BORDER_TOT_Y;
	  ______gtk_window_constrain_size (window, w, h, &w, &h);
	  if (w != deco->last_w || h != deco->last_h)
	    {
	      ______gtk_window_reposition (window, DECORATION_BORDER_LEFT, DECORATION_BORDER_TOP);
	      __gdk_window_resize (widget->window, w, h);
	      deco->maximized = TRUE;
	    }
	}
	...
	{
	  ______gtk_window_reposition (window, deco->last_x, deco->last_y);
	  ______gtk_window_constrain_size (window, deco->last_w, deco->last_h,
				      &deco->last_w, &deco->last_h);
	  __gdk_window_resize (widget->window, deco->last_w, deco->last_h);
	  deco->maximized = FALSE;
	}
    }
  return TRUE;
  ...
  if (deco->decorated)
    {
      ...
      frame = window->frame;
      __gdk_drawable_get_size (frame, &width, &height);

      /* Top */
      __gtk_paint_flat_box (widget->style, frame, GTK_STATE_NORMAL,
	...
	border_state = GTK_STATE_PRELIGHT;
      ...
      __gtk_paint_box (widget->style, frame, border_state, 
      ...
      if (deco->maximizable)
	{
	  ...
	  if (area)
	    __gdk_gc_set_clip_rectangle (widget->style->bg_gc[widget->state], area);

	  __gdk_draw_rectangle (frame, widget->style->bg_gc[widget->state], TRUE,
			      x1, y1, x2 - x1, y2 - y1);

	  __gdk_draw_line (frame, widget->style->black_gc, x1 + 1, y1 + 1, x2 - 2, y1 + 1);

	  __gdk_draw_rectangle (frame, widget->style->black_gc, FALSE,
			      x1 + 1, y1 + 2,
			      DECORATION_BUTTON_SIZE - 3, DECORATION_BUTTON_SIZE - 4);

	  if (area)
	    __gdk_gc_set_clip_rectangle (widget->style->black_gc, NULL);
	}
	...
	__gdk_gc_set_clip_rectangle (widget->style->bg_gc[widget->state], area);
      ...
      if (area)
	__gdk_gc_set_clip_rectangle (widget->style->bg_gc[widget->state], NULL);
      
      if (area)
	__gdk_gc_set_clip_rectangle (widget->style->black_gc, area);

      __gdk_draw_line (frame, widget->style->black_gc, x1, y1, x2-1, y2-1);

      __gdk_draw_line (frame, widget->style->black_gc, x1, y2-1, x2-1, y1);
	...
	{
	  if (area)
	    __gdk_gc_set_clip_rectangle (widget->style->fg_gc [border_state], area);

	  __gdk_draw_layout (frame,
			   widget->style->fg_gc [border_state],
			   DECORATION_BORDER_LEFT, 1,
			   deco->title_layout);
	  if (area)
	    __gdk_gc_set_clip_rectangle (widget->style->fg_gc [border_state], NULL);
	}
...
gtk_decorated_window_move_resize_window (GtkWindow   *window,
					 ...
					 gint         height)
{
  __gdk_window_move_resize (GTK_WIDGET (window)->window,
			  x, y, width, height);
}
#endif
