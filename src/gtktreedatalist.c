				  ...
				  gpointer      user_data)
{
  ...
  GType type = __gtk_tree_model_get_column_type (model, column);
  GValue a_value = {0, };
  GValue b_value = {0, };
  gint retval;
  const gchar *stra, *strb;

  __gtk_tree_model_get_value (model, a, column, &a_value);
  __gtk_tree_model_get_value (model, b, column, &b_value);
