...
#include <gdk.h>		/* For __gdk_rectangle_union() */
#include <gdkpixmap.h>
 ...
 * regular __gdk_window_begin_paint_region()  happen on a window of this impl window
...
static void __gdk_window_clear_backing_region (GdkWindow *window,
					     GdkRegion *region);
static void gdk_window_redirect_free      (GdkWindowRedirect *redirect);
static void apply_redirect_to_children    (GdkWindow   *private,
					   GdkWindowRedirect *redirect);
static void remove_redirect_from_children (GdkWindow   *private,
					   GdkWindowRedirect *redirect);

static void recompute_visible_regions   (GdkWindow *private,
					 gboolean recalculate_siblings,
					 gboolean recalculate_children);
static void __gdk_window_flush_outstanding_moves (GdkWindow *window);
static void __gdk_window_flush_recursive  (GdkWindow *window);
static void do_move_region_bits_on_impl (GdkWindow *private,
...
static void __gdk_window_invalidate_region_full (GdkWindow       *window,
					       const GdkRegion *region,
					       gboolean         invalidate_children,
					       ClearBg          clear_bg);
static void __gdk_window_invalidate_rect_full (GdkWindow          *window,
...
gdk_window_class_init (GdkWindowClass *klass)
{
   ...
   * GdkWindow:cursor:
   *
   * The mouse pointer for a #GdkWindow. See __gdk_window_set_cursor() and
   * __gdk_window_get_cursor() for details.
   *
   * Since: 2.18
   */
...
gdk_window_finalize (GObject *object)
{
  ...
  GdkWindow *obj = (GdkWindow *) object;
  ...
  if (obj->shape)
    __gdk_region_destroy (obj->shape);

  if (obj->input_shape)
    __gdk_region_destroy (obj->input_shape);

  if (obj->cursor)
    __gdk_cursor_unref (obj->cursor);
                         ...
                         GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CURSOR:
      __gdk_window_set_cursor (window, g_value_get_boxed (value));
      break;
                         ...
                         GParamSpec *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_CURSOR:
      g_value_set_boxed (value, __gdk_window_get_cursor (window));
      break;
...
{
  return (GdkWindow *)gdk_window_get_impl_window ((GdkWindow *)window);
}
...
{
  return gdk_window_has_impl ((GdkWindow *)window);
}
		   ...
		   GdkRegion *region)
{
  ...
  for (l = private->children; l; l = l->next)
	...
	continue;
      ...
      if (__gdk_region_rect_in (region, &r) == GDK_OVERLAP_RECTANGLE_OUT)
	continue;

      child_region = __gdk_region_rectangle (&r);

      if (child->shape)
	{
	  /* Adjust shape region to parent window coords */
	  __gdk_region_offset (child->shape, child->x, child->y);
	  __gdk_region_intersect (child_region, child->shape);
	  __gdk_region_offset (child->shape, -child->x, -child->y);
	}
      else if (private->window_type == GDK_WINDOW_FOREIGN)
	{
	  ...
	  if (shape)
	    {
	      __gdk_region_intersect (child_region, shape);
	      __gdk_region_destroy (shape);
	    }
	}
      ...
      if (for_input)
	{
	  if (child->input_shape)
	    __gdk_region_intersect (child_region, child->input_shape);
	  else if (private->window_type == GDK_WINDOW_FOREIGN)
	    {
      ...
      __gdk_region_subtract (region, child_region);
      __gdk_region_destroy (child_region);

    }
}

static GdkVisibilityState
effective_visibility (GdkWindow *private)
{
  GdkVisibilityState native;

  if (!__gdk_window_is_viewable ((GdkWindow *)private))
...
static void
gdk_window_update_visibility (GdkWindow *private)
{
...
static void
gdk_window_update_visibility_recursively (GdkWindow *private,
...
static gboolean
should_apply_clip_as_shape (GdkWindow *private)
{
  return
...
static void
apply_clip_as_shape (GdkWindow *private)
{
  GdkRectangle r;
  ...
  if (!__gdk_region_rect_equal (private->clip_region, &r))
				    ...
				    gboolean recalculate_children)
{
  ...
  if (recalculate_clip)
      ...
      if (private->viewable)
	{
	  ...
	  r.width = private->width;
	  r.height = private->height;
	  new_clip = __gdk_region_rectangle (&r);

	  if (!gdk_window_is_toplevel (private))
	    {
	      __gdk_region_intersect (new_clip, private->parent->clip_region);
	  ...
	  __gdk_region_offset (new_clip, -private->x, -private->y);

	  if (private->shape)
	    __gdk_region_intersect (new_clip, private->shape);
	}
      else
	new_clip = __gdk_region_new ();

      if (private->clip_region == NULL ||
	  !__gdk_region_equal (private->clip_region, new_clip))
	clip_region_changed = TRUE;

      if (private->clip_region)
	__gdk_region_destroy (private->clip_region);
      private->clip_region = new_clip;

      old_clip_region_with_children = private->clip_region_with_children;
      private->clip_region_with_children = __gdk_region_copy (private->clip_region);
      if (private->window_type != GDK_WINDOW_ROOT)
	remove_child_area (private, NULL, FALSE, private->clip_region_with_children);

      if (clip_region_changed ||
	  !__gdk_region_equal (private->clip_region_with_children, old_clip_region_with_children))
	  private->clip_tag = new_region_tag ();

      if (old_clip_region_with_children)
	__gdk_region_destroy (old_clip_region_with_children);
    }
  ...
  if (clip_region_changed)
    {
      ...
      if (__gdk_region_empty (private->clip_region))
	visibility = GDK_VISIBILITY_FULLY_OBSCURED;
      else
        {
          if (private->shape)
            {
	      fully_visible = __gdk_region_equal (private->clip_region,
	                                        private->shape);
            }
          else
            {
	      ...
	      r.width = private->width;
	      r.height = private->height;
	      fully_visible = __gdk_region_rect_equal (private->clip_region, &r);
	    }
...
{
  recompute_visible_regions ((GdkWindow *)window, TRUE, FALSE);
}
 ...
 **/
GdkWindow*
__gdk_window_new (GdkWindow     *parent,
		...
		gint           attributes_mask)
{
  ...
  if (!parent)
    {
      GDK_NOTE (MULTIHEAD,
		g_warning ("__gdk_window_new(): no parent specified reverting to parent = default root window"));
  ...
  if (GDK_WINDOW_DESTROYED (parent))
    {
      g_warning ("__gdk_window_new(): parent is destroyed\n");
      return NULL;
    }
  ...
  if (attributes_mask & GDK_WA_VISUAL)
    ...
    visual = __gdk_screen_get_system_visual (screen);
  ...
  if (private->parent->window_type == GDK_WINDOW_ROOT)
    native = TRUE; /* Always use native windows for toplevels */
	   ...
	   ((attributes_mask & GDK_WA_COLORMAP &&
	     attributes->colormap != __gdk_drawable_get_colormap ((GdkDrawable *)private->parent)) ||
	    (attributes_mask & GDK_WA_VISUAL &&
	     attributes->visual != __gdk_drawable_get_visual ((GdkDrawable *)private->parent))))
  ...
  __gdk_window_set_cursor (window, ((attributes_mask & GDK_WA_CURSOR) ?
				  (attributes->cursor) :
				  NULL));
  ...
  for (l = g_list_last (private->children); l != NULL; l = l->prev)
      ...
      else
	{
	  show = impl_iface->reparent ((GdkWindow *)child,
				       ...
				       child->x, child->y);
	  if (show)
	    __gdk_window_show_unraised ((GdkWindow *)child);
	}
    }
}
 ...
 **/
void
__gdk_window_reparent (GdkWindow *window,
		     GdkWindow *new_parent,
		     gint       x,
		     gint       y)
{
  GdkWindow *private;
  GdkWindow *new_parent_private;
  ...
  screen = __gdk_drawable_get_screen (GDK_DRAWABLE (window));
  if (!new_parent)
    new_parent = __gdk_screen_get_root_window (screen);

  private = (GdkWindow *) window;
  new_parent_private = (GdkWindow *)new_parent;
  ...
  if (window->extension_events != 0)
    {
      ...
      __gdk_input_set_extension_events ((GdkWindow *)window, 0,
  ...
  if (mask != 0)
    {
	 ...
	 change the mask. */
      __gdk_input_set_extension_events ((GdkWindow *)window, mask,
 ...
 */
gboolean
__gdk_window_ensure_native (GdkWindow *window)
{
  GdkWindow *private;
  GdkWindow *impl_window;
  ...
  screen = __gdk_drawable_get_screen ((GdkDrawable *)window);
  visual = __gdk_drawable_get_visual ((GdkDrawable *)window);
  ...
  attributes.colormap = __gdk_drawable_get_colormap ((GdkDrawable *)window);
  ...
  if (__gdk_window_is_viewable (window))
...
{
  GdkWindow *obj = (GdkWindow*) window;
  while (obj->filters)
 ...
 *
 * Internal function to destroy a window. Like __gdk_window_destroy(),
 * but does not drop the reference count created by __gdk_window_new().
 **/
static void
			       ...
			       gboolean   foreign_destroy)
{
  ...
  display = __gdk_drawable_get_display (GDK_DRAWABLE (window));
  screen = __gdk_drawable_get_screen (GDK_DRAWABLE (window));
  ...
  switch (private->window_type)
      ...
      else
	{
	  if (private->parent)
	    {
	      GdkWindow *parent_private = (GdkWindow *)private->parent;
	  ...
	  if (private->window_type == GDK_WINDOW_FOREIGN)
	      ...
	      while (tmp)
		{
		  ...
		  temp_private = (GdkWindow*) temp_window;
		  if (temp_private)
	  ...
	  __gdk_drawable_set_colormap (GDK_DRAWABLE (window), NULL);
	  ...
	  if (private->clip_region_with_children)
	    {
	      __gdk_region_destroy (private->clip_region_with_children);
 ...
 **/
void
__gdk_window_destroy (GdkWindow *window)
{
 ...
 **/
void
__gdk_window_set_user_data (GdkWindow *window,
			  gpointer   user_data)
{
  g_return_if_fail (GDK_IS_WINDOW (window));

  ((GdkWindow*)window)->user_data = user_data;
}

/**
 * __gdk_window_get_user_data:
 * @window: a #GdkWindow
 * @data: (out): return location for user data
 *
 * Retrieves the user data for @window, which is normally the widget
 * that @window belongs to. See __gdk_window_set_user_data().
 *
 **/
void
__gdk_window_get_user_data (GdkWindow *window,
			  gpointer  *data)
{
  g_return_if_fail (GDK_IS_WINDOW (window));

  *data = ((GdkWindow*)window)->user_data;
}

/**
 * __gdk_window_get_window_type:
 * @window: a #GdkWindow
 *
 ...
 **/
GdkWindowType
__gdk_window_get_window_type (GdkWindow *window)
{
 ...
 **/
gboolean
__gdk_window_is_destroyed (GdkWindow *window)
{
...
/**
 * __gdk_window_has_native:
 * @window: a #GdkWindow
 *
 * Checks whether the window has a native window or not. Note that
 * you can use __gdk_window_ensure_native() if a native window is needed.
 *
 * Returns: %TRUE if the %window has a native window, %FALSE otherwise.
 *
 * Since: 2.22
 */
gboolean
__gdk_window_has_native (GdkWindow *window)
{
  GdkWindow *w;
 ...
 * __gdk_window_get_geometry() which queries the X server for the
 ...
 **/
void
__gdk_window_get_position (GdkWindow *window,
			 gint      *x,
			 gint      *y)
{
  GdkWindow *obj;
...
/**
 * __gdk_window_get_parent:
 * @window: a #GdkWindow
 *
 * Obtains the parent of @window, as known to GDK. Does not query the
 * X server; thus this returns the parent as passed to __gdk_window_new(),
 * not the actual parent. This should never matter unless you're using
 * Xlib calls mixed with GDK calls on the X11 platform. It may also
 * matter for toplevel windows, because the window manager may choose
 * to reparent them.
 *
 * Note that you should use __gdk_window_get_effective_parent() when
 * writing generic code that walks up a window hierarchy, because
 * __gdk_window_get_parent() will most likely not do what you expect if
 * there are offscreen windows in the hierarchy.
 *
 * Return value: parent of @window
 **/
GdkWindow*
__gdk_window_get_parent (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);

  return (GdkWindow*) ((GdkWindow*) window)->parent;
}

/**
 * __gdk_window_get_effective_parent:
 * @window: a #GdkWindow
 *
 * Obtains the parent of @window, as known to GDK. Works like
 * __gdk_window_get_parent() for normal windows, but returns the
 * window's embedder for offscreen windows.
 *
 * See also: __gdk_offscreen_window_get_embedder()
 *
 ...
 **/
GdkWindow *
__gdk_window_get_effective_parent (GdkWindow *window)
{
  GdkWindow *obj;
  ...
  if (gdk_window_is_offscreen (obj))
    return __gdk_offscreen_window_get_embedder (window);
  else
    return (GdkWindow *) obj->parent;
}

/**
 * __gdk_window_get_toplevel:
 * @window: a #GdkWindow
 *
 ...
 *
 * Note that you should use __gdk_window_get_effective_toplevel() when
 * you want to get to a window's toplevel as seen on screen, because
 * __gdk_window_get_toplevel() will most likely not do what you expect
 * if there are offscreen windows in the hierarchy.
 *
 * Return value: the toplevel window containing @window
 **/
GdkWindow *
__gdk_window_get_toplevel (GdkWindow *window)
{
  GdkWindow *obj;
...
/**
 * __gdk_window_get_effective_toplevel:
 * @window: a #GdkWindow
 *
 * Gets the toplevel window that's an ancestor of @window.
 *
 * Works like __gdk_window_get_toplevel(), but treats an offscreen window's
 * embedder as its parent, using __gdk_window_get_effective_parent().
 *
 * See also: __gdk_offscreen_window_get_embedder()
 *
 ...
 **/
GdkWindow *
__gdk_window_get_effective_toplevel (GdkWindow *window)
{
  GdkWindow *parent;

  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);

  while ((parent = __gdk_window_get_effective_parent (window)) != NULL &&
	 (__gdk_window_get_window_type (parent) != GDK_WINDOW_ROOT))
    window = parent;
 ...
 **/
GList*
__gdk_window_get_children (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);
  ...
  return g_list_copy (GDK_WINDOW (window)->children);
}

/**
 * __gdk_window_peek_children:
 * @window: a #GdkWindow
 *
 * Like __gdk_window_get_children(), but does not copy the list of
 ...
 **/
GList *
__gdk_window_peek_children (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);
  ...
  return GDK_WINDOW (window)->children;
}

/**
 * __gdk_window_add_filter:
 * @window: a #GdkWindow
 ...
 *
 * See __gdk_display_add_client_message_filter() if you are interested
 * in X ClientMessage events.
 **/
void
__gdk_window_add_filter (GdkWindow     *window,
...
/**
 * __gdk_window_remove_filter:
 * @window: a #GdkWindow
 * @function: previously-added filter function
 * @data: user data for previously-added filter function
 *
 * Remove a filter previously added with __gdk_window_add_filter().
 *
 **/
void
__gdk_window_remove_filter (GdkWindow     *window,
			  GdkFilterFunc  function,
			  gpointer       data)
{
  GdkWindow *private;
 ...
 **/
GList *
__gdk_screen_get_toplevel_windows (GdkScreen *screen)
{
  ...
  tmp_list = ((GdkWindow *)root_window)->children;
  while (tmp_list)
    {
...
/**
 * __gdk_window_get_toplevels:
 *
 * Obtains a list of all toplevel windows known to GDK on the default
 * screen (see __gdk_screen_get_toplevel_windows()).
 * A toplevel window is a child of the root window (see
 * __gdk_get_default_root_window()).
 *
 * The returned list should be freed with g_list_free(), but
 ...
 *
 * Deprecated: 2.16: Use __gdk_screen_get_toplevel_windows() instead.
 */
GList *
__gdk_window_get_toplevels (void)
{
  return __gdk_screen_get_toplevel_windows (__gdk_screen_get_default ());
}

/**
 * __gdk_window_is_visible:
 * @window: a #GdkWindow
 *
 * Checks whether the window has been mapped (with __gdk_window_show() or
 * __gdk_window_show_unraised()).
 *
 * Return value: %TRUE if the window is mapped
 **/
gboolean
__gdk_window_is_visible (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), FALSE);
 ...
 **/
gboolean
__gdk_window_is_viewable (GdkWindow *window)
{
 ...
 **/
GdkWindowState
__gdk_window_get_state (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
...
gdk_window_begin_implicit_paint (GdkWindow *window, GdkRectangle *rect)
{
  ...
  paint->region = __gdk_region_new (); /* Empty */
...
static void
__gdk_window_flush_implicit_paint (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindow *impl_window;
  GdkWindowPaint *paint;
  ...
  paint->flushed = TRUE;
  region = __gdk_region_copy (private->clip_region_with_children);
  ...
  for (list = private->paint_stack; list != NULL; list = list->next)
    {
      ...
      __gdk_region_subtract (region, tmp_paint->region);
    }

  __gdk_region_offset (region, private->abs_x, private->abs_y);
  __gdk_region_intersect (region, paint->region);

  if (!GDK_WINDOW_DESTROYED (window) && !__gdk_region_empty (region))
    {
      /* Remove flushed region from the implicit paint */
      __gdk_region_subtract (paint->region, region);

      /* Some regions are valid, push these to window now */
      tmp_gc = _gdk_drawable_get_scratch_gc ((GdkDrawable *)window, FALSE);
      _gdk_gc_set_clip_region_internal (tmp_gc, region, TRUE);
      __gdk_draw_drawable (private->impl, tmp_gc, paint->pixmap,
			 0, 0, paint->x_offset, paint->y_offset, -1, -1);
      /* Reset clip region of the cached GdkGC */
      __gdk_gc_set_clip_region (tmp_gc, NULL);
    }
  else
...
gdk_window_end_implicit_paint (GdkWindow *window)
{
  ...
  if (!GDK_WINDOW_DESTROYED (window) && !__gdk_region_empty (paint->region))
    {
      /* Some regions are valid, push these to window now */
      tmp_gc = _gdk_drawable_get_scratch_gc ((GdkDrawable *)window, FALSE);
    ...
    __gdk_region_destroy (paint->region);
...
/**
 * __gdk_window_begin_paint_rect:
 * @window: a #GdkWindow
 * @rectangle: rectangle you intend to draw to
 *
 * A convenience wrapper around __gdk_window_begin_paint_region() which
 * creates a rectangular region for you. See
 * __gdk_window_begin_paint_region() for details.
 *
 **/
void
__gdk_window_begin_paint_rect (GdkWindow          *window,
			     const GdkRectangle *rectangle)
{
  ...
  region = __gdk_region_rectangle (rectangle);
  __gdk_window_begin_paint_region (window, region);
  __gdk_region_destroy (region);
}

/**
 * __gdk_window_begin_paint_region:
 * @window: a #GdkWindow
 ...
 * backing store.  When you call __gdk_window_end_paint(), the backing
 ...
 * __gdk_window_end_paint(). If you draw to @window directly without
 * calling __gdk_window_begin_paint_region(), the user may see flicker
 * as individual drawing operations are performed in sequence.  The
 * clipping and background-initializing features of
 * __gdk_window_begin_paint_region() are conveniences for the
 * programmer, so you can avoid doing that work yourself.
 *
 * When using GTK+, the widget system automatically places calls to
 * __gdk_window_begin_paint_region() and __gdk_window_end_paint() around
 * emissions of the expose_event signal. That is, if you're writing an
 * expose event handler, you can assume that the exposed area in
 * #GdkEventExpose has already been cleared to the window background,
 * is already set as the clip region, and already has a backing store.
 * Therefore in most cases, application code need not call
 * __gdk_window_begin_paint_region(). (You can disable the automatic
 * calls around expose events on a widget-by-widget basis by calling
 * __gtk_widget_set_double_buffered().)
 *
 * If you call this function multiple times before calling the
 * matching __gdk_window_end_paint(), the backing stores are pushed onto
 * a stack. __gdk_window_end_paint() copies the topmost backing store
 * onscreen, subtracts the topmost region from all other regions in
 * the stack, and pops the stack. All drawing operations affect only
 * the topmost backing store in the stack. One matching call to
 * __gdk_window_end_paint() is required for each call to
 * __gdk_window_begin_paint_region().
 *
 **/
void
__gdk_window_begin_paint_region (GdkWindow       *window,
			       const GdkRegion *region)
{
  ...
  paint = g_new (GdkWindowPaint, 1);
  paint->region = __gdk_region_copy (region);
  paint->region_tag = new_region_tag ();

  __gdk_region_intersect (paint->region, private->clip_region_with_children);
  __gdk_region_get_clipbox (paint->region, &clip_box);

  /* Convert to impl coords */
  __gdk_region_offset (paint->region, private->abs_x, private->abs_y);

  /* Mark the region as valid on the implicit paint */

  if (implicit_paint)
    __gdk_region_union (implicit_paint->region, paint->region);
  ...
  for (list = private->paint_stack; list != NULL; list = list->next)
    {
      ...
      __gdk_region_subtract (tmp_paint->region, paint->region);
    }

  private->paint_stack = g_slist_prepend (private->paint_stack, paint);

  if (!__gdk_region_empty (paint->region))
    {
      __gdk_window_clear_backing_region (window,
				       paint->region);
    }
		     ...
		     int            *y_offset_out)
{
  ...
  tmpreg = __gdk_region_rectangle (&dest_rect);
  __gdk_region_intersect (visible_region, tmpreg);
  __gdk_region_destroy (tmpreg);

  /* Compensate for the dest pos */
  x_offset += private->redirect->dest_x;
  y_offset += private->redirect->dest_y;

  __gdk_gc_set_clip_region (gc, visible_region); /* This resets clip origin! */

  /* offset clip and tiles from window coords to pixmaps coords */
  __gdk_gc_offset (gc, -x_offset, -y_offset);

  __gdk_region_destroy (visible_region);
...
/**
 * __gdk_window_end_paint:
 * @window: a #GdkWindow
 *
 * Indicates that the backing store created by the most recent call to
 * __gdk_window_begin_paint_region() should be copied onscreen and
 * deleted, leaving the next-most-recent backing store or no backing
 * store at all as the active paint region. See
 * __gdk_window_begin_paint_region() for full details. It is an error to
 * call this function without a matching
 * __gdk_window_begin_paint_region() first.
 *
 **/
void
__gdk_window_end_paint (GdkWindow *window)
{
#ifdef USE_BACKING_STORE
  GdkWindow *private = (GdkWindow *)window;
  ...
  if (private->paint_stack == NULL)
    {
      g_warning (G_STRLOC": no preceding call to __gdk_window_begin_paint_region(), see documentation");
      return;
    }
  ...
  if (!paint->uses_implicit)
    {
      __gdk_window_flush_outstanding_moves (window);

      full_clip = __gdk_region_copy (private->clip_region_with_children);
      __gdk_region_intersect (full_clip, paint->region);
      _gdk_gc_set_clip_region_internal (tmp_gc, full_clip, TRUE); /* Takes ownership of full_clip */
      __gdk_gc_set_clip_origin (tmp_gc, - x_offset, - y_offset);
      __gdk_draw_drawable (private->impl, tmp_gc, paint->pixmap,
  ...
  if (private->redirect)
    {
      ...
      __gdk_draw_drawable (private->redirect->pixmap, tmp_gc, paint->pixmap,
			 clip_box.x - paint->x_offset,
			 clip_box.y - paint->y_offset,
			 clip_box.x + x_offset,
			 clip_box.y + y_offset,
  ...
  for (composited = private;
       ...
       composited = composited->parent)
    {
      ...
      __gdk_drawable_get_size (GDK_DRAWABLE (composited->parent),
			     &width, &height);
	...
	{
	  __gdk_window_invalidate_rect (GDK_WINDOW (composited->parent),
				      &clip_box, FALSE);
	  break;
	}
    }
			     ...
			     int dx, int dy)
{
  ...
  while (!gdk_window_is_toplevel (private) &&
         !private->composited &&
         __gdk_drawable_get_visual ((GdkDrawable *) private) == __gdk_drawable_get_visual ((GdkDrawable *) private->parent))
    {
  ...
  __gdk_region_offset (dest_region, -dx, -dy); /* Move to source region */
  impl_iface = GDK_WINDOW_IMPL_GET_IFACE (private->impl);
  ...
  __gdk_draw_drawable ((GdkDrawable *)impl_window->impl,
		     tmp_gc,
		     private->impl,
			    ...
			    int dx, int dy)
{
  ...
  move->dest_region  = __gdk_region_copy (region);
  move->dx = dx;
  move->dy = dy;
...
{
  __gdk_region_destroy (move->dest_region);
		    ...
		    int dx, int dy)
{
  ...
  if (__gdk_region_empty (new_dest_region))
    return;
  ...
  new_total_region = __gdk_region_copy (new_dest_region);
  __gdk_region_offset (new_total_region, -dx, -dy);
  __gdk_region_union (new_total_region, new_dest_region);
  ...
  for (l = g_list_last (impl_window->outstanding_moves); l != NULL; l = prev)
    {
      ...
      old_total_region = __gdk_region_copy (old_move->dest_region);
      __gdk_region_offset (old_total_region, -old_move->dx, -old_move->dy);
      __gdk_region_union (old_total_region, old_move->dest_region);

      __gdk_region_intersect (old_total_region, new_total_region);
      /* If these regions intersect then its not safe to push the
	 new region before the old one */
      if (!__gdk_region_empty (old_total_region))
	{
	  /* The area where the new moves source overlaps the old ones
	     destination */
	  source_overlaps_destination = __gdk_region_copy (new_dest_region);
	  __gdk_region_offset (source_overlaps_destination, -dx, -dy);
	  __gdk_region_intersect (source_overlaps_destination, old_move->dest_region);
	  __gdk_region_offset (source_overlaps_destination, dx, dy);
	  ...
	  /* We can do all sort of optimizations here, but to do things safely it becomes
	     ...
	     copy. */
	  if (__gdk_region_equal (source_overlaps_destination, new_dest_region))
	    {
		 ...
		 by the destination of the new one */
	      ...
	      non_overwritten = __gdk_region_copy (old_move->dest_region);
	      __gdk_region_subtract (non_overwritten, new_dest_region);
	      /* move to source region */
	      __gdk_region_offset (non_overwritten, -old_move->dx, -old_move->dy);

	      __gdk_region_intersect (non_overwritten, new_dest_region);
	      if (__gdk_region_empty (non_overwritten))
		{
		  added_move = TRUE;
		  ...
		  impl_window->outstanding_moves =
		    g_list_insert_before (impl_window->outstanding_moves,
					  l, move);
		  __gdk_region_subtract (old_move->dest_region, new_dest_region);
		}
	      __gdk_region_destroy (non_overwritten);
	    }

	  __gdk_region_destroy (source_overlaps_destination);
	  __gdk_region_destroy (old_total_region);
	  break;
	}
      __gdk_region_destroy (old_total_region);
    }

  __gdk_region_destroy (new_total_region);
  ...
  if (impl_window->update_area)
    {
      ...
      update_area = __gdk_region_copy (region);

      /* Convert from target to source */
      __gdk_region_offset (update_area, -dx, -dy);
      __gdk_region_intersect (update_area, impl_window->update_area);
	 ...
	 area. However, it would just use waste cycles. */
      ...
      /* Convert back */
      __gdk_region_offset (update_area, dx, dy);
      __gdk_region_union (impl_window->update_area, update_area);

      /* This area of the destination is now invalid,
	 so no need to copy to it.  */
      __gdk_region_subtract (region, update_area);

      __gdk_region_destroy (update_area);
    }
  ...
  if (impl_window->implicit_paint)
    {
      ...
      exposing = __gdk_region_copy (implicit_paint->region);
      __gdk_region_intersect (exposing, region);
      __gdk_region_subtract (region, exposing);

      impl_window_add_update_area (impl_window, exposing);
      __gdk_region_destroy (exposing);
    }
 ...
 */
static void
__gdk_window_flush_outstanding_moves (GdkWindow *window)
{
  GdkWindow *private;
  GdkWindow *impl_window;
  GdkWindowRegionMove *move;
 ...
 **/
void
__gdk_window_flush (GdkWindow *window)
{
  __gdk_window_flush_outstanding_moves (window);
  __gdk_window_flush_implicit_paint (window);
}
 ...
 */
static void
__gdk_window_flush_if_exposing (GdkWindow *window)
{
  GdkWindow *private;
  GdkWindow *impl_window;
  ...
  if (impl_window->implicit_paint)
    __gdk_window_flush (window);
}


static void
__gdk_window_flush_recursive_helper (GdkWindow *window,
				   GdkWindow *impl)
{
  GdkWindow *child;
  GList *l;
  ...
  for (l = window->children; l != NULL; l = l->next)
    {
      ...
      if (child->impl == impl)
	/* Same impl, ignore */
	__gdk_window_flush_recursive_helper (child, impl);
      else
	__gdk_window_flush_recursive (child);
    }
}

static void
__gdk_window_flush_recursive (GdkWindow *window)
{
  __gdk_window_flush ((GdkWindow *)window);
  __gdk_window_flush_recursive_helper (window, window->impl);
}
...
/**
 * __gdk_window_get_internal_paint_info:
 * @window: a #GdkWindow
 ...
 * pixmap as the result of a __gdk_window_begin_paint_region() calls.
 ...
 **/
void
__gdk_window_get_internal_paint_info (GdkWindow    *window,
...
start_draw_helper (GdkDrawable *drawable,
		   ...
		   gint *y_offset_out)
{
  GdkWindow *private = (GdkWindow *)drawable;
  gint x_offset, y_offset;
  GdkDrawable *impl;
  ...
  if (x_offset != 0 || y_offset != 0)
    {
      __gdk_gc_set_clip_origin (gc,
			      old_clip_x - x_offset,
			      old_clip_y - y_offset);
      __gdk_gc_set_ts_origin (gc,
  ...
  *y_offset_out = y_offset;
  ...
  if (paint)
  ...
  else
    {
	 ...
	 guarantee ordering. */
      __gdk_window_flush ((GdkWindow *)drawable);
...
#define END_DRAW					    \
    if (x_offset != 0 || y_offset != 0)			    \
     {                                                      \
       __gdk_gc_set_clip_origin (gc, old_clip_x, old_clip_y); \
       __gdk_gc_set_ts_origin (gc, old_ts_x, old_ts_y);       \
     }                                                      \
  }
		      ...
		      GdkGCValuesMask  mask)
{
  ...
  return __gdk_gc_new_with_values (((GdkWindow *) drawable)->impl,
				 values, mask);
}
			   ...
			   gint         height)
{
  ...
  BEGIN_DRAW;
  __gdk_draw_rectangle (impl, gc, filled,
			 ...
			 gint         npoints)
{
  ...
  __gdk_draw_polygon (impl, gc, filled, new_points, npoints);
			 ...
			 gint            text_length)
{
  ...
  BEGIN_DRAW;
  __gdk_draw_text_wc (impl, font, gc,
		    x - x_offset, y - y_offset, text, text_length);
  END_DRAW;
}
  ...
  for (list = private->paint_stack; list != NULL; list = list->next)
    {
      ...
      overlap = __gdk_region_rect_in (paint->region, &rect);
  ...
  if (implicit_paint)
    {
      ...
      overlap = __gdk_region_rect_in (implicit_paint->region, &rect);
      if (overlap == GDK_OVERLAP_RECTANGLE_IN)
	{
...
gdk_window_get_clip_region (GdkDrawable *drawable)
{
  ...
  result = __gdk_region_copy (private->clip_region);

  if (private->paint_stack)
    {
      GdkRegion *paint_region = __gdk_region_new ();
      GSList *tmp_list = private->paint_stack;
      ...
      while (tmp_list)
	{
	  ...
	  __gdk_region_union (paint_region, paint->region);

	  tmp_list = tmp_list->next;
	}

      __gdk_region_intersect (result, paint_region);
      __gdk_region_destroy (paint_region);
    }
...
gdk_window_get_visible_region (GdkDrawable *drawable)
{
  ...
  return __gdk_region_copy (private->clip_region);
}
			  ...
			  GdkDrawable *original_src)
{
  ...
  if (!private->paint_stack)
	  ...
	  GDK_IS_WINDOW (original_src))
	{
	  ...
	  r.width = width;
	  r.height = height;
	  exposure_region = __gdk_region_rectangle (&r);

	  if (_gdk_gc_get_subwindow (gc) == GDK_CLIP_BY_CHILDREN)
	    clip = private->clip_region_with_children;
	  else
	    clip = private->clip_region;
	  __gdk_region_intersect (exposure_region, clip);

	  _gdk_gc_remove_drawable_clip (gc);
	  clip = _gdk_gc_get_clip_region (gc);
	  if (clip)
	    {
	      __gdk_region_offset (exposure_region,
				 old_clip_x,
				 old_clip_y);
	      __gdk_region_intersect (exposure_region, clip);
	      __gdk_region_offset (exposure_region,
	  ...
	  clip = __gdk_drawable_get_visible_region (original_src);
	  __gdk_region_offset (clip,
			     xdest - (xsrc - GDK_WINDOW (original_src)->abs_x),
			     ydest - (ysrc - GDK_WINDOW (original_src)->abs_y));
	  __gdk_region_subtract (exposure_region, clip);
	  __gdk_region_destroy (clip);

	  __gdk_window_invalidate_region_full (GDK_WINDOW (private),
					      exposure_region,
					      _gdk_gc_get_subwindow (gc) == GDK_INCLUDE_INFERIORS,
					      CLEAR_BG_ALL);

	  __gdk_region_destroy (exposure_region);
	}
    }
			...
			gint         npoints)
{
  ...
  __gdk_draw_points (impl, gc, new_points, npoints);
			  ...
			  gint         nsegs)
{
  ...
  __gdk_draw_segments (impl, gc, new_segs, nsegs);
		       ...
		       gint         npoints)
{
  ...
  __gdk_draw_lines (impl, gc, new_points, npoints);
			...
			PangoGlyphString *glyphs)
{
  ...
  BEGIN_DRAW;
  __gdk_draw_glyphs (impl, gc, font,
				    ...
				    PangoGlyphString *glyphs)
{
  ...
  __gdk_draw_glyphs_transformed (impl, gc, matrix, font, x, y, glyphs);
...
#  define GDK_CAIRO_REPEAT_IS_FAST 1
  ...
  else
    {
      ...
      __gdk_cairo_set_source_color (method->cr, &private->bg_color);
    }
}

static void
__gdk_window_clear_backing_region (GdkWindow *window,
				 GdkRegion *region)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindowPaint *paint = private->paint_stack->data;
  ...
  clip = __gdk_region_copy (paint->region);
  __gdk_region_intersect (clip, region);
  __gdk_region_get_clipbox (clip, &clipbox);
  ...
  if (method.cr)
    {
      ...
      __gdk_cairo_region (method.cr, clip);
  ...
  else
    {
      ...
      __gdk_gc_set_clip_region (method.gc, clip);
      __gdk_draw_rectangle ((GdkDrawable *)window, method.gc, TRUE,
			  clipbox.x, clipbox.y,
...
static void
__gdk_window_clear_backing_region_redirect (GdkWindow *window,
					  GdkRegion *region)
{
  GdkWindow *private = (GdkWindow *)window;
  ...
  clip_region = _gdk_window_calculate_full_clip_region (window,
							...
							TRUE,
							&x_offset, &y_offset);
  __gdk_region_intersect (clip_region, region);
  ...
  __gdk_region_offset (clip_region, x_offset, y_offset);
  ...
  if (method.cr)
    {
      ...
      __gdk_cairo_region (method.cr, clip_region);
      cairo_fill (method.cr);
  ...
  else
    {
      ...
      __gdk_region_get_clipbox (clip_region, &clipbox);
      __gdk_gc_set_clip_region (method.gc, clip_region);
      __gdk_draw_rectangle (redirect->pixmap, method.gc, TRUE,
			  clipbox.x, clipbox.y,
			  clipbox.width, clipbox.height);
      g_object_unref (method.gc);
  ...
  __gdk_region_destroy (clip_region);
  cairo_surface_destroy (paint.surface);
}

static void
__gdk_window_clear_backing_region_direct (GdkWindow *window,
					GdkRegion *region)
{
  GdkWindow *private = (GdkWindow *)window;
  BackingRectMethod method;
  ...
  paint.y_offset = 0;
  paint.pixmap = window;
  paint.surface = _gdk_drawable_ref_cairo_surface ((GdkDrawable *)window);

  method.cr = NULL;
  method.gc = NULL;
  setup_backing_rect_method (&method, window, &paint, 0, 0);

  clip = __gdk_region_copy (private->clip_region_with_children);
  __gdk_region_intersect (clip, region);
  __gdk_region_get_clipbox (clip, &clipbox);
 ...
 **/
void
__gdk_window_clear (GdkWindow *window)
{
  gint width, height;

  g_return_if_fail (GDK_IS_WINDOW (window));

  __gdk_drawable_get_size (GDK_DRAWABLE (window), &width, &height);

  __gdk_window_clear_area (window, 0, 0,
			 width, height);
}
...
static gboolean
clears_as_native (GdkWindow *private)
{
  GdkWindow *next;
...
static void
__gdk_window_clear_region_internal (GdkWindow *window,
				  GdkRegion *region,
				  gboolean   send_expose)
{
  ...
  if (private->paint_stack)
    __gdk_window_clear_backing_region (window, region);
  else
    {
      if (private->redirect)
	__gdk_window_clear_backing_region_redirect (window, region);
	...
	{
	  GdkRegion *copy;
	  copy = __gdk_region_copy (region);
	  __gdk_region_intersect (copy,
	  ...
	  __gdk_region_destroy (copy);
	}
      else
	{
	  __gdk_window_clear_backing_region_direct (window, region);
	  if (send_expose)
	    __gdk_window_invalidate_region (window, region, FALSE);
	}
    }
}

static void
__gdk_window_clear_area_internal (GdkWindow *window,
				gint       x,
				gint       y,
				gint       width,
				gint       height,
				gboolean   send_expose)
{
  ...
  __gdk_window_clear_region_internal (window,
				    region,
				    send_expose);
  __gdk_region_destroy (region);
 ...
 **/
void
__gdk_window_clear_area (GdkWindow *window,
		       gint       x,
		       gint       y,
		       gint       width,
		       gint       height)
{
  __gdk_window_clear_area_internal (window,
				  x, y,
				  width, height,
				  FALSE);
}
 ...
 * @height: height of rectangle to clear
 *
 * Like __gdk_window_clear_area(), but also generates an expose event for
 * the cleared area.
 *
 ...
 **/
void
__gdk_window_clear_area_e (GdkWindow *window,
			 gint       x,
			 gint       y,
			 gint       width,
			 gint       height)
		       ...
		       gint         height)
{
  ...
  BEGIN_DRAW;
  __gdk_draw_image (impl, gc, image, xsrc, ysrc,
			    ...
			    gint           n_trapezoids)
{
  ...
  __gdk_draw_trapezoids (impl, gc, trapezoids, n_trapezoids);
...
gdk_window_real_get_visual (GdkDrawable *drawable)
{
  ...
  colormap = __gdk_drawable_get_colormap (drawable);
  return colormap ? __gdk_colormap_get_visual (colormap) : NULL;
}
...
gdk_window_real_get_depth (GdkDrawable *drawable)
{
  ...
  return ((GdkWindow *)GDK_WINDOW (drawable))->depth;
}

static GdkScreen*
gdk_window_real_get_screen (GdkDrawable *drawable)
{
  return __gdk_drawable_get_screen (GDK_WINDOW (drawable)->impl);
}
			      ...
			      GdkColormap *cmap)
{
  ...
  private = (GdkWindow *)drawable;

  /* different colormap than parent, requires native window */
  if (!private->input_only &&
      cmap != __gdk_drawable_get_colormap ((GdkDrawable *)(private->parent)))
    __gdk_window_ensure_native ((GdkWindow *)drawable);

  __gdk_drawable_set_colormap (private->impl, cmap);
}
...
gdk_window_real_get_colormap (GdkDrawable *drawable)
{
  ...
  return __gdk_drawable_get_colormap (((GdkWindow*)drawable)->impl);
}
			  ...
			  gint             height)
{
  ...
  return __gdk_drawable_copy_to_image (private->impl,
				     image,
				     src_x - x_offset,
				     src_y - y_offset,
				     dest_x, dest_y,
...
static void
gdk_window_drop_cairo_surface (GdkWindow *private)
{
  if (private->cairo_surface)
    {
...
{
  GdkWindow *private = (GdkWindow*) data;
...
gdk_window_create_cairo_surface (GdkDrawable *drawable,
				 ...
				 int height)
{
  return _gdk_windowing_create_cairo_surface (GDK_WINDOW(drawable)->impl,
					      width, height);
}
  ...
  if (!private->paint_stack)
    {
      ...
      cairo_new_path (cr);
      __gdk_cairo_region (cr, private->clip_region_with_children);
      ...
      if (paint->uses_implicit)
	{
	  ...
	  cairo_new_path (cr);
	  __gdk_cairo_region (cr, paint->region);
	  cairo_restore (cr);
			...
			GdkWindow *ancestor)
{
  while (window)
    {
      GdkWindow *parent = (GdkWindow*) ((GdkWindow*) window)->parent;
...
gdk_window_add_update_window (GdkWindow *window)
{
   ...
   *  It could be added during execution of __gtk_widget_destroy() when
  ...
  for (tmp = update_windows; tmp; tmp = tmp->next)
    {
      GdkWindow *parent = GDK_WINDOW (window)->parent;
...
gdk_window_is_toplevel_frozen (GdkWindow *window)
{
  ...
  toplevel = (GdkWindow *)__gdk_window_get_toplevel (window);
...
{
  if (window &&
      (GDK_WINDOW (window)->update_freeze_count ||
       gdk_window_is_toplevel_frozen (window)))
    return;

  if (!update_idle)
    update_idle =
      __gdk_threads_add_idle_full (GDK_PRIORITY_REDRAW,
				 gdk_window_update_idle,
				 NULL, NULL);
}

void
___gdk_window_process_updates_recurse (GdkWindow *window,
				     GdkRegion *expose_region)
{
  ...
  if (__gdk_region_empty (expose_region))
    return;
  ...
  for (l = children; l != NULL; l = l->next)
      ...
      if (child->impl == private->impl)
	{
	  ...
	  __gdk_region_intersect (child_region, expose_region);
	  __gdk_region_subtract (expose_region, child_region);
	  __gdk_region_offset (child_region, -child->x, -child->y);
	  ___gdk_window_process_updates_recurse ((GdkWindow *)child, child_region);
	}
      else
	{
  ...
  if (!__gdk_region_empty (expose_region) &&
      ...
      if (private->event_mask & GDK_EXPOSURE_MASK)
	{
	  ...
	  __gdk_region_get_clipbox (expose_region, &event.expose.area);
	       ...
	       private->window_type != GDK_WINDOW_FOREIGN)
	{
	  ...
	  __gdk_window_begin_paint_region (window, expose_region);
	  __gdk_window_clear_region_internal (window, expose_region, FALSE);
 ...
 */
static void
__gdk_window_process_updates_internal (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindowImplIface *impl_iface;
   ...
   * __gdk_window_flush_outstanding_moves(). However, we
  ...
  while (private->update_area &&
	 iteration++ < 2)
    {
      ...
      if (_gdk_event_func && __gdk_window_is_viewable (window))
	{
	  GdkRegion *expose_region;
	  gboolean end_implicit;

	  /* Clip to part visible in toplevel */
	  __gdk_region_intersect (update_area, private->clip_region);

	  if (debug_updates)
	    {
	      /* Make sure we see the red invalid area before redrawing. */
	      __gdk_display_sync (__gdk_drawable_get_display ((GdkDrawable *)window));
	      g_usleep (70000);
	    }
	  ...
	  if (private->outstanding_moves)
	    {
	      ...
	      remove = __gdk_region_copy (update_area);
	      ...
	      for (l = g_list_last (private->outstanding_moves); l != NULL; l = prev)
		{
		  ...
		  __gdk_region_subtract (move->dest_region, remove);

		  /* However if any of the destination we do need has a source
		     in the updated region we do need that as a destination for
		     the earlier moves */
		  __gdk_region_offset (move->dest_region, -move->dx, -move->dy);
		  __gdk_region_subtract (remove, move->dest_region);

		  if (__gdk_region_empty (move->dest_region))
		    {
		      gdk_window_region_move_free (move);
	   ...
	   * begin/end_paint pair. That will be lead to a __gdk_window_flush
	  ...
	  __gdk_region_get_clipbox (update_area, &clip_box);
	  end_implicit = gdk_window_begin_implicit_paint (window, &clip_box);
	  expose_region = __gdk_region_copy (update_area);
	  if (!end_implicit)
	    {
	  ...
	  __gdk_region_destroy (expose_region);
	}
      if (!save_region)
	__gdk_region_destroy (update_area);
    }
...
{
  GSList *displays = __gdk_display_manager_list_displays (__gdk_display_manager_get ());
  GSList *tmp_list;

  for (tmp_list = displays; tmp_list; tmp_list = tmp_list->next)
    __gdk_display_flush (tmp_list->data);

  g_slist_free (displays);
}

/* Currently it is not possible to override
 * __gdk_window_process_all_updates in the same manner as
 * __gdk_window_process_updates and __gdk_window_invalidate_maybe_recurse
 ...
 * NULL. __gdk_window_process_all_updates can then walk the list of open
 ...
 * __gdk_window_process_all_updates:
 *
 * Calls __gdk_window_process_updates() for all windows (see #GdkWindow)
 * in the application.
 *
 **/
void
__gdk_window_process_all_updates (void)
{
  ...
  while (tmp_list)
    {
      GdkWindow *private = (GdkWindow *)tmp_list->data;
      ...
      if (!GDK_WINDOW_DESTROYED (tmp_list->data))
	{
	  if (private->update_freeze_count ||
	    ...
	    __gdk_window_process_updates_internal (tmp_list->data);
	}
...
/**
 * __gdk_window_process_updates:
 * @window: a #GdkWindow
 * @update_children: whether to also process updates for child windows
 *
 * Sends one or more expose events to @window. The areas in each
 * expose event will cover the entire update area for the window (see
 * __gdk_window_invalidate_region() for details). Normally GDK calls
 * __gdk_window_process_all_updates() on your behalf, so there's no
 ...
 **/
void
__gdk_window_process_updates (GdkWindow *window,
			    gboolean   update_children)
{
  ...
  if ((impl_window->update_area ||
    ...
    {
      __gdk_window_process_updates_internal ((GdkWindow *)impl_window);
  ...
  if (update_children)
	...
	{
	  __gdk_window_process_updates (node->data, TRUE);
...
static void
__gdk_window_invalidate_rect_full (GdkWindow          *window,
				  const GdkRectangle *rect,
				  gboolean            invalidate_children,
				  ClearBg             clear_bg)
{
  ...
  if (!rect)
    {
      window_rect.x = 0;
      window_rect.y = 0;
      __gdk_drawable_get_size (GDK_DRAWABLE (window),
			     &window_rect.width,
			     &window_rect.height);
      rect = &window_rect;
    }
  ...
  region = __gdk_region_rectangle (rect);
  __gdk_window_invalidate_region_full (window, region, invalidate_children, clear_bg);
  __gdk_region_destroy (region);
}

/**
 * __gdk_window_invalidate_rect:
 * @window: a #GdkWindow
 * @rect: (allow-none): rectangle to invalidate or %NULL to invalidate the whole
 *      window
 * @invalidate_children: whether to also invalidate child windows
 *
 * A convenience wrapper around __gdk_window_invalidate_region() which
 * invalidates a rectangular region. See
 * __gdk_window_invalidate_region() for details.
 **/
void
__gdk_window_invalidate_rect (GdkWindow          *window,
			    const GdkRectangle *rect,
			    gboolean            invalidate_children)
{
  __gdk_window_invalidate_rect_full (window, rect, invalidate_children, CLEAR_BG_NONE);
}
		 ...
		 const GdkRegion *region)
{
  ...
  __gdk_region_get_clipbox (region, &clipbox);

  __gdk_draw_rectangle ((GdkDrawable *)window,
		      ugly_gc,
		      TRUE,
		      clipbox.x, clipbox.y,
		      clipbox.width, clipbox.height);
...
{
  if (impl_window->update_area)
    __gdk_region_union (impl_window->update_area, region);
  else
    {
      gdk_window_add_update_window ((GdkWindow *)impl_window);
      impl_window->update_area = __gdk_region_copy (region);
 ...
 */
static void
__gdk_window_invalidate_maybe_recurse_full (GdkWindow       *window,
					  const GdkRegion *region,
					  ClearBg          clear_bg,
					  ...
					  gpointer   user_data)
{
  ...
  visible_region = __gdk_drawable_get_visible_region ((GdkDrawable *)window);
  __gdk_region_intersect (visible_region, region);
  ...
  while (tmp_list)
      ...
      if (!child->input_only)
	{
	  ...
	  child_region = __gdk_region_rectangle (&child_rect);

	  /* remove child area from the invalid area of the parent */
	  if (GDK_WINDOW_IS_MAPPED (child) && !child->shaped &&
	      !child->composited &&
	      !gdk_window_is_offscreen (child))
	    __gdk_region_subtract (visible_region, child_region);

	  if (child_func && (*child_func) ((GdkWindow *)child, user_data))
	    {
	      GdkRegion *tmp = __gdk_region_copy (region);

	      __gdk_region_offset (tmp, - child_rect.x, - child_rect.y);
	      __gdk_region_offset (child_region, - child_rect.x, - child_rect.y);
	      __gdk_region_intersect (child_region, tmp);

	      __gdk_window_invalidate_maybe_recurse_full ((GdkWindow *)child,
							child_region, clear_bg, child_func, user_data);

	      __gdk_region_destroy (tmp);
	    }
  ...
  if (!__gdk_region_empty (visible_region)  ||
	...
	draw_ugly_color (window, region);
      ...
      /* Convert to impl coords */
      __gdk_region_offset (visible_region, private->abs_x, private->abs_y);
...
/**
 * __gdk_window_invalidate_maybe_recurse:
 * @window: a #GdkWindow
 * @region: a #GdkRegion
 ...
 * __gdk_window_process_updates() sends one or more expose events to the
 * window, which together cover the entire update area. An
 * application would normally redraw the contents of @window in
 * response to those expose events.
 *
 * GDK will call __gdk_window_process_all_updates() on your behalf
 ...
 **/
void
__gdk_window_invalidate_maybe_recurse (GdkWindow       *window,
				     const GdkRegion *region,
				     gboolean       (*child_func) (GdkWindow *,
								   gpointer),
				     gpointer   user_data)
{
  __gdk_window_invalidate_maybe_recurse_full (window, region, CLEAR_BG_NONE,
...
static void
__gdk_window_invalidate_region_full (GdkWindow       *window,
				    const GdkRegion *region,
				    gboolean         invalidate_children,
				    ClearBg          clear_bg)
{
  __gdk_window_invalidate_maybe_recurse_full (window, region, clear_bg,
					    invalidate_children ?
...
/**
 * __gdk_window_invalidate_region:
 * @window: a #GdkWindow
 * @region: a #GdkRegion
 ...
 * unaffected. See __gdk_window_invalidate_maybe_recurse if you need
 * fine grained control over which children are invalidated.
 **/
void
__gdk_window_invalidate_region (GdkWindow       *window,
			      const GdkRegion *region,
			      gboolean         invalidate_children)
{
  __gdk_window_invalidate_maybe_recurse (window, region,
				       invalidate_children ?
					 true_predicate : (gboolean (*) (GdkWindow *, gpointer))NULL,
				       NULL);
}
  ...
  for (l = private->outstanding_moves; l != NULL; l = l->next)
    {
      ...
      move_region = __gdk_region_copy (move->dest_region);
      __gdk_region_offset (move_region, -move->dx, -move->dy);

      /* Move area of region that intersects with move source
	 by dx, dy of the move*/
      __gdk_region_intersect (move_region, region);
      __gdk_region_subtract (region, move_region);
      __gdk_region_offset (move_region, move->dx, move->dy);
      __gdk_region_union (region, move_region);

      __gdk_region_destroy (move_region);
    }

  __gdk_window_invalidate_maybe_recurse_full (window, region, CLEAR_BG_WINCLEARED,
 ...
 * __gdk_window_get_update_area() returns %NULL. You are responsible for
 * calling __gdk_region_destroy() on the returned region if it's non-%NULL.
 *
 * Return value: the update area for @window
 **/
GdkRegion *
__gdk_window_get_update_area (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindow *impl_window;
  ...
  if (impl_window->update_area)
    {
      tmp_region = __gdk_region_copy (private->clip_region_with_children);
      /* Convert to impl coords */
      __gdk_region_offset (tmp_region, private->abs_x, private->abs_y);
      __gdk_region_intersect (tmp_region, impl_window->update_area);

      if (__gdk_region_empty (tmp_region))
	{
	  __gdk_region_destroy (tmp_region);
	  return NULL;
	}
      else
	{
	  __gdk_region_subtract (impl_window->update_area, tmp_region);

	  if (__gdk_region_empty (impl_window->update_area) &&
	      impl_window->outstanding_moves == NULL)
	    {
	      __gdk_region_destroy (impl_window->update_area);
	      impl_window->update_area = NULL;
  ...
  if (private->update_area)
    {
      ...
      __gdk_region_destroy (private->update_area);
      private->update_area = NULL;
    }
}

/**
 * __gdk_window_freeze_updates:
 * @window: a #GdkWindow
 *
 * Temporarily freezes a window such that it won't receive expose
 * events.  The window will begin receiving expose events again when
 * __gdk_window_thaw_updates() is called. If __gdk_window_freeze_updates()
 * has been called more than once, __gdk_window_thaw_updates() must be called
 * an equal number of times to begin processing exposes.
 **/
void
__gdk_window_freeze_updates (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindow *impl_window;
...
/**
 * __gdk_window_thaw_updates:
 * @window: a #GdkWindow
 *
 * Thaws a window frozen with __gdk_window_freeze_updates().
 **/
void
__gdk_window_thaw_updates (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindow *impl_window;
...
/**
 * __gdk_window_freeze_toplevel_updates_libgtk_only:
 * @window: a #GdkWindow
 *
 * Temporarily freezes a window and all its descendants such that it won't
 * receive expose events.  The window will begin receiving expose events
 * again when __gdk_window_thaw_toplevel_updates_libgtk_only() is called. If
 * __gdk_window_freeze_toplevel_updates_libgtk_only()
 * has been called more than once,
 * __gdk_window_thaw_toplevel_updates_libgtk_only() must be called
 * an equal number of times to begin processing exposes.
 *
 ...
 **/
void
__gdk_window_freeze_toplevel_updates_libgtk_only (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
...
/**
 * __gdk_window_thaw_toplevel_updates_libgtk_only:
 * @window: a #GdkWindow
 *
 ...
 **/
void
__gdk_window_thaw_toplevel_updates_libgtk_only (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
...
/**
 * __gdk_window_set_debug_updates:
 * @setting: %TRUE to turn on update debugging
 *
 * With update debugging enabled, calls to
 * __gdk_window_invalidate_region() clear the invalidated region of the
 * screen to a noticeable color, and GDK pauses for a short time
 * before sending exposes to windows during
 * __gdk_window_process_updates().  The net effect is that you can see
 ...
 * usually more useful than calling __gdk_window_set_debug_updates()
 * yourself, though you might want to use this function to enable
 * updates sometime after application startup time.
 *
 **/
void
__gdk_window_set_debug_updates (gboolean setting)
{
  debug_updates = setting;
}

/**
 * __gdk_window_constrain_size:
 ...
 */
void
__gdk_window_constrain_size (GdkGeometry *geometry,
			   guint        flags,
			   gint         width,
			   gint         height,
			   gint        *new_width,
 ...
 * __gdk_window_at_pointer()), or %NULL if the window containing the
 * pointer isn't known to GDK
 **/
GdkWindow*
__gdk_window_get_pointer (GdkWindow	  *window,
			gint		  *x,
			gint		  *y,
			GdkModifierType   *mask)
{
  ...
  if (window)
  ...
  else
    {
      ...
      display = __gdk_screen_get_display (screen);
      window = __gdk_screen_get_root_window (screen);

      GDK_NOTE (MULTIHEAD,
		g_message ("Passing NULL for window to __gdk_window_get_pointer()\n"
 ...
 * for it with __gdk_window_foreign_new())
 *
 * NOTE: For multihead-aware widgets or applications use
 * __gdk_display_get_window_at_pointer() instead.
 *
 * Return value: (transfer none): window under the mouse pointer
 **/
GdkWindow*
__gdk_window_at_pointer (gint *win_x,
		       gint *win_y)
{
  return __gdk_display_get_window_at_pointer (__gdk_display_get_default (), win_x, win_y);
}
 ...
 **/
GdkWindow *
__gdk_get_default_root_window (void)
{
  return __gdk_screen_get_root_window (__gdk_screen_get_default ());
}

/**
 * __gdk_window_foreign_new:
 ...
 **/
GdkWindow *
__gdk_window_foreign_new (GdkNativeWindow anid)
{
  return __gdk_window_foreign_new_for_display (__gdk_display_get_default (), anid);
}

static void
get_all_native_children (GdkWindow *private,
			 GList **native)
{
  GdkWindow *child;
  GList *l;
...
static inline void
__gdk_window_raise_internal (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
	      ...
	      gboolean val)
{
  ...
  if (!_gdk_native_windows &&
      ...
      !gdk_window_is_toplevel (w))
    {
       ...
       * except when explicitly done via __gdk_window_show/hide, as this may
...
static void
__gdk_window_show_internal (GdkWindow *window, gboolean raise)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
  ...
  if (raise)
    ...
    __gdk_window_raise_internal (window);
  ...
  if (!was_mapped || raise)
	...
	{
	  _gdk_synthesize_crossing_events_for_geometry_change (window);
	  __gdk_window_invalidate_rect_full (window, NULL, TRUE, CLEAR_BG_ALL);
	}
    }
}
 ...
 * order. In contrast, __gdk_window_show() will raise the window
 ...
 */
void
__gdk_window_show_unraised (GdkWindow *window)
{
  __gdk_window_show_internal (window, FALSE);
}
 ...
 * request to move the window in the Z-order, __gdk_window_raise() only
 * requests the restack, does not guarantee it.
 */
void
__gdk_window_raise (GdkWindow *window)
{
  GdkWindow *private;
  ...
  __gdk_window_flush_if_exposing (window);

  old_region = NULL;
  if (__gdk_window_is_viewable (window) &&
      !private->input_only)
    old_region = __gdk_region_copy (private->clip_region);
  ...
  if (old_region)
    {
      new_region = __gdk_region_copy (private->clip_region);

      __gdk_region_subtract (new_region, old_region);
      __gdk_window_invalidate_region_full (window, new_region, TRUE, CLEAR_BG_ALL);

      __gdk_region_destroy (old_region);
      __gdk_region_destroy (new_region);
    }
}

static void
__gdk_window_lower_internal (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkWindow *parent = private->parent;
  GdkWindowImplIface *impl_iface;
...
gdk_window_invalidate_in_parent (GdkWindow *private)
{
  ...
  __gdk_rectangle_intersect (&r, &child, &r);

  __gdk_window_invalidate_rect_full (GDK_WINDOW (private->parent), &r, TRUE, CLEAR_BG_ALL);
}
 ...
 *
 * If @window is a toplevel, the window manager may choose to deny the
 * request to move the window in the Z-order, __gdk_window_lower() only
 * requests the restack, does not guarantee it.
 *
 * Note that __gdk_window_show() raises the window again, so don't call this
 * function before __gdk_window_show(). (Try __gdk_window_show_unraised().)
 */
void
__gdk_window_lower (GdkWindow *window)
{
  GdkWindow *private;
  ...
  /* Keep children in (reverse) stacking order */
  __gdk_window_lower_internal (window);
 ...
 *
 * If @window is a toplevel, the window manager may choose to deny the
 * request to move the window in the Z-order, __gdk_window_restack() only
 * requests the restack, does not guarantee it.
 *
 * Since: 2.18
 */
void
__gdk_window_restack (GdkWindow     *window,
		    GdkWindow     *sibling,
		    gboolean       above)
{
  GdkWindow *private;
  ...
  if (gdk_window_is_toplevel (private))
    {
      g_return_if_fail (gdk_window_is_toplevel (GDK_WINDOW (sibling)));
      impl_iface = GDK_WINDOW_IMPL_GET_IFACE (private->impl);
 ...
 * @window: a #GdkWindow
 *
 * Like __gdk_window_show_unraised(), but also raises the window to the
 * top of the window stack (moves the window to the front of the
 * Z-order).
 *
 * This function maps a window so it's visible onscreen. Its opposite
 * is __gdk_window_hide().
 *
 * When implementing a #GtkWidget, you should call this function on the widget's
 * #GdkWindow as part of the "map" method.
 */
void
__gdk_window_show (GdkWindow *window)
{
  __gdk_window_show_internal (window, TRUE);
}
 ...
 * part of __gtk_widget_hide().
 */
void
__gdk_window_hide (GdkWindow *window)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
  ...
  if (gdk_window_has_impl (private))
      ...
      if (display->keyboard_grab.window != NULL)
	{
	  if (is_parent_of (window, display->keyboard_grab.window))
	    {
	      /* Call this ourselves, even though __gdk_display_keyboard_ungrab
...
/**
 * __gdk_window_withdraw:
 * @window: a toplevel #GdkWindow
 *
 * Withdraws a window (unmaps it and asks the window manager to forget about it).
 * This function is not really useful as __gdk_window_hide() automatically
 * withdraws toplevel windows before hiding them.
 **/
void
__gdk_window_withdraw (GdkWindow *window)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
  gboolean was_mapped;
 ...
 **/
void
__gdk_window_set_events (GdkWindow       *window,
		       GdkEventMask     event_mask)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
...
/**
 * __gdk_window_get_events:
 * @window: a #GdkWindow
 *
 * Gets the event mask for @window. See __gdk_window_set_events().
 *
 * Return value: event mask for @window
 **/
GdkEventMask
__gdk_window_get_events (GdkWindow *window)
{
  GdkWindow *private;
...
static void
__gdk_window_move_resize_toplevel (GdkWindow *window,
				 gboolean   with_move,
				 gint       x,
				 gint       y,
				 gint       width,
  ...
  if (expose)
    {
       ...
       */
      __gdk_region_subtract (new_region, old_region);
      __gdk_window_invalidate_region_full (window, new_region, TRUE, CLEAR_BG_WINCLEARED);
				    ...
				    int y_offset)
{
  ...
  for (l = window->children; l != NULL; l = l->next)
	...
	continue;
      ...
      if (child->impl != impl)
	{
	  tmp = __gdk_region_copy (child->clip_region);
	  __gdk_region_offset (tmp,
			     x_offset + child->x,
			     y_offset + child->y);
	  if (*region == NULL)
	    *region = tmp;
			     ...
			     gboolean include_this)
{
  ...
  if (include_this && gdk_window_has_impl (window) && window->viewable)
    return __gdk_region_copy (window->clip_region);
...
static void
__gdk_window_move_resize_internal (GdkWindow *window,
				 gboolean   with_move,
				 gint       x,
				 gint       y,
				 gint       width,
				 gint       height)
{
  ...
  if (gdk_window_is_toplevel (private))
    {
      __gdk_window_move_resize_toplevel (window, with_move, x, y, width, height);
      return;
    }
  ...
  if (__gdk_window_is_viewable (window) &&
      !private->input_only)
    {
      ...
      __gdk_region_offset (old_region, private->x, private->y);

      old_native_child_region = collect_native_child_region (private, TRUE);
      if (old_native_child_region)
	{
	  /* Adjust region to parent window coords */
	  __gdk_region_offset (old_native_child_region, private->x, private->y);
	   ...
	   * affecting this.
	   */
	  __gdk_window_flush_recursive (private->parent);
	}
    }
  ...
  if (old_native_child_region)
    {
      ...
      /* Adjust region to parent window coords */
      __gdk_region_offset (new_native_child_region, private->x, private->y);
    }
  ...
  if (expose)
    {
      new_region = __gdk_region_copy (private->clip_region);
      /* Adjust region to parent window coords */
      __gdk_region_offset (new_region, private->x, private->y);
       ...
       */
      copy_area = __gdk_region_copy (new_region);

      __gdk_region_union (new_region, old_region);
      ...
      if (old_native_child_region)
	{
	   ...
	   */
	  __gdk_region_subtract (old_region, old_native_child_region);
	}
      __gdk_region_offset (old_region, dx, dy);

      __gdk_region_intersect (copy_area, old_region);
      ...
      if (new_native_child_region)
	{
	  ...
	  __gdk_region_offset (new_native_child_region, dx, dy);
	  __gdk_region_subtract (copy_area, new_native_child_region);
	  __gdk_region_offset (new_native_child_region, -dx, -dy);
	}

      __gdk_region_subtract (new_region, copy_area);

      /* Convert old region to impl coords */
      __gdk_region_offset (old_region, -dx + private->abs_x - private->x, -dy + private->abs_y - private->y);

      /* convert from parent coords to impl */
      __gdk_region_offset (copy_area, private->abs_x - private->x, private->abs_y - private->y);
      ...
      if (old_native_child_region)
	{
	  ...
	  __gdk_region_offset (old_native_child_region, dx, dy);
	  __gdk_region_intersect (old_native_child_region, new_native_child_region);
	  __gdk_region_subtract (new_region, old_native_child_region);
	}
      __gdk_window_invalidate_region_full (GDK_WINDOW (private->parent), new_region, TRUE, CLEAR_BG_ALL);
  ...
  if (old_native_child_region)
    {
      __gdk_region_destroy (old_native_child_region);
      __gdk_region_destroy (new_native_child_region);
    }
 ...
 * you should probably use __gtk_window_move() on a #GtkWindow widget
 * anyway, instead of using GDK functions. For child windows,
 * the move will reliably succeed.
 *
 * If you're also planning to resize the window, use __gdk_window_move_resize()
 * to both move and resize simultaneously, for a nicer visual effect.
 **/
void
__gdk_window_move (GdkWindow *window,
		 gint       x,
		 gint       y)
{
  __gdk_window_move_resize_internal (window, TRUE, x, y, -1, -1);
}
 ...
 * use __gtk_window_resize() instead of this low-level GDK function.
 *
 * Windows may not be resized below 1x1.
 *
 * If you're also planning to move the window, use __gdk_window_move_resize()
 * to both move and resize simultaneously, for a nicer visual effect.
 **/
void
__gdk_window_resize (GdkWindow *window,
		   gint       width,
		   gint       height)
{
  __gdk_window_move_resize_internal (window, FALSE, 0, 0, width, height);
}


/**
 * __gdk_window_move_resize:
 * @window: a #GdkWindow
 ...
 * @height: new height
 *
 * Equivalent to calling __gdk_window_move() and __gdk_window_resize(),
 * except that both operations are performed at once, avoiding strange
 * visual effects. (i.e. the user may be able to see the window first
 * move, then resize, if you don't use __gdk_window_move_resize().)
 **/
void
__gdk_window_move_resize (GdkWindow *window,
			gint       x,
			gint       y,
			gint       width,
			gint       height)
{
  __gdk_window_move_resize_internal (window, TRUE, x, y, width, height);
}
 ...
 **/
void
__gdk_window_scroll (GdkWindow *window,
		   gint       dx,
		   gint       dy)
{
  GdkWindow *private = (GdkWindow *) window;
  ...
  if (old_native_child_region)
    {
       ...
       * affecting this.
       */
      __gdk_window_flush_recursive (private);
    }
  ...
  while (tmp_list)
    {
      ...
      GdkWindow *child_obj = GDK_WINDOW (child);
  ...
  copy_area = __gdk_region_copy (private->clip_region);
  if (old_native_child_region)
    {
      /* Don't copy from inside native children, as this is copied by
       * the native window move.
       */
      __gdk_region_subtract (copy_area, old_native_child_region);

      /* Don't copy any bits that would cause a read from the moved
	 native windows, as we can't read that data */
      __gdk_region_subtract (copy_area, new_native_child_region);
    }
  __gdk_region_offset (copy_area, dx, dy);
  __gdk_region_intersect (copy_area, private->clip_region);

  /* And the rest need to be invalidated */
  noncopy_area = __gdk_region_copy (private->clip_region);
  __gdk_region_subtract (noncopy_area, copy_area);

  /* convert from window coords to impl */
  __gdk_region_offset (copy_area, private->abs_x, private->abs_y);
  ...
  if (old_native_child_region)
    {
      ...
      __gdk_region_offset (old_native_child_region, dx, dy);
      __gdk_region_intersect (old_native_child_region, new_native_child_region);
      __gdk_region_subtract (noncopy_area, old_native_child_region);
    }
  __gdk_window_invalidate_region_full (window, noncopy_area, TRUE, CLEAR_BG_ALL);

  __gdk_region_destroy (noncopy_area);
 ...
 */
void
__gdk_window_move_region (GdkWindow       *window,
			const GdkRegion *region,
			gint             dx,
			gint             dy)
{
  ...
  copy_area = __gdk_region_copy (region);
  __gdk_region_intersect (copy_area, private->clip_region_with_children);
  ...
  /* Invalidate parts of the region (source and dest) not covered
     by the copy */
  nocopy_area = __gdk_region_copy (region);
  __gdk_region_offset (nocopy_area, dx, dy);
  __gdk_region_union (nocopy_area, region);
  __gdk_region_subtract (nocopy_area, copy_area);

  /* convert from window coords to impl */
  __gdk_region_offset (copy_area, private->abs_x, private->abs_y);
  move_region_on_impl (impl_window, copy_area, dx, dy); /* Takes ownership of copy_area */

  __gdk_window_invalidate_region_full (window, nocopy_area, FALSE, CLEAR_BG_ALL);
  __gdk_region_destroy (nocopy_area);
}

/**
 * __gdk_window_set_background:
 * @window: a #GdkWindow
 * @color: an allocated #GdkColor
 *
 * Sets the background color of @window. (However, when using GTK+,
 * set the background of a widget with __gtk_widget_modify_bg() - if
 * you're an application - or __gtk_style_set_background() - if you're
 * implementing a custom widget.)
 *
 * The @color must be allocated; __gdk_rgb_find_color() is the best way
 * to allocate a color.
 *
 * See also __gdk_window_set_back_pixmap().
 */
void
__gdk_window_set_background (GdkWindow      *window,
			   const GdkColor *color)
{
  GdkWindow *private;
  GdkColormap *colormap = __gdk_drawable_get_colormap ((GdkDrawable *)window);
  GdkWindowImplIface *impl_iface;
  ...
  __gdk_colormap_query_color (colormap, private->bg_color.pixel, &private->bg_color);
 ...
 */
void
__gdk_window_set_back_pixmap (GdkWindow *window,
			    GdkPixmap *pixmap,
...
/**
 * __gdk_window_get_background_pattern:
 * @window: a window
 *
 ...
 **/
cairo_pattern_t *
__gdk_window_get_background_pattern (GdkWindow *window)
{
  GdkWindow *private = (GdkWindow *) window;
 ...
 *   directly. Use __gdk_window_set_cursor() to unset the cursor of the
 *   window
 *
 * Since: 2.18
 */
GdkCursor *
__gdk_window_get_cursor (GdkWindow *window)
{
  GdkWindow *private;
...
/**
 * __gdk_window_set_cursor:
 * @window: a #GdkWindow
 * @cursor: (allow-none): a cursor
 *
 * Sets the mouse pointer for a #GdkWindow. Use __gdk_cursor_new_for_display()
 * or __gdk_cursor_new_from_pixmap() to create the cursor. To make the cursor
 * invisible, use %GDK_BLANK_CURSOR. Passing %NULL for the @cursor argument
 * to __gdk_window_set_cursor() means that @window will use the cursor of its
 * parent window. Most windows should use this default.
 */
void
__gdk_window_set_cursor (GdkWindow *window,
		       GdkCursor *cursor)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
  ...
  if (private->cursor)
    {
      __gdk_cursor_unref (private->cursor);
      private->cursor = NULL;
    }
  ...
  if (!GDK_WINDOW_DESTROYED (window))
    {
      if (cursor)
	private->cursor = __gdk_cursor_ref (cursor);
 ...
 * #GdkEventConfigure. __gdk_window_get_position() in contrast gets the
 * position from the most recent configure event.
 *
 * <note>
 * If @window is not a toplevel, it is <emphasis>much</emphasis> better
 * to call __gdk_window_get_position() and __gdk_drawable_get_size() instead,
 * because it avoids the roundtrip to the X server and because
 * __gdk_drawable_get_size() supports the full 32-bit coordinate space,
 * whereas __gdk_window_get_geometry() is restricted to the 16-bit
 * coordinates of X11.
 *</note>
 **/
void
__gdk_window_get_geometry (GdkWindow *window,
			 gint      *x,
			 gint      *y,
			 gint      *width,
			 gint      *height,
			 gint      *depth)
{
  GdkWindow *private, *parent;
  GdkWindowImplIface *impl_iface;

  if (!window)
    {
      GDK_NOTE (MULTIHEAD,
		g_message ("__gdk_window_get_geometry(): Window needs "
			   "to be non-NULL to be multi head safe"));
      window = __gdk_screen_get_root_window ((__gdk_screen_get_default ()));
    }
...
/**
 * __gdk_window_get_origin:
 * @window: a #GdkWindow
 * @x: return location for X coordinate
 * @y: return location for Y coordinate
 *
 * Obtains the position of a window in root window coordinates.
 * (Compare with __gdk_window_get_position() and
 * __gdk_window_get_geometry() which return the position of a window
 * relative to its parent window.)
 *
 * Return value: not meaningful, ignore
 */
gint
__gdk_window_get_origin (GdkWindow *window,
		       gint      *x,
		       gint      *y)
{
  GdkWindow *private;
 ...
 * __gdk_window_get_origin() but allows you go pass
 * in any position in the window, not just the origin.
 *
 * Since: 2.18
 */
void
__gdk_window_get_root_coords (GdkWindow *window,
			    gint       x,
			    gint       y,
			    gint      *root_x,
			    gint      *root_y)
 ...
 * __gdk_window_get_parent() for normal windows, and the window's
 * embedder as returned by __gdk_offscreen_window_get_embedder() for
 * offscreen windows.
 *
 * For normal windows, calling this function is equivalent to adding
 * the return values of __gdk_window_get_position() to the child coordinates.
 ...
 *
 * See also: __gdk_window_coords_from_parent()
 *
 * Since: 2.22
 **/
void
__gdk_window_coords_to_parent (GdkWindow *window,
                             gdouble    x,
                             gdouble    y,
                             gdouble   *parent_x,
                             gdouble   *parent_y)
...
/**
 * __gdk_window_coords_from_parent:
 * @window: a child window
 ...
 * the return values of __gdk_window_get_position() from the parent coordinates.
 * For offscreen windows however (which can be arbitrarily transformed),
 ...
 *
 * See also: __gdk_window_coords_to_parent()
 *
 * Since: 2.22
 **/
void
__gdk_window_coords_from_parent (GdkWindow *window,
                               gdouble    parent_x,
                               gdouble    parent_y,
                               gdouble   *x,
                               gdouble   *y)
...
/**
 * __gdk_window_get_deskrelative_origin:
 * @window: a toplevel #GdkWindow
 * @x: return location for X coordinate
 * @y: return location for Y coordinate
 *
 ...
 **/
gboolean
__gdk_window_get_deskrelative_origin (GdkWindow *window,
				    gint      *x,
				    gint      *y)
{
  GdkWindow *private;
...
/**
 * __gdk_window_shape_combine_mask:
 * @window: a #GdkWindow
 * @mask: shape mask
 ...
 */
void
__gdk_window_shape_combine_mask (GdkWindow *window,
			       GdkBitmap *mask,
			       gint       x,
			       gint       y)
{
  ...
  __gdk_window_shape_combine_region (window,
				   region,
				   x, y);
...
/**
 * __gdk_window_shape_combine_region:
 * @window: a #GdkWindow
 ...
 * __gdk_window_shape_combine_mask() to use a bitmap as the mask.
 *
 ...
 */
void
__gdk_window_shape_combine_region (GdkWindow       *window,
				 ...
				 gint             offset_y)
{
  ...
  if (private->shape)
    __gdk_region_destroy (private->shape);
  ...
  if (shape_region)
    {
      private->shape = __gdk_region_copy (shape_region);
      __gdk_region_offset (private->shape, offset_x, offset_y);
    }
  else
    private->shape = NULL;
  ...
  if (old_region)
    {
      ...
      diff = __gdk_region_copy (new_region);
      __gdk_region_subtract (diff, old_region);

      __gdk_window_invalidate_region_full (window, diff, TRUE, CLEAR_BG_ALL);

      __gdk_region_destroy (diff);

      if (!gdk_window_is_toplevel (private))
	{
	  /* New area in the non-root parent window, needs invalidation */
	  diff = __gdk_region_copy (old_region);
	  __gdk_region_subtract (diff, new_region);

	  /* Adjust region to parent window coords */
	  __gdk_region_offset (diff, private->x, private->y);

	  __gdk_window_invalidate_region_full (GDK_WINDOW (private->parent), diff, TRUE, CLEAR_BG_ALL);
		 ...
		 gboolean merge)
{
  ...
  region = __gdk_region_rectangle (&r);
  remove_child_area (private, NULL, FALSE, region);

  if (merge && private->shape)
    __gdk_region_subtract (region, private->shape);

  __gdk_window_shape_combine_region (window, region, 0, 0);

  __gdk_region_destroy (region);
}

/**
 * __gdk_window_set_child_shapes:
 * @window: a #GdkWindow
 *
 * Sets the shape mask of @window to the union of shape masks
 * for all children of @window, ignoring the shape mask of @window
 * itself. Contrast with __gdk_window_merge_child_shapes() which includes
 * the shape mask of @window in the masks to be merged.
 **/
void
__gdk_window_set_child_shapes (GdkWindow *window)
{
  g_return_if_fail (GDK_IS_WINDOW (window));
...
/**
 * __gdk_window_merge_child_shapes:
 * @window: a #GdkWindow
 *
 * Merges the shape masks for any child windows into the
 * shape mask for @window. i.e. the union of all masks
 * for @window and its children will become the new mask
 * for @window. See __gdk_window_shape_combine_mask().
 *
 * This function is distinct from __gdk_window_set_child_shapes()
 * because it includes @window's shape mask in the set of shapes to
 * be merged.
 */
void
__gdk_window_merge_child_shapes (GdkWindow *window)
{
  g_return_if_fail (GDK_IS_WINDOW (window));
...
/**
 * __gdk_window_input_shape_combine_mask:
 * @window: a #GdkWindow
 * @mask: (allow-none): shape mask, or %NULL
 * @x: X position of shape mask with respect to @window
 * @y: Y position of shape mask with respect to @window
 *
 * Like __gdk_window_shape_combine_mask(), but the shape applies
 ...
 */
void
__gdk_window_input_shape_combine_mask (GdkWindow *window,
				     GdkBitmap *mask,
				     gint       x,
				     gint       y)
{
  ...
  __gdk_window_input_shape_combine_region (window,
					 region,
					 x, y);
...
/**
 * __gdk_window_input_shape_combine_region:
 * @window: a #GdkWindow
 * @shape_region: region of window to be non-transparent
 * @offset_x: X position of @shape_region in @window coordinates
 * @offset_y: Y position of @shape_region in @window coordinates
 *
 * Like __gdk_window_shape_combine_region(), but the shape applies
 * only to event handling. Mouse events which happen while
 * the pointer position corresponds to an unset bit in the
 * mask will be passed on the window below @window.
 *
 ...
 */
void
__gdk_window_input_shape_combine_region (GdkWindow       *window,
				       const GdkRegion *shape_region,
				       gint             offset_x,
				       gint             offset_y)
{
  ...
  if (private->input_shape)
    __gdk_region_destroy (private->input_shape);

  if (shape_region)
    {
      private->input_shape = __gdk_region_copy (shape_region);
      __gdk_region_offset (private->input_shape, offset_x, offset_y);
    }
  else
		       ...
		       gboolean merge)
{
  ...
  if (merge && private->input_shape)
    __gdk_region_subtract (region, private->input_shape);

  __gdk_window_input_shape_combine_region (window, region, 0, 0);
}


/**
 * __gdk_window_set_child_input_shapes:
 * @window: a #GdkWindow
 *
 * Sets the input shape mask of @window to the union of input shape masks
 * for all children of @window, ignoring the input shape mask of @window
 * itself. Contrast with __gdk_window_merge_child_input_shapes() which includes
 * the input shape mask of @window in the masks to be merged.
 *
 * Since: 2.10
 **/
void
__gdk_window_set_child_input_shapes (GdkWindow *window)
{
  g_return_if_fail (GDK_IS_WINDOW (window));
...
/**
 * __gdk_window_merge_child_input_shapes:
 * @window: a #GdkWindow
 *
 * Merges the input shape masks for any child windows into the
 * input shape mask for @window. i.e. the union of all input masks
 * for @window and its children will become the new input mask
 * for @window. See __gdk_window_input_shape_combine_mask().
 *
 * This function is distinct from __gdk_window_set_child_input_shapes()
 ...
 **/
void
__gdk_window_merge_child_input_shapes (GdkWindow *window)
{
  g_return_if_fail (GDK_IS_WINDOW (window));
...
/**
 * __gdk_window_set_static_gravities:
 * @window: a #GdkWindow
 ...
 */
gboolean
__gdk_window_set_static_gravities (GdkWindow *window,
				 gboolean   use_static)
{
  GdkWindow *private;
  GdkWindowImplIface *impl_iface;
...
/**
 * __gdk_window_get_composited:
 * @window: a #GdkWindow
 *
 * Determines whether @window is composited.
 *
 * See __gdk_window_set_composited().
 *
 ...
 **/
gboolean
__gdk_window_get_composited (GdkWindow *window)
{
  GdkWindow *private;
 ...
 * __gdk_window_set_opacity() if you need translucent toplevel
 * windows.
 *
 ...
 * You must call __gdk_display_supports_composite() to check if
 ...
 */
void
__gdk_window_set_composited (GdkWindow *window,
			   gboolean   composited)
{
  GdkWindow *private = (GdkWindow *)window;
  GdkDisplay *display;
  ...
  if (!__gdk_display_supports_composite (display) && composited)
    {
      g_warning ("__gdk_window_set_composited called but "
...
static void
remove_redirect_from_children (GdkWindow   *private,
			       GdkWindowRedirect *redirect)
{
  GList *l;
  GdkWindow *child;
...
/**
 * __gdk_window_remove_redirection:
 * @window: a #GdkWindow
 *
 * Removes any active redirection started by
 * __gdk_window_redirect_to_drawable().
 *
 * Since: 2.14
 **/
void
__gdk_window_remove_redirection (GdkWindow *window)
{
  GdkWindow *private;
 ...
 */
gboolean
__gdk_window_get_modal_hint (GdkWindow *window)
{
  GdkWindow *private;
 ...
 */
gboolean
__gdk_window_get_accept_focus (GdkWindow *window)
{
  GdkWindow *private;
 ...
 */
gboolean
__gdk_window_get_focus_on_map (GdkWindow *window)
{
  GdkWindow *private;
 ...
 */
gboolean
__gdk_window_is_input_only (GdkWindow *window)
{
  GdkWindow *private;
 ...
 */
gboolean
__gdk_window_is_shaped (GdkWindow *window)
{
  GdkWindow *private;
...
static void
apply_redirect_to_children (GdkWindow   *private,
			    GdkWindowRedirect *redirect)
{
  GList *l;
  GdkWindow *child;
 ...
 * @dest_x, @dest_y.
 *
 * Only drawing between __gdk_window_begin_paint_region() or
 * __gdk_window_begin_paint_rect() and __gdk_window_end_paint() is
 * redirected.
 *
 * Redirection is active until __gdk_window_remove_redirection()
 * is called.
 *
 * Since: 2.14
 **/
void
__gdk_window_redirect_to_drawable (GdkWindow   *window,
				 GdkDrawable *drawable,
				 gint         src_x,
				 gint         src_y,
				 gint         dest_x,
				 ...
				 gint         height)
{
  ...
  if (private->redirect)
    __gdk_window_remove_redirection (window);

  if (width == -1 || height == -1)
    {
      gint w, h;
      __gdk_drawable_get_size (GDK_DRAWABLE (window), &w, &h);
      if (width == -1)
	width = w;
      if (height == -1)
	height = h;
					...
					gint      *base_y_offset)
{
  ...
  real_clip_region = __gdk_region_rectangle (&visible_rect);
  ...
  for (; parentwin != NULL &&
	 (parentwin == private || lastwin != (GdkWindow*) base_window);
	...
	{
	  x_offset += GDK_WINDOW (lastwin)->x;
	  y_offset += GDK_WINDOW (lastwin)->y;
	}
	   ...
	   cur = cur->next)
	{
	  ...
	  GdkWindow *child_private = (GdkWindow *)child;
	  ...
	  __gdk_region_get_clipbox (real_clip_region, &real_clip_rect);
	  ...
	  tmpreg = __gdk_region_rectangle (&visible_rect);
	  __gdk_region_subtract (real_clip_region, tmpreg);
	  __gdk_region_destroy (tmpreg);
	}
      ...
      tmpreg = __gdk_region_rectangle (&visible_rect);
      __gdk_region_intersect (real_clip_region, tmpreg);
      __gdk_region_destroy (tmpreg);
    }
			...
			GdkRegion *damaged_region)
{
  ...
  __gdk_region_get_clipbox (event.expose.region, &event.expose.area);
  display = __gdk_drawable_get_display ((GdkDrawable *)event.expose.window);
  _gdk_event_queue_append (display, __gdk_event_copy (&event));
}
...
get_event_parent (GdkWindow *window)
{
  if (gdk_window_is_offscreen (window))
    return (GdkWindow *)__gdk_offscreen_window_get_embedder ((GdkWindow *)window);
  else
...
{
  GdkWindow *private = GDK_WINDOW (w);
  GdkWindow *parent;
 	  	             ...
 	  	             GdkWindow *child)
{
  ...
  while (w != NULL)
	...
	return TRUE;
      ...
      w = (GdkWindow *)get_event_parent ((GdkWindow *)w);
    }
...
update_cursor (GdkDisplay *display)
{
  ...
  if (/* have grab */
    ...
    cursor_window = (GdkWindow *)grab->window;
  else
    /* otherwise use the cursor from the pointer window */
    cursor_window = (GdkWindow *)pointer_window;
  ...
  toplevel = (GdkWindow *)get_event_toplevel (pointer_window);
                 ...
                 gdouble          y)
{
  return
    ...
    y >= 0 && y < window->height &&
    (window->shape == NULL ||
     __gdk_region_point_in (window->shape,
			  x, y)) &&
    (window->input_shape == NULL ||
     __gdk_region_point_in (window->input_shape,
			  x, y));
}
				   ...
				   gdouble   *window_y)
{
  ...
  for (l = children; l != NULL; l = l->next)
    __gdk_window_coords_from_parent (l->data, x, y, &x, &y);
                           ...
                           int        y)
{
  ...
  if (point_in_window (private, x, y))
      ...
      for (l = private->children; l != NULL; l = l->next)
	{
	  ...
	  __gdk_window_coords_from_parent ((GdkWindow *)sub,
                                         x, y,
                                         &child_x, &child_y);
 ...
 * the display just as __gdk_display_beep().
 *
 * Since: 2.12
 **/
void
__gdk_window_beep (GdkWindow *window)
{
  GdkDisplay *display;
  GdkWindow *toplevel;
  ...
  if (toplevel && !gdk_window_is_offscreen ((GdkWindow *)toplevel))
    _gdk_windowing_window_beep (toplevel);
  else
    __gdk_display_beep (display);
}
...
{
  GdkEvent *event = __gdk_event_new (type);
  guint32 the_time;
  GdkModifierType the_state;

  the_time = __gdk_event_get_time (event_in_queue);
  __gdk_event_get_state (event_in_queue, &the_state);
  ...
  if (event_in_queue)
    {
    if (before_event)
      _gdk_event_queue_insert_before (__gdk_drawable_get_display ((GdkDrawable *)window), event_in_queue, event);
    else
      _gdk_event_queue_insert_after (__gdk_drawable_get_display ((GdkDrawable *)window), event_in_queue, event);
    }
  else
    _gdk_event_queue_append (__gdk_drawable_get_display ((GdkDrawable *)window), event);
				 ...
				 gboolean                    non_linear)
{
  ...
  if (a) /* There might not be a source (i.e. if no previous pointer_in_window) */
    {
      toplevel = (GdkWindow *)__gdk_window_get_toplevel ((GdkWindow *)a);
  ...
  if (b) /* Might not be a dest, e.g. if we're moving out of the window */
    {
      toplevel = (GdkWindow *)__gdk_window_get_toplevel ((GdkWindow *)b);
 ...
 *
 * Side effects:
 *   requires a corresponding call to __gdk_pointer_ungrab
 *
 *--------------------------------------------------------------
 */
GdkGrabStatus
__gdk_pointer_grab (GdkWindow *	  window,
		  ...
		  guint32	  time)
{
  ...
  if (confine_to)
    {
      if (!__gdk_window_ensure_native (confine_to))
	{
  ...
  if (!_gdk_window_has_impl (window) &&
      !__gdk_window_is_viewable (window))
  ...
  while (gdk_window_is_offscreen ((GdkWindow *)native))
    {
      native = __gdk_offscreen_window_get_embedder (native);

      if (native == NULL ||
	  (!_gdk_window_has_impl (native) &&
	   !__gdk_window_is_viewable (native)))
	return GDK_GRAB_NOT_VIEWABLE;

      native = __gdk_window_get_toplevel (native);
    }
 ...
 */
void
__gdk_window_geometry_changed (GdkWindow *window)
{
  _gdk_synthesize_crossing_events_for_geometry_change (window);
}
...
do_synthesize_crossing_event (gpointer data)
{
  ...
  GdkWindow *changed_toplevel_priv;
  GdkWindow *new_window_under_pointer;
  gulong serial;

  changed_toplevel = data;
  changed_toplevel_priv = (GdkWindow *)changed_toplevel;
...
_gdk_synthesize_crossing_events_for_geometry_change (GdkWindow *changed_window)
{
  GdkDisplay *display;
  GdkWindow *toplevel;
  GdkWindow *toplevel_priv;

  if (_gdk_native_windows)
    return; /* We use the native crossing events if all native */

  display = __gdk_drawable_get_display ((GdkDrawable *)changed_window);

  toplevel = get_event_toplevel (changed_window);
  toplevel_priv = (GdkWindow *)toplevel;
  ...
  if (toplevel == display->pointer_info.toplevel_under_pointer &&
      !toplevel_priv->synthesize_crossing_event_queued)
    {
      ...
      __gdk_threads_add_idle_full (GDK_PRIORITY_EVENTS - 1,
		     ...
		     gulong                      serial)
{
  ...
  __gdk_event_get_coords (source_event, &toplevel_x, &toplevel_y);
  __gdk_event_get_state (source_event, &state);
  time_ = __gdk_event_get_time (source_event);
		    ...
		    gulong serial)
{
  ...
  __gdk_event_get_state (source_event, &state);
  time_ = __gdk_event_get_time (source_event);
  display = __gdk_drawable_get_display ((GdkDrawable *)source_event->any.window);
  toplevel_window = convert_native_coords_to_toplevel (event_window,
						       toplevel_x, toplevel_y,
						       &toplevel_x, &toplevel_y);
  ...
  if (type == GDK_BUTTON_PRESS &&
	     ...
	     parent->window_type != GDK_WINDOW_ROOT)
	{
      ...
      _gdk_display_add_pointer_grab  (display,
				      ...
				      event_window,
				      FALSE,
				      __gdk_window_get_events (pointer_window),
				      serial,
				      time_,
				      TRUE);
		  ...
		  int indent)
{
  ...
  if (gdk_window_has_impl (window))
    {
#ifdef GDK_WINDOWING_X11
      g_print (" impl(0x%lx)", __gdk_x11_drawable_get_xid (GDK_DRAWABLE (window)));
#endif
    }
  ...
  if (!__gdk_window_is_visible ((GdkWindow *)window))
    g_print (" hidden");

  g_print (" abs[%d,%d]",
	   window->abs_x, window->abs_y);

  __gdk_region_get_clipbox (window->clip_region, &r);
  if (__gdk_region_empty (window->clip_region))
		...
		GdkEvent *event)
{
  ...
  core_pointer = __gdk_display_get_core_pointer (display);
			  ...
			  gulong      serial)
{
  ...
  if (__gdk_event_get_time (event) != GDK_CURRENT_TIME)
    display->last_event_time = __gdk_event_get_time (event);
  ...
  event_private = GDK_WINDOW (event_window);
  ...
  if (_gdk_native_windows)
	  ...
	  _gdk_display_has_pointer_grab (display, serial) == NULL)
	{
	  _gdk_display_add_pointer_grab  (display,
					  ...
					  event_window,
					  FALSE,
					  __gdk_window_get_events (event_window),
					  serial,
					...
					gulong serial)
{
  ...
  display = __gdk_drawable_get_display ((GdkDrawable *)native_window);
...
/**
 * __gdk_window_create_similar_surface:
 ...
 **/
cairo_surface_t *
__gdk_window_create_similar_surface (GdkWindow *     window,
                                   ...
                                   int             height)
{
  ...
  window_surface = _gdk_drawable_ref_cairo_surface ((GdkDrawable *)window);
 ...
 */
GdkScreen*
__gdk_window_get_screen (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);

  return __gdk_drawable_get_screen (GDK_DRAWABLE (window));
}

/**
 * __gdk_window_get_display:
 * @window: a #GdkWindow
 *
 ...
 */
GdkDisplay *
__gdk_window_get_display (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);

  return __gdk_drawable_get_display (GDK_DRAWABLE (window));
}

/**
 * __gdk_window_get_visual:
 * @window: a #GdkWindow
 *
 ...
 */
GdkVisual*
__gdk_window_get_visual (GdkWindow *window)
{
  g_return_val_if_fail (GDK_IS_WINDOW (window), NULL);

  return __gdk_drawable_get_visual (GDK_DRAWABLE (window));
}

/**
 * __gdk_window_get_width:
 * @window: a #GdkWindow
 *
 ...
 */
int
__gdk_window_get_width (GdkWindow *window)
{
  gint width, height;
 ...
 */
int
__gdk_window_get_height (GdkWindow *window)
{
  gint width, height;
