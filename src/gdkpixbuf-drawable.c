		   ...
		   gboolean     alpha)
{
  ...
  v = __gdk_colormap_get_visual (cmap);
  ...
  for (yy = y1; yy < y2; yy++)
    {
      ...
      for (xx = x1; xx < x2; xx++)
	{
	  pixel = __gdk_image_get_pixel (image, xx, yy);
	  switch (v->type)
	    {
...
/**
 * __gdk_pixbuf_get_from_drawable:
 ...
 * 
 * If the drawable @src has no colormap (__gdk_drawable_get_colormap()
 * returns %NULL), then a suitable colormap must be specified.
 * Typically a #GdkWindow or a pixmap created by passing a #GdkWindow
 * to __gdk_pixmap_new() will already have a colormap associated with
 ...
 * black. For taking screenshots, __gdk_colormap_get_system() returns
 ...
 *
 * This function calls __gdk_drawable_get_image() internally and
 * converts the resulting image to a #GdkPixbuf, so the
 * documentation for __gdk_drawable_get_image() may also be relevant.
 * 
 ...
 **/
GdkPixbuf *
__gdk_pixbuf_get_from_drawable (GdkPixbuf   *dest,
			      GdkDrawable *src,
			      GdkColormap *cmap,
			      int src_x,  int src_y,
			      ...
			      int width,  int height)
{
  ...
  if (cmap == NULL)
    cmap = __gdk_drawable_get_colormap (src);

  depth = __gdk_drawable_get_depth (src);
  ...
  if (depth != 1 && cmap == NULL)
    {
      g_warning ("%s: Source drawable has no colormap; either pass "
                 ...
                 "with __gdk_drawable_set_colormap()", G_STRLOC);
      return NULL;
    }
  ...
  if (GDK_IS_PIXMAP (src))
    {
      __gdk_drawable_get_size (src, &src_width, &src_height);
      if (width < 0)
        width = src_width;
      if (height < 0)
        height = src_height;
  ...
  for (y0 = 0; y0 < height; y0 += GDK_SCRATCH_IMAGE_HEIGHT)
      ...
      for (x0 = 0; x0 < width; x0 += GDK_SCRATCH_IMAGE_WIDTH)
	{
	  ...
	  image = _gdk_image_get_scratch (__gdk_drawable_get_screen (src), 
					  width1, height1, depth, &xs0, &ys0);

	  __gdk_drawable_copy_to_image (src, image,
				      src_x + x0, src_y + y0,
				       xs0, ys0, width1, height1);

	  __gdk_pixbuf_get_from_image (dest, image, cmap,
 ...
 * @height: Height in pixels of region to get.
 * 
 * Same as __gdk_pixbuf_get_from_drawable() but gets the pixbuf from
 * an image.
 * 
 * Return value: @dest, newly-created pixbuf if @dest was %NULL, %NULL on error
 **/
GdkPixbuf*
__gdk_pixbuf_get_from_image (GdkPixbuf   *dest,
                           GdkImage    *src,
                           GdkColormap *cmap,
                           int          src_x,
                           int          src_y,
                           ...
                           int          height)
{
  ...
  if (cmap == NULL)
    cmap = __gdk_image_get_colormap (src);
  ...
  if (src->depth != 1 && cmap == NULL)
    {
      g_warning ("%s: Source image has no colormap; either pass "
                 ...
                 "with __gdk_image_set_colormap()", G_STRLOC);
      return NULL;
    }
