 ...
 *
 * To start the animation, use __gtk_spinner_start(), to stop it
...
STLWRT_DEFINE_TYPE (GtkSpinner, gtk_spinner, GTK_TYPE_DRAWING_AREA);
...
static GType      __gtk_spinner_accessible_get_type (void);
...
gtk_spinner_timeout (gpointer data)
{
  ...
  __gtk_widget_queue_draw (GTK_WIDGET (data));
...
gtk_spinner_screen_changed (GtkWidget* widget, GdkScreen* old_screen)
{
  ...
  if (!colormap)
    {
      colormap = __gdk_screen_get_rgb_colormap (new_screen);
    }

  __gtk_widget_set_colormap (widget, colormap);
}
  ...
  if (priv->active != active)
    {
      ...
      if (active && __gtk_widget_get_realized (GTK_WIDGET (spinner)) && priv->timeout == 0)
        {
...
{
  return __gtk_spinner_accessible_get_type ();
}
...
gtk_spinner_accessible_new (GObject *obj)
{
  ...
  accessible = g_object_new (__gtk_spinner_accessible_get_type (), NULL);
...
static GType
__gtk_spinner_accessible_factory_get_type (void)
{
  static GType type = 0;
...
static GType
__gtk_spinner_accessible_get_type (void)
{
  static GType type = 0;
  ...
  if (first_time)
    {
      ...
      if (g_type_is_a (derived_atk_type, GTK_TYPE_ACCESSIBLE))
        atk_registry_set_factory_type (registry,
                                       GTK_TYPE_SPINNER,
                                       __gtk_spinner_accessible_factory_get_type ());
      first_time = FALSE;
    }
 ...
 */
void
__gtk_spinner_start (GtkSpinner *spinner)
{
 ...
 */
void
__gtk_spinner_stop (GtkSpinner *spinner)
{
  g_return_if_fail (GTK_IS_SPINNER (spinner));
