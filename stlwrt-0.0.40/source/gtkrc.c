...
{
  gchar **paths = ______gtk_get_module_path ("immodules");
...
{
  if (!context->reloading)
    ______gtk_rc_context_get_default_font_name (settings);
}
...
void
______gtk_rc_context_destroy (GtkSettings *settings)
{
  GtkRcContext *context;
  ...
  ______gtk_settings_reset_rc_values (context->settings);
...
void
______gtk_rc_style_set_rc_property (GtkRcStyle *rc_style,
			       GtkRcProperty *property)
{
...
void
______gtk_rc_style_unset_rc_property (GtkRcStyle *rc_style,
				 GQuark      type_name,
				 GQuark      property_name)
{
  ...
  node = (GtkRcProperty *) ______gtk_rc_style_lookup_rc_property (rc_style,
                                                             type_name,
                                                             property_name);
...
GSList *
______gtk_rc_style_get_color_hashes (GtkRcStyle *rc_style)
{
...
gtk_rc_reset_widgets (GtkSettings *settings)
{
  ...
  ______gtk_icon_set_invalidate_caches ();
...
const gchar*
______gtk_rc_context_get_default_font_name (GtkSettings *settings)
{
  GtkRcContext *context;
  gchar *new_font_name;
				 ...
				 gboolean     force_load)
{
  ...
  if (force_load || mtime_modified)
    {
      ______gtk_binding_reset_parsed ();
      gtk_rc_clear_styles (context);
  ...
  while (sets)
    {
      ...
      if (rc_set->type == GTK_PATH_WIDGET_CLASS)
        {
          if (______gtk_rc_match_widget_class (rc_set->path, path_length, path, path_reversed))
...
__gtk_rc_get_style (GtkWidget *widget)
{
  ...
  if (rc_styles)
      ...
      if (!context->default_style)
	{
	  ...
	  ______gtk_style_init_for_settings (context->default_style, context->settings);
	}
		    ...
		    GtkPathType  path_type)
{
  ...
  *new_style = *rc_style;
  ...
  if (path_type == GTK_PATH_WIDGET_CLASS)
    {
      rc_set->pspec = NULL;
      rc_set->path = ______gtk_rc_parse_widget_class_path (pattern);
    }
  else
    {
		       ...
		       GtkRcStyle   *rc_style)
{
  ...
  ______gtk_style_init_for_settings (style, context->settings);
			...
			GScanner     *scanner)
{
  ...
  switch (token)
    ...
    case GTK_RC_TOKEN_BINDING:
      return ______gtk_binding_parse_binding (scanner);
	  ...
	  if (token == G_TOKEN_NONE)
	    {
	      ...
	      ______gtk_settings_set_property_value_from_rc (context->settings,
							name,
							&svalue);
	    }
	  g_free (prop.origin);
...
const GtkRcProperty*
______gtk_rc_style_lookup_rc_property (GtkRcStyle *rc_style,
				  GQuark      type_name,
				  GQuark      property_name)
{
...
{
  return ______gtk_find_module (module_file, "engines");
}
  ...
  switch (token)
    ...
    case G_TOKEN_IDENTIFIER:
	  ...
	  token = g_scanner_get_next_token (scanner);
          ...
          ______gtk_style_shade (&c1, color, l);
...
GSList *
______gtk_rc_parse_widget_class_path (const gchar *pattern)
{
  GSList *result;
  PathElt *path_elt;
  const gchar *current;
...
void
______gtk_rc_free_widget_class_path (GSList *list)
{
...
gtk_rc_set_free (GtkRcSet *rc_set)
{
  ...
  ______gtk_rc_free_widget_class_path (rc_set->path);
...
gboolean
______gtk_rc_match_widget_class (GSList  *list,
                            gint     length,
                            gchar   *path,
                            gchar   *path_reversed)
{
