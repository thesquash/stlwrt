...
free_style_cache (GtkTextLayout *text_layout)
{
  if (text_layout->one_style_cache)
    {
      __gtk_text_attributes_unref (text_layout->one_style_cache);
...
gtk_text_layout_finalize (GObject *object)
{
  ...
  if (layout->default_style)
    __gtk_text_attributes_unref (layout->default_style);
                            ...
                            GtkTextBuffer *buffer)
{
  ...
  if (layout->buffer)
    {
      _gtk_text_btree_remove_view (______gtk_text_buffer_get_btree (layout->buffer),
                                  layout);
  ...
  if (buffer)
    {
      ...
      _gtk_text_btree_add_view (______gtk_text_buffer_get_btree (buffer), layout);
  ...
  if (layout->cursor_visible != cursor_visible)
    {
       ...
       */
      __gtk_text_buffer_get_iter_at_mark (layout->buffer, &iter,
                                        __gtk_text_buffer_get_insert (layout->buffer));

      gtk_text_layout_get_line_yrange (layout, &iter, &y, &height);
      gtk_text_layout_emit_changed (layout, y, height, height);

      gtk_text_layout_invalidate_cache (layout, ___gtk_text_iter_get_text_line (&iter), TRUE);
    }
}
  ...
  if (layout->one_display_cache)
    {
      ...
      gint cache_y = _gtk_text_btree_find_line_top (______gtk_text_buffer_get_btree (layout->buffer),
						    line, layout);
                           ...
                           gint *first_line_y)
{
  ...
  first_btree_line =
    _gtk_text_btree_find_line_by_y (______gtk_text_buffer_get_btree (layout->buffer),
  ...
  if (!last_btree_line)
    last_btree_line =
      _gtk_text_btree_get_end_iter_line (______gtk_text_buffer_get_btree (layout->buffer));
...
gtk_text_layout_invalidate_all (GtkTextLayout *layout)
{
  ...
  __gtk_text_buffer_get_bounds (layout->buffer, &start, &end);
...
gtk_text_layout_update_cursor_line(GtkTextLayout *layout)
{
  ...
  priv->cursor_line = ___gtk_text_iter_get_text_line (&iter);
}
                                 ...
                                 const GtkTextIter *end)
{
  ...
  last_line = ___gtk_text_iter_get_text_line (end);
  line = ___gtk_text_iter_get_text_line (start);
  ...
  if (layout->one_display_cache)
    {
      ...
      _gtk_text_btree_get_iter_at_line (______gtk_text_buffer_get_btree (layout->buffer),
                                        &line_start, line, 0);
      line_end = line_start;
...
gtk_text_layout_is_valid (GtkTextLayout *layout)
{
  ...
  return _gtk_text_btree_is_valid (______gtk_text_buffer_get_btree (layout->buffer),
                                  layout);
}
...
{
  _gtk_text_btree_get_view_size (______gtk_text_buffer_get_btree (layout->buffer),
				layout,
                                 ...
                                 gint           y1)
{
   ...
   */
  line = ___gtk_text_iter_get_text_line (anchor);
  ...
  while (line && seen < -y0)
    {
	  ...
	  old_height = line_data ? line_data->height : 0;
          ...
          _gtk_text_btree_validate_line (______gtk_text_buffer_get_btree (layout->buffer),
                                         line, layout);
  ...
  if (first_line)
    {
      ...
      line_top = _gtk_text_btree_find_line_top (______gtk_text_buffer_get_btree (layout->buffer),
                                                first_line, layout);
                          ...
                          gint           max_pixels)
{
  ...
  while (max_pixels > 0 &&
         _gtk_text_btree_validate (______gtk_text_buffer_get_btree (layout->buffer),
                                   layout,  max_pixels,
	   ...
	   GPtrArray     *tags)
{
  ...
  */
  if (layout->one_style_cache != NULL)
    {
      __gtk_text_attributes_ref (layout->one_style_cache);
  ...
  if (tags == NULL || tags->len == 0)
    {
      ...
      __gtk_text_attributes_ref (layout->default_style);
      __gtk_text_attributes_ref (layout->default_style);
  ...
  __gtk_text_attributes_copy_values (layout->default_style,
                                   style);
  ...
  g_assert (layout->one_style_cache == NULL);
  __gtk_text_attributes_ref (style); /* ref held by layout->one_style_cache */
            ...
            gint                start)
{
   ...
   */
  if (_gtk_text_btree_mark_is_insert (______gtk_text_buffer_get_btree (layout->buffer),
                                     seg->body.mark.obj) &&
      (!layout->cursor_visible ||
       __gtk_text_buffer_get_selection_bounds (layout->buffer, NULL, NULL)))
    return;

  if (layout->overwrite_mode &&
      _gtk_text_btree_mark_is_insert (______gtk_text_buffer_get_btree (layout->buffer),
				      seg->body.mark.obj))
    {
      GtkTextIter iter;
      gboolean cursor_at_line_end;

      _gtk_text_btree_get_iter_at_mark (______gtk_text_buffer_get_btree (layout->buffer),
			    ...
			    const GtkTextIter  *iter)
{
  ...
  g_return_val_if_fail (___gtk_text_iter_get_text_line (iter) == display->line, 0);
			    ...
			    gint                trailing)
{
  g_return_if_fail (!_gtk_text_line_is_last (display->line,
                                             ______gtk_text_buffer_get_btree (layout->buffer)));
  ...
  if (___gtk_text_iter_get_text_line (iter) != display->line)
    {
               ...
               gint          *line_top)
{
  ...
  *line = _gtk_text_btree_find_line_by_y (______gtk_text_buffer_get_btree (layout->buffer),
                                         layout, y, line_top);
  if (*line == NULL)
    {
      *line = _gtk_text_btree_get_end_iter_line (______gtk_text_buffer_get_btree (layout->buffer));
      
      if (line_top)
        *line_top =
          _gtk_text_btree_find_line_top (______gtk_text_buffer_get_btree (layout->buffer),
                                        *line, layout);
    }
}
                                      ...
                                      GdkRectangle   *weak_pos)
{
  ...
  line = ___gtk_text_iter_get_text_line (iter);
  display = gtk_text_layout_get_line_display (layout, line, FALSE);
  ...
  __gtk_text_buffer_get_iter_at_mark (layout->buffer, &insert_iter,
                                    __gtk_text_buffer_get_insert (layout->buffer));
                                 ...
                                 gint              *height)
{
  ...
  g_return_if_fail (GTK_IS_TEXT_LAYOUT (layout));
  g_return_if_fail (_gtk_text_iter_get_btree (iter) == ______gtk_text_buffer_get_btree (layout->buffer));

  line = ___gtk_text_iter_get_text_line (iter);

  if (y)
    *y = _gtk_text_btree_find_line_top (______gtk_text_buffer_get_btree (layout->buffer),
                                       line, layout);
  if (height)
    {
      GtkTextLineData *line_data = _gtk_text_line_get_data (line, layout);
                         ...
                         gint           y)
{
  ...
  line = _gtk_text_btree_find_line_by_y (______gtk_text_buffer_get_btree (layout->buffer), layout, y, &line_top);
  if (!line)
    {
      line = _gtk_text_btree_get_end_iter_line (______gtk_text_buffer_get_btree (layout->buffer));
      
      line_top = _gtk_text_btree_find_line_top (______gtk_text_buffer_get_btree (layout->buffer), line, layout);
    }
  ...
  if (found_line)
    ...
    __gtk_text_buffer_get_iter_at_offset (layout->buffer, iter, 0);
}
                                        ...
                                        GtkTextIter   *iter)
{
  ...
  if (!found_after)
    __gtk_text_buffer_get_end_iter (layout->buffer, iter);
                                    ...
                                    gint           count)
{
  ...
  while (count != 0)
    {
      GtkTextLine *line = ___gtk_text_iter_get_text_line (iter);
      gint line_byte;
      gint extra_back = 0;
      gboolean strong;
