...
gtk_page_setup_unix_dialog_finalize (GObject *object)
{
  ...
  GtkPageSetupUnixDialogPrivate *priv = gtk_page_setup_unix_dialog_get_props (dialog)->priv;
			  ...
			  GtkPageSetupUnixDialog *dialog)
{
  ...
  if (__gtk_combo_box_get_active_iter (combo_box, &iter))
    {
      __gtk_tree_model_get (__gtk_combo_box_get_model (combo_box), &iter,
      ...
      if (priv->print_settings)
	{
	  ...
	  __gtk_print_settings_set (priv->print_settings,
				 ...
				 gpointer   user_data)
{
  ...
  GtkPageSetupUnixDialogPrivate *priv = gtk_page_setup_unix_dialog_get_props (page_setup_dialog)->priv;
		    ...
		    GtkPageSetupUnixDialog *dialog)
{
  ...
  if (__gtk_combo_box_get_active_iter (combo_box, &iter))
	...
	unit_str = _("inch");
      ...
      w = double_to_string (__gtk_page_setup_get_paper_width (page_setup, unit),
			    unit);
      h = double_to_string (__gtk_page_setup_get_paper_height (page_setup, unit),
			    unit);
      ...
      top = double_to_string (__gtk_page_setup_get_top_margin (page_setup, unit), unit);
      bottom = double_to_string (__gtk_page_setup_get_bottom_margin (page_setup, unit), unit);
      left = double_to_string (__gtk_page_setup_get_left_margin (page_setup, unit), unit);
      right = double_to_string (__gtk_page_setup_get_right_margin (page_setup, unit), unit);
      ...
      __gtk_widget_set_tooltip_text (priv->paper_size_label, str);
      g_free (str);
  ...
  else
    {
      ...
      __gtk_widget_set_tooltip_text (priv->paper_size_label, NULL);
      if (priv->last_setup)
	g_object_unref (priv->last_setup);
		     ...
		     const gchar *stock_id)
{
  ...
  radio_button = __gtk_radio_button_new (group);
  image = __gtk_image_new_from_stock (stock_id, GTK_ICON_SIZE_LARGE_TOOLBAR);
  __gtk_stock_lookup (stock_id, &item);
  label = __gtk_label_new (item.label);
  hbox = __gtk_hbox_new (0, 6);
  __gtk_container_add (GTK_CONTAINER (radio_button), hbox);
  __gtk_container_add (GTK_CONTAINER (hbox), image);
  __gtk_container_add (GTK_CONTAINER (hbox), label);

  __gtk_widget_show_all (radio_button);
...
{
  GtkPageSetupUnixDialogPrivate *priv = gtk_page_setup_unix_dialog_get_props (ps_dialog)->priv;
  ...
  table = __gtk_table_new (5, 4, FALSE);
  __gtk_table_set_row_spacings (GTK_TABLE (table), 6);
  __gtk_table_set_col_spacings (GTK_TABLE (table), 12);
  __gtk_container_set_border_width (GTK_CONTAINER (table), 5);
  __gtk_box_pack_start (GTK_BOX (gtk_dialog_get_props (dialog)->vbox), table, TRUE, TRUE, 0);
  __gtk_widget_show (table);

  label = __gtk_label_new_with_mnemonic (_("_Format for:"));
  ...
  __gtk_cell_layout_set_attributes (GTK_CELL_LAYOUT (combo), cell,
  ...
  label = __gtk_label_new_with_mnemonic (_("_Orientation:"));
  __gtk_misc_set_alignment (GTK_MISC (label), 0.0, 0.5);
  __gtk_table_attach (GTK_TABLE (table), label,
		    0, 1, 3, 4,
		    GTK_FILL, 0, 0, 0);
  ...
  __gtk_table_attach (GTK_TABLE (table), radio_button,
		    1, 2, 3, 4,
		    GTK_EXPAND|GTK_FILL, 0, 0, 0);
  __gtk_label_set_mnemonic_widget (GTK_LABEL (label), radio_button);

  radio_button = create_radio_button (__gtk_radio_button_get_group (GTK_RADIO_BUTTON (radio_button)),
  ...
  __gtk_table_attach (GTK_TABLE (table), radio_button,
		    1, 2, 4, 5,
		    GTK_EXPAND|GTK_FILL, 0, 0, 0);
  __gtk_widget_show (radio_button);

  __gtk_table_set_row_spacing (GTK_TABLE (table), 3, 0);
				...
				GtkWindow   *parent)
{
  ...
  if (parent)
    __gtk_window_set_transient_for (GTK_WINDOW (result), parent);
...
get_orientation (GtkPageSetupUnixDialog *dialog)
{
  ...
  if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->portrait_radio)))
    return GTK_PAGE_ORIENTATION_PORTRAIT;
  if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->landscape_radio)))
    return GTK_PAGE_ORIENTATION_LANDSCAPE;
  if (__gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->reverse_landscape_radio)))
		 ...
		 GtkPageOrientation      orientation)
{
  ...
  switch (orientation)
    ...
    case GTK_PAGE_ORIENTATION_REVERSE_PORTRAIT:
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->reverse_portrait_radio), TRUE);
      break;
    case GTK_PAGE_ORIENTATION_PORTRAIT:
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->portrait_radio), TRUE);
      break;
    case GTK_PAGE_ORIENTATION_LANDSCAPE:
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->landscape_radio), TRUE);
      break;
    case GTK_PAGE_ORIENTATION_REVERSE_LANDSCAPE:
      __gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (priv->reverse_landscape_radio), TRUE);
      break;
    }
}
  ...
  if (page_setup)
    {
      ...
      set_orientation (dialog, __gtk_page_setup_get_orientation (page_setup));
    }
}
...
gtk_page_setup_unix_dialog_get_page_setup (GtkPageSetupUnixDialog *dialog)
{
  ...
  __gtk_page_setup_set_orientation (page_setup, get_orientation (dialog));
  ...
  if (print_settings)
    {
      ...
      format_for_printer = __gtk_print_settings_get (print_settings, "format-for-printer");
