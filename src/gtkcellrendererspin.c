  ...
  if (!canceled)
    {
      ...
      new_text = __gtk_entry_get_text (GTK_ENTRY (widget));
					...
					gpointer     data)
{
  if (event->state == 0)
	...
	{
	  __gtk_spin_button_spin (GTK_SPIN_BUTTON (widget), GTK_SPIN_STEP_FORWARD, 1);
	  return TRUE;
	}
      else if (event->keyval == GDK_Down)
	{
	  __gtk_spin_button_spin (GTK_SPIN_BUTTON (widget), GTK_SPIN_STEP_BACKWARD, 1);
	  return TRUE;
	}
    }
				      ...
				      GtkCellRendererState flags)
{
  ...
  spin = __gtk_spin_button_new (priv->adjustment,
  ...
  if (gtk_cell_renderer_text_get_props (cell_text)->text)
    __gtk_spin_button_set_value (GTK_SPIN_BUTTON (spin),
			       g_ascii_strtod (gtk_cell_renderer_text_get_props (cell_text)->text, NULL));
  ...
  __gtk_widget_show (spin);

  return GTK_CELL_EDITABLE (spin);
}

/**
 * __gtk_cell_renderer_spin_new:
 *
 ...
 */
GtkCellRenderer *
__gtk_cell_renderer_spin_new (void)
{
