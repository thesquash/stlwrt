...
static gboolean      __gtk_drag_highlight_expose   (GtkWidget      *widget,
...
gtk_drag_get_ipc_widget_for_screen (GdkScreen *screen)
{
  ...
  if (drag_widgets)
    ...
    {
      result = __gtk_window_new (GTK_WINDOW_POPUP);
      __gtk_window_set_screen (GTK_WINDOW (result), screen);
      __gtk_window_resize (GTK_WINDOW (result), 1, 1);
      __gtk_window_move (GTK_WINDOW (result), -100, -100);
      __gtk_widget_show (result);
    }  
...
gtk_drag_get_ipc_widget (GtkWidget *widget)
{
  ...
  result = gtk_drag_get_ipc_widget_for_screen (__gtk_widget_get_screen (widget));
               ...
               guint32    time)
{
  ...
  root = __gdk_screen_get_root_window (__gtk_widget_get_screen (widget));
  ...
  __gdk_window_add_filter (NULL, root_key_filter, (gpointer) GDK_WINDOW_XID (window));
}
                 ...
                 guint32    time)
{
  ...
  __gdk_window_remove_filter (NULL, root_key_filter, (gpointer) GDK_WINDOW_XID (window));
...
grab_dnd_keys (GtkWidget *widget,
               guint32    time)
{
  __gdk_keyboard_grab (widget->window, FALSE, time);
}
...
ungrab_dnd_keys (GtkWidget *widget,
                 guint32    time)
{
  __gdk_display_keyboard_ungrab (__gtk_widget_get_display (widget), time);
}
			      ...
			      gint        height)
{
  ...
  if (!__gdk_display_supports_cursor_color (display))
    return FALSE;

  if (!__gdk_display_supports_cursor_alpha (display))
    return FALSE;

  __gdk_display_get_maximal_cursor_size (display, 
                                       &max_width,
                                       &max_height);
		     ...
		     GtkDragSourceInfo *info)
{
  ...
  if (!info)
	...
	if (drag_cursors[i].cursor != NULL)
	  {
	    __gdk_cursor_unref (drag_cursors[i].cursor);
  ...
  if (drag_cursors[i].cursor != NULL)
    {
      if (display != __gdk_cursor_get_display (drag_cursors[i].cursor))
	{
	  __gdk_cursor_unref (drag_cursors[i].cursor);
	  drag_cursors[i].cursor = NULL;
	}
  ...
  if (drag_cursors[i].cursor == NULL)
    drag_cursors[i].cursor = __gdk_cursor_new_from_name (display, drag_cursors[i].name);
  
  if (drag_cursors[i].cursor == NULL)
    drag_cursors[i].cursor = __gdk_cursor_new_from_pixbuf (display, drag_cursors[i].pixbuf, 0, 0);
  ...
  if (info && info->icon_pixbuf) 
    {
      ...
      if (info->drag_cursors[i] != NULL)
        {
          if (display == __gdk_cursor_get_display (info->drag_cursors[i]))
	    return info->drag_cursors[i];
	  
	  __gdk_cursor_unref (info->drag_cursors[i]);
      ...
      hot_x = hot_y = 0;
      cursor_pixbuf = __gdk_cursor_get_image (drag_cursors[i].cursor);
      if (!cursor_pixbuf)
      ...
      if (gtk_drag_can_use_rgba_cursor (display, width, height))
	{
	  ...
	  info->drag_cursors[i] = 
	    __gdk_cursor_new_from_pixbuf (display, pixbuf, ref_x, ref_y);
...
gtk_drag_update_cursor (GtkDragSourceInfo *info)
{
  ...
  cursor = gtk_drag_get_cursor (__gdk_cursor_get_display (info->cursor), 
				drag_cursors[i].action, info);
  
  if (cursor != info->cursor)
    {
      __gdk_pointer_grab (info->ipc_widget->window, FALSE,
...
void 
__gtk_drag_get_data (GtkWidget      *widget,
		   GdkDragContext *context,
		   GdkAtom         target,
		   guint32         time)
{
  ...
  __gtk_selection_convert (selection_widget,
			 __gdk_drag_get_selection (context),
			 target,
			 time);
}
 ...
 */
GtkWidget *
__gtk_drag_get_source_widget (GdkDragContext *context)
{
  GSList *tmp_list;
  ...
  while (tmp_list)
    {
      ...
      if (__gtk_widget_get_window (ipc_widget) == __gdk_drag_context_get_source_window (context))
	{
	  GtkDragSourceInfo *info;
...
void 
__gtk_drag_finish (GdkDragContext *context,
		 gboolean        success,
		 gboolean        del,
		 guint32         time)
{
  ...
  if (success && del)
    {
      target = __gdk_atom_intern_static_string ("DELETE");
    }
  else if (__gdk_drag_context_get_protocol (context) == GDK_DRAG_PROTO_MOTIF)
    {
      target = __gdk_atom_intern_static_string (success ? 
					      "XmTRANSFER_SUCCESS" : 
					      "XmTRANSFER_FAILURE");
    }
  ...
  if (target != GDK_NONE)
    {
      GtkWidget *selection_widget = gtk_drag_get_ipc_widget_for_screen (__gdk_window_get_screen (__gdk_drag_context_get_source_window (context)));
  ...
  if (!(success && del))
    __gdk_drop_finish (context, success, time);
}

/*************************************************************
 * __gtk_drag_highlight_expose:
...
static gboolean
__gtk_drag_highlight_expose (GtkWidget      *widget,
			   GdkEventExpose *event,
			   gpointer        data)
{
  ...
  if (__gtk_widget_is_drawable (widget))
      ...
      else
	{
	  x = 0;
	  y = 0;
          width = __gdk_window_get_width (widget->window);
          height = __gdk_window_get_height (widget->window);
	}
      
      __gtk_paint_shadow (widget->style, widget->window,
...
void 
__gtk_drag_highlight (GtkWidget  *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));

  g_signal_connect_after (widget, "expose-event",
			  G_CALLBACK (__gtk_drag_highlight_expose),
			  NULL);
...
void 
__gtk_drag_unhighlight (GtkWidget *widget)
{
  g_return_if_fail (GTK_IS_WIDGET (widget));
...
static void
__gtk_drag_dest_set_internal (GtkWidget       *widget,
 ...
 *     Later you can access the list with __gtk_drag_dest_get_target_list()
 *     and __gtk_drag_dest_find_target().
 ...
 * invokations of __gdk_drag_status() in the context of #GtkWidget:drag-motion,
 * and invokations of __gtk_drag_finish() in #GtkWidget:drag-data-received.
 * Especially the later is dramatic, when your own #GtkWidget:drag-motion
 * handler calls __gtk_drag_get_data() to inspect the dragged data.
 *
 ...
 *
 *   __gdk_window_get_pointer (__gtk_widget_get_window (widget),
 *                           NULL, NULL, &mask);
 *   if (mask & GDK_CONTROL_MASK)
 *     __gdk_drag_status (context, GDK_ACTION_COPY, time);
 *   else
 ...
 */
void
__gtk_drag_dest_set (GtkWidget            *widget,
		   GtkDestDefaults       flags,
		   ...
		   GdkDragAction         actions)
{
  ...
  if (targets)
    site->target_list = __gtk_target_list_new (targets, n_targets);
  else
  ...
  __gtk_drag_dest_set_internal (widget, site);
}

/*************************************************************
 * __gtk_drag_dest_set_proxy:
...
void 
__gtk_drag_dest_set_proxy (GtkWidget      *widget,
			 GdkWindow      *proxy_window,
...
void 
__gtk_drag_dest_unset (GtkWidget *widget)
{
  GtkDragDestSite *old_site;
...
/**
 * __gtk_drag_dest_get_target_list:
 * @widget: a #GtkWidget
 * 
 ...
 **/
GtkTargetList*
__gtk_drag_dest_get_target_list (GtkWidget *widget)
{
  GtkDragDestSite *site;
...
/**
 * __gtk_drag_dest_set_target_list:
 ...
 **/
void
__gtk_drag_dest_set_target_list (GtkWidget      *widget,
                               GtkTargetList  *target_list)
{
  GtkDragDestSite *site;
  ...
  if (!site)
    {
      g_warning ("Can't set a target list on a widget until you've called __gtk_drag_dest_set() "
                 "to make the widget into a drag destination");
      return;
    }

  if (target_list)
    __gtk_target_list_ref (target_list);
  
  if (site->target_list)
    __gtk_target_list_unref (site->target_list);

  site->target_list = target_list;
}

/**
 * __gtk_drag_dest_add_text_targets:
 * @widget: a #GtkWidget that's a drag destination
 *
 * Add the text targets supported by #GtkSelection to
 * the target list of the drag destination. The targets
 * are added with @info = 0. If you need another value, 
 * use __gtk_target_list_add_text_targets() and
 * __gtk_drag_dest_set_target_list().
 * 
 * Since: 2.6
 **/
void
__gtk_drag_dest_add_text_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;

  target_list = __gtk_drag_dest_get_target_list (widget);
  if (target_list)
    __gtk_target_list_ref (target_list);
  else
...
/**
 * __gtk_drag_dest_add_image_targets:
 * @widget: a #GtkWidget that's a drag destination
 *
 * Add the image targets supported by #GtkSelection to
 * the target list of the drag destination. The targets
 * are added with @info = 0. If you need another value, 
 * use __gtk_target_list_add_image_targets() and
 * __gtk_drag_dest_set_target_list().
 * 
 * Since: 2.6
 **/
void
__gtk_drag_dest_add_image_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;
  ...
  if (target_list)
    ...
    target_list = __gtk_target_list_new (NULL, 0);
  __gtk_target_list_add_image_targets (target_list, 0, FALSE);
  __gtk_drag_dest_set_target_list (widget, target_list);
  __gtk_target_list_unref (target_list);
}

/**
 * __gtk_drag_dest_add_uri_targets:
 * @widget: a #GtkWidget that's a drag destination
 *
 * Add the URI targets supported by #GtkSelection to
 * the target list of the drag destination. The targets
 * are added with @info = 0. If you need another value, 
 * use __gtk_target_list_add_uri_targets() and
 * __gtk_drag_dest_set_target_list().
 * 
 * Since: 2.6
 **/
void
__gtk_drag_dest_add_uri_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;
  ...
  if (target_list)
    ...
    target_list = __gtk_target_list_new (NULL, 0);
  __gtk_target_list_add_uri_targets (target_list, 0);
  __gtk_drag_dest_set_target_list (widget, target_list);
  __gtk_target_list_unref (target_list);
}

/**
 * __gtk_drag_dest_set_track_motion:
 * @widget: a #GtkWidget that's a drag destination
 ...
 **/
void
__gtk_drag_dest_set_track_motion (GtkWidget *widget,
...
/**
 * __gtk_drag_dest_get_track_motion:
 * @widget: a #GtkWidget that's a drag destination
 * 
 ...
 **/
gboolean
__gtk_drag_dest_get_track_motion (GtkWidget *widget)
{
  GtkDragDestSite *site;
...
/*************************************************************
 * ______gtk_drag_dest_handle_event:
...
void
______gtk_drag_dest_handle_event (GtkWidget *toplevel,
			    GdkEvent  *event)
{
  GtkDragDestInfo *info;
  GdkDragContext *context;
  ...
  switch (event->type)
	 ...
	 *
	 * Currently __gdk_window_get_position doesn't provide reliable
	 * information for embedded windows, so we call the much more
	 * expensive __gdk_window_get_origin().
	 */
	if (GTK_IS_PLUG (toplevel))
	  __gdk_window_get_origin (toplevel->window, &tx, &ty);
	else
#endif /* GDK_WINDOWING_X11 */
	  __gdk_window_get_position (toplevel->window, &tx, &ty);
	...
	if (event->type == GDK_DRAG_MOTION)
	  {
	    if (!found)
	      __gdk_drag_status (context, 0, event->dnd.time);
	  }
	else if (event->type == GDK_DROP_START && !info->proxy_source)
	  {
	    __gdk_drop_reply (context, found, event->dnd.time);
            if ((__gdk_drag_context_get_protocol (context) == GDK_DRAG_PROTO_MOTIF) && !found)
	      __gtk_drag_finish (context, FALSE, FALSE, event->dnd.time);
	  }
      }
      break;
 ...
 * value from __gtk_drag_dest_get_target_list(), but some widgets may
 ...
 **/
GdkAtom
__gtk_drag_dest_find_target (GtkWidget      *widget,
                           GdkDragContext *context,
                           GtkTargetList  *target_list)
{
  GList *tmp_target;
  ...
  while (tmp_target)
    {
      ...
      tmp_source = __gdk_drag_context_list_targets (context);
      while (tmp_source)
	{
			     ...
			     gpointer          data)
{
  ...
  if (info->proxy_data && 
    ...
    {
      __gtk_selection_data_set (info->proxy_data,
			      selection_data->type,
  ...
  if (selection_data->target == __gdk_atom_intern_static_string ("DELETE"))
    {
      __gtk_drag_finish (context, TRUE, FALSE, time);
    }
  else if ((selection_data->target == __gdk_atom_intern_static_string ("XmTRANSFER_SUCCESS")) ||
	   (selection_data->target == __gdk_atom_intern_static_string ("XmTRANSFER_FAILURE")))
    {
      /* Do nothing */
    }
  else
      ...
      if (site && site->target_list)
	{
	  ...
	  if (__gtk_target_list_find (site->target_list, 
                      ...
                      GtkDragDestCallback  callback)
{
  if (!__gtk_widget_get_mapped (widget) ||
      !__gtk_widget_get_sensitive (widget))
    return FALSE;

  /* Get the widget at the pointer coordinates and travel up
   * the widget hierarchy from there.
   */
  widget = ______gtk_widget_find_at_coords (__gtk_widget_get_window (widget),
                                       x, y, &x, &y);
  if (!widget)
    return FALSE;
  ...
  while (widget)
      ...
      if (g_object_get_data (G_OBJECT (widget), "gtk-drag-dest"))
	{
      ...
      if (!found)
        {
           ...
           */
          parent = __gtk_widget_get_parent (widget);
      ...
      if (!__gtk_widget_translate_coordinates (widget, parent, x, y, &x, &y))
        return FALSE;
		      ...
		      guint32          time)
{
  ...
  if (dest_info->proxy_source)
    {
      __gdk_drag_abort (dest_info->proxy_source->context, time);
      gtk_drag_source_info_destroy (dest_info->proxy_source);
      dest_info->proxy_source = NULL;
    }
  
  ipc_widget = gtk_drag_get_ipc_widget (widget);
  context = __gdk_drag_begin (__gtk_widget_get_window (ipc_widget),
			    __gdk_drag_context_list_targets (dest_info->context));
  ...
  source_info->target_list = __gtk_target_list_new (NULL, 0);
  tmp_list = __gdk_drag_context_list_targets (dest_info->context);
  while (tmp_list)
    {
      __gtk_target_list_add (source_info->target_list,
...
gtk_drag_dest_realized (GtkWidget *widget)
{
  ...
  if (__gtk_widget_is_toplevel (toplevel))
    __gdk_window_register_dnd (toplevel->window);
}
				 ...
				 GtkWidget *previous_toplevel)
{
  ...
  if (__gtk_widget_is_toplevel (toplevel) && __gtk_widget_get_realized (toplevel))
    __gdk_window_register_dnd (toplevel->window);
}
		     ...
		     guint           time)
{
  ...
  if (site->do_proxy)
	...
	{
	  __gdk_drag_abort (info->proxy_source->context, time);
		      ...
		      guint           time)
{
  ...
  if (site->do_proxy)
	...
	{
	  __gdk_drag_find_window_for_screen (info->proxy_source->context,
					   NULL,
					   __gdk_window_get_screen (current_event->dnd.window),
					   current_event->dnd.x_root, 
					   current_event->dnd.y_root,
					   &dest_window, &proto);
	}
      
      __gdk_drag_motion (info->proxy_source->context, 
		       dest_window, proto,
		       current_event->dnd.x_root, 
		       current_event->dnd.y_root, 
                       __gdk_drag_context_get_suggested_action (context),
                       __gdk_drag_context_get_actions (context),
                       time);

      if (!site->proxy_window && dest_window)
	g_object_unref (dest_window);

      selection = __gdk_drag_get_selection (info->proxy_source->context);
      if (selection && 
	  selection != __gdk_drag_get_selection (info->context))
  ...
  if (site->track_motion || site->flags & GTK_DEST_DEFAULT_MOTION)
    {
      if (__gdk_drag_context_get_suggested_action (context) & site->actions)
	action = __gdk_drag_context_get_suggested_action (context);
      else
	{
	  gint i;
	  ...
	  for (i = 0; i < 8; i++)
	    {
	      if ((site->actions & (1 << i)) &&
		  (__gdk_drag_context_get_actions (context) & (1 << i)))
		{
		  action = (1 << i);
		  break;
		}
      ...
      if (action && __gtk_drag_dest_find_target (widget, context, NULL))
	{
	  if (!site->have_drag)
	    {
	      site->have_drag = TRUE;
  ...
  if (site->do_proxy)
    {
	  ...
	  (__gdk_drag_context_get_protocol (info->context) == GDK_DRAG_PROTO_ROOTWIN))
	{
  ...
  else
    {
      ...
      if (site->flags & GTK_DEST_DEFAULT_DROP)
	{
	  GdkAtom target = __gtk_drag_dest_find_target (widget, context, NULL);
...
__gtk_drag_begin_internal (GtkWidget         *widget,
			 ...
			 GdkEvent          *event)
{
  ...
  cursor = gtk_drag_get_cursor (__gtk_widget_get_display (widget), 
			        suggested_action,
				NULL);
  
  if (event)
    {
      time = __gdk_event_get_time (event);
      if (time == GDK_CURRENT_TIME)
        time = __gtk_get_current_event_time ();
    }

  if (__gdk_pointer_grab (ipc_widget->window, FALSE,
			GDK_POINTER_MOTION_MASK |
  ...
  context = __gdk_drag_begin (ipc_widget->window, targets);
  g_list_free (targets);
  ...
  if (event && event->type == GDK_MOTION_NOTIFY)
    {
      info->cur_screen = __gtk_widget_get_screen (widget);
      info->cur_x = event->motion.x_root;
      info->cur_y = event->motion.y_root;
    }
  else 
    {
      __gdk_display_get_pointer (__gtk_widget_get_display (widget),
  ...
  if (!info->icon_window && !info->icon_pixbuf)
	...
	switch (site->icon_type)
	  ...
	  case GTK_IMAGE_PIXBUF:
	    __gtk_drag_set_icon_pixbuf (context,
				      site->icon_data.pixbuf.pixbuf,
				      -2, -2);
	    break;
	  case GTK_IMAGE_STOCK:
	    __gtk_drag_set_icon_stock (context,
				     site->icon_data.stock.stock_id,
				     -2, -2);
	    break;
	  case GTK_IMAGE_ICON_NAME:
	    __gtk_drag_set_icon_name (context,
  ...
  if (info->icon_pixbuf)  
    {
      ...
      if (cursor != info->cursor)
        {
	  __gdk_pointer_grab (widget->window, FALSE,
	 	            GDK_POINTER_MOTION_MASK |
		  	    GDK_BUTTON_RELEASE_MASK,
			    NULL,
			    cursor, time);
 ...
 * from the mouse, using __gdk_event_copy(), and pass it to this function
 * (remember to free the event with __gdk_event_free() when you are done).  If you
 * can really not pass a real event, pass #NULL instead.
 * 
 * Return value: (transfer none): the context for this drag.
 **/
GdkDragContext *
__gtk_drag_begin (GtkWidget         *widget,
		...
		GdkEvent          *event)
{
  ...
  return __gtk_drag_begin_internal (widget, NULL, targets,
				  actions, button, event);
}

/**
 * __gtk_drag_source_set:
 * @widget: a #GtkWidget
 ...
 */
void
__gtk_drag_source_set (GtkWidget            *widget,
		     GdkModifierType       start_button_mask,
		     const GtkTargetEntry *targets,
		     gint                  n_targets,
		     GdkDragAction         actions)
...
void 
__gtk_drag_source_unset (GtkWidget        *widget)
{
  GtkDragSourceSite *site;
...
/**
 * __gtk_drag_source_get_target_list:
 * @widget: a #GtkWidget
 *
 ...
 **/
GtkTargetList *
__gtk_drag_source_get_target_list (GtkWidget *widget)
{
  GtkDragSourceSite *site;
...
/**
 * __gtk_drag_source_set_target_list:
 ...
 **/
void
__gtk_drag_source_set_target_list (GtkWidget     *widget,
                                 GtkTargetList *target_list)
{
  GtkDragSourceSite *site;
  ...
  if (site == NULL)
    {
      g_warning ("__gtk_drag_source_set_target_list() requires the widget "
...
/**
 * __gtk_drag_source_add_text_targets:
 ...
 **/
void
__gtk_drag_source_add_text_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;

  target_list = __gtk_drag_source_get_target_list (widget);
  if (target_list)
    __gtk_target_list_ref (target_list);
  else
    target_list = __gtk_target_list_new (NULL, 0);
  __gtk_target_list_add_text_targets (target_list, 0);
  __gtk_drag_source_set_target_list (widget, target_list);
  __gtk_target_list_unref (target_list);
}

/**
 * __gtk_drag_source_add_image_targets:
 * @widget: a #GtkWidget that's is a drag source
 *
 ...
 **/
void
__gtk_drag_source_add_image_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;
  ...
  if (target_list)
    ...
    target_list = __gtk_target_list_new (NULL, 0);
  __gtk_target_list_add_image_targets (target_list, 0, TRUE);
  __gtk_drag_source_set_target_list (widget, target_list);
  __gtk_target_list_unref (target_list);
}

/**
 * __gtk_drag_source_add_uri_targets:
 * @widget: a #GtkWidget that's is a drag source
 *
 * Add the URI targets supported by #GtkSelection to
 * the target list of the drag source.  The targets
 ...
 **/
void
__gtk_drag_source_add_uri_targets (GtkWidget *widget)
{
  GtkTargetList *target_list;
...
static void
__gtk_drag_source_unset_icon (GtkDragSourceSite *site)
{
  switch (site->icon_type)
    {
    case GTK_IMAGE_EMPTY:
 ...
 * Use __gtk_drag_source_set_icon_pixbuf() instead.
 **/
void 
__gtk_drag_source_set_icon (GtkWidget     *widget,
			  ...
			  GdkBitmap     *mask)
{
  ...
  __gtk_drag_source_unset_icon (site);
...
/**
 * __gtk_drag_source_set_icon_pixbuf:
 * @widget: a #GtkWidget
 ...
 **/
void 
__gtk_drag_source_set_icon_pixbuf (GtkWidget   *widget,
				 GdkPixbuf   *pixbuf)
{
  GtkDragSourceSite *site;
...
/**
 * __gtk_drag_source_set_icon_stock:
 * @widget: a #GtkWidget
 ...
 **/
void 
__gtk_drag_source_set_icon_stock (GtkWidget   *widget,
				const gchar *stock_id)
{
  GtkDragSourceSite *site;
...
/**
 * __gtk_drag_source_set_icon_name:
 * @widget: a #GtkWidget
 ...
 **/
void 
__gtk_drag_source_set_icon_name (GtkWidget   *widget,
		   ...
		   gint              *hot_y)
{
  if (get_can_change_screen (info->icon_window))
    __gtk_window_set_screen (GTK_WINDOW (info->icon_window),
			   info->cur_screen);
      
  if (__gtk_widget_get_screen (info->icon_window) != info->cur_screen)
    {
      if (!info->fallback_icon)
	{
	  ...
	  if (!default_icon_pixmap)
	    ...
	    __gtk_drag_set_icon_pixmap (info->context, 
      ...
      __gtk_widget_hide (info->icon_window);
      
      *icon_window = info->fallback_icon;
      __gtk_window_set_screen (GTK_WINDOW (*icon_window), info->cur_screen);
    ...
    {
      if (info->fallback_icon)
	__gtk_widget_hide (info->fallback_icon);
  ...
  if (info->icon_window)
    {
      ...
      __gtk_window_move (GTK_WINDOW (icon_window), 
		       info->cur_x - hot_x, 
		       info->cur_y - hot_y);

      if (__gtk_widget_get_visible (icon_window))
	__gdk_window_raise (icon_window->window);
      else
	__gtk_widget_show (icon_window);
    }
}
 ...
 **/
void 
__gtk_drag_set_icon_widget (GdkDragContext    *context,
			  GtkWidget         *widget,
			  gint               hot_x,
			  gint               hot_y)
{
		     ...
		     GdkPixbuf *pixbuf)
{
  ...
  __gdk_pixbuf_render_pixmap_and_mask_for_colormap (pixbuf,
						  __gtk_widget_get_colormap (window),
						  &pixmap, &mask, 128);
  
  __gdk_window_set_back_pixmap (window->window, pixmap, FALSE);
		       ...
		       gboolean           force_window)
{
  ...
  screen = __gdk_window_get_screen (__gdk_drag_context_get_source_window (context));

  /* Push a NULL colormap to guard against __gtk_widget_push_colormap() */
  __gtk_widget_push_colormap (NULL);
  window = __gtk_window_new (GTK_WINDOW_POPUP);
  __gtk_window_set_type_hint (GTK_WINDOW (window), GDK_WINDOW_TYPE_HINT_DND);
  __gtk_window_set_screen (GTK_WINDOW (window), screen);
  set_can_change_screen (window, TRUE);
  __gtk_widget_pop_colormap ();

  __gtk_widget_set_events (window, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
  __gtk_widget_set_app_paintable (window, TRUE);

  if (stock_id)
    {
      pixbuf = __gtk_widget_render_icon (window, stock_id,
				       GTK_ICON_SIZE_DND, NULL);

      if (!pixbuf)
	{
	  g_warning ("Cannot load drag icon from stock_id %s", stock_id);
	  __gtk_widget_destroy (window);
	  return;
	}
  ...
  display = __gdk_window_get_display (__gdk_drag_context_get_source_window (context));
 ...
 **/
void 
__gtk_drag_set_icon_pixbuf  (GdkDragContext *context,
			   GdkPixbuf      *pixbuf,
			   gint            hot_x,
			   gint            hot_y)
{
 ...
 **/
void 
__gtk_drag_set_icon_stock  (GdkDragContext *context,
			  const gchar    *stock_id,
			  gint            hot_x,
			  gint            hot_y)
{
 ...
 * they are no longer needed. In general, __gtk_drag_set_icon_pixbuf()
 * will be more convenient to use.
 **/
void 
__gtk_drag_set_icon_pixmap (GdkDragContext    *context,
			  GdkColormap       *colormap,
			  GdkPixmap         *pixmap,
			  GdkBitmap         *mask,
			  gint               hot_x,
			  gint               hot_y)
{
  ...
  __gtk_widget_set_events (window, GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK);
  __gtk_widget_set_app_paintable (GTK_WIDGET (window), TRUE);
 ...
 **/
void 
__gtk_drag_set_icon_name (GdkDragContext *context,
			const gchar    *icon_name,
			gint            hot_x,
			gint            hot_y)
{
  ...
  if (__gtk_icon_size_lookup_for_settings (settings,
					 GTK_ICON_SIZE_DND,
					 &width, &height))
  ...
  pixbuf = __gtk_icon_theme_load_icon (icon_theme, icon_name,
		  		     icon_size, 0, NULL);
  if (pixbuf)
    set_icon_stock_pixbuf (context, NULL, pixbuf, hot_x, hot_y, FALSE);
  else
 ...
 **/
void 
__gtk_drag_set_icon_default (GdkDragContext *context)
{
  g_return_if_fail (GDK_IS_DRAG_CONTEXT (context));

  if (!default_icon_pixmap)
    __gtk_drag_set_icon_stock (context, GTK_STOCK_DND, -2, -2);
  else
    __gtk_drag_set_icon_pixmap (context, 
			      default_icon_colormap, 
			      default_icon_pixmap, 
 ...
 **/
void 
__gtk_drag_set_default_icon (GdkColormap   *colormap,
			   GdkPixmap     *pixmap,
			   GdkBitmap     *mask,
			   gint           hot_x,
			   gint           hot_y)
...
/*************************************************************
 * ______gtk_drag_source_handle_event:
 *     Called from widget event handling code on Drag events
 *     for drag sources.
 *
 *   arguments:
...
void
______gtk_drag_source_handle_event (GtkWidget *widget,
			       GdkEvent  *event)
{
  GtkDragSourceInfo *info;
  GdkDragContext *context;
	...
	if (info->proxy_dest)
	  {
		...
		if (info->proxy_dest->proxy_drop_wait)
		  {
		    gboolean result = __gdk_drag_context_get_selected_action (context) != 0;
		    
		    /* Aha - we can finally pass the MOTIF DROP on... */
		    __gdk_drop_reply (info->proxy_dest->context, result, info->proxy_dest->proxy_drop_time);
		    if (result)
		      __gdk_drag_drop (info->context, info->proxy_dest->proxy_drop_time);
		    else
		      __gtk_drag_finish (info->proxy_dest->context, FALSE, FALSE, info->proxy_dest->proxy_drop_time);
		  }
		else
		  {
		    __gdk_drag_status (info->proxy_dest->context,
				     __gdk_drag_context_get_selected_action (event->dnd.context),
				     event->dnd.time);
		  }
	      }
	  }
				 ...
				 guint32            time)
{
  ...
  __gtk_selection_owner_set_for_display (__gtk_widget_get_display (info->widget),
				       info->ipc_widget,
				       selection,
				       time);
  ...
  while (tmp_list)
    {
      ...
      __gtk_selection_add_target (info->ipc_widget,
				selection,
				pair->target,
				pair->info);
      tmp_list = tmp_list->next;
  ...
  if (__gdk_drag_context_get_protocol (info->context) == GDK_DRAG_PROTO_MOTIF)
    {
      __gtk_selection_add_target (info->ipc_widget,
				selection,
				__gdk_atom_intern_static_string ("XmTRANSFER_SUCCESS"),
				TARGET_MOTIF_SUCCESS);
      __gtk_selection_add_target (info->ipc_widget,
				selection,
				__gdk_atom_intern_static_string ("XmTRANSFER_FAILURE"),
				TARGET_MOTIF_FAILURE);
    }

  __gtk_selection_add_target (info->ipc_widget,
			    selection,
			    __gdk_atom_intern_static_string ("DELETE"),
			    TARGET_DELETE);
}
  ...
  if (info->proxy_dest)
    {
      ...
      __gtk_drag_finish (info->proxy_dest->context, success, FALSE, 
      ...
      else
	{
	  ...
	  info->cur_screen = __gtk_widget_get_screen (info->widget);
	  ...
	  __gdk_threads_add_timeout (ANIM_STEP_TIME, gtk_drag_anim_timeout, anim);
	}
    }
}
...
gtk_drag_source_release_selections (GtkDragSourceInfo *info,
				    guint32            time)
{
  GdkDisplay *display = __gtk_widget_get_display (info->widget);
  GList *tmp_list = info->selections;
  
  while (tmp_list)
    {
      GdkAtom selection = GDK_POINTER_TO_ATOM (tmp_list->data);
      if (__gdk_selection_owner_get_for_display (display, selection) == info->ipc_widget->window)
...
gtk_drag_drop (GtkDragSourceInfo *info, 
	       guint32            time)
{
  if (__gdk_drag_context_get_protocol (info->context) == GDK_DRAG_PROTO_ROOTWIN)
    {
      GtkSelectionData selection_data;
      GList *tmp_list;
       ...
       */
      GdkAtom target1 = __gdk_atom_intern_static_string ("application/x-rootwindow-drop");
      GdkAtom target2 = __gdk_atom_intern_static_string ("application/x-rootwin-drop");
	...
	__gtk_widget_hide (info->icon_window);
      ...
      __gdk_drag_drop (info->context, time);
      info->drop_timeout = __gdk_threads_add_timeout (DROP_ABORT_TIME,
			  ...
			  gpointer        data)
{
  ...
  switch (event->type)
      ...
      if (site->state & event->motion.state & site->start_button_mask)
	{
	  ...
	  if (__gtk_drag_check_threshold (widget, site->x, site->y,
					event->motion.x, event->motion.y))
	    {
	      site->state = 0;
	      __gtk_drag_begin_internal (widget, site, site->target_list,
				       site->actions, 
				       i, event);
			...
			gpointer          data)
{
  ...
  if (!null_atom)
    null_atom = __gdk_atom_intern_static_string ("NULL");
  ...
  switch (sel_info)
    ...
    case TARGET_DELETE:
      g_signal_emit_by_name (info->widget,
			     "drag-data-delete", 
			     info->context);
      __gtk_selection_data_set (selection_data, null_atom, 8, NULL, 0);
      break;
      ...
      if (info->proxy_dest)
	{
	  ...
	  __gtk_drag_get_data (info->widget,
	...
	{
	  if (__gtk_target_list_find (info->target_list, 
				    selection_data->target, 
				    &target_info))
	    {
	      g_signal_emit_by_name (info->widget, "drag-data-get",
...
gtk_drag_remove_icon (GtkDragSourceInfo *info)
{
  if (info->icon_window)
	...
	__gtk_widget_destroy (info->icon_window);

      if (info->fallback_icon)
	{
	  __gtk_widget_destroy (info->fallback_icon);
...
gtk_drag_source_info_destroy (GtkDragSourceInfo *info)
{
  ...
  __gtk_selection_remove_all (info->ipc_widget);
  ...
  __gtk_target_list_unref (info->target_list);
  ...
  if (info->last_event)
    {
      ...
      if (!__gdk_drag_motion (info->context, dest_window, protocol,
			    info->cur_x, info->cur_y, action, 
			    possible_actions,
			    time))
	{
	  __gdk_event_free ((GdkEvent *)info->last_event);
	  info->last_event = NULL;
	}
...
gtk_drag_add_update_idle (GtkDragSourceInfo *info)
{
   ...
   */
  if (!info->update_idle)
    info->update_idle = __gdk_threads_add_idle_full (GDK_PRIORITY_REDRAW + 5,
		 ...
		 GdkEvent          *event)
{
  ...
  if (event)
    info->last_event = __gdk_event_copy ((GdkEvent *)event);
...
gtk_drag_end (GtkDragSourceInfo *info, guint32 time)
{
  ...
  GdkDisplay *display = __gtk_widget_get_display (source_widget);
  ...
  if (info->last_event)
    {
      __gdk_event_free (info->last_event);
      info->last_event = NULL;
    }
  ...
  __gtk_grab_remove (info->ipc_widget);

  if (__gtk_widget_get_realized (source_widget))
    {
      GdkEvent *send_event;

      /* Send on a release pair to the original widget to convince it
       * to release its grab. We need to call __gtk_propagate_event()
       * here, instead of __gtk_widget_event() because widget like
       * GtkList may expect propagation.
       */

      send_event = __gdk_event_new (GDK_BUTTON_RELEASE);
      send_event->button.window = g_object_ref (__gtk_widget_get_root_window (source_widget));
      ...
      __gtk_propagate_event (source_widget, send_event);
...
gtk_drag_cancel (GtkDragSourceInfo *info, GtkDragResult result, guint32 time)
{
  ...
  __gdk_drag_abort (info->context, time);
  ...
  if (event->is_hint)
    {
      ...
      __gdk_display_get_pointer (display, &screen, &x_root, &y_root, NULL);
      event->x_root = x_root;
      event->y_root = y_root;
    }
  else
    screen = __gdk_event_get_screen ((GdkEvent *)event);
		 ...
		 gpointer           data)
{
  ...
  dx = dy = 0;
  state = event->state & __gtk_accelerator_get_default_mod_mask ();
   ...
   */
  root_window = __gtk_widget_get_root_window (widget);
  __gdk_window_get_pointer (root_window, NULL, NULL, &state);
  event->state = state;
  ...
  if (dx != 0 || dy != 0)
    {
      info->cur_x += dx;
      info->cur_y += dy;
      __gdk_display_warp_pointer (__gtk_widget_get_display (widget), 
  ...
  if (!was_grabbed)
    {
	 ...
	 gtk_drag_cancel calls __gtk_grab_remove (via gtk_drag_end) */
			    ...
			    gpointer        data)
{
  ...
  if ((__gdk_drag_context_get_selected_action (info->context) != 0) &&
      (__gdk_drag_context_get_dest_window (info->context) != NULL))
    {
      gtk_drag_end (info, event->time);
      gtk_drag_drop (info, event->time);
    }
 ...
 **/
gboolean
__gtk_drag_check_threshold (GtkWidget *widget,
			  gint       start_x,
			  gint       start_y,
			  gint       current_x,
			  gint       current_y)
