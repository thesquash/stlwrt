...
gtk_tool_button_construct_contents (GtkToolItem *tool_item)
{
  ...
  if (need_label)
      ...
      else
	{
	  ...
	  if (elide)
	    label_text = ______gtk_toolbar_elide_underscores (label_text);
	  else
			   ...
			   const gchar   *label)
{
  ...
  if (label)
    {
      elided_label = ______gtk_toolbar_elide_underscores (label);
...
GtkWidget *
______gtk_tool_button_get_button (GtkToolButton *button)
{
  g_return_val_if_fail (GTK_IS_TOOL_BUTTON (button), NULL);
