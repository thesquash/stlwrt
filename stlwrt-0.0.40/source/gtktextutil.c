  ...
  for (i = 0; i < G_N_ELEMENTS (bidi_menu_entries); i++)
    {
      ...
      menuitem = __gtk_menu_item_new_with_mnemonic (_(bidi_menu_entries[i].label));
                                 ...
                                 gsize      len)
{
  ...
  drawable = __gdk_pixmap_new (widget->window,
                             pixmap_width  + 2,
                             pixmap_height + 2,
                             -1);
  ...
  __gdk_cairo_set_source_color (cr, &widget->style->text [__gtk_widget_get_state (widget)]);
                                      ...
                                      GtkTextIter   *end)
{
  ...
  cairo_t           *cr;
   ...
   new_buffer = __gtk_text_buffer_new (__gtk_text_buffer_get_tag_table (buffer));
   __gtk_text_buffer_get_start_iter (new_buffer, &iter);

   __gtk_text_buffer_insert_range (new_buffer, &iter, start, end);
   ...
   if (GTK_IS_TEXT_VIEW (widget))
     {
       ...
       layout_width = layout_width
         - __gtk_text_view_get_border_window_size (GTK_TEXT_VIEW (widget), GTK_TEXT_WINDOW_LEFT)
         - __gtk_text_view_get_border_window_size (GTK_TEXT_VIEW (widget), GTK_TEXT_WINDOW_RIGHT);
     }

   style->direction = __gtk_widget_get_direction (widget);
   ...
   gtk_text_layout_draw (layout, widget, drawable,
                         widget->style->text_gc [__gtk_widget_get_state (widget)],
