...
gtk_link_button_init (GtkLinkButton *link_button)
{
  ...
  /* enable drag source */
  __gtk_drag_source_set (GTK_WIDGET (link_button),
  		       GDK_BUTTON1_MASK,
			      ...
			      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_URI:
      __gtk_link_button_set_uri (link_button, g_value_get_string (value));
      break;
    case PROP_VISITED:
      __gtk_link_button_set_visited (link_button, g_value_get_boolean (value));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
set_link_color (GtkLinkButton *link_button)
{
  ...
  label = __gtk_bin_get_child (GTK_BIN (link_button));
  ...
  __gtk_widget_modify_fg (label, GTK_STATE_NORMAL, link_color);
  __gtk_widget_modify_fg (label, GTK_STATE_ACTIVE, link_color);
  __gtk_widget_modify_fg (label, GTK_STATE_PRELIGHT, link_color);
  __gtk_widget_modify_fg (label, GTK_STATE_SELECTED, link_color);
  ...
  if (GTK_IS_LABEL (label))
    {
      ...
      __gtk_label_set_attributes (GTK_LABEL (label), attributes);
			  ...
			  GdkEventButton *event)
{
  ...
  if (__gtk_widget_get_realized (GTK_WIDGET (link_button)))
    {
      GtkWidget *menu_item;
      
      if (priv->popup_menu)
	__gtk_widget_destroy (priv->popup_menu);

      priv->popup_menu = __gtk_menu_new ();
      
      __gtk_menu_attach_to_widget (GTK_MENU (priv->popup_menu),
		      		 GTK_WIDGET (link_button),
				 popup_menu_detach);

      menu_item = __gtk_image_menu_item_new_with_mnemonic (_("Copy URL"));
      __gtk_image_menu_item_set_image (GTK_IMAGE_MENU_ITEM (menu_item),
		      		     __gtk_image_new_from_stock (GTK_STOCK_COPY,
							       GTK_ICON_SIZE_MENU));
      g_signal_connect (menu_item, "activate",
		        G_CALLBACK (copy_activate_cb), link_button);
      __gtk_widget_show (menu_item);
      __gtk_menu_shell_append (GTK_MENU_SHELL (priv->popup_menu), menu_item);
      
      if (button)
        __gtk_menu_popup (GTK_MENU (priv->popup_menu), NULL, NULL,
		        NULL, NULL,
			button, time);
      else
        {
          __gtk_menu_popup (GTK_MENU (priv->popup_menu), NULL, NULL,
			  ...
			  button, time);
	  __gtk_menu_shell_select_first (GTK_MENU_SHELL (priv->popup_menu), FALSE);
	}
    }
}
...
gtk_link_button_clicked (GtkButton *button)
{
  ...
  if (uri_func)
  ...
  else
    {
      ...
      if (__gtk_widget_has_screen (GTK_WIDGET (button)))
        screen = __gtk_widget_get_screen (GTK_WIDGET (button));
      else
        screen = NULL;
  ...
  __gtk_link_button_set_visited (link_button, TRUE);
}
 ...
 */
GtkWidget *
__gtk_link_button_new (const gchar *uri)
{
  gchar *utf8_uri = NULL;
  GtkWidget *retval;
...
/**
 * __gtk_link_button_new_with_label:
 * @uri: a valid URI
 ...
 */
GtkWidget *
__gtk_link_button_new_with_label (const gchar *uri,
				const gchar *label)
{
  GtkWidget *retval;
  ...
  if (!label)
    return __gtk_link_button_new (uri);
                                  ...
                                  gpointer      data)
{
  ...
  if (!__gtk_widget_get_tooltip_text (widget)
    ...
    {
      __gtk_tooltip_set_text (tooltip, uri);
      return TRUE;
    }
 ...
 */
void
__gtk_link_button_set_uri (GtkLinkButton *link_button,
			 const gchar   *uri)
{
  ...
  __gtk_link_button_set_visited (link_button, FALSE);
}

/**
 * __gtk_link_button_get_uri:
 * @link_button: a #GtkLinkButton
 *
 * Retrieves the URI set using __gtk_link_button_set_uri().
 *
 ...
 */
const gchar *
__gtk_link_button_get_uri (GtkLinkButton *link_button)
{
...
/**
 * __gtk_link_button_set_uri_hook:
 ...
 *
 * If no uri hook has been set, GTK+ defaults to calling __gtk_show_uri().
 *
...
__gtk_link_button_set_uri_hook (GtkLinkButtonUriFunc func,
			      gpointer             data,
...
/**
 * __gtk_link_button_set_visited:
 * @link_button: a #GtkLinkButton
 * @visited: the new 'visited' state
 *
 * Sets the 'visited' state of the URI where the #GtkLinkButton
 * points.  See __gtk_link_button_get_visited() for more details.
 *
 * Since: 2.14
 */
void
__gtk_link_button_set_visited (GtkLinkButton *link_button,
                             gboolean       visited)
{
  g_return_if_fail (GTK_IS_LINK_BUTTON (link_button));
 ...
 *
 * The state may also be changed using __gtk_link_button_set_visited().
 *
 ...
 */
gboolean
__gtk_link_button_get_visited (GtkLinkButton *link_button)
{
