...
{
  gint space_size = ______gtk_toolbar_get_default_space_size();
  GtkWidget *parent = GTK_WIDGET (tool_item)->parent;
  
  if (GTK_IS_TOOLBAR (parent))
    {
      __gtk_widget_style_get (parent,
...
gtk_separator_tool_item_create_menu_proxy (GtkToolItem *item)
{
  ...
  menu_item = __gtk_separator_menu_item_new();
				      ...
				      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_DRAW:
      __gtk_separator_tool_item_set_draw (item, g_value_get_boolean (value));
      break;
    default:
				      ...
				      GParamSpec   *pspec)
{
  ...
  switch (prop_id)
    {
    case PROP_DRAW:
      g_value_set_boolean (value, __gtk_separator_tool_item_get_draw (item));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
				      ...
				      GtkRequisition *requisition)
{
  ...
  GtkOrientation orientation = __gtk_tool_item_get_orientation (item);
				...
				GdkEventExpose *event)
{
  ...
  if (priv->draw)
	...
	toolbar = GTK_TOOLBAR (widget->parent);
      ...
      ______gtk_toolbar_paint_space_line (widget, toolbar,
 ...
 */
GtkToolItem *
__gtk_separator_tool_item_new (void)
{
  GtkToolItem *self;
...
/**
 * __gtk_separator_tool_item_get_draw:
 * @item: a #GtkSeparatorToolItem 
 * 
 * Returns whether @item is drawn as a line, or just blank. 
 * See __gtk_separator_tool_item_set_draw().
 * 
 ...
 */
gboolean
__gtk_separator_tool_item_get_draw (GtkSeparatorToolItem *item)
{
...
/**
 * __gtk_separator_tool_item_set_draw:
 * @item: a #GtkSeparatorToolItem
 * @draw: whether @item is drawn as a vertical line
 * 
 * Whether @item is drawn as a vertical line, or just blank.
 * Setting this to %FALSE along with __gtk_tool_item_set_expand() is useful
 * to create an item that forces following items to the end of the toolbar.
 * 
 * Since: 2.4
 */
void
__gtk_separator_tool_item_set_draw (GtkSeparatorToolItem *item,
				  gboolean              draw)
{
