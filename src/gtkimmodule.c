...
STLWRT_DEFINE_FTYPE (GtkIMModule, gtk_im_module, G_TYPE_TYPE_MODULE, G_TYPE_FLAG_NONE, ;)
...
{
  /* STLWRT_LIBDIR here is supposed to still have the definition from
   * Makefile.am, i.e. the build-time value. Do *not* include gtkprivate.h
   * in this file.
   */
  if (strncmp (*path, STLWRT_LIBDIR, strlen (STLWRT_LIBDIR)) == 0)
    {
      ...
      gchar *tem = *path;
      *path = g_strconcat (_gtk_get_libdir (), tem + strlen (STLWRT_LIBDIR), NULL);
      g_free (tem);
    }
}
...
{
  /* As above, but for STLWRT_LOCALEDIR. Use separate function in case
   * STLWRT_LOCALEDIR isn't a subfolder of STLWRT_LIBDIR.
   */
  if (strncmp (*path, STLWRT_LOCALEDIR, strlen (STLWRT_LOCALEDIR)) == 0)
    {
      extern const gchar *_gtk_get_localedir ();
      gchar *tem = *path;
      *path = g_strconcat (_gtk_get_localedir (), tem + strlen (STLWRT_LOCALEDIR), NULL);
      g_free (tem);
    }
}
#endif
...
gtk_im_module_initialize (void)
{
  ...
  gchar *filename = __gtk_rc_get_im_module_file();
  FILE *file;
...
_gtk_im_module_create (const gchar *context_id)
{
  ...
  if (!context)
     return __gtk_im_context_simple_new ();
  else
    return context;
}
...
_gtk_im_module_get_default_context_id (GdkWindow *client_window)
{
  ...
  if (GDK_IS_DRAWABLE (client_window))
    {
      screen = __gdk_window_get_screen (client_window);
   ...
   */
  tmp_locale = ___gtk_get_lc_ctype ();
