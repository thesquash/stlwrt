...
static void         __gtk_tree_model_filter_clear_cache_helper              (GtkTreeModelFilter     *filter,
                                                                           FilterLevel            *level);
...
STLWRT_DEFINE_TYPE_WITH_CODE (GtkTreeModelFilter, gtk_tree_model_filter, G_TYPE_OBJECT,
...
gtk_tree_model_filter_finalize (GObject *object)
{
  ...
  if (filter->priv->virtual_root)
    __gtk_tree_path_free (filter->priv->virtual_root);
                                   ...
                                   gboolean            emit_inserted)
{
  ...
  if (!parent_level)
    {
      if (filter->priv->virtual_root)
        {
          if (__gtk_tree_model_get_iter (filter->priv->child_model, &root, filter->priv->virtual_root) == FALSE)
            return;
          length = __gtk_tree_model_iter_n_children (filter->priv->child_model, &root);

          if (__gtk_tree_model_iter_children (filter->priv->child_model, &iter, &root) == FALSE)
            return;
        }
      else
        {
          if (!__gtk_tree_model_get_iter_first (filter->priv->child_model, &iter))
            return;
          length = __gtk_tree_model_iter_n_children (filter->priv->child_model, NULL);
        }
    }
  else
    {
      ...
      __gtk_tree_model_filter_convert_iter_to_child_iter (filter,
                                                        &child_parent_iter,
                                                        &parent_iter);
      if (__gtk_tree_model_iter_children (filter->priv->child_model, &iter, &child_parent_iter) == FALSE)
        return;
      ...
      __gtk_tree_model_filter_convert_iter_to_child_iter (filter,
                                                        &child_parent_iter,
                                                        &parent_iter);
      length = __gtk_tree_model_iter_n_children (filter->priv->child_model, &child_parent_iter);
    }
          ...
          if (emit_inserted)
            {
              ...
              f_path = __gtk_tree_model_get_path (GTK_TREE_MODEL (filter),
                                                &f_iter);
              __gtk_tree_model_row_inserted (GTK_TREE_MODEL (filter),
                                           f_path, &f_iter);
              __gtk_tree_path_free (f_path);

              if (__gtk_tree_model_iter_children (filter->priv->child_model,
                                                &children, &iter))
  ...
  while (__gtk_tree_model_iter_next (filter->priv->child_model, &iter));
                                ...
                                GtkTreePath *root)
{
  ...
  retval = __gtk_tree_path_copy (root);

  for (i = 0; i < __gtk_tree_path_get_depth (src); i++)
    __gtk_tree_path_append_index (retval, __gtk_tree_path_get_indices (src)[i]);
                                   ...
                                   GtkTreePath *root)
{
  ...
  if (__gtk_tree_path_get_depth (src) <= __gtk_tree_path_get_depth (root))
    return NULL;

  depth = __gtk_tree_path_get_depth (src);
  indices = __gtk_tree_path_get_indices (src);

  for (i = 0; i < __gtk_tree_path_get_depth (root); i++)
    if (indices[i] != __gtk_tree_path_get_indices (root)[i])
      return NULL;

  retval = __gtk_tree_path_new ();

  for (; i < depth; i++)
    __gtk_tree_path_append_index (retval, indices[i]);
...
gtk_tree_model_filter_increment_stamp (GtkTreeModelFilter *filter)
{
  ...
  __gtk_tree_model_filter_clear_cache (filter);
}
                               ...
                               GtkTreeIter        *child_iter)
{
  if (filter->priv->visible_func)
  ...
  else if (filter->priv->visible_column >= 0)
   {
     ...
     __gtk_tree_model_get_value (filter->priv->child_model, child_iter,
...
static void
__gtk_tree_model_filter_clear_cache_helper (GtkTreeModelFilter *filter,
                                          FilterLevel        *level)
{
  gint i;
  ...
  for (i = 0; i < level->array->len; i++)
    {
      if (g_array_index (level->array, FilterElt, i).zero_ref_count > 0)
        __gtk_tree_model_filter_clear_cache_helper (filter, g_array_index (level->array, FilterElt, i).children);
     }
                                   ...
                                   gint               *index)
{
  ...
  if (level->parent_elt_index >= 0)
    ...
    {
      if (filter->priv->virtual_root)
        c_parent_path = __gtk_tree_path_copy (filter->priv->virtual_root);
      else
        c_parent_path = NULL;
    }
  ...
  if (c_parent_path)
    {
      __gtk_tree_model_get_iter (filter->priv->child_model,
                               &c_parent_iter,
                               c_parent_path);
      len = __gtk_tree_model_iter_n_children (filter->priv->child_model,
                                            &c_parent_iter);

      c_path = __gtk_tree_path_copy (c_parent_path);
      __gtk_tree_path_free (c_parent_path);
    }
  else
    {
      len = __gtk_tree_model_iter_n_children (filter->priv->child_model, NULL);
      c_path = __gtk_tree_path_new ();
    }

  __gtk_tree_path_append_index (c_path, offset);
  __gtk_tree_model_get_iter (filter->priv->child_model, &c_iter, c_path);
  __gtk_tree_path_free (c_path);
  ...
  if (length > 1)
    {
      ...
      path = __gtk_tree_model_get_path (GTK_TREE_MODEL (filter), iter);
      elt->visible = FALSE;
      gtk_tree_model_filter_increment_stamp (filter);
      iter->stamp = filter->priv->stamp;
      __gtk_tree_model_row_deleted (GTK_TREE_MODEL (filter), path);
      __gtk_tree_path_free (path);
  ...
  if (emit_child_toggled)
    {
      ...
      ppath = __gtk_tree_model_get_path (GTK_TREE_MODEL (filter), &piter);

      __gtk_tree_model_row_has_child_toggled (GTK_TREE_MODEL (filter),
                                            ppath, &piter);
				       ...
				       FilterElt          *elt)
{
  ...
  __gtk_tree_model_filter_convert_iter_to_child_iter (filter, &c_iter, &iter);

  if (__gtk_tree_model_iter_has_child (filter->priv->child_model, &c_iter))
    {
      GtkTreePath *path = __gtk_tree_model_get_path (GTK_TREE_MODEL (filter),
                                                   &iter);
      __gtk_tree_model_row_has_child_toggled (GTK_TREE_MODEL (filter),
                                            path,
                                            &iter);
                                   ...
                                   gpointer      data)
{
  ...
  if (!c_path)
    {
      c_path = __gtk_tree_model_get_path (c_model, c_iter);
      free_c_path = TRUE;
    }
  ...
  if (c_iter)
    ...
    __gtk_tree_model_get_iter (c_model, &real_c_iter, c_path);

  /* is this node above the virtual root? */
  if (filter->priv->virtual_root
      && (__gtk_tree_path_get_depth (filter->priv->virtual_root)
          >= __gtk_tree_path_get_depth (c_path)))
    goto done;
  ...
  if (current_state == TRUE && requested_state == TRUE)
    {
      ...
      if (gtk_tree_model_filter_elt_is_visible_in_target (level, elt))
        {
          __gtk_tree_model_row_changed (GTK_TREE_MODEL (filter), path, &iter);

          /* and update the children */
          if (__gtk_tree_model_iter_children (c_model, &children, &real_c_iter))
  ...
  if (gtk_tree_model_filter_elt_is_visible_in_target (level, elt))
    {
      ...
      if (!signals_emitted)
        __gtk_tree_model_row_inserted (GTK_TREE_MODEL (filter), path, &iter);
      ...
      if (level->parent_level && level->visible_nodes == 1)
        {
          ...
          __gtk_tree_model_get_iter (GTK_TREE_MODEL (filter), &iter, path);
      ...
      if (!signals_emitted
          && __gtk_tree_model_iter_children (c_model, &children, c_iter))
        gtk_tree_model_filter_update_children (filter, level, elt);
    }
                                    ...
                                    gpointer      data)
{
  ...
  if (filter->priv->virtual_root)
    {
      if (__gtk_tree_path_get_depth (filter->priv->virtual_root) >=
          __gtk_tree_path_get_depth (c_path))
        {
          gint level;
          ...
          level = __gtk_tree_path_get_depth (c_path) - 1;
          v_indices = __gtk_tree_path_get_indices (filter->priv->virtual_root);
          c_indices = __gtk_tree_path_get_indices (c_path);
  ...
  if (filter->priv->virtual_root)
    ...
    real_path = __gtk_tree_path_copy (c_path);

  if (__gtk_tree_path_get_depth (real_path) - 1 >= 1)
    {
      /* find the parent level */
      while (i < __gtk_tree_path_get_depth (real_path) - 1)
        {
          gint j;
          ...
          elt = bsearch_elt_with_offset (level->array,
                                         __gtk_tree_path_get_indices (real_path)[i],
                                         &j);
          ...
          if (!elt->children)
            {
              ...
              tmppath = __gtk_tree_model_get_path (GTK_TREE_MODEL (data),
                                                 &tmpiter);

              if (tmppath)
                {
                  __gtk_tree_model_row_has_child_toggled (GTK_TREE_MODEL (data),
                                                        tmppath, &tmpiter);
  ...
  offset = __gtk_tree_path_get_indices (real_path)[__gtk_tree_path_get_depth (real_path) - 1];
...
done_and_emit:
  ...
  __gtk_tree_path_free (path);
  path = __gtk_tree_model_get_path (GTK_TREE_MODEL (data), &iter);
  ...
  if (real_path)
    __gtk_tree_path_free (real_path);
                                             ...
                                             gpointer      data)
{
  ...
  if (filter->priv->virtual_root && !filter->priv->root
      && !__gtk_tree_path_compare (c_path, filter->priv->virtual_root))
    {
      gtk_tree_model_filter_build_level (filter, NULL, -1, TRUE);
      return;
    }
   ...
   */
  if (elt->ref_count > 1 && __gtk_tree_model_iter_has_child (c_model, c_iter))
                                   ...
                                   gpointer      data)
{
  ...
  if (filter->priv->virtual_root &&
      (__gtk_tree_path_is_ancestor (c_path, filter->priv->virtual_root) ||
       !__gtk_tree_path_compare (c_path, filter->priv->virtual_root)))
    {
      gint i;
      GtkTreePath *path2;
      ...
      gtk_tree_model_filter_increment_stamp (filter);
      path2 = __gtk_tree_path_new ();
      __gtk_tree_path_append_index (path2, 0);

      for (i = 0; i < level2->visible_nodes; i++)
        __gtk_tree_model_row_deleted (GTK_TREE_MODEL (data), path2);

      __gtk_tree_path_free (path2);
      gtk_tree_model_filter_free_level (filter, filter->priv->root);
  ...
  if (filter->priv->virtual_root)
    {
      if (__gtk_tree_path_get_depth (filter->priv->virtual_root) >=
          __gtk_tree_path_get_depth (c_path))
        {
          ...
          level2 = __gtk_tree_path_get_depth (c_path) - 1;
          v_indices = __gtk_tree_path_get_indices (filter->priv->virtual_root);
          c_indices = __gtk_tree_path_get_indices (c_path);
  ...
  if (emit_child_toggled && parent_level)
    {
      ...
      path2 = __gtk_tree_model_get_path (GTK_TREE_MODEL (filter), &iter2);
      __gtk_tree_model_row_has_child_toggled (GTK_TREE_MODEL (filter),
                                            path2, &iter2);
      __gtk_tree_path_free (path2);
      filter->priv->in_row_deleted = FALSE;
                                      ...
                                      gpointer      data)
{
  ...
  if (c_path == NULL || __gtk_tree_path_get_depth (c_path) == 0)
    {
      length = __gtk_tree_model_iter_n_children (c_model, NULL);
      ...
      if (filter->priv->virtual_root)
        {
          ...
          for (i = 0; i < length; i++)
            if (new_order[i] == __gtk_tree_path_get_indices (filter->priv->virtual_root)[0])
              new_pos = i;

          if (new_pos < 0)
            return;

          __gtk_tree_path_get_indices (filter->priv->virtual_root)[0] = new_pos;
          return;
        }
	  ...
	  __gtk_tree_path_is_ancestor (c_path, filter->priv->virtual_root))
        {
          ...
          level = __gtk_tree_path_get_depth (c_path);
          ...
          length = __gtk_tree_model_iter_n_children (c_model, &real_c_iter);

          for (i = 0; i < length; i++)
            if (new_order[i] == __gtk_tree_path_get_indices (filter->priv->virtual_root)[level])
              new_pos = i;

          if (new_pos < 0)
            return;

          __gtk_tree_path_get_indices (filter->priv->virtual_root)[level] = new_pos;
          return;
        }
      ...
      if (!path)
        {
          /* root level mode */
          if (!c_iter)
            __gtk_tree_model_get_iter (c_model, c_iter, c_path);
          length = __gtk_tree_model_iter_n_children (c_model, c_iter);
          path = __gtk_tree_path_new ();
          level = FILTER_LEVEL (filter->priv->root);
        }
      else
        {
          ...
          __gtk_tree_model_filter_convert_iter_to_child_iter (GTK_TREE_MODEL_FILTER (filter), &child_iter, &iter);
          length = __gtk_tree_model_iter_n_children (c_model, &child_iter);
        }
    }
  ...
  /* emit rows_reordered */
  if (!__gtk_tree_path_get_indices (path))
    __gtk_tree_model_rows_reordered (GTK_TREE_MODEL (data), path, NULL,
                                   tmp_array);
  else
    {
      /* get a path taking only visible nodes into account */
      ...
      __gtk_tree_model_rows_reordered (GTK_TREE_MODEL (data), path, &iter,
                                     tmp_array);
    }
...
gtk_tree_model_filter_get_flags (GtkTreeModel *model)
{
  ...
  flags = __gtk_tree_model_get_flags (GTK_TREE_MODEL_FILTER (model)->priv->child_model);
...
gtk_tree_model_filter_get_n_columns (GtkTreeModel *model)
{
  ...
  return __gtk_tree_model_get_n_columns (filter->priv->child_model);
}
                                       ...
                                       gint          index)
{
  ...
  return __gtk_tree_model_get_column_type (filter->priv->child_model, index);
}
  ...
  while (level)
    {
      ...
      __gtk_tree_path_prepend_index (retval, index);
      elt_index = level->parent_elt_index;
      level = level->parent_level;
    }
                                 ...
                                 GValue       *value)
{
  ...
  __gtk_tree_model_get_value (GTK_TREE_MODEL_FILTER (model)->priv->child_model,
                                      ...
                                      GtkTreeIter  *iter)
{
   ...
   */
  if (!elt->children
      && __gtk_tree_model_iter_has_child (filter->priv->child_model, &child_iter))
                                ...
                                GtkTreeIter  *iter)
{
  ...
  __gtk_tree_model_ref_node (filter->priv->child_model, &child_iter);
  ...
  if (propagate_unref)
    {
      GtkTreeIter child_iter;
      __gtk_tree_model_filter_convert_iter_to_child_iter (GTK_TREE_MODEL_FILTER (model), &child_iter, iter);
      __gtk_tree_model_unref_node (filter->priv->child_model, &child_iter);
    }
                                     ...
                                     GtkTreePath       *path)
{
  ...
  child_path = __gtk_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  draggable = __gtk_tree_drag_source_row_draggable (GTK_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path);
                                     ...
                                     GtkSelectionData  *selection_data)
{
  ...
  child_path = __gtk_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  gotten = __gtk_tree_drag_source_drag_data_get (GTK_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path, selection_data);
  __gtk_tree_path_free (child_path);
                                        ...
                                        GtkTreePath       *path)
{
  ...
  child_path = __gtk_tree_model_filter_convert_path_to_child_path (tree_model_filter, path);
  deleted = __gtk_tree_drag_source_drag_data_delete (GTK_TREE_DRAG_SOURCE (tree_model_filter->priv->child_model), child_path);
  __gtk_tree_path_free (child_path);
  ...
  if (child_model)
    {
      ...
      filter->priv->child_flags = __gtk_tree_model_get_flags (child_model);
                                ...
                                GtkTreePath        *path)
{
  ...
  len = __gtk_tree_path_get_depth (path);
  p = __gtk_tree_path_copy (path);
  while (len--)
    {
      GtkTreeIter iter;

      __gtk_tree_model_get_iter (GTK_TREE_MODEL (filter->priv->child_model), &iter, p);
      __gtk_tree_model_ref_node (GTK_TREE_MODEL (filter->priv->child_model), &iter);
      __gtk_tree_path_up (p);
    }
  ...
  while (len--)
    {
      ...
      __gtk_tree_model_get_iter (GTK_TREE_MODEL (filter->priv->child_model), &iter, p);
      __gtk_tree_model_unref_node (GTK_TREE_MODEL (filter->priv->child_model), &iter);
      __gtk_tree_path_up (p);
    }
                                ...
                                GtkTreePath        *root)
{
  ...
  if (!root)
    ...
    filter->priv->virtual_root = __gtk_tree_path_copy (root);
}

/* public API */

/**
 * __gtk_tree_model_filter_new:
 ...
 */
GtkTreeModel *
__gtk_tree_model_filter_new (GtkTreeModel *child_model,
                           GtkTreePath  *root)
{
  GtkTreeModel *retval;
...
/**
 * __gtk_tree_model_filter_get_model:
 ...
 */
GtkTreeModel *
__gtk_tree_model_filter_get_model (GtkTreeModelFilter *filter)
{
...
/**
 * __gtk_tree_model_filter_set_visible_func:
 * @filter: A #GtkTreeModelFilter.
 ...
 * __gtk_tree_model_filter_refilter() to keep the visibility information of 
 * the model uptodate.
 *
 ...
 *
 *   __gtk_tree_model_get (model, iter, 0, &str, -1);
 ...
 */
void
__gtk_tree_model_filter_set_visible_func (GtkTreeModelFilter            *filter,
...
/**
 * __gtk_tree_model_filter_set_modify_func:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
void
__gtk_tree_model_filter_set_modify_func (GtkTreeModelFilter           *filter,
                                       gint                          n_columns,
                                       GType                        *types,
...
/**
 * __gtk_tree_model_filter_set_visible_column:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
void
__gtk_tree_model_filter_set_visible_column (GtkTreeModelFilter *filter,
                                          gint column)
{
  g_return_if_fail (GTK_IS_TREE_MODEL_FILTER (filter));
...
/**
 * __gtk_tree_model_filter_convert_child_iter_to_iter:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
gboolean
__gtk_tree_model_filter_convert_child_iter_to_iter (GtkTreeModelFilter *filter,
                                                  ...
                                                  GtkTreeIter        *child_iter)
{
  ...
  child_path = __gtk_tree_model_get_path (filter->priv->child_model, child_iter);
  g_return_val_if_fail (child_path != NULL, FALSE);

  path = __gtk_tree_model_filter_convert_child_path_to_path (filter,
                                                           child_path);
  __gtk_tree_path_free (child_path);
  ...
  ret = __gtk_tree_model_get_iter (GTK_TREE_MODEL (filter), filter_iter, path);
  __gtk_tree_path_free (path);
...
/**
 * __gtk_tree_model_filter_convert_iter_to_child_iter:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
void
__gtk_tree_model_filter_convert_iter_to_child_iter (GtkTreeModelFilter *filter,
                                                  GtkTreeIter        *child_iter,
                                                  GtkTreeIter        *filter_iter)
{
  g_return_if_fail (GTK_IS_TREE_MODEL_FILTER (filter));
  ...
  if (GTK_TREE_MODEL_FILTER_CACHE_CHILD_ITERS (filter))
  ...
  else
    {
      ...
      path = gtk_tree_model_filter_elt_get_path (filter_iter->user_data,
                                                 ...
                                                 filter->priv->virtual_root);
      __gtk_tree_model_get_iter (filter->priv->child_model, child_iter, path);
      __gtk_tree_path_free (path);
    }
}
                                                       ...
                                                       gboolean            fetch_children)
{
  ...
  if (!filter->priv->virtual_root)
    real_path = __gtk_tree_path_copy (child_path);
  else
  ...
  retval = __gtk_tree_path_new ();
  child_indices = __gtk_tree_path_get_indices (real_path);

  if (filter->priv->root == NULL && build_levels)
    gtk_tree_model_filter_build_level (filter, NULL, -1, FALSE);
  level = FILTER_LEVEL (filter->priv->root);

  for (i = 0; i < __gtk_tree_path_get_depth (real_path); i++)
    {
      gint j;
...
/**
 * __gtk_tree_model_filter_convert_child_path_to_path:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
GtkTreePath *
__gtk_tree_model_filter_convert_child_path_to_path (GtkTreeModelFilter *filter,
                                                  GtkTreePath        *child_path)
{
  GtkTreeIter iter;
  GtkTreePath *path;
...
/**
 * __gtk_tree_model_filter_convert_path_to_child_path:
 * @filter: A #GtkTreeModelFilter.
 ...
 */
GtkTreePath *
__gtk_tree_model_filter_convert_path_to_child_path (GtkTreeModelFilter *filter,
                                                  GtkTreePath        *filter_path)
{
  ...
  /* convert path */
  retval = __gtk_tree_path_new ();
  filter_indices = __gtk_tree_path_get_indices (filter_path);
  if (!filter->priv->root)
    gtk_tree_model_filter_build_level (filter, NULL, -1, FALSE);
  level = FILTER_LEVEL (filter->priv->root);

  for (i = 0; i < __gtk_tree_path_get_depth (filter_path); i++)
    {
      FilterElt *elt;
      ...
      __gtk_tree_path_append_index (retval, elt->offset);
      level = elt->children;
    }
...
static gboolean
__gtk_tree_model_filter_refilter_helper (GtkTreeModel *model,
                                       GtkTreePath  *path,
                                       GtkTreeIter  *iter,
                                       gpointer      data)
{
...
/**
 * __gtk_tree_model_filter_refilter:
 * @filter: A #GtkTreeModelFilter.
 *
 ...
 */
void
__gtk_tree_model_filter_refilter (GtkTreeModelFilter *filter)
{
  g_return_if_fail (GTK_IS_TREE_MODEL_FILTER (filter));

  /* S L O W */
  __gtk_tree_model_foreach (filter->priv->child_model,
                          __gtk_tree_model_filter_refilter_helper,
                          filter);
}

/**
 * __gtk_tree_model_filter_clear_cache:
 * @filter: A #GtkTreeModelFilter.
 *
 * This function should almost never be called. It clears the @filter
 * of any cached iterators that haven't been reffed with
 * __gtk_tree_model_ref_node(). This might be useful if the child model
 ...
 */
void
__gtk_tree_model_filter_clear_cache (GtkTreeModelFilter *filter)
{
  g_return_if_fail (GTK_IS_TREE_MODEL_FILTER (filter));
