...
/**
 * __gtk_disable_setlocale:
 * 
 * Prevents __gtk_init(), __gtk_init_check(), __gtk_init_with_args() and
 * __gtk_parse_args() from automatically
 ...
 **/
void
__gtk_disable_setlocale (void)
{
  if (pre_initialized)
    g_warning ("__gtk_disable_setlocale() must be called before __gtk_init()");
 ...
 */
gboolean
______gtk_module_has_mixed_deps (GModule *module_to_check)
{
  GModule *module;
  gpointer func;
  gboolean result;
			     ...
			     char ***argv)
{
  ...
  if (______gtk_module_has_mixed_deps (NULL))
    g_error ("GTK+ 2.x symbols detected. Using GTK+ 2.x and GTK+ 3 in the same process is not supported");

  __gdk_pre_parse_libgtk_only ();
  __gdk_event_handler_set ((GdkEventFunc)__gtk_main_do_event, NULL, NULL);
			      ...
			      char ***argv)
{
  ...
  if (gtk_modules_string)
    {
      ______gtk_modules_init (argc, argv, gtk_modules_string->str);
      g_string_free (gtk_modules_string, TRUE);
    }
  else
    {
      ______gtk_modules_init (argc, argv, NULL);
    }
}
 ...
 */
void
__gtk_init_abi_check (int *argc, char ***argv, int num_checks, size_t sizeof_GtkWindow, size_t sizeof_GtkBox)
{
...
gboolean
__gtk_init_check_abi_check (int *argc, char ***argv, int num_checks, size_t sizeof_GtkWindow, size_t sizeof_GtkBox)
{
  check_sizeof_GtkWindow (sizeof_GtkWindow);
  if (num_checks >= 2)
    check_sizeof_GtkBox (sizeof_GtkBox);
 ...
 * __gtk_set_locale:
 *
 * Initializes internationalization support for GTK+. __gtk_init()
 ...
 **/
gchar *
__gtk_set_locale (void)
{
  return __gdk_set_locale ();
}

/**
 * ______gtk_get_lc_ctype:
 *
...
__gtk_main_do_event (GdkEvent *event)
{
  ...
  if (event->type == GDK_SETTING)
    {
      ______gtk_settings_handle_event (&event->setting);
      return;
    }

  if (event->type == GDK_OWNER_CHANGE)
    {
      ______gtk_clipboard_handle_event (&event->owner_change);
      return;
    }
  ...
  if (!event_widget)
	...
	______gtk_selection_incr_event (event->any.window,
				   &event->property);
  ...
  switch (event->type)
    ...
    case GDK_ENTER_NOTIFY:
      ...
      ______gtk_widget_set_pointer_window (event_widget, event->any.window);
    ...
    case GDK_DROP_FINISHED:
      ______gtk_drag_source_handle_event (event_widget, event);
      break;
    case GDK_DRAG_ENTER:
    case GDK_DRAG_LEAVE:
    case GDK_DRAG_MOTION:
    case GDK_DROP_START:
      ______gtk_drag_dest_handle_event (event_widget, event);
      break;
    default:
                        ...
                        
{
  ...
  if (is_shadowed)
	...
	______gtk_widget_synthesize_crossing (child, info->new_grab_widget,
  ...
  else
    {
      ...
      if (was_shadowed && GTK_WIDGET_HAS_POINTER (child)
	  && __gtk_widget_is_sensitive (child))
	______gtk_widget_synthesize_crossing (info->old_grab_widget, child,
					 info->from_grab ? GDK_CROSSING_GTK_GRAB
					 : GDK_CROSSING_GTK_UNGRAB);
    }

  if (was_shadowed != is_shadowed)
    ______gtk_widget_grab_notify (child, was_shadowed);
...
__gtk_grab_add (GtkWidget *widget)
{
  ...
  if (!__gtk_widget_has_grab (widget) && __gtk_widget_is_sensitive (widget))
    {
      ______gtk_widget_set_has_grab (widget, TRUE);
...
__gtk_grab_remove (GtkWidget *widget)
{
  ...
  if (__gtk_widget_has_grab (widget))
    {
      ______gtk_widget_set_has_grab (widget, FALSE);
...
void
__gtk_init_add (GtkFunction function,
	      gpointer	  data)
{
  GtkInitFunction *init;
...
guint
__gtk_key_snooper_install (GtkKeySnoopFunc snooper,
			 gpointer	 func_data)
{
...
void
__gtk_key_snooper_remove (guint snooper_id)
{
...
guint
__gtk_quit_add_full (guint		main_level,
		   GtkFunction		function,
		   GtkCallbackMarshal	marshal,
		   gpointer		data,
		   GDestroyNotify	destroy)
...
void
__gtk_quit_add_destroy (guint              main_level,
		      GtkObject         *object)
{
  GtkObject **object_p;
  ...
  *object_p = object;
  g_signal_connect (object,
		    ...
		    object_p);
  __gtk_quit_add (main_level, (GtkFunction) gtk_quit_destructor, object_p);
}

guint
__gtk_quit_add (guint	  main_level,
	      GtkFunction function,
	      gpointer	  data)
{
  return __gtk_quit_add_full (main_level, function, NULL, data, NULL);
}

void
__gtk_quit_remove (guint id)
{
  GtkQuitFunction *quitf;
  GList *tmp_list;
...
void
__gtk_quit_remove_by_data (gpointer data)
{
  GtkQuitFunction *quitf;
  GList *tmp_list;
...
guint
__gtk_timeout_add_full (guint32		 interval,
		      GtkFunction	 function,
		      GtkCallbackMarshal marshal,
		      gpointer		 data,
		      GDestroyNotify	 destroy)
...
guint
__gtk_timeout_add (guint32     interval,
		 GtkFunction function,
		 gpointer    data)
{
...
void
__gtk_timeout_remove (guint tag)
{
  g_source_remove (tag);
}

guint
__gtk_idle_add_full (gint			priority,
		   GtkFunction		function,
		   GtkCallbackMarshal	marshal,
		   gpointer		data,
		   GDestroyNotify	destroy)
...
guint
__gtk_idle_add (GtkFunction function,
	      gpointer	  data)
{
...
guint	    
__gtk_idle_add_priority (gint        priority,
		       GtkFunction function,
		       gpointer	   data)
{
...
void
__gtk_idle_remove (guint tag)
{
  g_source_remove (tag);
}

void
__gtk_idle_remove_by_data (gpointer data)
{
  if (!g_idle_remove_by_data (data))
    g_warning ("__gtk_idle_remove_by_data(%p): no such idle", data);
}

guint
__gtk_input_add_full (gint		source,
		    GdkInputCondition	condition,
  ...
  if (marshal)
    {
      ...
      return __gdk_input_add_full (source,
				 condition,
    ...
    return __gdk_input_add_full (source, condition, function, data, destroy);
}

void
__gtk_input_remove (guint tag)
{
  g_source_remove (tag);
}
...
gtk_print (gchar *str)
{
  ...
  if (!window)
    {
      ...
      __gtk_window_set_title (GTK_WINDOW (window), "Messages");
      
      box1 = __gtk_vbox_new (FALSE, 0);
      __gtk_container_add (GTK_CONTAINER (window), box1);
      __gtk_widget_show (box1);
      
      
      box2 = __gtk_vbox_new (FALSE, 10);
      __gtk_container_set_border_width (GTK_CONTAINER (box2), 10);
      __gtk_box_pack_start (GTK_BOX (box1), box2, TRUE, TRUE, 0);
      __gtk_widget_show (box2);
      
      
      table = __gtk_table_new (2, 2, FALSE);
      __gtk_table_set_row_spacing (GTK_TABLE (table), 0, 2);
      __gtk_table_set_col_spacing (GTK_TABLE (table), 0, 2);
      __gtk_box_pack_start (GTK_BOX (box2), table, TRUE, TRUE, 0);
      __gtk_widget_show (table);
      
      text = gtk_text_new (NULL, NULL);
      gtk_text_set_editable (GTK_TEXT (text), FALSE);
      __gtk_table_attach_defaults (GTK_TABLE (table), text, 0, 1, 0, 1);
      __gtk_widget_show (text);
      __gtk_widget_realize (text);
      
      hscrollbar = __gtk_hscrollbar_new (GTK_TEXT (text)->hadj);
      __gtk_table_attach (GTK_TABLE (table), hscrollbar, 0, 1, 1, 2,
			GTK_EXPAND | GTK_FILL, GTK_FILL, 0, 0);
      __gtk_widget_show (hscrollbar);
      
      vscrollbar = __gtk_vscrollbar_new (GTK_TEXT (text)->vadj);
      __gtk_table_attach (GTK_TABLE (table), vscrollbar, 1, 2, 0, 1,
			GTK_FILL, GTK_EXPAND | GTK_FILL, 0, 0);
      __gtk_widget_show (vscrollbar);
      
      separator = __gtk_hseparator_new ();
      __gtk_box_pack_start (GTK_BOX (box1), separator, FALSE, TRUE, 0);
      __gtk_widget_show (separator);
      
      
      box2 = __gtk_vbox_new (FALSE, 10);
      __gtk_container_set_border_width (GTK_CONTAINER (box2), 10);
      __gtk_box_pack_start (GTK_BOX (box1), box2, FALSE, TRUE, 0);
      __gtk_widget_show (box2);
      
      
      button = __gtk_button_new_with_label ("close");
      gtk_signal_connect_object (GTK_OBJECT (button), "clicked",
				 G_CALLBACK (__gtk_widget_hide),
				 GTK_OBJECT (window));
      __gtk_box_pack_start (GTK_BOX (box2), button, TRUE, TRUE, 0);
      __gtk_widget_set_can_default (button, TRUE);
      __gtk_widget_grab_default (button);
  ...
  if (!__gtk_widget_get_visible (window))
    __gtk_widget_show (window);
}
#endif

gboolean
______gtk_boolean_handled_accumulator (GSignalInvocationHint *ihint,
				  GValue                *return_accu,
